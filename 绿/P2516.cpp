...
int main() {
    readstr(s+1),n=strlen(s+1)-1;
    readstr(t+1),m=strlen(t+1)-1;
    _rep(i,0,N-1) g[0][i]=1; g[1][0]=1;
    _rep(i,1,n) _rep(j,1,m) {
        ...
        if (s[i]!=t[j] && f[i&1][j]==f[i&1^1][j-1]) ((g[i&1][j]-=g[i&1^1][j-1])+=mod)%=mod;
    }
    writeln(f[n&1][m]),write(g[n&1][m]);
    return 0;
}
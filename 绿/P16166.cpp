...
inline int solve1() { // r: 10
    _rep(i,1,n) _rep(j,1,m) a[i][j]=b[i][j];
    _rep(i,1,n) _rep(j,1,m) if (a[i][j] && ((a[i][j]&1)^(i&1))) return inf;
    _rep(i,1,n) _rep(j,1,m) if (!a[i][j]) {
        a[i][j]=max(a[i-1][j],a[i][j-1])+1;
        if ((a[i][j]&1)^(i&1)) a[i][j]++;
    }
    _rep(i,1,n) {
        int las=0;
        _rep(j,1,m) {
            if (a[i][j]<=las) return inf;
            las=a[i][j];
        }
    }
    _rep(j,1,m) {
    ...
    int ans=0;
    _rep(i,1,n) _rep(j,1,m) ans+=a[i][j];
    return ans;
}

inline int solve2() { // r: 01
    _rep(i,1,n) _rep(j,1,m) a[i][j]=b[i][j];
    _rep(i,1,n) _rep(j,1,m) if (a[i][j] && !((a[i][j]&1)^(i&1))) return inf;
    _rep(i,1,n) _rep(j,1,m) if (!a[i][j]) {
        a[i][j]=max(a[i-1][j],a[i][j-1])+1;
...
inline int solve3() { // c: 10
    _rep(i,1,n) _rep(j,1,m) a[i][j]=b[i][j];
    _rep(i,1,n) _rep(j,1,m) if (a[i][j] && ((a[i][j]&1)^(j&1))) return inf;
    _rep(i,1,n) _rep(j,1,m) if (!a[i][j]) {
        a[i][j]=max(a[i-1][j],a[i][j-1])+1;
...
i32 main() {
    read(n,m);
    _rep(i,1,n) _rep(j,1,m) read(a[i][j]),b[i][j]=a[i][j];
    if (n==1) {
        int las=0;
        bool ok=true;
        _rep(j,1,m) {
            if (!a[1][j]) a[1][j]=las+1;
            else if (a[1][j]<=las) ok=false;
            las=a[1][j];
        }
        if (!ok) puts("-1");
        else {
            int ans=0;
            _rep(j,1,m) ans+=a[1][j];
            write(ans);
        }
        return 0;
    } else if (m==1) {
        ...
        bool ok=true;
        _rep(i,1,n) {
            if (!a[i][1]) a[i][1]=las+1;
            else if (a[i][1]<=las) ok=false;
            las=a[i][1];
        }
        if (!ok) puts("-1");
        else {
            int ans=0;
            _rep(i,1,n) ans+=a[i][1];
            write(ans);
        }
        return 0;
    }
    int ans=min4(solve1(),solve2(),solve3(),solve4());
    if (ans==inf) ans=-1;
    write(ans);
    return 0;
}
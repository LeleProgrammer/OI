...
i32 main() {
    ...
    _rep(i,1,m) read(b[i]),b[i]+=b[i-1];
    _rep(l,1,n) _rep(r,l,n) va.emplace_back(a[r]-a[l-1]);
    _rep(l,1,m) _rep(r,l,m) vb.emplace_back(b[r]-b[l-1]);
    _sort(va),_sort(vb);
    _for(k,va) {
        int pos=LB(vb,k)-1;
        ans+=pos+1;
    ...
    ans%=mod;
    write(ans*Math::inverse((i64)va.size()*vb.size()%mod)%mod);
    return 0;
}
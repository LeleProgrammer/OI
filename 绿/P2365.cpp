...
i32 main() {
    read(n,s);
    _rep(i,1,n) read(t[i],c[i]);
    _rep(i,1,n) pret[i]=pret[i-1]+t[i],prec[i]=prec[i-1]+c[i];
    q[++tt]=0;
    _rep(i,1,n) {
        int l=hh,r=tt;
        while (l<r) {
            int mid=l+r>>1;
            if ((gety(q[mid+1])-gety(q[mid]))>=(getx(q[mid+1])-getx(q[mid]))*getk(i)) r=mid;
            else l=mid+1;
        }
        f[i]=gety(q[r])-getk(i)*getx(q[r])+prec[i]*pret[i]+prec[n]*s;
        if (r-1>=hh) chmin(f[i],gety(q[r-1])-getk(i)*getx(q[r-1])+prec[i]*pret[i]+prec[n]*s);
        while (tt-hh>=1 && (gety(i)-gety(q[tt]))*(getx(q[tt])-getx(q[tt-1]))<=(gety(q[tt])-gety(q[tt-1]))*(getx(i)-getx(q[tt]))) tt--;
        q[++tt]=i;
    }
    write(f[n]);
    return 0;

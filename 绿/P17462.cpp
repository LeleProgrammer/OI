...
inline void solve(int st) {
    ...
    while (!q.empty()) {
        ...
        flag[u]=true;
        _for(e,vec[u]) {
            int v=e.v,nex=min(e.l,dis[u]-e.t);
            if (dis[v]<nex && !flag[v]) q.emplace(pii{dis[v]=nex,v});
        }
    }
}
...
int main() {
    ...
    while (m--) {
        int u,v,l,t;
        read(u,v,l,t);
        vec[v].emplace_back(node{u,l,t});
    }
    _rep(i,1,n) {
        solve(i);
        _rep(j,1,n) arr[j][i]=dis[j];
    }
    while (q--) {
        int x,y,s;
        read(x,y,s);

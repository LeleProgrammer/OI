...
inline pii solve(int k1,int b1,int k2,int b2) {
    if (k1==k2 && b1==b2) return {inf,inf};
    if (k1==k2) return {inf,0};
    int a=b2-b1,b=k1-k2;
    int k=1;
    if (a<0) k=-k,a=-a;
    if (b<0) k=-k,b=-b;
...
i32 main() {
    read(n,q);
    _rep(i,1,n) read(k[i],b[i]);
    _rep(i,1,n-1) p[i]=solve(k[i],b[i],k[i+1],b[i+1]);
    _rep(i,1,n-1) if (p[i]!=pii{inf,inf} && p[i]!=pii{inf,0}) {
        int j=i;
        while (j+1<=n && (p[j]==p[i] || p[j]==pii{inf,inf})) ++j;
        _rep(t,i,j-1) nex[t]=j;
        i=j-1;
    }
    _rep(i,1,n-1) {
        int j=i;
        while (j+1<=n && p[j]==pii{inf,inf}) ++j;
        _rep(t,i,j-1) suf[t]=j;
        if (i==j) suf[j]=j,i=j;
        else i=j-1;
    }
    while (q--) {
        int l,r;
        read(l,r);
        if (suf[l]>=r || r<=nex[suf[l]]) puts("Yes");
        else puts("No");
    }
    return 0;
}

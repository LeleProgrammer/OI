...
int main() {
    ...
    read(n);
    _rep(i,1,8) _rep(j,1,8) read(a[i][j]);
    _rep(i,1,8) _rep(j,1,8) p[i][j]=p[i-1][j]+p[i][j-1]-p[i-1][j-1]+a[i][j];
    _rep(i,1,8) _rep(j,i,8) _rep(x,1,8) _rep(y,x,8) f[i][j][x][y][1]=p2(p[j][y]-p[i-1][y]-p[j][x-1]+p[i-1][x-1]);
    _rep(k,2,n) _rep(i,1,8) _rep(j,i,8) _rep(x,1,8) _rep(y,x,8) {
        _rep(t,i,j-1) chmin(f[i][j][x][y][k],min(f[i][t][x][y][1]+f[t+1][j][x][y][k-1],f[i][t][x][y][k-1]+f[t+1][j][x][y][1]));
        _rep(t,x,y-1) chmin(f[i][j][x][y][k],min(f[i][j][x][t][1]+f[i][j][t+1][y][k-1],f[i][j][x][t][k-1]+f[i][j][t+1][y][1]));
    }
    write(f[1][8][1][8][n]);
    return 0;
}
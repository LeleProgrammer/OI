...
i32 main() {
    ...
    _rep(tt,1,T) {
        ...
        int ans=0,cnt=0;
        _rep(i,1,n) {
            int q=arr[i].q,l=arr[i].l,v=arr[i].v;
            if (cnt+q<=l*x) {
                ans+=q*v;
                cnt+=q;
                st.emplace(pii{v,q});
                ...
                q-=inc,cnt+=inc;
                ans+=inc*v;
                st.emplace(pii{v,inc});
                while (q && !st.empty() && st.top().first<v) {
                    auto tp=st.top(); st.pop();
                    int val=tp.first,tot=tp.second;
                    int dec=min(tot,q);
                    tot-=dec,q-=dec;
                    ans-=dec*val;
                    ans+=dec*v;
                    st.emplace(pii{v,dec});
                    if (tot) st.emplace(pii{val,tot});
                }
            }
        }
        printf("Case #%lld: %lld\n",tt,ans);

...
inline bool check(i128 mid) {
    vector<int> nums;
    while (mid) nums.emplace_back(mid%10),mid/=10;
    i128 cur=0,cnt=0;
    _rrep(i,(int)nums.size()-1,0) {
        _rep(j,0,nums[i]-1) {
            _rep(k,0,i) cnt+=(cur+k+(j==x))*C(i,k)*ksm(9,i-k);
            if (cnt>y) return false;
        }
        if (nums[i]==x) cur++;
    }
    _for(t,nums) if (t==x) cnt++;
    return cnt<=y;
}

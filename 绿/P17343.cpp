...
inline int init(int u,int d) {
    if (vl[u]==0) return 0;
    else if (vl[u]==-1) return 1;
    char cur;
    if (d&1) cur=ch;
    else if (ch=='A') cur='O';
    else cur='A';
    ...
    if (vl[u]==0) {
        ...
        _graph(i,u) {
            ...
            if (cur=='A') {
                g[u][1]+=g[e[i]][1];
                g[u][0]+=g[e[i]][1];
                vec.emplace_back(g[e[i]][0]-g[e[i]][1]);
            } else {
                g[u][0]+=g[e[i]][0];
                g[u][1]+=g[e[i]][0];
                vec.emplace_back(g[e[i]][1]-g[e[i]][0]);
            }
        }
        _sort(vec);
        if (cur=='A') {
            g[u][0]+=vec[0];
            _rep(i,1,(int)vec.size()-1) if (vec[i]<=0) g[u][0]+=vec[i];
        } else {
            g[u][1]+=vec[0];
            _rep(i,1,(int)vec.size()-1) if (vec[i]<=0) g[u][1]+=vec[i];
        }
    }
}
...
int main() {
    ...
    _rep(i,1,n) {
        ...
        while (rem--) {
            ...
            else _for(c,s) val=val*10+c-'0';
            vec[i].emplace_back(val);
            id[i].emplace_back(++idx);
            vl[idx]=val;
            if (val>0) nex+=val;
        }
        rem=nex;
    ...
    _rep(i,1,n) {
        ...
        _rep(j,0,(int)vec[i].size()-1) {
            ...
            _rep(k,1,vec[i][j]) add(u,id[i+1][ptr++]);
        }
    }
    int v=init(1,1);
    dfs(1,1);

...
inline int dfs(int r,int b,int mnr) {
    if (~f[r][b][mnr]) return f[r][b][mnr];
    if (r<mnr) return f[r][b][mnr]=0;
    int cur=0;
    f[r][b][mnr]=0;
    _rep(i,1,inf) {
        cur+=i-1;
        ...
        if (mnr*i>r) break;
        if (!mnr && i==1) continue;
        chmax(f[r][b][mnr],dfs(r-mnr*i,b-cur,mnr+1)+i-!mnr);
    }
    return f[r][b][mnr];
}

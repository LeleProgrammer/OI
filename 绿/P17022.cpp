...
inline const pii& suf(set<pii>& mp) {
    return *prev(mp.end());
}

inline const pii& suf2(set<pii>& mp) {
    return *prev(prev(mp.end()));
}

int main() {
    read(n);
    _rep(i,1,n) _rep(j,1,n) read(a[i][j]),nums.emplace_back(a[i][j]);
    _sort(nums),_unique(nums);
    _rep(i,1,n) _rep(j,1,n) a[i][j]=LB(nums,a[i][j])+1;
    _rep(x,1,n) {
        memset(mp0,0,sizeof(mp0));
        memset(mp1,0,sizeof(mp1));
        vec0.clear(),vec1.clear();
        vec0.emplace(pii{0,0});
        vec1.emplace(pii{0,-1});
        cnt0=cnt1=0;
        _rep(y,1,n) {
            _rep(i,1,x) {
                if ((i+y)&1) {
                    cnt1++;
                    vec1.erase(pii{mp1[a[i][y]],a[i][y]});
                    mp1[a[i][y]]++;
                    vec1.emplace(pii{mp1[a[i][y]],a[i][y]});
                } else {
                    cnt0++;
                    vec0.erase(pii{mp0[a[i][y]],a[i][y]});
                    mp0[a[i][y]]++;
                    vec0.emplace(pii{mp0[a[i][y]],a[i][y]});
                }
                while (vec1.size()>=4) vec1.erase(vec1.begin());
                while (vec0.size()>=4) vec0.erase(vec0.begin());
            }
            if (suf(vec0).second!=suf(vec1).second) {
                writesp((cnt0+cnt1)-(suf(vec0).first+suf(vec1).first));
            } else {
                int ans1=(cnt0+cnt1)-(suf(vec0).first+suf2(vec1).first);
                int ans2=(cnt0+cnt1)-(suf2(vec0).first+suf(vec1).first);
                writesp(min(ans1,ans2));
            }
        }
        putchar(10);
    }

...
i32 main() {
    ...
    while (k--) {
        ...
        if (x>n || y>n) ans++;
            ...
            if (find(x)==find(n+y) || find(y)==find(n+x)) ans++;
            else merge(x,y),merge(n+x,n+y),merge(n*2+x,n*2+y);
        } else {
            if (find(x)==find(y) || find(x)==find(n+y)) ans++;
            else merge(x+n,y),merge(n*2+x,n+y),merge(x,n*2+y);
        }
    }
    write(ans);
    return 0;

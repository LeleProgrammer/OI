...
u64 pw[N];
char s[N];
u64 pre1[N],pre2[N],suf1[N],suf2[N];
...
int main() {
    ...
    _rep(tt,1,T) {
        ...
        readstr(s+1);
        suf1[n+1]=suf2[n+1]=0;
        _rep(i,1,n) pre1[i]=pre1[i-1]*P+s[i]-'a'+1;
        _rep(i,1,n) pre2[i]=pre2[i-1]+pw[i-1]*(s[i]-'a'+1);
        _rrep(i,n,1) suf1[i]=suf1[i+1]*P+s[i]-'a'+1;
        _rrep(i,n,1) suf2[i]=suf2[i+1]+pw[n-i]*(s[i]-'a'+1);
        _rep(i,1,n) if (pre1[i]==pre2[i] && suf1[i+1]==suf2[i+1]) {
            printf("Case #%d: ",tt);
            _rep(j,1,i) putchar(s[j]);
            putchar(10);
            break;
        }
    }

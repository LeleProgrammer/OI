...
i32 main() {
    read(n,q);
    _rep(i,1,n) read(a[i]);
    _rep(j,0,M-1) _rep(i,1,n) p[j][i]=(p[j][i-1]^kthbit(a[i],j));
    while (q--) {
        int op,k,l,r;
        read(op,k,l,r);
        if (!op) {
            int ans=0;
            _rep(j,0,M-1) if (kthbit(k,j)) ans^=((1LL<<j)*(p[j][r]^p[j][l-1]));
            writeln(ans);
        } else {
            int ans=0;
            _rep(j,0,M-1) {
                if (kthbit(k,j)) ans^=((1LL<<j)*((r-l+1)&1));
                else ans^=((1LL<<j)*(p[j][r]^p[j][l-1]));
            }
            writeln(ans);
        }
    }

...
int n,m,B,q;
string ans[N];
vector<piii> vecx[N],vecy[N];
vector<piii> qx[N],qy[N];
...
inline pii query(int u,int l,int r) {
    if (tr[u].l>=l && tr[u].r<=r) return {tr[u].mn,tr[u].mx};
    pushdown(u);
    int mid=tr[u].l+tr[u].r>>1;
    pii ans={inf,-inf};
    if (l<=mid) {
        auto ret=query(u<<1,l,r);
        chmin(ans.first,ret.first);
        chmax(ans.second,ret.second);
    }
    if (r>mid) {
...
int main() {
    ...
    while (B--) {
        int x1,y1,x2,y2;
        read(x1,y1,x2,y2);
        vecx[x1].emplace_back(piii{1,{y1,y2}});
        vecx[x2+1].emplace_back(piii{-1,{y1,y2}});
        vecy[y1].emplace_back(piii{1,{x1,x2}});
        vecy[y2+1].emplace_back(piii{-1,{x1,x2}});
    }
    read(q);
    _rep(i,1,q) {
        int op,c,l,r;
        read(op,c,l,r);
        if (op==1) qx[c].emplace_back(piii{i,{l,r}});
        else qy[c].emplace_back(piii{i,{l,r}});
    }
    build(1,1,m);
    _rep(i,1,n) {
        _for(t,vecx[i]) {
        ...
        _for(t,qx[i]) {
            ...
            if (!ret.second) ans[id]="MISS";
            else if (ret.first) ans[id]="SUNK";
            else ans[id]="HIT";
        }
    }
    build(1,1,n);

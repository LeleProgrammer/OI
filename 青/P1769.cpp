...
inline map<int,f128> solve(int l,int r) {
    map<int,f128> res;
    if (l==r) {
        res[l]=1;
        return res;
    ...
    _for(i,L) _for(j,R) {
        res[i.first]+=(f128)p[i.first][j.first]/100.0*i.second*j.second;
        res[j.first]+=(f128)p[j.first][i.first]/100.0*i.second*j.second;
    }
    return res;
}

int comp(const f128& a,const f128& b) {
...
i32 main() {
    read(n);
    _rep(i,1,1<<n) _rep(j,1,1<<n) read(p[i][j]);
    auto res=solve(1,1<<n);
    int ans=1;
    _for(t,res) {
        int sgn=comp(res[ans],t.second);
        if (!sgn) chmin(ans,t.first);
        else if (sgn<0) ans=t.first;
    }
    write(ans);
    return 0;
}
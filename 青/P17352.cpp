...
i32 main() {
    ...
    f[1][0][1][0][1][0]=1;
    f[1][0][0][1][2][0]=1;
    _rep(i,2,n) _rep(a,0,i) _rep(b,0,i) if (a+b<=i) {
        int c=i-a-b;
        _rep(t,0,2) {
            if (t==0 && !a) continue;
            if (t==1 && !b) continue;
            ...
            _rep(s,0,2) {
                if (t!=s) f[i][a][b][c][t][0]+=f[i-1][pa][pb][pc][s][0]+f[i-1][pa][pb][pc][s][1];
                else if (t==1) f[i][a][b][c][t][1]+=f[i-1][pa][pb][pc][s][0];
            }
            if (i==n && max3(a,b,c)-min3(a,b,c)<=d) ans+=f[i][a][b][c][t][0]+f[i][a][b][c][t][1];
        }
    }
    if (n&1) {
        memset(f,0,sizeof(f));
        ...
        _rep(i,2,n) _rep(a,0,i) _rep(b,0,i) if (a+b<=i) {
            ...
            _rep(t,0,2) {
                ...
                if (i==n && max3((a<<1)-(t==0),(b<<1)-(t==1),(c<<1)-(t==2))-min3((a<<1)-(t==0),(b<<1)-(t==1),(c<<1)-(t==2))<=d) ans-=f[i][a][b][c][t][0];
            }
        }
    } else {
        memset(f,0,sizeof(f));
        ...
        _rep(i,2,n) _rep(a,0,i) _rep(b,0,i) if (a+b<=i) {
            ...
            _rep(t,0,2) {
                ...
                if (i==n && t==1 && max3(a<<1,b<<1,c<<1)-min3(a<<1,b<<1,c<<1)<=d) ans-=f[i][a][b][c][t][0];
            }
        }
        if (n==1 && d>=2) ans--;
    }

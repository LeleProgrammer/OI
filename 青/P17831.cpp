...
inline void pushdown(int u) {
    ...
    if (~root.set) {
        ...
        left.mn=root.set;
        right.mn=root.set;
        left.sum=(left.r-left.l+1)*root.set;
        right.sum=(right.r-right.l+1)*root.set;
        root.set=-1;
    }
}
...
inline int bina(int u,int p,int k) {
    if (tr[u].l==tr[u].r) return tr[u].l;
    pushdown(u);
    int mid=tr[u].l+tr[u].r>>1;
    if (p<=mid) return bina(u<<1,p,k);
    if (tr[u<<1|1].mn>k) return bina(u<<1,p,k);
    else return bina(u<<1|1,p,k);
}
...
i32 main() {
    ...
    int nex=0;
    _rep(i,1,n) {
        if (las[a[i]] && query(1,las[a[i]],las[a[i]])>i-las[a[i]]) {
            int pos=bina(1,las[a[i]],i-las[a[i]])+1;
            modify(1,pos,las[a[i]],i-las[a[i]]);
            chmax(nex,las[a[i]]);
        }
        las[a[i]]=i;
        if (nex) ans+=query(1,1,nex);
    }
    write(ans);
    return 0;
}
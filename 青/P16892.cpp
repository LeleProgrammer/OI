...
inline int query(int u,int k) {
    ...
    else if (k==tr[u<<1].cnt) return tr[u<<1].sum;
    else return tr[u<<1].sum+query(u<<1|1,k-tr[u<<1].cnt);
}
...
int main() {
    ...
    _rep(tt,1,T) {
        read(n,k,x,d);
        _rep(i,0,d) vecl[i].clear(),vecr[i].clear();
        _rep(i,1,n) cnt[i]=0;
        read(m);
        _rep(i,1,m) {
            int p,l,r;
            read(p,l,r);
            vecl[r].emplace_back(p);
            vecr[l+1].emplace_back(p);
        }
        build(1,0,m);
        _rep(i,0,x-1) _for(t,vecr[i]) {
            modify(1,cnt[t],-1);
            cnt[t]++;
            modify(1,cnt[t],1);
        }

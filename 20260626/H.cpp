...
i32 main() {
    ...
    while (q--) {
        ...
        if (op==1) {
            ...
            bool ok=true;
            _rep(i,x,x+h-1) _rep(j,y,y+w-1) if (arr[i][j]) ok=false;
            if (!ok) puts("Error");
            else {
                ++idx;
                _rep(i,x,x+h-1) _rep(j,y,y+w-1) arr[i][j]=idx;
                writeln(idx);
            }
        } else if (op==2) {
            int id;
            read(id);
            bool ok=false;
            _rep(i,1,n) _rep(j,1,m) if (arr[i][j]==id) ok=true;
            if (!ok) {
                puts("Error");
                continue;
            }
            _rep(i,1,n) _rep(j,1,m) if (arr[i][j]==id) arr[i][j]=0;
            puts("Remove");
        } else {
            int x,y;
            read(x,y);
    ...
    set<int> st;
    _rep(i,1,n) _rep(j,1,m) if (arr[i][j]) st.emplace(arr[i][j]);
    int ans=0;

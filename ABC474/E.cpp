...
i32 main() {
    ...
    while (T--) {
        read(n); m=n>>1;
        _rep(i,1,n) read(arr[i].a,arr[i].b),arr[i].flag=false;
        int sum=0;
        _rep(i,1,n) sum+=arr[i].a;
        int mna=inf;
        _rep(i,1,n) chmin(mna,arr[i].a);
        _sorta(arr,1,n);
        int ext=n-m*2;
        _rep(i,1,n) {
            if (i<=m) sum+=arr[i].b-arr[i].a;
            else {
                if (ext) {
                    if (arr[i].b+mna<arr[i].a) sum=sum-arr[i].a+arr[i].b+mna;
                    ext=0;
                } else {
                    if (arr[i].b+mna*2<arr[i].a) sum=sum-arr[i].a+arr[i].b+mna*2;
                }
            }
        }
        writeln(sum);

...
i32 main() {
    ...
    _rep(i,1,n) _rep(j,1,m) (suma[i]+=a[i][j])%=mod;
    _rep(k,1,m) {
        memset(f,0,sizeof(f));
        f[0][n+10]=1;
        _rep(i,1,n) _rep(j,-i-5,i+5) f[i][j+n+10]=(f[i-1][j+n+10]+f[i-1][j+n+10-1]*a[i][k]+f[i-1][j+n+10+1]*(suma[i]-a[i][k]))%mod;
        _rep(i,1,n) (ans+=f[n][i+n+10])%=mod;
    }
    int tot=1;
    _rep(i,1,n) (tot*=suma[i]+1)%=mod;
    tot--;
    ans=tot-ans;
    ans=(ans%mod+mod)%mod;
    write(ans);

...
inline void init() {
    pre[1]=0;
    _rep(i,2,n) if (s[i]==s[i-1]) pre[i]=pre[i-1]; else pre[i]=i-1;
    nex[n]=n+1;
    _rrep(i,n-1,1) if (s[i]==s[i+1]) nex[i]=nex[i+1]; else nex[i]=i+1;
}
...
int main() {
    ...
    while (T--) {
        ...
        while (l<=r) {
            if (l==r) putchar(s[l++]);
            else if (s[l]<s[r]) putchar(s[l++]);
            else if (s[r]<s[l]) putchar(s[r--]);
            else {
                int L=nex[l],R=pre[r];
                if (L>R) {
                    int t=r-l+1;
                    int cnt=t/2+1;
                    while (cnt--) putchar(s[l]);
                    break;
                }
                if (s[L]<s[l] && s[R]<s[r]) while (l<L && r>R) putchar(s[l++]),r--;
                else if (s[L]>s[l] && s[R]>s[r]) {
                    while (l<L) putchar(s[l++]);
                    while (r>R) putchar(s[r--]);
                } else if (s[L]<s[l] && s[R]>s[r]) while (l<L) putchar(s[l++]);
                else while (r>R) putchar(s[r--]);
            }
        }
        putchar(10);
    }

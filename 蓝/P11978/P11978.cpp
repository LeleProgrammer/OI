...
vector<pii> encode_map(int n,int k,int& x,vector<pii> E) {
    ...
    _for(t,E) add(t.first,t.second),add(t.second,t.first),deg[t.first]++,deg[t.second]++;
    dfs(1,0,1);
    int mx=0;
    _rep(i,1,n) if (deg[i]==1) {
        if (!mx) mx=i;
        else if (dep[i]>dep[mx]) mx=i;
    }
    int yes=0,no=0;
    if (dep[mx]>=3) {
        int g=p[p[mx]];
        if (deg[g]==k+1) yes=g;
        else if (deg[g]==k) no=g;
    }
    vector<pii> ret;
    if (yes) ret.emplace_back(pii{mx,yes}),k--;
    int i=1;
    while (k) {
        if (i==p[mx] || i==mx || (n>4 && i==no) || i==yes) i++;
        else ret.emplace_back(pii{mx,i++}),k--;
    }
    x=p[mx];
    return ret;
}
...
vector<pii> decode_map(int n,int k,int x,vector<pii> E) {
    ...
    _for(t,E) add(t.first,t.second),add(t.second,t.first),deg[t.first]++,deg[t.second]++;
    int u;
    _graph(i,x) if (deg[e[i]]==k+1) {
        u=e[i];
        break;
    }
    _graph(i,u) if (e[i]!=x) flag[i]=flag[i^1]=true;
    vector<pii> ret;
    _reps(i,0,tot-1,i+=2) if (!flag[i]) ret.emplace_back(pii{e[i],e[i^1]});
    return ret;
}
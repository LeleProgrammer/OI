...
struct val {
    ...
    int cnt0,cnt1;
    int flp;
    val(): l(0),r(0),cnt0(0),cnt1(0),flp(0) {}
};
...
inline void modify(val& u,const vi2& k) {
    if (k.a==0) {
        if (k.b) u.cnt1=1;
        else u.cnt0=1;
    } else {
...
SEG<val,pushup,pushdown,vi2,modify,0> seg[6];

int main() {
    _rep(i,0,5) seg[i].reserve(N);
    read(n,m);
    _rep(i,0,n-1) {
        read(arr[i]);
        nums[i%6].emplace_back(i);
        seg[i%6].modify(1,nums[i%6].size()-1,nums[i%6].size()-1,vi2{0,arr[i]});
    }
    while (m--) {
        int a,x,y;
        read(a,x,y),x--,y--;
        if (a==1) {
            _reps(i,x,x+5,i+=2) {
                int l=LB(nums[i%6],i);
                int r=UB(nums[i%6],y)-1;
                if (l<=r) seg[i%6].modify(1,l,r,vi2{1,0});
            }
        } else if (a==2) {
            _reps(i,x,x+5,i+=3) {
                int l=LB(nums[i%6],i);
            ...
            _rep(i,x,x+5) {
                int l=LB(nums[i%6],i);
                int r=UB(nums[i%6],y)-1;
                if (l<=r) ans+=seg[i%6].query(1,l,r).cnt1;
            }
            writeln(ans);
        }
    }

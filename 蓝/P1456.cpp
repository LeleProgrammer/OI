...
int main() {
    while (~scanf("%d",&n)) {
        DSU dsu(N,merge);
        _rep(i,1,n) st[i].clear();
        _rep(i,1,n) read(s[i]),st[i].emplace(s[i]),sz[i]=1;
        read(m);
        while (m--) {
            int x,y;
            read(x,y);
            if (dsu.connected(x,y)) puts("-1");
            else {
                ...
                st[y].erase(prev(st[y].end()));
                st[x].emplace(a>>1);
                st[y].emplace(b>>1);
                _for(g,st[y]) st[x].emplace(g);
                dsu.merge(x,y);
                writeln(*prev(st[x].end()));
            }
        }
    }
    return 0;

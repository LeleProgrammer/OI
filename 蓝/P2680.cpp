...
inline bool check(int mid) {
    ...
    _rep(t,1,n) {
        int u=rec[t];
        _graph(i,u) if (e[i]!=fa[u]) g[u]+=g[e[i]];
        if (g[u]==cnt) chmax(ans,pre[u]-pre[fa[u]]);
    }
    _rep(i,1,m) if (arr[i].c-ans>mid) return false;
    return true;
}
...
int main() {
    ...
    LCA::dfs(1,0,1);
    _rep(i,1,n) rec[i]=i;
    sort(rec+1,rec+1+n,[&](const int& a,const int& b) {
        return LCA::dep[a]>LCA::dep[b];
    });
    dfs(1,0);
    _rep(i,1,m) {
        read(arr[i].u,arr[i].v);
        arr[i].g=lca(arr[i].u,arr[i].v);
        arr[i].c=pre[arr[i].u]+pre[arr[i].v]-pre[arr[i].g]*2;
        // cout<<"test: "<<i<<" "<<arr[i].g<<" "<<arr[i].c<<endl;
    }
    int l=0,r=1e9;
    while (l<r) {
        int mid=l+r>>1;

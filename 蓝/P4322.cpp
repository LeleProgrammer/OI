...
inline void dfs(int u) {
    ...
    _graph(i,u) {
        dfs(e[i]);
        _rrep(j,sz[u],0) _rep(t,0,sz[e[i]]) chmax(f[u][j+t],f[u][j]+f[e[i]][t]);
        sz[u]+=sz[e[i]];
    }
    if (u) _rrep(i,sz[u],1) f[u][i]=f[u][i-1]+g[u];
}

inline bool check(double mid) {
    _rep(i,0,n) _rep(j,0,n) f[i][j]=-inf;
    _rep(i,1,n) g[i]=p[i]-mid*s[i];
    dfs(0);
    return f[0][k]>=0;
}

...
i32 main() {
    ...
    _rep(i,1,q) {
        ...
        arr[i]={r1,r2,i};
        arr[i+q]={r1,l2-1,i+q};
        arr[i+2*q]={l1-1,r2,i+2*q};
        arr[i+3*q]={l1-1,l2-1,i+3*q};
    }
    _sorta(arr,1,q*4);
    int i=0,j=0;
    _rep(t,1,q*4) {
        int l=arr[t].l,r=arr[t].r,id=arr[t].i;
        while (i<l) add1(a[++i]);
        while (i>l) del1(a[i--]);
        while (j<r) add2(a[++j]);
        while (j>r) del2(a[j--]);
    ...
    _rep(i,1,q) writeln(ans[i]-ans[i+q]-ans[i+2*q]+ans[i+3*q]);
    return 0;
}
...
= [r1,r2] - [r1,l2-1] - [l1-1,r2] + [l1-1,l2-1]
*/
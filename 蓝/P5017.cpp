...
i32 main() {
    ...
    _rep(i,1,N-1) g[i]=g[i-1]+cnt[i]*i,p[i]=p[i-1]+cnt[i];
    q[++tt]=0;
    _rep(i,1,N-1) {
        if (i-m>=1) {
            while (tt-hh>=1 && (Y(i-m)-Y(q[tt]))*(X(q[tt])-X(q[tt-1]))<=(Y(q[tt])-Y(q[tt-1]))*(X(i-m)-X(q[tt]))) tt--;
            q[++tt]=i-m;
        }
        while (tt-hh>=1 && Y(q[hh])-K(i)*X(q[hh])>=Y(q[hh+1])-K(i)*X(q[hh+1])) hh++;
        f[i]=Y(q[hh])-K(i)*X(q[hh])+p[i]*i-g[i];
    }
    write(f[N-1]);
    return 0;
}
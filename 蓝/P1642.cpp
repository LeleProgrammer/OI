...
inline void dfs(int u,int fa) {
    ...
    _graph(i,u) {
        ...
        dfs(e[i],u);
        sz[u]+=sz[e[i]];
        _rrep(j,sz[u],0) _rep(t,0,min(sz[e[i]],j)) chmax(g[u][j],g[u][j-t]+g[e[i]][t]);
    }
    _rrep(i,sz[u],1) g[u][i]=g[u][i-1]+c[u];
}

inline bool check(double mid) {
    _rep(i,1,n) _rep(j,0,n) g[i][j]=-inf;
    _rep(i,1,n) c[i]=a[i]-mid*b[i];
    dfs(1,0);
    _rep(i,1,n) if (g[i][n-m]>=0) return true;
    return false;
}

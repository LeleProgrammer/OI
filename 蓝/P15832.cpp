...
inline void change(int u,int p,int k) {
    if (tr[u].l==p && tr[u].r==p) tr[u].mx=tr[u].sum=k;
    else {
        int mid=tr[u].l+tr[u].r>>1;
        if (p<=mid) change(u<<1,p,k);
        else change(u<<1|1,p,k);
        pushup(u);
    }
}
...
inline void modify(int u,int l,int r) {
    if (k==1) return;
    if (!tr[u].mx) return;
    if (tr[u].l==tr[u].r) tr[u].mx/=k,tr[u].sum/=k;
    else {
        int mid=tr[u].l+tr[u].r>>1;

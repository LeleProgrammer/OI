...
i32 main() {
    read(n,m);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n) seg.modify(1,i,i,a[i]-a[i-1]);
    while (m--) {
        char op[2];
        int l,r,d;
        readstr(op),read(l,r);
        if (*op=='C') {
            ...
            if (l==r) writeln(seg.query(1,1,l).sum);
            else writeln(abs(gcd(seg.query(1,1,l).sum,seg.query(1,l+1,r).g)));
        }
    }
    return 0;
}
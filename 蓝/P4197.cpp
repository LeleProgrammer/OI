...
int n,m,q,idx,root=1;
int p[N],ht[N];
int v[N];
int fa[N][K];
int dep[N];
vector<pii> vec[N]; // qid, k
int ans[M];
int rt[N];
...
inline int kth(int u,int l,int r,int k) {
    if (l==r) return l;
    int mid=l+r>>1;
    if (tr[tr[u].r].sum>=k) return kth(tr[u].r,mid+1,r,k);
    else return kth(tr[u].l,l,mid,k-tr[tr[u].r].sum);
}
...
inline void solve(int u,int fa) {
    _graph(i,u) {
        if (e[i]==fa) continue;
        solve(e[i],u);
        merge(rt[u],rt[e[i]],1,V);
    }
    if (u<=n) insert(rt[u],1,V,ht[u]);
    _for(t,vec[u]) {
        int id=t.first,k=t.second;
        if (tr[rt[u]].sum<k) ans[id]=-1;
        else ans[id]=kth(rt[u],1,V,k);
    }
}
...
int main() {
    ...
    _rep(i,1,q) {
        int u,x,k;
        read(u,x,k);
        _rrep(i,K-1,0) if (fa[u][i] && v[fa[u][i]]<=x) u=fa[u][i];
        vec[u].emplace_back(pii{i,k});
    }
    solve(root,0);

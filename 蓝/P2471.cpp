...
inline void build() {
    _rep(i,1,n) mx[i][0]=arr[i];
    _rep(j,1,K-1) _rep(i,1,n) {
        mx[i][j]=mx[i][j-1];
        if (i+(1<<j)<=n) chmax(mx[i][j],mx[i+(1<<(j-1))][j-1]);
    }
}
...
int main() {
    ...
    _rep(i,1,n) read(y[i],arr[i]),vec.emplace_back(y[i]);
    _sort(vec),_unique(vec);
    _rep(i,1,n) y[i]=LB(vec,y[i]);
    build();
    read(m);
    while (m--) {
        int l,r;
        ...
        int R=LB(vec,r)-1;
        int mid=query(L,R);
        if (vec[L-1]==l && vec[R+1]==r) {
            if ((R+1)-(L-1)+1==r-l+1 && mid<arr[R+1] && arr[L-1]>=arr[R+1]) puts("true");
            else if (arr[L-1]<arr[R+1]) puts("false");
            else if (mid>=arr[R+1]) puts("false");
            else puts("maybe");
        } else if (vec[L-1]==l && vec[R+1]!=r) {
            if (mid>=arr[L-1]) puts("false");
            else puts("maybe");
        } else if (vec[L-1]!=l && vec[R+1]==r) {
            if (mid>=arr[R+1]) puts("false");
            else puts("maybe");
        } else puts("maybe");
    }
    return 0;

...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,1,idx) nums.emplace_back(arr[i].l),nums.emplace_back(arr[i].r);
        nums.emplace_back(-inf);
        _sort(nums),_unique(nums);
        _rep(i,1,idx) arr[i].l=LB(nums,arr[i].l),arr[i].r=LB(nums,arr[i].r);
        sort(arr+1,arr+1+idx);
        int ans=0;
        _rep(i,1,idx) {
            int j=i;
            while (j+1<=idx && arr[j+1].pos==arr[i].pos) ++j;
            _rep(k,i,j) seg.modify(1,arr[k].l,arr[k].r,arr[k].d);
            chmax(ans,seg.tr[1].mx);
            i=j;
        }
        writeln(ans);
    }

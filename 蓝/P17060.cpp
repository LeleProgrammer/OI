...
char s[N];
char t[N];
int poss[505][26]; // 0: impos' 1: pos' 2: conf'
pii dat[26]; // {0,c}: >=c {1,c}: ==c
pii cnt[26]; // {G/Y,B}
int eid[505][26];
char ans[505];
int in[N],out[N];

constexpr inline int get(const int& i,const int& c) {
    return i*26+c+1;
}

constexpr inline int get2(const int& i) {
    return (m+1)*26+i;
}

int main() {
    // FRR("P17060_1.in");
    Graph::init_graph(N,M,true);
    DINIC::init();
    S=N-1,T=N-2;
    read(n,m);
    SS=(m+1)*26+m+1,TT=(m+1)*26+m+2;
    _rep(i,1,m) _rep(j,0,25) poss[i][j]=1;
    _rep(tt,1,n-1) {
        readstr(s+1);
        readstr(t+1);
        memset(cnt,0,sizeof(cnt));
        _rep(i,1,m) {
            if (t[i]=='G') poss[i][s[i]-'a']=2;
            else poss[i][s[i]-'a']=0;
            if (t[i]=='Y' || t[i]=='G') cnt[s[i]-'a'].first++;
            else if (t[i]=='B') cnt[s[i]-'a'].second++;
        }
        _rep(i,0,25) if (cnt[i].first || cnt[i].second) {
            if (cnt[i].second) dat[i]={1,cnt[i].first};
            else if (!dat[i].first) chmax(dat[i].second,cnt[i].first);
        }
    }
    _rep(i,1,m) {
        bool two=false;
        _rep(j,0,25) if (poss[i][j]==2) {
            dat[j].second--;
            two=true;
            break;
        }
        if (!two) continue;
        _rep(j,0,25) if (poss[i][j]!=2) poss[i][j]=0;
    }
    _rep(i,1,m) ans[i]='@';
    _rep(i,0,25) {
        int mn,mx;
        if (!dat[i].first) mn=dat[i].second,mx=500;
        else mn=mx=dat[i].second;
        in[i+1]+=mn,out[SS]+=mn;
        add_flow(SS,i+1,mx-mn);
    }
    _rep(i,1,m) _rep(j,0,25) if (poss[i][j]==1) {
        int mn=0,mx=1;
        eid[i][j]=tot;
        add_flow(j+1,get(i,j),mx-mn);
        in[get(i,j)]+=mn,out[j+1]+=mn;
        add_flow(get(i,j),get2(i),mx-mn);
        out[get(i,j)]+=mn,in[get2(i)]+=mn;
    } else if (poss[i][j]==2) ans[i]='a'+j;
    _rep(i,1,m) if (ans[i]=='@') {
        int mn=1,mx=1;
        add_flow(get2(i),TT,mx-mn);
        in[TT]+=mn,out[get2(i)]+=mn;
    }
    _rep(i,1,(m+1)*26+m+2) {
        if (in[i]>out[i]) add_flow(S,i,in[i]-out[i]);
        else add_flow(i,T,out[i]-in[i]);
    }
    add_flow(TT,SS,inf);
    dinic();
    S=SS,T=TT;
    dinic();
    _rep(i,1,m) if (ans[i]=='@') _rep(j,0,25) if (eid[i][j] && !f[eid[i][j]]) ans[i]='a'+j;
    _rep(i,1,m) putchar(ans[i]);
    return 0;
}
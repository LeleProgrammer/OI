...
int main() {
    ...
    DINIC::init();
    read(n),S=N-1,T=N-2;
    _rep(i,1,n) _rep(j,1,n) read(arr[get(i,j)]);
    _rep(i,1,n) _rep(j,1,n) {
        if (arr[get(i,j)]>1) add_flow(S,get(i,j),arr[get(i,j)]-1);
        else if (!arr[get(i,j)]) add_flow(get(i,j),T,1),cnt++;
    }
    _rep(i,1,n) _rep(j,1,n) {
        if (arr[get(i,j)]>1) {
            add_flow(get(i,j),n*n+i,inf);
            add_flow(get(i,j),n*n+n+j,inf);
        } else if (!arr[get(i,j)]) {
            add_flow(n*n+i,get(i,j),inf);
            add_flow(n*n+n+j,get(i,j),inf);
        }
    }
    write(cnt*2-dinic());
    return 0;

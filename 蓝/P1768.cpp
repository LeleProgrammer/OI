...
inline void add2(int u,int v,int A,int B) {
    e[tot]=v,a[tot]=A,b[tot]=B,ne[tot]=h[u],h[u]=tot++;
}

inline bool check(double mid) {
    _rep(i,1,n+1) dis[i]=inf,flag[i]=false,cnt[i]=0;
    queue<int> q;
    dis[ss]=0,flag[ss]=true; q.emplace(ss);
    while (!q.empty()) {
        int u=q.front(); q.pop();
        flag[u]=false;
        cnt[u]++;
        if (cnt[u]>100) return true;
        _graph(i,u) {
            double w=-(a[i]-mid*b[i]);
            if (dis[e[i]]>dis[u]+w) {
                dis[e[i]]=dis[u]+w;
                if (!flag[e[i]]) {
                    flag[e[i]]=true;

...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,m) if (arr[i][j]==2) {
        cnt2++;
        _rep(p,0,3) {
            int nx=i+dx4[p];
            int ny=j+dy4[p];
    ...
    _rep(i,1,n) _rep(j,1,m) if (flag[i][j]) arr[i][j]=1;
    _rep(i,1,n) _rep(j,1,m) if (!arr[i][j]) {
        cnt0++;
        if ((i+j)&1) {
        ...
        _rep(p,0,3) {
            ...
            int ny=j+dy4[p];
            if (nx<1 || ny<1 || nx>n || ny>m) continue;
            add_flow(get(i,j),get(nx,ny),inf);
        }
    }
    write(cnt2+cnt0-dinic());
    return 0;
}
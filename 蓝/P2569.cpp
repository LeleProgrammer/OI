...
i32 main() {
    _rep(i,0,N-1) _rep(j,0,N-1) f[i][j]=-inf;
    read(n,m,w);
    _rep(i,1,n) read(ap[i],bp[i],as[i],bs[i]);
    f[0][0]=0;
    _rep(i,1,n) {
        _rep(j,0,as[i]) chmax(f[i][j],-ap[i]*j);
        _rep(j,0,m) chmax(f[i][j],f[i-1][j]);
        if (i-w-1<0) continue;
        int cur=m;
        deque<int> q;
        _rrep(j,m,0) {
            while (cur>=j-as[i] && ~cur) {
                while (!q.empty() && f[i-w-1][q.front()]+q.front()*ap[i]<=f[i-w-1][cur]+cur*ap[i]) q.pop_front();
                q.emplace_front(cur);
                cur--;
            }
            while (!q.empty() && q.back()>j) q.pop_back();
            if (q.empty()) continue;
            chmax(f[i][j],f[i-w-1][q.back()]+q.back()*ap[i]-j*ap[i]);
        }
        while (!q.empty()) q.pop_front();
        cur=0;
        _rep(j,0,m) {
            while (cur<=j+bs[i] && cur<=m) {
                while (!q.empty() && f[i-w-1][q.front()]+q.front()*bp[i]<=f[i-w-1][cur]+cur*bp[i]) q.pop_front();
                q.emplace_front(cur);
                cur++;
            }
            while (!q.empty() && q.back()<j) q.pop_back();
            if (q.empty()) continue;
            chmax(f[i][j],f[i-w-1][q.back()]+q.back()*bp[i]-j*bp[i]);
        }
    }
    int ans=0;
    _rep(i,0,m) chmax(ans,f[n][i]);
    write(ans);
    return 0;
}
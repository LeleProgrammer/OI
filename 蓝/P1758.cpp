...
int main() {
    ...
    readstr(b+1);
    f[0][0][0]=1;
    _rep(k,1,n+m) _rep(i,0,min(n,k)) _rep(j,0,min(n,k)) {
        f[k&1][i][j]=0;
        if (a[i]==a[j] && i && j) f[k&1][i][j]+=f[k&1^1][i-1][j-1];
        if (a[i]==b[k-j] && i) f[k&1][i][j]+=f[k&1^1][i-1][j];
        if (b[k-i]==a[j] && j) f[k&1][i][j]+=f[k&1^1][i][j-1];
        if (b[k-i]==b[k-j]) f[k&1][i][j]+=f[k&1^1][i][j];
        f[k&1][i][j]%=mod;
    }
    write(f[(n+m)&1][n][n]);
    return 0;

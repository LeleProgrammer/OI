...
inline void addl(int x) {
    if (x%4==0) cur+=cnt23[x>>1];
    cnt[x]++;
    if (x%4==0) cnt42[x>>1]+=cnt[x>>1];
    if (x%2==0) cnt23[x]+=cnt[x/2*3];
}

inline void addr(int x) {
    if (x%3==0) cur+=cnt42[x/3*2];
    cnt[x]++;
    if (x%2==0) cnt42[x]+=cnt[x*2];
    if (x%3==0) cnt23[x/3*2]+=cnt[x/3*2];
}

...
int main() {
    ...
    while (m--) {
        ...
        ptr=0;
        int root=vec[0];
        _rep(i,1,(int)vec.size()-1) root=lca(root,vec[i]);
        stk[++ptr]=root;
        dec.emplace_back(root);
        _for(u,vec) {
            if (u==root) continue;
            int g=lca(u,stk[ptr]);
            while (true) {
                if (dep[g]<=dep[stk[ptr-1]]) {
                    add(stk[ptr-1],stk[ptr]);
                    add(stk[ptr],stk[ptr-1]);
                    ptr--;
                } else break;
            }
            if (g==stk[ptr]) {
        ...
        solve(root,0);
        while (!dec.empty()) {
            int u=dec.back(); dec.pop_back();
            h[u]=-1;
        }
        tot=0;
    }
    Graph::restore_graph();
    _for(t,edg) add(t.first,t.second),add(t.second,t.first);
    solve2(1,0);
    writeln(ans);
    _rep(i,0,(int)edg.size()-1) {
        int a=edg[i].first,b=edg[i].second;
        if (dep[a]<dep[b]) swap(a,b);
        if (f[a]>=k) writesp(i+1);
    }
    return 0;
}
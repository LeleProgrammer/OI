...
i32 main() {
    ...
    if (x<=n && y>=n+1) {
        prex[0]=1;
        _rep(i,1,m) prex[i]=(prex[i-1]+calc(i,n-x))%mod;
        prey[0]=1;
        _rep(i,1,m) prey[i]=(prey[i-1]+calc(i,y-n-1))%mod;
        _rep(k,1,m) {
            int stx=calc(k-1,x);
            int edx=prex[m-k];
            int sty=prey[m-k];
            int edy=calc(k-1,n*2-y+1);
            // cout<<"test: "<<k<<" "<<stx<<" "<<edx<<" "<<sty<<" "<<edy<<endl;
            ans=(ans+stx*edx%mod*sty%mod*edy%mod)%mod;
        }
        write(ans);
    } else if (x<=n && y<=n) {
        prey[0]=1;
        _rep(i,1,m) prey[i]=(prey[i-1]+calc(i,n-y))%mod;
        int ri=0;
        _rep(k,1,m) ri=(ri+calc(k-1,n))%mod;
        _rep(k,1,m) {
            int st=calc(k-1,x);
            int ed=prey[m-k];
            ans=(ans+st*ed%mod*ri%mod)%mod;
        }
        write(ans);
    } else {
        prex[0]=1;
        _rep(i,1,m) prex[i]=(prex[i-1]+calc(i,x-n-1))%mod;
        int le=0;
        _rep(k,1,m) le=(le+calc(k-1,n))%mod;
        _rep(k,1,m) {
            int st=prex[m-k];
            int ed=calc(k-1,n*2-y+1);
            ans=(ans+st*ed%mod*le%mod)%mod;
        }
        write(ans);
    }
    return 0;

...
inline int swa(int sta,int a,int b,int c,int d) {
    a=6-a,b=6-b,c=6-c,d=6-d;
    int p=(a-1)*5+b-1,q=(c-1)*5+d-1;
    if (kthbit(sta,p)==kthbit(sta,q)) return sta;
    else return flpbit(flpbit(sta,p),q);
}

inline void dfs1(int cur,int x,int y) {
    int d=dis[x][y][cur];
    _rep(p,0,7) {
        int nx=x+dx[p];
        int ny=y+dy[p];
        if (nx<1 || ny<1 || nx>5 || ny>5) continue;
        int nex=swa(cur,x,y,nx,ny);
        if (!dis[nx][ny].count(nex) || dis[nx][ny][nex]>d+1) {
            dis[nx][ny][nex]=d+1;
            if (d!=7) dfs1(nex,nx,ny);
        }
    }
}

inline void dfs2(int cur,int x,int y,int d) {
    if (dis[x][y].count(cur)) chmin(ans,dis[x][y][cur]+d);
    _rep(p,0,7) {
        int nx=x+dx[p];
        int ny=y+dy[p];
        if (nx<1 || ny<1 || nx>5 || ny>5) continue;
        int nex=swa(cur,x,y,nx,ny);
        if (d!=7) dfs2(nex,nx,ny,d+1);
    }
}
...
int main() {
    ...
    while (T--) {
        ...
        _rep(i,1,5) _rep(j,1,5) dis[i][j].clear();
        dis[x][y][st]=0;
        ans=inf;
        dfs1(st,x,y);
        dfs2(33000480,3,3,0);

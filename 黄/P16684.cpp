...
int main() {
    ...
    while (n--) {
        int x,y,r;
        read(x,y,r);
        _rep(i,1,1000) if (i>=x-r && i<=x+r) {
            int d=sqrt(r*r-(i-x)*(i-x))+2;
            _rep(j,y-d,y+d) if ((i-x)*(i-x)+(j-y)*(j-y)<=r*r) {
                p[i][max(0,j)]++;
                break;
            }
            _rrep(j,y+d,y-d) if ((i-x)*(i-x)+(j-y)*(j-y)<=r*r) {
                if (j<=1000) p[i][j+1]--;
                break;
            }
        }
    }
    _rep(i,1,1000) _rep(j,1,1000) p[i][j]+=p[i][j-1];
    read(q);
    while (q--) {
        int x,y;
        read(x,y);

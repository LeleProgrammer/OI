...
int main() {
    read(x,y,z);
    _rep(i,0,1) _rep(j,0,N-1) _rep(k,0,N-1) _rep(t,0,2) f[i][j][k][t]=-inf;
    f[0][0][0][1]=0;
    _rep(i,0,x) {
        _rep(j,0,N-1) _rep(k,0,N-1) _rep(t,0,2) f[i&1^1][j][k][t]=-inf;
        _rep(j,0,y) _rep(k,0,z) _rep(t,0,2) {
            chmax(f[i&1^1][j][k][max(0,t-1)],f[i&1][j][k][t]+t);
            chmax(f[i&1][j+1][k][t],f[i&1][j][k][t]+t);
            chmax(f[i&1][j][k+1][min(2,t+1)],f[i&1][j][k][t]+t);
        }
    }
    write(max3(f[x&1][y][z][0],f[x&1][y][z][1],f[x&1][y][z][2]));
    return 0;
}
...
inline void solve() {
    ...
    while (!q.empty()) {
        ...
        _rep(p,0,3) {
            ...
            if (dis[nx][ny]>dis[x][y]+1 || (dis[nx][ny]==dis[x][y]+1 && mx[nx][ny]<mx[x][y]+arr[nx][ny])) {
                dis[nx][ny]=dis[x][y]+1;;
                mx[nx][ny]=mx[x][y]+arr[nx][ny];
                q.emplace(pii{nx,ny});
            }
        }
    }
...
int main() {
    ...
    _rep(tt,1,T) {
        read(n,m);
        read(sx,sy,tx,ty);
        _rep(i,0,n-1) _rep(j,0,m-1) read(arr[i][j]);
        solve();
        printf("Case #%d: ",tt);
        if (dis[tx][ty]>1e9) puts("Mission Impossible.");
        else writeln(mx[tx][ty]);
    }
    return 0;
}
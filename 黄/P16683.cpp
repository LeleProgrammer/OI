...
i32 main() {
    ...
    while (m--) {
        ...
        f[1][0]=a[1],f[1][2]=a[1],f[1][1]=(a[1]^k);
        _rep(i,2,n) {
            f[i][0]=f[i-1][0]+a[i];
            f[i][1]=min(f[i-1][1],f[i-1][0])+(a[i]^k);
            f[i][2]=min3(f[i-1][0],f[i-1][2],f[i-1][1])+a[i];
        }
        writesp(min3(f[n][0],f[n][1],f[n][2]));
        memset(f,0,sizeof(f));
        f[1][0]=a[1],f[1][2]=a[1],f[1][1]=(a[1]^k);
        _rep(i,2,n) {
            f[i][0]=f[i-1][0]+a[i];
            f[i][1]=max(f[i-1][1],f[i-1][0])+(a[i]^k);
            f[i][2]=max3(f[i-1][0],f[i-1][2],f[i-1][1])+a[i];
        }
        writeln(max3(f[n][0],f[n][1],f[n][2]));
    }
    return 0;
}
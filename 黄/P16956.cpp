...
i32 main() {
    ...
    while (m--) {
        int x,y;
        read(x,y);
        vec[x+1].emplace_back(y);
    }
    f[1][0]=a[1];
    _rep(i,2,n) {
        sort(_rall(vec[i]));
        if (vec[i].empty()) f[i][0]=max(f[i-1][0],f[i-1][1])+a[i];
        else {
            f[i][0]=max(f[i-1][0],f[i-1][1])+a[i]; // 0
            chmax(f[i][0],f[i-1][0]-a[i-1]+vec[i][0]+a[i]); // L
            chmax(f[i][1],max(f[i-1][0],f[i-1][1])+vec[i][0]); // R
            if (vec[i].size()>=2) chmax(f[i][1],f[i-1][0]-a[i-1]+vec[i][0]+vec[i][1]); // LR
        }
    }

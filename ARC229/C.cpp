...
i32 main() {
    ...
    while (T--) {
        ...
        b[1]=a[n],b[n]=a[n-1];
        vector<int> v0,v1;
        _rep(i,1,n-2) if (a[i]&1) v1.emplace_back(a[i]); else v0.emplace_back(a[i]);
        int las=(b[1]&1);
        _rep(i,2,n-1) {
            if (!las) {
                if (!v1.empty()) b[i]=v1.back(),v1.pop_back();
                else b[i]=v0.back(),v0.pop_back();
            } else {
                if (!v0.empty()) b[i]=v0.back(),v0.pop_back();
                else b[i]=v1.back(),v1.pop_back();
            }
            las=(b[i]&1);
        }
        int ans=0;
        _rep(i,1,n-1) ans+=(b[i]+b[i+1])/2;
        writeln(ans);
    }
    return 0;
}

...
int main() {
    ...
    while (T--) {
        ...
        DSU p1(n+5,nullptr),p2(n+5,nullptr),p3(n+5,nullptr);
        set<pii> st;
        while (m--) {
            int a,b,c;
            read(a,b,c);
            if (c==1) p1.merge(a,b);
            else if (c==2) p2.merge(a,b);
            else p3.merge(a,b);
            st.emplace(pii{a,b});
        }
        while (true) {
            ...
            _for(t,st) {
                ...
                if (p2.connected(a,b) && p3.connected(a,b) && !p1.connected(a,b)) {
                    p1.merge(a,b);
                    ok=false;
                }
            }
        ...
        _rep(i,1,n) mp[p1.find(i)].emplace_back(i);
        writeln(mp[p1.find(1)].size());
        _for(t,mp[p1.find(1)]) writesp(t);
        putchar(10);
    }
    return 0;
}
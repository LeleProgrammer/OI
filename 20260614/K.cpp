...
int main() {
    ...
    vector<pii> vec;
    _rep(j,1,m) vec.emplace_back(pii{1,j});
    _rep(i,1,n) vec.emplace_back(pii{i,m});
    bool ok=true;
    _for(t,vec) if (s[t.first][t.second]=='#') {
        ok=false;
        break;
    }
    if (ok) puts("YES");
        ...
        vec.clear();
        _rep(j,1,m) vec.emplace_back(pii{n,j});
        _rep(i,1,n) vec.emplace_back(pii{i,1});
        ok=true;
            _for(t,vec) if (s[t.first][t.second]=='#') {
            ok=false;
            break;

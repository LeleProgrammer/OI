...
inline int solve(int i,int t) {
    int l=1,r=a[i];
    while (l<r) {
        int mid=l+r>>1;
        if (f(a[i],mid)<=t) r=mid;
        else l=mid+1;
    }
    return r;
}
...
inline bool check(int mid) {
    sum=0;
    _rep(i,1,n) sum+=(b[i]=solve(i,mid));
    if (sum<k) return true; // 小于等于，找到最后一个 <k 的再调整
    else return false;
}
...
i32 main() {
    ...
    _rep(i,1,n) if (sum!=k && b[i]!=a[i] && f(a[i],b[i])==dest) b[i]++,sum++;

...
using namespace IO;
using Graph::h,Graph::ne,Graph::w,Graph::e,Graph::add;
...
inline void topo() {
    ...
    while (!q.empty()) {
        int u=q.front(); q.pop();
        _graph(i,u) {
            _rep(j,2,n) chmin(f[e[i]][j],f[u][j-1]+w[i]);
...
i32 main() {
    ...
    while (m--) {
        ...
        add(a,b,c);
        indeg[b]++;
        rev[b].emplace_back(pii{a,c});
    }
    topo();
    int ans;
    _rrep(i,n,1) if (f[n][i]<=t) {
        ans=i;
        break;
    }
    writeln(ans);
    ...
    while (u!=1) {
        vec.emplace_back(u);
        _for(v,rev[u]) if (f[v.first][cur-1]+v.second==f[u][cur]) {
            cur--,u=v.first;
            break;
        }
    }
    vec.emplace_back(1);
    _riter(it,vec) writesp(*it);
    return 0;
}
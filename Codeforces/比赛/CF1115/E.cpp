...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,0,62) {
            ...
            int kb=kthbit(n,i);
            _rep(sta,0,(1<<6)-1) {
                int pre0=kthbit(sta,0);
                int pre1=kthbit(sta,1);
                int pre2=kthbit(sta,2);
                int pre3=kthbit(sta,3);
                int pre4=kthbit(sta,4);
                int pre5=kthbit(sta,5);
                _rep(u,0,1) _rep(v,0,1) {
                    if (u && v) continue;
                    if (u!=v && pre1) continue;
                    int k=0;
                    if (u) k=setbit(k,0);
                    if (u|v) k=setbit(k,1);
                    if (pre2+v-u>0) k=setbit(k,2);
                    if ((u+pre0+pre3)>>1) k=setbit(k,3);
                    if ((((u+pre0+pre3)&1)+v+pre4)>>1) k=setbit(k,4);
                    if (((((u+pre0+pre3)&1)+v+pre4)&1)+pre5-kb>0) k=setbit(k,5);
                    f[nex][k]=(f[nex][k]+f[cur][sta])%mod;
                }
            }
            swap(cur,nex);
        }

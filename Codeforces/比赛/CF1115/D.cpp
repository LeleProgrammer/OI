...
i32 main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,n) read(a[i]);
        _rep(i,1,n) c[i]=a[i]-a[i-1];
        _rep(i,2,n-1) if (check(i)) {
            int j=i;
            while (j+1<=n-1 && check(j+1)) ++j;
            vector<int> vec;
            _rep(k,i,j+1) vec.emplace_back(c[k]);
            _sort(vec);
            _rep(k,i,j+1) c[k]=vec[k-i];
            i=j;
        }
        int cur=0;
        _rep(i,1,n) {

...
inline vector<int> ask(const int& k) {
    putchar('?');
    putchar(' ');
    writeln(k);
    fflush(stdout);
...
inline void solve(int cur,int dep) {
    auto t=ask(cur);
    if (t.empty()) return;
    if (dep<t.size() && t[dep]==las[dep]) {
        _for(i,t) sz[i]++;
        sz[t.back()]--;
        ans.emplace_back(pii{t[dep],t[dep+1]});
        if (flag[t[dep+1]]) {
            int inc=sz[t[dep+1]];
            _for(i,t) sz[i]+=inc;
            sz[t.back()]-=inc;
        ...
        sz[t.back()]--;
        dep=(int)t.size()-1;
        if (dep) ans.emplace_back(pii{t[dep-1],t[dep]});
        if (flag[t[dep]]) {
            int inc=sz[t[dep]];
            _for(i,t) sz[i]+=inc;
            sz[t.back()]-=inc;
...
int main() {
    ...
    while (T--) {
        ...
        putchar('!'); putchar(' ');
        writeln(ans.size());
        _iter(it,ans) writesp(it->first),writeln(it->second);
        fflush(stdout);
    }
    return 0;
}

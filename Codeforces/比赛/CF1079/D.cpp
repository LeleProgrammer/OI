...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,1,n) if (a[i]<=n) vec[a[i]].emplace_back(i);
        _rep(i,1,n) {
            _for(j,vec[i]) {
                int cur=j;
                while (cur<=n && !flag[cur]) flag[cur]=true,g.emplace_back(cur),cur+=i;
            }
            _for(j,g) if (j-a[j]*i>=1 && a[j-a[j]*i]==i) ++ans;
            while (!g.empty()) flag[g.back()]=false,g.pop_back();
        }
        writeln(ans);
    }
    return 0;

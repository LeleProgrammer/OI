...
int main() {
    ...
    while (T--) {
        ...
        _rep(i,1,n) scanf("%d",&x[i]),flag[x[i]]=true;
        int k=0;
        _rep(i,1,n) {
            if (!flag[i]) {
                k=i;
        ...
        if (k) {
            ...
            int res;
            read(res);
            if (res) puts("! B"),fflush(stdout);
            else puts("! A"),fflush(stdout);
        } else {
            int p;
            _rep(i,1,n) {
                if (x[i]==1) k=i;
            ...
            int res;
            scanf("%d",&res);
            if (res<n-1) puts("! A"),fflush(stdout);
            else {
                printf("? %d %d\n",p,k),fflush(stdout);
                int res2;
                scanf("%d",&res2);
                if (res!=res2) puts("! A"),fflush(stdout);
                else puts("! B"),fflush(stdout);
            }
        }
    }
    return 0;

...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,1,n) {
            int k=a[i];
            _rep(j,2,sqrt(k)) while (k%j==0) mp[i][j]++,k/=j;
            if (k!=1) mp[i][k]++;
        }
        vector<int> ans;
        int cur=1;
        _rep(k,2,16) {
            map<int,int> pk;
            int t=k;
            _rep(j,2,sqrt(t)) while (t%j==0) pk[j]++,t/=j;
            if (t!=1) pk[t]++;
            map<int,int> cp;
            t=cur;
            _rep(j,2,sqrt(t)) while (t%j==0) cp[j]++,t/=j;
            if (t!=1) cp[t]++;
            bool ok=false;
            int l=1;
            set<int> st;
            ...
            _rep(r,1,n) {
                ...
                while (l<r) {
                    _for(dat,mp[l]) {
                        int pri=dat.first;
                        int cnt=dat.second;
                        g[pri].erase(LB2(g[pri],cnt));
                        if (g[pri].empty()) st.erase(pri);
                    }
                    bool flag=true;
                    _for(dat,cp) {
                        int pri=dat.first;

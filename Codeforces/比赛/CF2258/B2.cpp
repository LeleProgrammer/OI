...
i32 main() {
    ...
    while (T--) {
        read(n,m),tot=0;
        _rep(i,0,m+1) cnt[i]=0,sum[i]=0;
        _rep(i,1,n) read(a[i]),cnt[a[i]]++,tot+=a[i];
        _rep(i,1,m) pre[i]=pre[i-1]+cnt[i];
        _rep(i,1,m) {
            int cur=0;
            _rep(j,1,m) {
                if (i*j>m) break;
                cur+=(pre[min(m,(j+1)*i-1)]-pre[i*j-1])*j;
            }
            sum[i]=cur;
        }
        vector<int> ans;
        _rep(k,1,m) {
            ...
            _rep(j,1,m) {
                ...
                int mx=s*j;
                if (mx<=m) {
                    _rep(g,1,s) inc+=g*(pre[min((g+1)*j-1,mx)]-pre[g*j-1]);
                } else {
                    mx=m;
                    inc=sum[j];
                }
                int f=(s-1)*(n-pre[mx])+inc;
                chmax(res,f);
            }
            ans.emplace_back(res);
        }
        while (ans.size()!=m) ans.emplace_back(tot);

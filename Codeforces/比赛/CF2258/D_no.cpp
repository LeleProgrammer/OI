...
i32 main() {
    ...
    while (T--) {
        ...
        vector<pii> vec; // len,type
        int ptr=1,cur=0;
        _for(t,mp) {
            int pos=t.first,b=t.second;
            if (cur) vec.emplace_back(pii{pos-ptr,cur});
            ptr=pos,cur^=b;
        }
        if (cur && ptr<=P) vec.emplace_back(pii{P-ptr+1,cur});
        vector<pii> arr;
        _for(t,vec) {
            int len=t.first,b=t.second;
            if (arr.empty()) arr.emplace_back(pii{len,b});
            else if (arr.back().second!=b) arr.emplace_back(pii{len,b});
            else arr.back().first+=len;
        }
        cout<<"test:"<<endl;
        _for(t,arr) cout<<"arr: "<<t.first<<" "<<t.second<<endl;
        swap(vec,arr);
        arr.clear();
        _for(t,vec) {
            int len=t.first,b=t.second;
            if (arr.empty()) arr.emplace_back(pii{len,b});
            else if (b==3) arr.emplace_back(pii{len,b});
            else if (arr.back().second==b) arr.back().first+=len;
            else if ((arr.back().second^b)==3) arr.emplace_back(pii{len,b});
            else if (arr.size()<=1) arr.emplace_back(pii{len,b});
            else if (arr[(int)arr.size()-2].second==b) {
                arr[(int)arr.size()-2].first+=arr.back().first;
                arr.pop_back();
                arr.back().first+=len;
            } else arr.emplace_back(pii{len,b});
        }
    }
    return 0;
}
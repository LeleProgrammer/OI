...
inline void merge(int x,int y) { // x: up, y: down
    bot[find(x)]=bot[find(y)];
    p[find(y)]=find(x);
}
...
i32 main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,n) vis[i]=false,nex[i].clear(),flag[i]=false,rem[i]=false;
        // Graph::restore_graph(n);
        _rep(i,1,n) read(arr[i]);
        vec.clear();
        _rep(i,1,n) vec.emplace_back(i);
        _rep(i,2,n) read(fa[i]),nex[fa[i]].emplace_back(i);
        dfs(1,0);
        ...
        int sum=0;
        _rep(i,1,n) sum+=arr[i];
        ans[n]=sum;
        _rep(i,1,n) p[i]=i,top[i]=bot[i]=i;
        int cur=n-1;
        _for(u,vec) {
            vis[u]=true;
            int up=top[find(u)];
            int down=bot[find(u)];
            bool ok=false;
            _for(v,nex[down]) if (!vis[v]) {
                ok=true;
                break;
            }
            if (ok) {
                rem[u]=true;
                ans[cur]=ans[cur+1]-arr[u];
                cur--;
            } else {
                while (fa[top[find(u)]] && vis[fa[top[find(u)]]]) {
                    if (rem[fa[top[find(u)]]]) {
                        rem[fa[top[find(u)]]]=false;
                        merge(fa[top[find(u)]],top[find(u)]);
                    } else break;
                }
                if (!fa[top[find(u)]]) continue;
                if (vis[fa[top[find(u)]]]) continue;
                if (bot[find(fa[top[find(u)]])]!=fa[top[find(u)]]) continue;
                ans[cur]=ans[cur+1]-arr[u];
                merge(fa[top[find(u)]],top[find(u)]);
                cur--;
            }

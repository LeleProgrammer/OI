...
inline void solve(int r,int sum) {
    int k=r;
    while (k-1>=1 && c[k-1]==c[k]) --k;
    int num=c[r];
    if (k==1) {
        if (c[k]) ok=false;
        if (sum%(r-k+1)) ok=false;
        _rep(i,k,r) d[i]=sum/(r-k+1);
    } else {
        solve(k-1,num);
        int rem=sum-num;
        if (rem%(r-k+1)) ok=false;
        else {
            _rep(i,k,r) d[i]=rem/(r-k+1);
            if (d[k]<=d[k-1]) ok=false;
        }
    }
}
...
i32 main() {
    read(T);
    while (T--) {
        read(n),mp.clear(),ok=true;
        _rep(i,1,n) read(b[i]),c[i]=b[i];
        _sorta(c,1,n);
        int m=n;
        while (m-1>=1 && c[m-1]==c[m]) --m;
        if (m==1) {
            if (c[m]) puts("-1");
            else {
                _rep(i,1,n) writesp(1);
                putchar(10);
            }
        } else {
            solve(m-1,c[m]);
            ...
            if (!ok) puts("-1");
                ...
                _rep(i,1,n) mp[c[i]]=d[i];
                _rep(i,1,n) writesp(mp[b[i]]);
                putchar(10);
            }
        }
    }

...
inline int solve(string s,string t) {
    vector<int> a,b;
    _rep(i,0,(int)s.size()-1) if (s[i]=='0') a.emplace_back(i);
    _rep(i,0,(int)s.size()-1) if (t[i]=='0') b.emplace_back(i);
    int ans=0;
    _rep(i,0,(int)a.size()-1) ans+=abs(a[i]-b[i]);
    return ans;
}
...
i32 main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,n) scanf("%1d",&a[i]);
        _rep(i,1,n) scanf("%1d",&b[i]);
        string b0,b1,a0,a1;
        _rep(i,1,n) if (i&1) a1+=a[i]+'0'; else a0+=a[i]+'0';
        _rep(i,1,n) if (i&1) b1+=b[i]+'0'; else b0+=b[i]+'0';
        string A0=a0,B0=b0,A1=a1,B1=b1;
        _sort(A0),_sort(B0),_sort(A1),_sort(B1);
        if (A0!=B0 || A1!=B1) puts("-1");
        else {
            int ans0=solve(a0,b0);
            int ans1=solve(a1,b1);
            writeln(ans0+ans1);
        }
    }
    return 0;

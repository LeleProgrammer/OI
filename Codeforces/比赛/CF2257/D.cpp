...
inline void init() {
    ...
    _rep(i,0,(int)vec.size()-1) {
        ...
        int h=s/d;
        num.emplace_back(h);
        cnt.emplace_back(d-las);
        pre.emplace_back(d-las);
        sum.emplace_back((d-las)*h);
        las=d;
    }
    _rep(i,1,(int)pre.size()-1) pre[i]+=pre[i-1];
    _rep(i,1,(int)sum.size()-1) sum[i]+=sum[i-1];
    // cout<<"num: "; _for(t,num) cout<<t<<" "; cout<<endl;
    // cout<<"cnt: "; _for(t,cnt) cout<<t<<" "; cout<<endl;
    // cout<<"pre: "; _for(t,pre) cout<<t<<" "; cout<<endl;
    // cout<<"sum: "; _for(t,sum) cout<<t<<" "; cout<<endl;
}
...
i32 main() {
    ...
    while (T--) {
        ...
        while (q--) {
            ...
            read(x,y);
            int pos=LB(pre,x);
            if (num[pos]>=y) writeln(x*y);
            else {
                int l=-1,r=(int)num.size()-1;
                while (l<r) {
                    int mid=(l+r>>1)+1;
                    if (num[mid]>y) l=mid;
                    else r=mid-1;
                ...
                if (~l) {
                    ans+=y*pre[l];
                    // cout<<"add 1: "<<y*pre[l]<<endl;
                }
                ans+=sum[pos-1];
                // cout<<"add 2: "<<sum[pos-1]<<endl;
                if (~l) {
                    ans-=sum[l];
                    // cout<<"dec: "<<sum[l]<<endl;
                }
                ans+=(x-pre[pos-1])*num[pos];
                // cout<<"add 3: "<<(x-pre[pos-1])*num[pos]<<endl;
                writeln(ans);
            }
        }
    }

...
i32 main() {
    ...
    while (T--) {
        read(n,x);
        _rep(i,1,n) {
            vec[i].clear(),a[i].clear(),b[i].clear();
            nex[i].clear(),cst[i].clear(),dta[i].clear();
            res[i]=0;
        }
        _rep(i,1,n) {
            int m;
            read(m);
            a[i].emplace_back(0);
            cst[i].emplace_back(0);
            dta[i].emplace_back(0);
            nex[i].emplace_back(0);
            _rep(j,1,m) {
                int v;
                read(v);
                a[i].emplace_back(v);
            ...
            _rep(l,1,m) {
                int r=l,fr=a[i][l],to=b[i][l];
                while (r+1<=m && fr>to) {
                    ++r;
                    int inc=max(0LL,a[i][r]-to);
                    fr+=inc,to+=inc;
                    to-=a[i][r],to+=b[i][r];
                }
                nex[i][l]=r;
        ...
        priority_queue<pii,vector<pii>,greater<pii>> top; // cost,id
        _rep(i,1,n) {
            ptr[i]=1;
            top.emplace(pii{cst[i][ptr[i]],i});
        }
        priority_queue<pair<int,pii>> ava; // delta,{id,floor cnt}
        int cur=x;
        while (true) {
            while (!top.empty() && top.top().first<=cur) {
                auto t=top.top(); top.pop();
                int cost=t.first,i=t.second;
                ava.emplace(pair<int,pii>{dta[i][ptr[i]],{i,nex[i][ptr[i]]}});
                ptr[i]=nex[i][ptr[i]]+1;
                if (ptr[i]<=(int)cst[i].size()-1) top.emplace(pii{cst[i][ptr[i]],i});
            }
            if (ava.empty() || ava.top().first<0) {
                int curmx=0,ans=1;
                _rep(i,1,n) if (res[i]>curmx) curmx=res[i],ans=i;
                _rep(i,1,n) {
                    int rem=cur;
                    while (res[i]!=(int)a[i].size()-1) {
                        if (rem>=a[i][res[i]+1]) {
                            rem-=a[i][res[i]+1];
                            rem+=b[i][res[i]+1];
                            res[i]++;
                    ...
                    if (res[i]>curmx) curmx=res[i],ans=i;
                    else if (res[i]==curmx && i<ans) ans=i;
                }
                writespln(curmx,ans);
                break;
            }
            while (!ava.empty() && ava.top().first>=0) {
                auto t=ava.top(); ava.pop();
                int delta=t.first,i=t.second.first,dest=t.second.second;
                cur+=delta;
                chmax(res[i],dest);
            }
        }

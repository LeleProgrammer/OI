...
int main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,n) read(a[i]),flag[i]=false,vec[i].clear(),pos[i].clear();
        int m=0;
        _rep(i,1,n) if (!flag[i]) {
            m++;
            _reps(j,i,n,j<<=1) {
                flag[j]=true;
                vec[m].emplace_back(a[j]);
                pos[m].emplace_back(j);
            }
        }
        _rep(i,1,m) _sort(vec[i]);
        _rep(i,1,m) _rep(j,0,(int)vec[i].size()-1) b[pos[i][j]]=vec[i][j];
        if (is_sorted(b+1,b+1+n)) puts("Yes");
        else puts("No");
    }
    return 0;
}
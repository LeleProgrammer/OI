...
inline bool check(int a,int b,int c,int aa,int bb,int cc) {
    i64 A=a-aa;
    i64 B=b-bb;
    i64 C=c-cc;
    return B*B-4*A*C<0;
...
int main() {
    read(T);
    while (T--) {
        read(n),st.clear(),res.clear();
        _rep(i,1,n) read(arr[i].a,arr[i].b,arr[i].c);
        _rep(i,1,n) st[{arr[i].a,arr[i].b}].emplace_back(arr[i].c),res[{arr[i].a,arr[i].b}].emplace_back(0);
        _iter(it,st) {
            if (it->first.first>0) sort(_all(it->second));
            else sort(_all(it->second),[&](const int& a,const int& b) {
                return a>b;
            });
        }
        _iter(it,res) _rep(i,0,(int)(it->second.size())-1) it->second[i]=(int)(it->second.size())-i;
        _iter(it,st) _iter(itt,st) {
            if ((it->first.first<0)^(itt->first.first<0)) {
                auto &rs=res[it->first];
                int cur=0;
                _rep(i,0,(int)(it->second.size())-1) {
                    while (cur<itt->second.size() && !check(it->first.first,it->first.second,it->second[i],itt->first.first,itt->first.second,itt->second[cur])) cur++;
                    chmax(rs[i],(int)(it->second.size())-i+(int)(itt->second.size())-cur);
                }
            }
        }
        _iter(it,st) {
            cout<<"test: "<<it->first.first<<" "<<it->first.second<<": ";
            _iter(itt,it->second) cout<<*itt<<" ";
            putchar(10);
        }
        _iter(it,res) {
            cout<<"res: "<<it->first.first<<" "<<it->first.second<<": ";
            _iter(itt,it->second) cout<<*itt<<" ";
            putchar(10);
        }
        _rep(i,1,n) {
            auto &ck=st[{arr[i].a,arr[i].b}];
            int ans=1;
            _rep(j,0,(int)(ck.size())-1) if (ck[j]<=arr[i].c) chmax(ans,res[{arr[i].a,arr[i].b}][j]);
            writesp(ans);
        }
        putchar(10);
    }

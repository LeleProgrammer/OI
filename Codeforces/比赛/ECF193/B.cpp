...
int main() {
    ...
    while (T--) {
        ...
        _rep(i,1,n) {
            ...
            vec.emplace_back(pii{a[i],j-i+1});
            i=j;
        }
        int ans=vec.size();
        _rep(i,0,(int)vec.size()-2) {
            if (vec[i].second==1 && vec[i+1].second==1) continue;
            else if (vec[i].second==1) {
                if (i==0 || vec[i+1].first!=vec[i-1].first) chmax(ans,(int)vec.size()+1);
            } else if (vec[i+1].second==1) {
                if (i+1==(int)vec.size()-1 || vec[i+2].first!=vec[i].first) chmax(ans,(int)vec.size()+1);
            } else chmax(ans,(int)vec.size()+2);
        }
        writeln(ans);
    }
    return 0;

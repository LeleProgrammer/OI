...
inline i64 calc1(int L,int R,int fr,int to) {
    int pos=fr;
    int l=pos+L-1,r=pos+R-1;
    hh=1,tt=0;
    _rep(i,l,r) if (i>=1 && i<=n) {
        while (hh<=tt && p[q[tt]]<=p[i]) tt--;
        q[++tt]=i;
    }
    i64 ans=-inf;
    if (hh<=tt && pos>=1 && pos<=n) ans=p[q[hh]]-p[pos-1];
    while (pos<to) {
        ++pos,++l,++r;
        while (hh<=tt && q[hh]<l) hh++;
        if (r>=1 && r<=n) {
            while (hh<=tt && p[q[tt]]<=p[r]) tt--;
            q[++tt]=r;
        }
        if (hh<=tt && pos>=1 && pos<=n) chmax(ans,p[q[hh]]-p[pos-1]);
    }
    return ans;
}

inline i64 calc2(int L,int R,int fr,int to) {
    int pos=fr;
    int l=pos-R+1,r=pos-L+1;
    hh=1,tt=0;
    _rep(i,l,r) if (i>=1 && i<=n) {
        while (hh<=tt && p[q[tt]-1]>=p[i-1]) tt--;
        q[++tt]=i;
    }
    i64 ans=-inf;
    if (hh<=tt && pos>=1 && pos<=n) ans=p[pos]-p[q[hh]-1];
    while (pos<to) {
        ++pos,++l,++r;
        while (hh<=tt && q[hh]<l) hh++;
        if (r>=1 && r<=n) {
            while (hh<=tt && p[q[tt]-1]>=p[r-1]) tt--;
            q[++tt]=r;
        }
        if (hh<=tt && pos>=1 && pos<=n) chmax(ans,p[pos]-p[q[hh]-1]);
    }
    return ans;
}

inline i64 calc3(int L,int R,int fr,int to) {
    int pos=fr;
    chmax(L,pos);
    hh=1,tt=0;
    _rep(i,L,R) if (i>=1 && i<=n) {
...
inline i64 solve1(int a,int b,int x,int y,int c) {
    // cout<<"calc1: "<<b-a+1<<" "<<b+c-a+1<<" "<<a<<" "<<x<<": "<<calc1(b-a+1,b+c-a+1,a,x)<<endl;
    return calc1(b-a+1,b+c-a+1,a,x);
}

inline i64 solve2(int a,int b,int x,int y,int c) {
    // cout<<"calc2: "<<b-(a+c)+1<<" "<<b-a+1<<" "<<b<<" "<<y<<": "<<calc2(b-(a+c)+1,b-a+1,b,y)<<endl;
    return calc2(b-(a+c)+1,b-a+1,b,y);
}

inline i64 solve3(int a,int b,int x,int y,int c) {
    // cout<<"calc3: "<<b<<" "<<b+c<<" "<<a<<" "<<x<<": "<<calc3(b,b+c,a,x)<<endl;
    return calc3(b,b+c,a,x);
}
...
i32 main() {
    ...
    while (qq--) {
        ...
        u64 ans=0;
        _rep(i,1,n) {
            i64 ans1=solve1(i-L+1,i,i,i+L-1,R-L); // big
            i64 ans2=solve1((2*i+2-L-R)/2,i,i-L+1,(2*i+R-L)/2,(2*i+R-L)/2-i); // up
            i64 ans3=solve3((2*i+2-L-R)/2,i,i-L+1,i,(2*i+R-L)/2-i); // squ
            i64 ans4=solve2(i-R+1,i,(2*i+2-L-R)/2,(2*i+R-L)/2,(2*i+2-L-R)/2-(i-R+1));  // down
            ans^=max4(ans1,ans2,ans3,ans4)*i;
        }
        writeln(ans);
    }
    return 0;

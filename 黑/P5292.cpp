...
inline void dfs(int u,int g,bool& bin) {
    vis[u]=g;
    _for(v,vec[u]) {
        if (s[v]!=s[u]) continue;
        if (!~vis[v]) {
            add(u,v),add(v,u);
            dfs(v,g^1,bin);
        } else if (vis[u]==vis[v]) bin=false;
    }
}

inline void solve() {
    queue<pii> q;
    _rep(i,1,n) q.emplace(pii{i,i}),f[i][i]=1;
    _rep(u,1,n) _graph(i,u) {
        int v=e[i];
        if (u==v) continue;
        if (f[u][v]) continue;
        if (s[u]!=s[v]) continue;
        q.emplace(pii{u,v}),f[u][v]=1;
        q.emplace(pii{v,u}),f[v][u]=1;
    }
    while (!q.empty()) {
        auto t=q.front(); q.pop();
        int u=t.first,v=t.second;
        _graph(i,u) _graph(j,v) if (s[e[i]]==s[e[j]] && !f[e[i]][e[j]]) {
            f[e[i]][e[j]]=1;
            q.emplace(pii{e[i],e[j]});
        }
    }
}
...
int main() {
    ...
    _rep(i,1,m) {
        ...
        vec[arr[i].a].emplace_back(arr[i].b);
        vec[arr[i].b].emplace_back(arr[i].a);
    }
    _rep(i,1,n) if (!~vis[i]) {
        bool bin=true;
        dfs(i,0,bin);

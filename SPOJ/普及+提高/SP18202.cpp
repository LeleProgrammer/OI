...
void sieve(int k,unordered_map<int,int>& hash) {
    _rep(i,2,sqrt(k)+1) while (k%i==0) hash[i]++,k/=i;
    if (k>=2) hash[k]++;
}
...
int qpow(int a,int b) {
    ...
    int k=qpow(a,b>>1);
    if (b&1) {
        if (k*k>mod || k*k*a>mod) exceed=true;
        return (((k*k)%mod)*a)%mod;
    }
    else {
        if (k*k>mod) exceed=true;
        return (k*k)%mod;
    }
}
...
string print(int k) {
    string ans;
    while (k) ans.push_back((k%10)+'0'),k/=10;
    reverse(ans.begin(),ans.end());
    while (ans.size()<9) ans.insert(ans.begin(),'0');
    return ans;
}
...
signed main() {
    ...
    _rep(i,1,n) sieve(a[i],ma);
    _rep(i,1,m) sieve(b[i],mb);
    _iter(it,ma) {
        ans=ans*qpow(it->first,min(it->second,mb[it->first]));
        if (ans>mod) exceed=true;
        ans%=mod;
    }
    if (exceed) cout<<print(ans);
    else printf("%lld",ans);
    return 0;
}
...
inline int cand(int x,int y) {
    ...
    while (p.size()<q.size()) p.emplace_back(0);
    while (p.size()>q.size()) q.emplace_back(0);
    vector<int> ans;
    ...
    int res=0;
    _riter(it,ans) res=res*k+*it;
    return res;
}

inline int cor(int x,int y) {
    vector<int> p,q;
    while (x) {
        int t=x%k; x/=k;
        p.emplace_back(t);
...
int main() {
    read(k);
    _rep(i,0,k-1) _rep(j,0,k-1) read(a[i][j]);
    _rep(i,0,k-1) _rep(j,0,k-1) read(b[i][j]);
    readstr(s); _for(c,s) n=n*k+c-'0';
    readstr(s); _for(c,s) x=x*k+c-'0';
    readstr(s); _for(c,s) y=y*k+c-'0';
    int ans=0;
    _rep(i,1,n) if (cand(i,x)==y || cor(i,x)==y) ans++;
    write(ans);
    return 0;
} 
...
inline bool check(int p) {
    _rep(sta,0,(1<<k)-1) {
        ...
        _rep(i,0,k-1) {
            if (kthbit(sta,i)) {
                _rep(j,y[i]-p,y[i]+p) if (j>=1 && j<=m) vec.emplace_back(pii{x[i],j});
            } else {
                _rep(j,x[i]-p,x[i]+p) if (j>=1 && j<=n) vec.emplace_back(pii{j,y[i]});
            }
        }
        int sum=0;
        _for(t,vec) {
            if (flag[t.first][t.second]) continue;
            flag[t.first][t.second]=true;
            sum+=f[t.first][t.second];
        }
        _for(t,vec) flag[t.first][t.second]=false;
...
i32 main() {
    read(n,m,k,w);
    _rep(i,1,n) _rep(j,1,m) read(f[i][j]);
    _rep(i,0,k-1) read(x[i],y[i]);
    int l=0,r=1001;
    while (l<r) {
        int mid=l+r>>1;
        if (check(mid)) r=mid;

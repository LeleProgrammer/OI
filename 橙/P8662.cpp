...
int main() {
    read(n);
    _rep(i,1,n) _rep(j,1,n) scanf(" %c",&s[i][j]);
    _rep(i,1,n) _rep(j,1,n) _rep(p,0,3) {
        int nx=i+dx4[p];
        int ny=j+dy4[p];
        if (nx<1 || ny<1 || nx>n || ny>n) continue;
        if (s[i][j]=='.' || s[nx][ny]=='.') continue;
        if (!dsu.connected(get(i,j),get(nx,ny))) dsu.merge(get(i,j),get(nx,ny));
    }
    map<int,set<int>> mp;
    _rep(i,1,n) _rep(j,1,n) if (s[i][j]=='#') {
        bool flag=false;
        _rep(p,0,3) {
            int nx=i+dx4[p];
            int ny=j+dy4[p];
        ...
        mp[dsu.find(get(i,j))].emplace(flag);
    }
    int ans=0;
    _for(t,mp) if (t.second.size()==1) ans++;
    write(ans);
    return 0;
}
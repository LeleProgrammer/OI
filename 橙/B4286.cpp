...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,m) if (arr[i][j]=='R') _rep(p,0,3) {
        int nx=i+dx4[p];
        int ny=j+dy4[p];
        if (nx<1 || ny<1 || nx>n || ny>m) continue;
        if (arr[nx][ny]=='R' && !dsu.connected(get(nx,ny),get(i,j))) dsu.merge(get(nx,ny),get(i,j));
    }
    _rep(i,1,n) _rep(j,1,m) if (arr[i][j]=='R') st.emplace(dsu.find(get(i,j)));
    write(st.size());
    return 0;
}
...
int main() {
    _rep(i,0,N-1) _rep(j,0,N-1) s[i][j]='#';
    read(T);
    _rep(tt,1,T) {
        read(n);
        ...
        while (x!=ex || y!=ey) {
            ...
            if (f=='E') {
                if (s[x-1][y]=='.') f='N',x--;
                else if (s[x][y+1]=='.') f='E',y++;
                else if (s[x+1][y]=='.') f='S',x++;
                else if (s[x][y-1]=='.') f='W',y--;
                else {
                    ok=false;
                    break;
                }
        ...
        if (!ok) puts("Edison ran out of energy.");
        else writeln(ans.size()),writestrln(ans);
    }
    return 0;
}
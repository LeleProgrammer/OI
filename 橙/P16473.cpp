...
int main() {
    ...
    _rep(tt,1,T) {
        ...
        int m=n;
        n*=n;
        _rep(i,0,n-1) _rep(j,0,n-1) read(a[i][j]);
        bool ok=true;
        _rep(i,0,n-1) _rep(j,0,n-1) if (a[i][j]<=0 || a[i][j]>=n+1) ok=false;
        _rep(i,0,n-1) {
            set<int> st;
            _rep(j,0,n-1) {
                if (st.count(a[i][j])) ok=false;
                st.emplace(a[i][j]);
            }
        }
        _rep(j,0,n-1) {
        ...
        _rep(p,0,m-1) _rep(q,0,m-1) {
            set<int> st;
            _rep(i,0,m-1) _rep(j,0,m-1) {
                if (st.count(a[p*m+i][q*m+j])) ok=false;
                st.emplace(a[p*m+i][q*m+j]);
            }
        }
        printf("Case #%d: ",tt);
        if (ok) puts("Yes");

...
inline void ntt(vector<int>& a,bool inv) {
    ...
    _reps(len,2,n,len<<=1) {
        int l=ksm(G,(cmod-1)/len,cmod);
        if (inv) l=inverse(l,cmod);
        _reps(i,0,n-1,i+=len) {
            int w=1;
            _rep(j,0,(len>>1)-1) {
                int L=a[i+j]%cmod;
                int R=a[i+j+(len>>1)]*w%cmod;
                a[i+j]=(L+R)%cmod;
                a[i+j+(len>>1)]=(L-R+cmod)%cmod;
                w=w*l%cmod;
            }
        }
    }
    if (inv) {
        int v=inverse(n,cmod);
        _rep(i,0,n-1) a[i]=a[i]*v%cmod;
    }
}

inline vector<int> solve(vector<int> a,vector<int> b) {
    int sz=a.size()+b.size()-1;
    int n=1;
    while (n<sz) n<<=1;
    a.resize(n);
    ...
    ntt(a,false);
    ntt(b,false);
    _rep(i,0,n-1) a[i]=a[i]*b[i]%cmod;
    ntt(a,true);
    a.resize(sz);
    return a;
}

inline i128 crt(vector<int>& a,vector<int>& p) {
    i128 mul=1;
    _for(n,p) mul*=n;
    i128 ans=0;
    _rep(i,0,2) {
        i128 m=mul/p[i];
        i128 inv=inverse(m,p[i]);
        i128 c=m*inv%mul;
        ans=(ans+a[i]*c)%mul;
    }
    return ans;
...
i32 main() {
    ...
    _rep(i,0,m) read(g[i]);
    vector<int> f1,f2;
    _rep(i,0,n) f1.emplace_back(f[i]);
    _rep(i,0,m) f2.emplace_back(g[i]);
    cmod=mod1; auto A=solve(f1,f2);
    cmod=mod2; auto B=solve(f1,f2);
    cmod=mod3; auto C=solve(f1,f2);
    vector<int> pvec;
    pvec.emplace_back(mod1);
    pvec.emplace_back(mod2);
    pvec.emplace_back(mod3);
    _rep(i,0,(int)A.size()-1) {
        vector<int> vec;
        vec.emplace_back(A[i]);
        vec.emplace_back(B[i]);
        vec.emplace_back(C[i]);
        writesp(crt(vec,pvec)%p);
    }
    return 0;
}
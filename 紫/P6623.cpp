...
inline void solve(int u,bool flag) {
    _graph(i,u) if (e[i]!=son[u]) solve(e[i],0);
    if (son[u]) solve(son[u],1);
    _graph(i,u) if (e[i]!=son[u]) inc(e[i]);
    _graph(i,u) val[u]^=val[e[i]];
    _rep(i,0,22) val[u]^=((cnt[i][dep[u]%(1<<i)]&1)<<i);
    val[u]^=(v[u]-dep[u]);
    if (flag) _rep(i,0,22) cnt[i][v[u]%(1<<i)]++;
    else _graph(i,u) dec(e[i]);
}
...
i32 main() {
    ...
    dfs(1,1);
    _rep(i,1,n) v[i]+=dep[i];
    solve(1,1);
    int ans=0;
    _rep(i,1,n) ans+=val[i];
    write(ans);
    return 0;
}
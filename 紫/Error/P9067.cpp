...
inline void insert(int root,int k,int g) {
    int u=root;
    _rrep(i,K-1,0) {
        int c=kthbit(k,i);
    ...
    vec[u].emplace_back(g);
}

inline void combine(int& x,int& y) { // x <- y
    if (!y) return;
    if (!x) x=y;
    else {
        combine(tr[x][0],tr[y][0]);
        combine(tr[x][1],tr[y][1]);
    }
}
...
inline void merge(int x,int y) {
    ...
    sz[x]+=sz[y];
    p[y]=x;
    if (dep[top[y]]<dep[top[x]]) top[x]=top[y];
}

inline void dfs(int u,int d) {
    dep[u]=d;
    _graph(i,u) dfs(e[i],d+1);
}

int main() {
    Graph::init_graph();
    read(n,m);
    _rep(i,2,n) read(fa[i]),add(fa[i],i);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n) {
        p[i]=i,sz[i]=1,top[i]=i;
        root[i]=++idx;
    }
    dfs(1,1);
    _rep(i,2,n) {

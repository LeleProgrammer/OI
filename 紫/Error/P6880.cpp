...
inline void dfs1(int u,int t) {
    if (vis[u].count(t)) return;
    vis[u].emplace(t);
    _graph(i,u) if (g[1][u]+w[i]==g[1][e[i]]) es[i+1].flag=true,dfs1(e[i],t);
}

inline void dfs2(int u,int t) {
    if (vis[u].count(t)) return;
    vis[u].emplace(t);
    _graph(i,u) if (g[n][u]+w[i]==g[n][e[i]]) es[i+1].flag=true,dfs2(e[i],t);
}

inline void dijkstra(int s,int no,int fr,int to,int co) {
    _rep(i,1,n) dis[i]=inf,flag[i]=false;
    q.emplace(pii{dis[s]=0,s});
    while (!q.empty()) {
        auto tp=q.top(); q.pop();
        int u=tp.second;
        if (flag[u]) continue;
        flag[u]=true;
        _graph(i,u) if (i!=no && !flag[e[i]] && dis[e[i]]>dis[u]+w[i]) q.emplace(pii{dis[e[i]]=dis[u]+w[i],e[i]});
        if (u==fr && !flag[to] && dis[to]>dis[u]+co) q.emplace(pii{dis[to]=dis[u]+co,to});
    }
}
...
i32 main() {
    Graph::init_graph();
    read(n,m);
    _rep(i,1,m) read(es[i].a,es[i].b,es[i].c,es[i].d);
    _rep(i,1,m) add(es[i].a,es[i].b,es[i].c);
    _rep(i,1,n) _rep(j,1,n) g[i][j]=inf;
    _rep(i,1,n) g[i][i]=0;
    _rep(i,1,m) chmin(g[es[i].a][es[i].b],es[i].c);
    ...
    dfs2(n);
    Graph::restore_graph();
    _rep(i,1,m) add(es[i].b,es[i].a,es[i].c);
    _rep(i,1,n) _rep(j,1,n) g[i][j]=inf;
    _rep(i,1,n) g[i][i]=0;
    _rep(i,1,m) chmin(g[es[i].b][es[i].a],es[i].c);
    _rep(k,1,n) _rep(i,1,n) _rep(j,1,n) chmin(g[i][j],g[i][k]+g[k][j]);
    memset(vis,0,sizeof(vis));
    dfs1(1);
    memset(vis,0,sizeof(vis));
    ...
    _rep(i,1,m) add(es[i].a,es[i].b,es[i].c);
    // cout<<"fir: "<<ans<<endl;
    _rep(i,1,m) if (es[i].flag) {
        dijkstra(1,i-1,es[i].b,es[i].a,es[i].c);
        int sum=dis[n];
        dijkstra(n,i-1,es[i].b,es[i].a,es[i].c);
        sum+=dis[1];
        chmin(ans,sum+es[i].d);
        // cout<<"now: "<<i<<" "<<sum-dis[1]<<" "<<dis[1]<<" "<<es[i].d<<endl;
    }
    if (ans>=inf) ans=-1;
    write(ans);
    return 0;

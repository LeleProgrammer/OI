...
int main() {
    ...
    while (T--) {
        ...
        read(n);
        _rep(i,1,n) read(a[i][0],a[i][1],a[i][2]);
        _rep(i,0,n-1) {
            memset(f[i&1^1],0,sizeof(f[i&1^1]));
            _rep(j,0,min(i,220)) _rep(k,0,min(i,220)) _rep(t,0,2) {
                chmax(f[i&1^1][j?j+1:0][k?k+1:0][t],f[i&1][j][k][t]+a[i+1][t]-(j?j+1:0)-(k?k+1:0));
                chmax(f[i&1^1][k?k+1:0][1][(t+1)%3],f[i&1][j][k][t]+a[i+1][(t+1)%3]-1-(k?k+1:0));
                chmax(f[i&1^1][1][j?j+1:0][(t+2)%3],f[i&1][j][k][t]+a[i+1][(t+2)%3]-1-(j?j+1:0));
                if (i==n-1) chmax(ans,max3(f[i&1^1][j?j+1:0][k?k+1:0][t],f[i&1^1][k?k+1:0][1][(t+1)%3],f[i&1^1][1][j?j+1:0][(t+2)%3]));
            }
        }
        writeln(ans);
    }

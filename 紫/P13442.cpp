...
int main() {
    ...
    _rep(t,1,tt) {
        Graph::restore_graph();
        read(n,k),S=N-1,T=N-2;
        _rep(i,1,n) _rep(j,1,k) read(p[i][j]);
        _rep(i,1,n) _rep(j,1,n) {
            bool ok=true;
            _rep(t,1,k) if (p[i][t]>=p[j][t]) {
                ok=false;
                break;
            }
            if (ok) add_flow(i,n+j,1);
        }
        _rep(i,1,n) add_flow(S,i,1),add_flow(n+i,T,1);
        writestr(string{"Case #"});
        write(t); putchar(':'),putchar(' ');
        writeln(n-dinic());
    }
    return 0;
}
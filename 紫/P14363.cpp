...
inline void dfs2(int u,int root,int c,u64 cur) {
    cur=cur*P+c;
    hs[root][cur]=g[u];
    // cout<<"hash: "<<root<<" "<<cur<<" "<<g[u]<<endl;
    _rep(i,1,26) if (tr[u][i]) dfs2(tr[u][i],root,i,cur);
}

inline void dfs(int u,int c) {
    _rep(i,1,28) if (tr[u][i]) {
        g[tr[u][i]]+=g[u];
        dfs(tr[u][i],i);
    }
    if (c==28) {
        hs[u][0]=g[u];
        // cout<<"start hash: "<<u<<" "<<0<<" "<<g[u]<<endl;
        _rep(i,1,26) if (tr[u][i]) dfs2(tr[u][i],u,i,0);
    }
}
    ...
    _rep(i,1,idx) {
        ...
        if (st) {
            ...
            if (v) {
                ...
                while (l<r) {
                    int mid=(l+r>>1)+1;
                    if (hs[v].count(pre[mid])) l=mid;
                    else r=mid-1;
                }
                if (hs[v].count(pre[l])) ans+=hs[v][pre[l]];
                // if (hs[v].count(pre[l])) cout<<"add: "<<l<<" "<<pre[l]<<" "<<hs[v][pre[l]]<<endl;
            }
        }
    }
    return ans;
...
int main() {
    ...
    _rep(i,1,n) {
        ...
        readstr(s2+1);
        int m=strlen(s1+1);
        if (!strcmp(s1+1,s2+1)) continue;
        int l,r;
        _rep(i,1,m) if (s1[i]!=s2[i]) {
            l=i;
            break;
        }
        _rrep(i,m,1) if (s1[i]!=s2[i]) {
            r=i;
            break;
        }
        idx=0;
        _rep(i,l,r) s[++idx]=s1[i];
        _rep(i,l,r) s[++idx]=s2[i];
        s[++idx]=A;
        _rep(i,r+1,m) s[++idx]=s1[i];
        s[++idx]=B;
        _rrep(i,l-1,1) s[++idx]=s1[i];
        s[idx+1]='\0';
        // cout<<"test 1: "<<s+1<<endl;
        insert();
    }
    dfs(0,-1);
    while (q--) {
        ...
        int l,r;
        _rep(i,1,m) if (t1[i]!=t2[i]) {
            l=i;
            break;
        }
        _rrep(i,m,1) if (t1[i]!=t2[i]) {
            r=i;
            break;
        }
        idx=0;
        _rep(i,l,r) s[++idx]=t1[i];
        _rep(i,l,r) s[++idx]=t2[i];
        s[++idx]=A;
        _rep(i,r+1,m) s[++idx]=t1[i];
        s[++idx]=B;
        s[idx+1]='\0';
        // cout<<"test 2: "<<s+1<<endl;
        xidx=0;
        _rrep(i,l-1,1) x[++xidx]=t1[i];
        _rep(i,1,xidx) pre[i]=pre[i-1]*P+x[i]-'a'+1;
        int ans=query();
        writeln(ans);
    }
    return 0;

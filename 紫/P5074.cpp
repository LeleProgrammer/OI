    ...
    while (T--) {
        ...
        _rep(i,0,n-1) {
            _rep(j,0,m-1) {
                ...
                f[q].clear();
                _for(t,f[p]) {
                    int sta=t.first,cnt=t.second;
                    int x=kthbit(sta,j),y=kthbit(sta,j+1);
                    if (!a[i][j]) {
                        if (!x && !y) f[q][sta]+=cnt;
                    } else {
                        if (!x && !y) f[q][setbit(setbit(sta,j),j+1)]+=cnt;
                        else if (!x && y) f[q][sta]+=cnt,f[q][clrbit(setbit(sta,j),j+1)]+=cnt;
                        else if (x && !y) f[q][sta]+=cnt,f[q][clrbit(setbit(sta,j+1),j)]+=cnt;
                        else f[q][clrbit(clrbit(sta,j),j+1)]+=cnt;
                    }
                }
            }
            unordered_map<int,int> nex;
            _for(t,f[q]) if (!kthbit(t.first,m)) nex[t.first<<1]+=t.second;
            f[q]=nex;
        }
        writeln(f[q][0]);
    }

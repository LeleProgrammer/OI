...
inline void add(int x) {
    cnt[x]++;
    if (cnt[x]>cnt[cur]) cur=x;
    else if (cnt[x]==cnt[cur] && x<cur) cur=x;
}
...
i32 main() {
    ...
    _rep(i,1,n) read(qs[i].l,qs[i].r),qs[i].i=i;
    sort(qs+1,qs+1+n);
    _rep(p,1,n) {
        int q=p-1;
        while (q+1<=n && get(qs[q+1].l)==get(qs[q+1].r) && get(qs[q+1].l)==get(qs[p].l)) ++q;
        ...
        p=q+1;
        q=p-1;
        while (q+1<=n && get(qs[q+1].l)==get(qs[p].l)) ++q;
        int st=1;
        _rep(t,p,q) chmax(st,qs[t].l);
        int j=st-1;
        _rep(t,p,q) {
            int l=qs[t].l,r=qs[t].r,id=qs[t].i;
            while (j<r) add(a[++j]);
            bkcur=cur;
            _rrep(i,st-1,l) add(a[i]);
            ans[id]=cur;
            _rrep(i,st-1,l) del(a[i]);
            cur=bkcur;
        }
        p=q;
    }
    _rep(i,1,n) writeln(nums[ans[i]]);
    return 0;
}
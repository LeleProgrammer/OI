...
struct Splay {
    ...
    int rev,add;
    inline void init() {
        p=s[0]=s[1]=sz=rev=add=0;
        v=mn=inf;
    }
    inline void init(int p,int v) {
        this->p=p,this->v=v,mn=v;
        sz=1;
        s[0]=s[1]=0,rev=0,add=0;
    }
} tr[N];
...
inline void pushup(int u) {
    tr[u].mn=min3(tr[tr[u].s[0]].mn,tr[tr[u].s[1]].mn,tr[u].v);
...
inline void pushdown(int u) {
    ...
    if (tr[u].add) {
        pushadd(tr[u].s[0],tr[u].add);
        pushadd(tr[u].s[1],tr[u].add);
        tr[u].add=0;
    }
}
...
inline void splay(int x,int k) {
    int y=x;
    stack<int> g;
    while (y) g.emplace(y),y=tr[y].p;
    while (!g.empty()) pushdown(g.top()),g.pop();
    while (tr[x].p!=k) {
        int y=tr[x].p;
        int z=tr[y].p;
        if (z!=k) {
...
i32 main() {
    ...
    while (m--) {
        ...
        if (s=="ADD") {
            ...
            int r=getk(y+2);
            x=getk(x);
            splay(x,0),splay(st,x);
            int u=tr[st].s[0];
            tr[u].p=0,tr[st].s[0]=0;
            pushup(st),pushup(x);
            splay(l,0),splay(r,l);
            tr[r].s[0]=u,tr[u].p=r;
            pushup(r),pushup(l);

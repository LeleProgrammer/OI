...
i32 main() {
    read(n,l),l++;
    _rep(i,1,n) read(c[i]);
    _rep(i,1,n) pre[i]=pre[i-1]+c[i];
    _rep(i,1,n) s[i]=pre[i]+i;
    q[++tt]=0;
    _rep(i,1,n) {
        while (tt-hh>=1 && gety(q[hh])-getk(i)*getx(q[hh])>=gety(q[hh+1])-getk(i)*getx(q[hh+1])) hh++;
        f[i]=gety(q[hh])-getk(i)*getx(q[hh])+(s[i]-l)*(s[i]-l);

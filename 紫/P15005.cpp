...
inline void merge(int x,int y) {
    ...
    bkp.emplace_back(pii{x,p[x]});
    p[x]=y;
    bksum.emplace_back(pii{y,sum[y]});
    sum[y]+=sum[x];
}

inline void proceed(int i) {
    if (!i) return;
    int op=ops[i].op,a=ops[i].a,b=ops[i].b;
    if (op==1) merge(a,b);
    else {
        a=find(a);
        bksum.emplace_back(pii{a,sum[a]});
        sum[a]+=b;
    }
}
...
inline void revert() {
    while (!bkp.empty()) {
        auto t=bkp.back(); bkp.pop_back();
        p[t.first]=t.second;
    }
    while (!bksum.empty()) {
        auto t=bksum.back(); bksum.pop_back();
...
inline void solve(int l,int r,vector<int>& vec) {
    ...
    if (vec.empty()) _rep(i,l,r) proceed(i);
    else {
        bkp.clear();
        bksum.clear();
        int mid=l+r>>1;
        _rep(i,l,mid) proceed(i);
        vector<int> vecl,vecr;
        _for(t,vec) {
            int x=ask[t].x,y=ask[t].y,z=ask[t].z;
            x=find(x),y=find(y);
            if (sum[x]+sum[y]>=z) vecl.emplace_back(t);
            else vecr.emplace_back(t);
        }
        revert();
        solve(l,mid,vecl),solve(mid+1,r,vecr);
    }
}
...
i32 main() {
    ...
    _rrep(i,d,1) read(ops[i].op,ops[i].a,ops[i].b);
    _rep(i,1,s) read(ask[i].x,ask[i].y,ask[i].z);
    _rep(i,1,d) {
        if (ops[i].op==1) del.emplace(pii{min(ops[i].a,ops[i].b),max(ops[i].a,ops[i].b)});
        else a[ops[i].a]-=ops[i].b;
    }
    _rep(i,1,n) p[i]=i,sum[i]=a[i];
    _rep(i,1,m) {
        int a=min(es[i].a,es[i].b);
        int b=max(es[i].a,es[i].b);
        if (del.count(pii{a,b})) continue;
        merge(a,b);
    }
    bkp.clear();
    bksum.clear();
    vector<int> st;
    _rep(i,1,s) st.emplace_back(i);
    solve(0,d+1,st);
    _rep(i,1,s) writeln(d-ans[i]);
    return 0;
}
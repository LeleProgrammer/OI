...
int main() {
    ...
    read(n);
    _rep(i,1,n) _rep(j,1,n) read(g[i][j]);
    _rep(i,1,n) {
        add(i+n,T,inf,0);
        _rep(j,1,n) add(i,i+n,1,j*2-1);
    }
    _rep(i,1,n) _rep(j,1,i-1)  {
        add(S,get(i,j),1,0);
        if (g[i][j]==1 || g[i][j]==2) add(get(i,j),i,1,0);
        if (g[i][j]==0 || g[i][j]==2) edg[i][j]=tot,add(get(i,j),j,1,0);
    }
    int res=solve().second;
    writeln(n*(n-1)*(n-2)/6-(res-n*(n-1)/2)/2);
    _rep(i,1,n) {
        _rep(j,1,n) {
            if (g[i][j]!=2) writesp(g[i][j]);
            else {
                int a=max(i,j),b=min(i,j);
                if (!f[edg[a][b]]) writesp(0^(i<j));
                else writesp(1^(i<j));
            }
        }
        putchar(10);

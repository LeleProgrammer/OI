...
struct Ter {
    ...
    Ter(): k(0) {}
    Ter(const int& a): k(a%pw[m+1]) {}
    Ter(const i128& a): k(a%pw[m+1]) {}

    inline const int kthbit(const int& g) const {
        return (k/pw[g])%3;
    }

    inline Ter setbit(const int& g,const int& p) {
        int t=kthbit(g);
        return Ter{k-pw[g]*t+pw[g]*p};
    }
...
inline void roll(unordered_map<int,i128>& f) {
    unordered_map<int,i128> g;
    _iter(it,f) if (Ter{it->first}.kthbit(m)==0) g[Ter{it->first}.lmove()]+=it->second;
    f=g;
}

inline int next2(Ter sta,int pos) {
    stack<int> stk;
    while (true) {
        if (sta.kthbit(pos)==0) {
            pos++;
            continue;
        } else if (sta.kthbit(pos)==1) stk.emplace(pos);
        else if (stk.size()==1) return pos;
        else stk.pop();
        pos++;
    }
    return 42;
...
inline int prev1(Ter sta,int pos) {
    stack<int> stk;
    _rep(i,0,pos) {
        if (sta.kthbit(i)==0) continue;
        else if (sta.kthbit(i)==1) stk.emplace(i);
        else if (i==pos) return stk.top();
        else stk.pop();
    }
    return 42;
}
...
int main() {
    ...
    _rep(i,0,n-1) {
        _rep(j,0,m-1) {
            ...
            f[cur].clear();
            _iter(it,f[las]) {
                auto sta=Ter{it->first};
                i128 cnt=it->second;
                if (sta.kthbit(j)==0 && sta.kthbit(j+1)==0) {
                    f[cur][sta.setbit(j,1).setbit(j+1,2)]+=cnt;
                ...
                } else if (sta.kthbit(j)==2 && sta.kthbit(j+1)==2) {

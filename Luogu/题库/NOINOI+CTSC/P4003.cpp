...
namespace Tree {
    ...
    namespace HLD {
        ...
        void modify_path(int u,int v,ModifyType& k,bool edge,function<void(int,int,int,ModifyType&)> modify) {
            ...
            if (edge && id[v]!=id[u]) modify(1,son[id[v]],id[u],k);
        }
        ...
        ValueType query_path(int u,int v,bool edge,function<ValueType(int,int,int)> query,function<void(ValueType&,ValueType&,ValueType&)> pushup) {
            ...
            if (edge && id[v]!=id[u]) {
                ValueType g=query(1,son[id[v]],id[u]);
                pushup(ans,ans,g);
            }
            return ans;
        }
...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,m) read(arr[get(i,j)]);
    _rep(i,1,n) _rep(j,1,m) if (!((i+j)&1)) {
        if (i>1) add(up(get(i,j)),down(get(i-1,j)),1,0);
        if (i<n) add(down(get(i,j)),up(get(i+1,j)),1,0);
        if (j>1) add(left(get(i,j)),right(get(i,j-1)),1,0);
        if (j<m) add(right(get(i,j)),left(get(i,j+1)),1,0);
    }
    _rep(i,1,n) _rep(j,1,m) {
        if ((i+j)&1) add(source(get(i,j)),T,INF,0);
        else add(S,source(get(i,j)),INF,0);
    }
    int res=0,ress=0;
    _rep(i,1,n) _rep(j,1,m) {
        int k=arr[get(i,j)];
        ...
        if (u && !r && !d && !l) {
            if (!rev) {
                add(source(cur),up(cur),1,0);
                add(up(cur),left(cur),1,1);
                add(up(cur),right(cur),1,1);
                add(up(cur),down(cur),1,2);
            } else {
                add(up(cur),source(cur),1,0);
                add(left(cur),up(cur),1,1);
                add(right(cur),up(cur),1,1);
                add(down(cur),up(cur),1,2);
            }
        } else if (l && !u && !r && !d) {
            if (!rev) {
                add(source(cur),left(cur),1,0);
                add(left(cur),up(cur),1,1);
                add(left(cur),down(cur),1,1);
                add(left(cur),right(cur),1,2);
            } else {
                add(left(cur),source(cur),1,0);
                add(up(cur),left(cur),1,1);
                add(down(cur),left(cur),1,1);
                add(right(cur),left(cur),1,2);
            }
        } else if (u && d && !l && !r) {
            if (!rev) {
                add(source(cur),up(cur),1,0);
                add(source(cur),down(cur),1,0);
            } else {
                add(up(cur),source(cur),1,0);
                add(down(cur),source(cur),1,0);
            }
        } else if (l && r && !u && !d) {
            if (!rev) {
                add(source(cur),left(cur),1,0);
                add(source(cur),right(cur),1,0);
            } else {
                add(left(cur),source(cur),1,0);
                add(right(cur),source(cur),1,0);
            }
        } else if (r && !l && !u && !d) {
            if (!rev) {
                add(source(cur),right(cur),1,0);
                add(right(cur),up(cur),1,1);
                add(right(cur),down(cur),1,1);
                add(right(cur),left(cur),1,2);
            } else {
                add(right(cur),source(cur),1,0);
                add(up(cur),right(cur),1,1);
                add(down(cur),right(cur),1,1);
                add(left(cur),right(cur),1,2);
            }
        } else if (d && !l && !r && !u) {
            if (!rev) {
            ...
            if (!rev) {
                add(source(cur),left(cur),1,0);
                add(source(cur),up(cur),1,0);
                add(left(cur),right(cur),1,1);
                add(up(cur),down(cur),1,1);
            } else {
                add(left(cur),source(cur),1,0);
                add(up(cur),source(cur),1,0);
                add(right(cur),left(cur),1,1);
                add(down(cur),up(cur),1,1);
            }
        } else if (u && l && r && !d) {
            if (!rev) {
                add(source(cur),up(cur),1,0);
    ...
    auto ans=solve();
    if (ans.first!=res || res!=ress) puts("-1");
    else write(ans.second);
    return 0;
}
...
struct val {
    int len;
    int mx,mn,sum;
    val(): len(0),mx(0),mn(0),sum(0) {}
};

void pushup(val& u,val& l,val& r) {
    u.mx=max(l.mx,l.sum+r.mx);
    u.mn=min(l.mn,l.sum+r.mn);
    u.sum=l.sum+r.sum;
}
...
void modify(val& u,const int& k) {
    u.mx=max(0LL,u.sum+k);
    u.mn=min(0LL,u.sum+k);
    u.sum+=k;
}

SEG<val,pushup,pushdown,const int,modify> seg;

int solve(int u,int c,val suf) {
    // cout<<"test: "<<seg.tr[u].l<<" "<<seg.tr[u].r<<" "<<seg.tr[u].v.mx<<" "<<seg.tr[u].v.mn<<" "<<seg.tr[u].v.sum<<endl;
    // cout<<"suf: "<<suf.mx<<" "<<suf.mn<<" "<<suf.sum<<endl;
    if (seg.tr[u].l==seg.tr[u].r) {
        if (seg.tr[u].v.sum<=0) { // up
            if (suf.mx>c) return suf.sum-(suf.mx-c);
            else return suf.sum-suf.mn;
        } else  { // down
            if (suf.mn<-c) return suf.sum-suf.mn;
            else return suf.sum-(suf.mx-c);
        }
    } else {
        val nex;
        pushup(nex,seg.tr[u<<1|1].v,suf);
        if (nex.mx-nex.mn<=c) return solve(u<<1,c,nex);
        else return solve(u<<1|1,c,suf);
    }
}
...
vector<i32> distribute_candies(vector<i32> c,vector<i32> l,vector<i32> r,vector<i32> v) {
    ...
    vector<i32> ans(n);
    _rep(i,1,q) {
        vec[l[i]+1].emplace_back(pii{i+1,v[i]});
        vec[r[i]+2].emplace_back(pii{i+1,-v[i]});
    }
    _rep(i,1,n) {
        _for(p,vec[i]) seg.modify(1,p.first,p.first,p.second);
        ans[i-1]=solve(1,c[i],val{});
    }
    return ans;
}

...
struct Ternary {
    ...
    Ternary(): k(0) {}
    Ternary(const int& val): k(val%pw[m+1]) {}

    inline const int kthbit(const int& g) const {
        return (k/pw[g])%3;
    }

    inline Ternary setbit(const int& g,const int& val) {
        int t=kthbit(g);
        int ans=k;
        ans-=pw[g]*t;
        ans+=pw[g]*val;
    ...
    inline Ternary setval(const int& val) {
        return Ternary{val};
    }

    inline Ternary lmove(const int& g) {
        return Ternary{k*pw[g]};
    }

    inline Ternary rmove(const int& g) {
        return Ternary{k/pw[g]};
    }
...
inline void init() {
    pw[0]=1;
    _rep(i,1,N-1) pw[i]=pw[i-1]*3;
}

inline int next2(const Ternary& a,const int& pos) {
    stack<int> stk;
    _rep(i,pos,m) {
        int g=a.kthbit(i);
        if (!g) continue;
        else if (g==1) stk.emplace();
        else stk.pop();
...
inline int prev1(const Ternary& a,const int& pos) {
    stack<int> stk;
    _rep(i,0,pos-1) {
        int g=a.kthbit(i);
        if (!g) continue;
        else if (g==1) stk.emplace(i);
        else stk.pop();
    }
...
inline void roll(unordered_map<int,int>& f) {
    unordered_map<int,int> g;
    _iter(it,f) if (Ternary{it->first}.kthbit(m)==0) g[Ternary{it->first}.lmove(1)]+=it->second;
    f=g;
}
...
i32 main() {
    init();
    read(n,m);
    _rep(i,0,n-1) readstr(s[i]);
    int ex,ey;
    _rrep(i,n-1,0) {
        bool ok=false;
        _rrep(j,m-1,0) if (s[i][j]=='.') {
            ex=i,ey=j;
            ok=true;
            break;
        }
    ...
    _rep(i,0,n-1) {
        _rep(j,0,m-1) {
            ...
            f[cur].clear();
            _iter(it,f[las]) {
                auto sta=Ternary{it->first};
                int cnt=it->second;
                if ((sta.kthbit(j)==0 && sta.kthbit(j+1)==0) || s[i][j]=='*') {
                    if (s[i][j]=='.') f[cur][sta.setbit(j,1).setbit(j+1,2)]+=cnt;
                    else if (sta.kthbit(j)==0 && sta.kthbit(j+1)==0) f[cur][sta]+=cnt;
                } else if (sta.kthbit(j)==0) {
                    f[cur][sta]+=cnt;
                    f[cur][sta.setbit(j,sta.kthbit(j+1)).setbit(j+1,0)]+=cnt;
                } else if (sta.kthbit(j+1)==0) {
                    f[cur][sta]+=cnt;
                    f[cur][sta.setbit(j+1,sta.kthbit(j)).setbit(j,0)]+=cnt;
                } else if (sta.kthbit(j)==1 && sta.kthbit(j+1)==1) {
                    f[cur][sta.setbit(j,0).setbit(j+1,0).setbit(next2(sta,j+1),1)]+=cnt;
                } else if (sta.kthbit(j)==2 && sta.kthbit(j+1)==1) {
                    f[cur][sta.setbit(j,0).setbit(j+1,0)]+=cnt;
                } else if (sta.kthbit(j)==1 && sta.kthbit(j+1)==2) {
                    if (i==ex && j==ey) f[cur][sta.setbit(j,0).setbit(j+1,0)]+=cnt;
                } else {
                    f[cur][sta.setbit(j,0).setbit(j+1,0).setbit(prev1(sta,j),2)]+=cnt;
                }
            }
        }
        if (i!=n-1) roll(f[cur]);

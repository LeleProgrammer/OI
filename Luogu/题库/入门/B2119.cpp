...
int main() {
    readstr(s);
    if (s.size()>=2 && s[(int)s.size()-1]=='r' && s[(int)s.size()-2]=='e') s.pop_back(),s.pop_back();
    else if (s.size()>=2 && s[(int)s.size()-1]=='y' && s[(int)s.size()-2]=='l') s.pop_back(),s.pop_back();
    else if (s.size()>=3 && s[(int)s.size()-1]=='g' && s[(int)s.size()-2]=='n' && s[(int)s.size()-3]=='i') s.pop_back(),s.pop_back(),s.pop_back();
    writestr(s);
    return 0;
}
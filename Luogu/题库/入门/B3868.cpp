...
int main() {
    ...
    while (T--) {
        ...
        int mx=0;
        _for(c,s) {
            if (isdigit(c)) chmax(mx,c-'0');
            else chmax(mx,c-'A'+10);
        }
        if (mx<2) writesp(1); else writesp(0);
        if (mx<8) writesp(1); else writesp(0);
        if (mx<10) writesp(1); else writesp(0);
        if (mx<16) writesp(1); else writesp(0);
        putchar(10);
    }
    return 0;
}
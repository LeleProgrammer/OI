...
i32 main() {
    ...
    _rep(i,1,n) read(a[i]);
    pre[1]=a[1];
    _rep(i,2,n) pre[i]=(i64)pre[i-1]*a[i]%p;
    pinv[n]=inverse(pre[n],p);
    _rrep(i,n-1,1) pinv[i]=(i64)pinv[i+1]*a[i+1]%p;
    inv[1]=pinv[1];
    _rep(i,2,n) inv[i]=(i64)pinv[i]*pre[i-1]%p;
    int t=k;
    _rep(i,1,n) ans=(ans+t*inv[i])%p,t=t*k%p;
    write(ans);
    return 0;
}
...
inline int wqs(int mid) {
    _rep(i,1,m) if (!arr[i].t) arr[i].c+=mid;
    ans.clear();
    sort(arr+1,arr+1+m);
    DSU dsu(n+5,nullptr);
    int res=0;
    _rep(i,1,m) {
        if (dsu.connected(arr[i].a,arr[i].b)) continue;
        dsu.merge(arr[i].a,arr[i].b);
        res+=(!arr[i].t);
...
int main() {
    ...
    _rep(i,1,m) {
        int u,v,c;
        read(u,v,c);
        arr[i]={u,v,randint(114514,1919810),c};
    }
    int l=-1919810,r=1919810;
    while (l<r) {
        int mid=(l+r>>1)+1;
        int res=wqs(mid);
        if (res<k) r=mid-1;
    ...
    else _for(i,ans) writespln(i.a,i.b,i.t);
    return 0;
}
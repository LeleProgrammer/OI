...
int main() {
    read(n);
    _rep(i,1,n) read(arr[i].x),read(arr[i].t),arr[i].t-=arr[i].x;
    sort(arr+1,arr+1+n);
    _rep(i,1,n) {
        if (arr[i].t<0) continue;
        int pos=upper_bound(vec.begin(),vec.end(),arr[i].t)-vec.begin();
        if (pos==(int)vec.size()) vec.emplace_back(arr[i].t);
        else vec[pos]=arr[i].t;
    }
    write(vec.size());
    return 0;
}
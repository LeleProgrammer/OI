...
int main() {
    ...
    _rep(i,1,n) read(x[i]),read(y[i]);
    _rep(i,1,n) l[i]=1,r[i]=m;
    while (k--) {
        int p,L,h;
    ...
    int cnt=0;
    _rep(i,1,n) {
        _rep(j,0,m) f[i&1][j]=inf;
        _rep(j,x[i],m+x[i]) f[i&1][min(j,m)]=min(f[i&1][min(j,m)],min(f[i&1][j-x[i]],f[(i&1)^1][j-x[i]])+1);
        _rep(j,1,m-y[i]) f[i&1][j]=min(f[i&1][j],f[(i&1)^1][j+y[i]]);
        bool ok=false;
        _rep(j,0,m) {
            if (j<l[i] || j>r[i]) f[i&1][j]=inf;
            if (f[i&1][j]!=inf) ok=true;
        }
        if (!ok) {
            puts("0");
            write(cnt);
    ...
    // _rep(i,1,m) printf("f[%d] = %d\n",i,f[0][i]);
    int ans=inf;
    _rep(i,1,m) ans=min(ans,f[n&1][i]);
    puts("1");
    write(ans);
    return 0;
}
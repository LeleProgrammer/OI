...
void dfs(int u,int a) {
    id[u]=a;
    _graph(i,u) if (!id[e[i]]) dfs(e[i],a);
}
...
int main() {
    ...
    while (r--) {
        ...
        g[a][b].emplace(pii{c,d});
        g[c][d].emplace(pii{a,b});
    }
    _rep(i,1,n) _rep(j,1,n) _rep(p,0,3) {
        int nx=i+dx[p];
        int ny=j+dy[p];
        if (nx<=0 || ny<=0 || nx>n || ny>n) continue;
        if (g[i][j].count(pii{nx,ny})) continue;
        add(get(i,j),get(nx,ny));
    }
    _rep(i,1,n) _rep(j,1,n) if (!id[get(i,j)]) dfs(get(i,j),++idx);
    while (k--) {
        int a,b;
        read(a),read(b);
        cnt[id[get(a,b)]]++;
    ...
    int ans=0,cur=0;
    _rep(i,1,idx) ans+=cur*cnt[i],cur+=cnt[i];
    write(ans);
    return 0;
}
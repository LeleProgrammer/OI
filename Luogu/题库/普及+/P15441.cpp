...
inline void dijkstra() {
    ...
    while (!q.empty()) {
        ...
        int u=tp.second;
        if (flag[u]) continue;
        _graph(i,u) if (dis[u]<=to[i] && dis[e[i]]>max(dis[u],fr[i])+w[i]) q.emplace(pii{dis[e[i]]=max(dis[u],fr[i])+w[i],e[i]});
    }
}
...
int main() {
    ...
    while (m--) {
        ...
        _rep(i,0,k-1) {
            add(n*i+u,n*(i+1)+v,w),add(n*i+v,n*(i+1)+u,w);
            fr[tot-2]=fr[tot-1]=0;
            to[tot-2]=to[tot-1]=inf;
        }
    }
    dijkstra();
    _rep(i,0,k) chmin(ans,dis[n*i+n]);
    if (ans==inf) ans=-1;
    write(ans);
    return 0;
}
...
int main() {
    ...
    _iter(it,st) {
        ...
        while (k--) g.emplace(1);
        while (!q.empty()) ans=min(ans,q.top()),q.pop();
        while (!g.empty()) q.emplace(g.front()),g.pop();
        if (next(it)!=st.end() && next(it)->first!=a+1) while (!q.empty()) ans=min(ans,q.top()),q.pop();
    }
    while (!q.empty()) ans=min(ans,q.top()),q.pop();
    write(ans);
    return 0;

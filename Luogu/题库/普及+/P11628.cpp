...
int main() {
    ...
    while (T--) {
        ...
        read(n),read(m);
        _rep(i,1,n) read(a[i]);
        _rep(i,1,n) read(b[i]),cnt[b[i]]++;
        _rep(i,1,n) if (cnt[m-a[i]]) cnt[m-a[i]]--,a[i]=0;
        _rep(i,1,m-1) {
            if (i==m-i) cnt[i]&=1;
        ...
        bool ok=true;
        _rep(i,1,m-1) if (cnt[i]!=0) ok=false;
        int g=0;
        vector<int> vec;
        _rep(i,1,n) if (a[i]) {
            if (a[i]*2!=m) vec.emplace_back(a[i]);
            else g++;
        }
        if (g&1) ok=false;
        if (!vec.empty()) {
            if (vec.size()!=2) ok=false;
            else if (vec[0]+vec[1]!=m) ok=false;
            else if (g) ok=false;
        }
        if (!ok) puts("No");
        else puts("Yes");

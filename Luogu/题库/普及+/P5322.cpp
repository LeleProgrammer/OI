...
int main() {
    ...
    _rep(i,1,s) _rep(j,1,n) read(arr[j][i]);
    _rep(i,1,n) sort(arr[i]+1,arr[i]+1+s);
    _rep(i,1,m) f[i]=-inf;
    _rep(i,1,n) _rrep(j,m,1) _rep(k,1,s) {
        if (arr[i][k]*2+1>j) break;
        f[j]=max(f[j],f[j-(arr[i][k]*2+1)]+i*k);
        ans=max(ans,f[j]);
    }
    write(ans);
    return 0;

...
int main() {
    memset(h,-1,sizeof(h));
    read(m),read(n);
    _rep(i,1,m) _rep(j,1,m) arr[get(i,j)]=2;
    _rep(i,1,n) {
        int x,y,c;
    ...
    _rep(i,1,m) _rep(j,1,m) {
        _rep(p,0,3) {
            ...
            if (arr[get(nx,ny)]==2 && arr[get(i,j)]!=2) add(get(i,j)+m*m*arr[get(i,j)],get(nx,ny)+m*m*arr[get(i,j)],2);
            else if (arr[get(nx,ny)]!=2 && arr[get(i,j)]==2) {
                add(get(i,j),get(nx,ny)+m*m*arr[get(nx,ny)],arr[get(nx,ny)]);
                add(get(i,j)+m*m,get(nx,ny)+m*m*arr[get(nx,ny)],arr[get(nx,ny)]^1);
            } else if (arr[get(nx,ny)]!=2 && arr[get(i,j)]!=2) {
                add(get(i,j)+m*m*arr[get(i,j)],get(nx,ny)+m*m*arr[get(nx,ny)],arr[get(nx,ny)]^arr[get(i,j)]);
            }
        }
    }
    dijkstra(get(1,1)+m*m*arr[get(1,1)]);
    int ans=min(dis[get(m,m)],dis[get(m,m)+m*m]);
    if (ans==inf) ans=-1;
    write(ans);
    return 0;
}
...
vector<int> prex[N],prey[N];
vector<int> sufx[N],sufy[N];

i32 main() {
    read(n);
    _rep(i,1,n) read(x[i],y[i]),x[i]+=10000,y[i]+=10000,vecx[y[i]].emplace_back(x[i]),vecy[x[i]].emplace_back(y[i]);
    _rep(x,0,20000) {
        if (vecy[x].empty()) continue;
        _rep(i,0,(int)vecy[x].size()-1) prey[x].emplace_back(0),sufy[x].emplace_back(0);
        prey[x][0]=vecy[x][0];
        _rep(i,1,(int)vecy[x].size()-1) prey[x][i]=prey[x][i-1]+vecy[x][i];
        sufy[x].back()=vecy[x].back();
        _rrep(i,(int)vecy[x].size()-2,0) sufy[x][i]=sufy[x][i+1]+vecy[x][i];
    }
    _rep(y,0,20000) {
        if (vecx[y].empty()) continue;
        _rep(i,0,(int)vecx[y].size()-1) prex[y].emplace_back(0),sufx[y].emplace_back(0);
        prex[y][0]=vecx[y][0];
        _rep(i,1,(int)vecx[y].size()-1) prex[y][i]=prex[y][i-1]+vecx[y][i];
        sufx[y].back()=vecx[y].back();
        _rrep(i,(int)vecx[y].size()-2,0) sufx[y][i]=sufx[y][i+1]+vecx[y][i];
    }
    _rep(i,1,n) {
        int ypos=LB(vecy[x[i]],y[i]);
        int xpos=LB(vecx[y[i]],x[i]);
        int sumx=((xpos+1)*x[i]-prex[y[i]][xpos])+(sufx[y[i]][xpos]-((int)vecx[y[i]].size()-xpos)*x[i]);
        int sumy=((ypos+1)*y[i]-prey[x[i]][ypos])+(sufy[x[i]][ypos]-((int)vecy[x[i]].size()-ypos)*y[i]);
        sumx%=mod,sumy%=mod;

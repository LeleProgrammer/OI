...
i32 main() {
    ...
    _rep(i,1,n) nums.emplace_back(x[i]);
    _sort(nums),_unique(nums);
    _rep(i,1,n) x[i]=LB(nums,x[i])+1;
    nums.clear();
    _rep(i,1,n) nums.emplace_back(y[i]);
    _sort(nums),_unique(nums);
    _rep(i,1,n) y[i]=LB(nums,y[i])+1;
    _rep(i,1,n) p[x[i]][y[i]]=1;
    ...
    _rep(i,1,n) _rep(j,i,n) {
        ...
        if (x1>x2) swap(x1,x2);
        if (y1>y2) swap(y1,y2);
        ans+=(query(x1,1,x2,y1-1)+1)*(query(x1,y2+1,x2,n)+1);
    }
    write(ans+1);
    return 0;
}
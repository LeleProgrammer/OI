...
int main() {
    ...
    while (T--) {
        ...
        while (m--) {
            ...
            scanf(" %c",&q);
            read(b);
            if (p=='m' && q=='m') add(a+n,b),add(b+n,a);
            else if (p=='h' && q=='h') add(a,b+n),add(b,a+n);
            else if (p=='m' && q=='h') add(a+n,b+n),add(b,a);
        ...
        _rep(i,1,n<<1) if (!dfn[i]) tarjan(i);
        bool ok=true;
        _rep(i,1,n) if (id[i]==id[i+n]) ok=false;
        if (ok) puts("GOOD");
        else puts("BAD");
    }
    return 0;

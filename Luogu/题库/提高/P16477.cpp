...
int main() {
    ...
    _rep(tt,1,T) {
        ...
        int cntr=0,cntb=0;
        _rep(i,1,n) _rep(j,1,n) if (s[i][j]=='R') cntr++; else if (s[i][j]=='B') cntb++;
        printf("Case #%d: ",tt);
        if (abs(cntr-cntb)>=2) puts("Impossible");
        else {
            Graph::restore_graph();
            DINIC::init();
            DINIC::S=2*n*n+1;
            DINIC::T=2*n*n+2;
            _rep(i,0,n*n+1) p[i]=i;
            _rep(i,1,n) _rep(j,1,n) if (s[i][j]=='R') {
                _rep(p,0,5) {
                    int nx=i+dx[p];
                    int ny=j+dy[p];
                    ...
                    add_flow(get(i,j)+n*n,get(nx,ny),1);
                }
                if (i==1) merge(0,get(i,j)),add_flow(DINIC::S,get(i,j),1);
                if (i==n) merge(n*n+1,get(i,j)),add_flow(get(i,j)+n*n,DINIC::T,1);
            }
            _rep(i,1,n) _rep(j,1,n) add_flow(get(i,j),get(i,j)+n*n,1);
            if (find(0)==find(n*n+1)) {
                if (DINIC::dinic()>1 || cntr<cntb) puts("Impossible");
                else puts("Red wins");
                continue;
            }
            Graph::restore_graph();
            ...
            DINIC::T=2*n*n+2;
            _rep(i,0,n*n+1) p[i]=i;
            _rep(i,1,n) _rep(j,1,n) if (s[i][j]=='B') {
                _rep(p,0,5) {
                    int nx=i+dx[p];
                    int ny=j+dy[p];
                    if (nx<1 || ny<1 || nx>n || ny>n) continue;
                ...
                if (j==1) merge(0,get(i,j)),add_flow(DINIC::S,get(i,j),1);
                if (j==n) merge(n*n+1,get(i,j)),add_flow(get(i,j)+n*n,DINIC::T,1);
            }
            _rep(i,1,n) _rep(j,1,n) add_flow(get(i,j),get(i,j)+n*n,1);
            if (find(0)==find(n*n+1)) {
                if (DINIC::dinic()>1 || cntb<cntr) puts("Impossible");
                else puts("Blue wins");
                continue;
            }
            puts("Nobody wins");

...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,n) if (arr[i][j]) {
        _rep(p,0,3) {
            int nx=i+dx4[p];
            int ny=j+dy4[p];
            if (nx<1 || ny<1 || nx>n || ny>n) continue;
            if (arr[nx][ny]) continue;
            if (!id[nx][ny]) id[nx][ny]=++idx;
        }
    }
    _rep(i,1,n) {
        if (!id[1][i] && !arr[1][i]) id[1][i]=++idx;
        if (!id[i][1] && !arr[i][1]) id[i][1]=++idx;
        if (!id[n][i] && !arr[n][i]) id[n][i]=++idx;
        if (!id[i][n] && !arr[i][n]) id[i][n]=++idx;
    }
    _rep(i,1,n) _rep(j,1,n) if (!arr[i][j]) {
        int k=j;
        while (k+1<=n && !arr[i][k+1]) ++k;
        _rep(t,j,k) L[i][t]=j,R[i][t]=k;
        j=k;
    }
    _rep(i,1,n) _rep(j,1,n) if (!arr[j][i]) {
        int k=j;
        while (k+1<=n && !arr[k+1][i]) ++k;
        _rep(t,j,k) U[t][i]=j,D[t][i]=k;
        j=k;
    }
    vector<pii> vec;
    _rep(i,1,n) _rep(j,1,n) if (id[i][j]) vec.emplace_back(pii{i,j});
    _for(a,vec) _for(b,vec) {
        int x=a.first,y=a.second,p=b.first,q=b.second;
        addr(get(id[x][y],id[p][q]),get(id[U[x][y]][y],id[U[p][q]][q]));
        addr(get(id[x][y],id[p][q]),get(id[D[x][y]][y],id[D[p][q]][q]));
        addr(get(id[x][y],id[p][q]),get(id[x][L[x][y]],id[p][L[p][q]]));
        addr(get(id[x][y],id[p][q]),get(id[x][R[x][y]],id[p][R[p][q]]));
    }
    _rep(i,1,n) _rep(j,1,n) if (id[i][j]) add(0,get(id[i][j],id[i][j]));
    bfs();
    while (q--) {
        int x,y,p,q;
        read(x,y,p,q);
        ...
        int ans=inf;
        if (id[x][y] && id[p][q]) ans=dis[get(id[x][y],id[p][q])];
        else ans=min4(
            dis[get(id[U[x][y]][y],id[U[p][q]][q])],
            dis[get(id[D[x][y]][y],id[D[p][q]][q])],
            dis[get(id[x][L[x][y]],id[p][L[p][q]])],
            dis[get(id[x][R[x][y]],id[p][R[p][q]])]
        )+1;
        if (ans>=inf) ans=-1;
        writeln(ans);
    }

...
inline void dfs(int u) {
    id[u]=++idx,sz[u]=1;
    bk[id[u]]=u;
    _graph(i,u) dfs(e[i]),sz[u]+=sz[e[i]];
}
...
int main() {
    ...
    _rep(i,1,n) read(v[i]);
    dfs(1);
    _rep(i,1,n+1) _rep(j,0,k) g[i][j]=-inf;
    g[1][0]=0;
    _rep(i,1,n) _rep(j,0,k) {
        chmax(g[i+sz[bk[i]]][j],g[i][j]);
        if (j+1<=k) chmax(g[i+1][j+1],g[i][j]);
        if (j+v[bk[i]]<=k) chmax(g[i+1][j+v[bk[i]]],g[i][j]+p[bk[i]]);
    }
    int ans=0;
    _rep(i,0,k) chmax(ans,g[n+1][i]);
    write(ans);
    return 0;
}
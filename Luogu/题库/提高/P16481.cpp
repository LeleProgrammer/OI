...
inline void dfs(int u) {
    ...
    _graph(i,u) {
        if (vis[e[i]]) {
            if (tp[e[i]]==tp[u]) val[find(u)]=(w[i]-d[u]-d[e[i]])/tp[u]; // *2
            continue;
        }
        d[e[i]]=w[i]-d[u];
        tp[e[i]]=-tp[u];
...
i32 main() {
    ...
    _rep(tt,1,T) {
        ...
        _rep(i,1,idx) if (!vis[i]) d[i]=0,tp[i]=1,dfs(i);
        read(q);
        vector<string> ans;
        while (q--) {
            string s;
            ...
            int A=mp[a],B=mp[b];
            if (find(A)!=find(B)) {
                if (val[find(A)]==inf) continue;
                if (val[find(B)]==inf) continue;
                int va=d[A]*2+tp[A]*val[find(A)];
                int vb=d[B]*2+tp[B]*val[find(B)];
                s+="=";
                s+=to_string(va+vb>>1);
                ans.emplace_back(s);
            } else {
                if (tp[A]==tp[B]) {
                    if (val[find(A)]==inf) continue;
                    s+="=";
                    s+=to_string(d[A]+d[B]+tp[A]*val[find(A)]);
                    ans.emplace_back(s);
                } else {
                    int v=d[A]+d[B];
                    s+="=";

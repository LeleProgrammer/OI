...
int T,n,m,q;
int sz[N],sum[N],res[N];
int sz2[N],sum2[N],res2[N];
int ret,ret2;
...
inline void dfs1(int u,int fa) {
    ...
    _graph(i,u) {
        if (e[i]==fa) continue;
        dfs1(e[i],u);
        ret+=cnt*(sum[e[i]]+sz[e[i]])+sz[e[i]]*sum[u];
        cnt+=sz[e[i]];
        sz[u]+=sz[e[i]];
        sum[u]+=sum[e[i]]+sz[e[i]];
    }
    ret+=sum[u];
}
...
inline void dfs2(int u,int fa) {
    ...
    _graph(i,u) {
        if (e[i]==fa) continue;
        dfs2(e[i],u);
        ret2+=cnt*(sum2[e[i]]+sz2[e[i]])+sz2[e[i]]*sum2[u];
        cnt+=sz2[e[i]];
        sz2[u]+=sz2[e[i]];
        sum2[u]+=sum2[e[i]]+sz2[e[i]];
    }
    ret2+=sum2[u];
}
...
i32 main() {
    ...
    _rep(tt,1,T) {
        ...
        dfs2(1,0); res2[1]=sum2[1];
        solve2(1,0);
        vector<double> ans;
        // _rep(i,1,n) cout<<"sum1: "<<i<<": "<<res[i]<<" "<<sum[i]<<endl;
        // _rep(i,1,m) cout<<"sum2: "<<i<<": "<<res2[i]<<" "<<sum2[i]<<endl;
        while (q--) {
            int x,y;
            read(x,y);
            // cout<<"test: "<<res[x]*m+res2[y]*n+n*m+ret+ret2<<" "<<ret<<" "<<ret2<<endl;
            ans.emplace_back((double)(res[x]*m+res2[y]*n+n*m+ret+ret2)/((n+m)*(n+m-1)/2));
        }
        printf("Case #%lld: ",tt);
        _for(t,ans) printf("%.12lf ",t);
        putchar(10);
    }
    return 0;
}
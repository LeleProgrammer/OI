...
inline bool check(int k) {
    _rep(i,2,sqrt(k)) if (k%i==0) return false;
    return true;
}

i32 main() {
    Math::init_mul(32000000);
    read(T);
    _rep(tt,1,T) {
        memset(f,0,sizeof(f));
        read(n);
        ...
        vector<int> cnt;
        _rep(i,1,Math::idx) {
            if (Math::pri[i]*Math::pri[i]>m) break;
            if (m%Math::pri[i]==0) {
                vec.emplace_back(Math::pri[i]);
                cnt.emplace_back(0);
                while (m%Math::pri[i]==0) m/=Math::pri[i],cnt.back()++;
            }
        }
        if (m!=1) vec.emplace_back(m),cnt.emplace_back(1);
        m=vec.size();
        _repb(sta,m) {
            int val=1;
            _rep(i,0,m-1) if (kthbit(sta,i)) val*=Math::ksm2(vec[i],cnt[i]);
            int num=0;
            while (val) {
                int k=val%10; val/=10;
                num+=k;
            }
            if (check(num)) f[sta]=0;
            else _rep(i,0,m-1) if (kthbit(sta,i)) f[sta]|=!f[clrbit(sta,i)];
        }
        printf("Case #%lld: ",tt);
        if (f[(1<<m)-1]) puts("Laurence");
        else puts("Seymour");
    }
    return 0;
}
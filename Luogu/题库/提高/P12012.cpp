...
inline void dfs1(int l,int r,int pos,int cur,bool take,bool& ans) {
    if (take && !cur) ans=true;
    if (pos==r+1) flag[cur+8000]+=take;
    else {
        dfs1(l,r,pos+1,cur,take,ans);
        dfs1(l,r,pos+1,cur+b[pos],true,ans);
        dfs1(l,r,pos+1,cur-b[pos],true,ans);
    }
}

inline void dfs3(int l,int r,int pos,int cur,bool take) {
    if (pos==r+1) flag[cur+8000]-=take;
    else {
        dfs3(l,r,pos+1,cur,take);
        dfs3(l,r,pos+1,cur+b[pos],true);
        dfs3(l,r,pos+1,cur-b[pos],true);
    }
}

inline void dfs2(int l,int r,int pos,int cur,bool take,bool& ans) {
    if (take && !cur) ans=true;
    if (ans) return;
    if (pos==r+1) {
        if (flag[-cur+8000]) ans=true;
    } else {
        dfs2(l,r,pos+1,cur,take,ans);
        dfs2(l,r,pos+1,cur+b[pos],true,ans);
        dfs2(l,r,pos+1,cur-b[pos],true,ans);
    }
}

inline int ksm(int a,int b,int p,bool& mod) {
    if (!b) return 1;
    else {
        int k=ksm(a,b>>1,p,mod);
        k*=k;
        if (k>=p) mod=true;
        k%=p;
        if (b&1) {
    ...
    while (m--) {
        ...
        if (op==1) {
            if (r-l+1>13) puts("Yuno");
                ...
                _rep(i,l,r) {
                    bool flag=false;
                    int t=ksm(3,tr.query(i),phi,flag);
                    if (flag) t+=phi;
                    if (phi==1) t=1;
                    b[i]=Math::ksm(a[i],t,v)+1,sum+=b[i];
                }
                int mid=l+r>>1;
                bool ans=false;
                dfs1(l,mid,l,0,false,ans);
                dfs2(mid+1,r,mid+1,0,false,ans);
                dfs3(l,mid,l,0,false);
                if (ans) puts("Yuno");
                else puts("Yuki");
            }
        } else tr.modify(l,1),tr.modify(r+1,-1);
    }
    return 0;
}
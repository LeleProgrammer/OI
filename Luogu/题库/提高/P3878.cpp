...
inline void dfs(int pos,int sum,int cnt) {
    if (pos==n/2+1) st[cnt].emplace(sum);
    else {
        dfs(pos+1,sum-a[pos],cnt);
        dfs(pos+1,sum+a[pos],cnt+1);
    }
}

inline void solve(int pos,int sum,int cnt) {
    if (pos==n+1) {
        if (n/2-cnt<0) return;
        auto it=LB2(st[n/2-cnt],-sum);
        if (it!=st[n/2-cnt].end()) chmin(ans,abs(sum+*it));
        if (it!=st[n/2-cnt].begin()) chmin(ans,abs(sum+*prev(it)));
    } else {
        solve(pos+1,sum-a[pos],cnt);
        solve(pos+1,sum+a[pos],cnt+1);
    }
}

...
inline void topo() {
    ...
    while (!q.empty()) {
        ...
        flag[u]=true;
        _graph(i,u) {
            if (inc[u]<arr[u]) ans+=arr[u]-inc[u];
            inc[e[i]]+=arr[u];
...
i32 main() {
    ...
    _rep(tt,1,T) {
        ...
        _rep(i,1,n) if (!flag[i]) {
            ...
            fa[cur]=vec.back();
            _for(u,vec) inc[u]+=arr[fa[u]];
            _for(u,vec) inc[u]=arr[u]-inc[u];
            // _for(u,vec) cout<<"inc: "<<u<<": "<<inc[u]<<endl;
            multiset<int> st;
            cur=0;
            _for(u,vec) st.emplace(inc[u]),cur+=max(0LL,inc[u]);
            int res=inf;
            _for(u,vec) {
                cur-=max(0LL,inc[u]);
                st.erase(LB2(st,inc[u]));
                inc[u]+=arr[fa[u]];
                st.emplace(inc[u]);
                cur+=max(0LL,inc[u]);
                chmin(res,cur);

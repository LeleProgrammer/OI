...
int a[N][N],c[N][N];
int cl[N];
vector<pair<int,i64>> sum[N],sum2[N];

inline void dfs1(int pos,int cost) {
    if (pos==n/2+1) {
        vector<int> vec;
        _rep(i,1,n/2) vec.emplace_back(a[i][cl[i]]);
        _rsort(vec);
        i64 k=0;
        _rep(i,0,n/2-1) {
            k+=vec[i];
            sum[i+1].emplace_back(cost,k);
        }
    } else {
        int cur=0;
        _rep(t,l[pos],k[pos]) {
...
inline void dfs2(int pos,int cost) {
    if (pos==n+1) {
        vector<int> vec;
        _rep(i,n/2+1,n) vec.emplace_back(a[i][cl[i]]);
        _rsort(vec);
        i64 k=0;
        _rep(i,0,n-n/2-1) {
            k+=vec[i];
            sum2[i+1].emplace_back(cost,k);
        }
    } else {
        int cur=0;
        _rep(t,l[pos],k[pos]) {
...
int main() {
    ...
    _rep(tt,1,T) {
        read(m,n);
        _rep(i,0,8) {
            sum[i].clear(),sum2[i].clear();
        }
        _rep(i,1,n) {
            read(k[i],l[i]);
            _rep(j,1,k[i]) read(a[i][j]);
            _rep(j,1,k[i]-1) read(c[i][j]);
        }
        dfs1(1,0),dfs2(n/2+1,0);
        sum[0].emplace_back(0,0);
        sum2[0].emplace_back(0,0);
        _rep(i,0,8) if (!sum2[i].empty()) {
            sort(_all(sum2[i]));
            _rep(j,1,(int)sum2[i].size()-1) chmax(sum2[i][j].second,sum2[i][j-1].second);
        }
        // _rep(i,0,8) {
        //     _for(t,sum[i]) cout<<"sum "<<i<<": "<<t.first<<" "<<t.second<<endl;
        //     _for(t,sum2[i]) cout<<"sum2 "<<i<<": "<<t.first<<" "<<t.second<<endl;
        // }
        i64 ans=0;
        _rep(i,0,8) if (!sum[i].empty() && !sum2[8-i].empty()) {
            _for(t,sum[i]) {
                int cost=t.first;
                i64 val=t.second;
                int rem=m-cost;
                int l=0,r=(int)sum2[8-i].size();
                while (l<r) {
                    int mid=l+r>>1;
                    if (sum2[8-i][mid].first<=rem) l=mid+1;
                    else r=mid;
                }
                if (!r) continue;
                r--;
                chmax(ans,val+sum2[8-i][r].second);
            }
        }
        printf("Case #%d: %lld\n",tt,ans);
    }

...
using namespace IO;
using Graph::e,Graph::ne,Graph::h,Graph::add;
...
inline void dfs(int u) {
    _graph(i,u) dfs(e[i]),cnt[u]+=cnt[e[i]];
}

inline void topo0() {
    queue<int> q;
    _rep(i,1,idx) if (!indeg[i]) q.emplace(i);
    while (!q.empty()) {
        int u=q.front(); q.pop();
        f[u]++;
        _graph(i,u) {
...
inline void dfs1(int u,int k) {
    ...
    _rep(i,0,25) {
        if (sum[arr[u].s[i]]>=k) {
            ans.push_back('a'+i);
            dfs1(arr[u].s[i],k-cnt[arr[u].s[i]]);
            return;
        } else k-=sum[arr[u].s[i]];
    }
    fail=true;
}
...
i32 main() {
    ...
    if (!t) {
        _rep(i,1,idx) _rep(j,0,25) if (arr[i].s[j]) add(arr[i].s[j],i),indeg[i]++;
        topo0();
        dfs0(1,k);
    } else {

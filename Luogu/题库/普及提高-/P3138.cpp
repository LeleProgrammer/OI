...
int main() {
    read(n);
    _rep(i,1,n) read(x[i]),read(y[i]),nums.emplace_back(x[i]),nums.emplace_back(y[i]);
    sort(nums.begin(),nums.end());
    nums.erase(unique(nums.begin(),nums.end()),nums.end());
    _rep(i,1,n) x[i]=lower_bound(nums.begin(),nums.end(),x[i])-nums.begin()+1,y[i]=lower_bound(nums.begin(),nums.end(),y[i])-nums.begin()+1;
    _rep(i,1,n) p[x[i]][y[i]]++;
    n<<=1;
    _rep(i,1,n) _rep(j,1,n) p[i][j]+=p[i-1][j]+p[i][j-1]-p[i-1][j-1];
    _rep(i,1,n) _rep(j,1,n) ans=min(ans,max(p[i][j],p[n][j]-p[i][j],p[i][n]-p[i][j],p[n][n]-p[n][j]-p[i][n]+p[i][j]));
    write(ans);
    return 0;
}
...
i32 main() {
    ...
    while (T--) {
        ...
        sort(t+1,t+1+n);
        int ans=inf;
        _rep(i,1,n-1) chmin(ans,min(abs(a-t[i]),abs(a-t[1]))+t[i]-t[1]+min(abs(b-t[i+1]),abs(b-t[n]))+t[n]-t[i+1]);
        chmin(ans,min4(abs(a-t[1])+t[n]-t[1],abs(a-t[n])+t[n]-t[1],abs(b-t[1])+t[n]-t[1],abs(b-t[n])+t[n]-t[1]));
        writeln(ans);
    }
    return 0;
}
...
int main() {
    read(n);
    _rep(i,1,n) read(s[i]),read(g[i]);
    _rep(i,0,1) _rep(j,0,M-1) f[i][j]=-INF;
    f[0][400000]=0;
    _rep(i,1,n) _rep(j,0,M-1) if (j-s[i]>=0 && j-s[i]<M) f[i&1][j]=max(f[(i&1)^1][j],f[(i&1)^1][j-s[i]]+g[i]);
    int ans=0;
    _rep(i,400000,M-1) if (f[n&1][i]>=0) ans=max(ans,i-400000+f[n&1][i]);
    write(ans);
    return 0;
}
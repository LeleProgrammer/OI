...
void dfs(int u,int fa,int dep) {
    layer[u]=dep,mx=max(mx,dep),cnt[dep]++,p[u]=fa,wid=max(wid,cnt[dep]);
    _graph(i,u) {
        if (e[i]==fa) continue;
        dfs(e[i],u,dep+1);
    }
...
int main() {
    ...
    while (u) {
        if (an.count(u)) {
            writeln((layer[x]-layer[u])*2+layer[y]-layer[u]);
            break;
        }
        u=p[u];
    }

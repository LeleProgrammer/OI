...
int query(int u,int d,int l,int r) {
    return arr[d][r]-arr[d][l-1]-arr[u-1][r]+arr[u-1][l-1];
}

int main() {
    read(n),read(m),read(k);
    _rep(i,1,n) _rep(j,1,m) read(arr[i][j]);
    _rep(i,1,n) _rep(j,1,m) arr[i][j]+=arr[i-1][j]+arr[i][j-1]-arr[i-1][j-1];
    _rep(u,1,n) _rep(l,1,m) {
        int r=l,d=n;
        while (r<=m) {
            while (query(u,d,l,r)>k && d>=u) d--;
            if (d<u) break;
            ans+=d-u+1;
            r++;
        }

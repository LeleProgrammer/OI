...
int main() {
    read(n),readstr(s+1);
    read(m),readstr(t+1);
    _rep(i,1,n) f[i][0]=get(s[i])+f[i-1][0];
    _rep(i,1,m) f[0][i]=get(t[i])+f[0][i-1];
    _rep(i,1,n) _rep(j,1,m) f[i][j]=max3(f[i-1][j-1]+get(s[i],t[j]),f[i-1][j]+get(s[i]),f[i][j-1]+get(t[j]));
    write(f[n][m]);
    return 0;
}
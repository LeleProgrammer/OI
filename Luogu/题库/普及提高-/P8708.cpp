...
i32 main() {
    read(n),read(k);
    _rep(i,1,n) read(a[i]),st[a[i]]++,pre[a[i]]++;
    pre[-INF]=0,pre[INF]=0;
    _iter(it,pre) if (it!=pre.begin()) it->second+=prev(it)->second;
    _iter(it,st) {
        int g=k-(it->first);
        int t=it->first;
        while (t) t/=10,g/=10;
        int res=prev(pre.upper_bound(g))->second;
        ans+=res*(it->second);
    }
    _iter(it,st) {
        int g=it->first,t=it->first;
        while (t) t/=10,g*=10;
        if (g+(it->first)<=k) ans-=it->second;
    }
    write(ans);
    return 0;
}
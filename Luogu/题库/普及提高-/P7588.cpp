...
void init() {
    ...
    _rep(i,1,idx) {
        if (checkDoublePrime(pri[i])) ppri[++cnt]=pri[i];
    }
}
...
int main() {
    ...
    while (T--) {
        scanf("%d %d",&l,&r);
        int a=lower_bound(ppri+1,ppri+1+cnt,l)-(ppri+1)+1;
        int b=upper_bound(ppri+1,ppri+1+cnt,r)-(ppri+1)+1-1;
        printf("%d\n",b-a+1);
    }
    return 0;
}
...
int main() {
    ...
    _rep(i,1,n) pre[i]=pre[i-1]*P+s[i]-'a'+1;
    _rep(i,1,m) {
        readstr(t[i]+1),len[i]=strlen(t[i]+1);
        u64 hash=0;
        _rep(j,1,len[i]) hash=hash*P+t[i][j]-'a'+1;
        _rep(j,1,n-len[i]+1) {
            if (get(j,j+len[i]-1)==hash) cnt[i][j]++;
            cnt[i][j]+=cnt[i][j-1];
        }
    }
    while (q--) {
        ...
        read(l,r);
        int ans=0;
        _rep(i,1,m) if (r-len[i]+1>=l) ans+=cnt[i][r-len[i]+1]-cnt[i][l-1];
        writeln(ans);
    }
    return 0;
}
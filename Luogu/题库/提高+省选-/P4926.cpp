...
inline bool check(double mid) {
    ...
    _rep(i,1,s) {
        int o=arr[i].o,a=arr[i].a,b=arr[i].b;
        double k=arr[i].k;
        if (o==1) add(b,a,log2(k-mid));
        else add(b,a,-log2(k+mid));
    }
    _rep(i,1,n) if (g[i]) add(n+1,i,log2(g[i])),add(i,n+1,-log2(g[i]));
    _rep(i,1,n+1) add(0,i,0);
    _rep(i,0,n+1) dis[i]=-inf;
    dis[n+1]=0;
    _rep(i,0,n+1) flag[i]=false,cnt[i]=0;
    queue<int> q;
    q.emplace(n+1),flag[n+1]=true;
    while (!q.empty()) {
        int u=q.front(); q.pop();
        flag[u]=false;
        _graph(i,u) if (dis[e[i]]<dis[u]+w[i]) {
            dis[e[i]]=dis[u]+w[i];
            if (!flag[e[i]]) {
                flag[e[i]]=true;
                cnt[e[i]]++;
                if (cnt[e[i]]==n+5) return true;
                q.emplace(e[i]);
            }
        }
    }
...
int main() {
    read(n,s,t);
    double r=2e9;
    _rep(i,1,s) read(arr[i].o,arr[i].a,arr[i].b,arr[i].k);
    _rep(i,1,s) if (arr[i].o==1) chmin(r,(double)arr[i].k);
    _rep(i,1,t) {
        int c,x;
        read(c,x);
        g[c]=x;

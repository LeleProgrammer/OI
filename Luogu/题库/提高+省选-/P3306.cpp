...
inline int bsgs(int a,int b,int p) { // a^x = b (mod p)
    ...
    int cur=b;
    _rep(i,0,k-1) mp[cur]=i,cur=cur*a%p;
    cur=ak;
    _rep(i,1,k) {
        if (mp.count(cur)) return i*k-mp[cur];
        cur=cur*ak%p;
    }
    return -1;
}
...
i32 main() {
    ...
    while (T--) {
        ...
        if (a==1) {
            ...
            int ans=((t-x)*inverse(b,p)%p+p)%p+1;
            writeln(ans);
            continue;
        }
        int g=(t+b*inverse(a-1,p))%p*inverse(x+b*inverse(a-1,p),p)%p;
        int n=bsgs(a,g,p)+1;
        if (!n) puts("-1");
        else writeln(n);
    }

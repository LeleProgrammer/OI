...
SEG<val,pushup,pushdown,const int,modify> sm,sl,sr;

int main() {
    read(n),read(k),read(q);
    _rep(i,1,n) read(arr[i]),a[i]=arr[i];
    _rep(i,1,k) read(B[i]);
    _rep(i,1,k) read(C[i]);
    int idx=0;
    _rep(i,1,k) while (B[i]--) c[++idx]=C[i];
    sort(a+1,a+1+n);
    sort(c+1,c+1+n,[&](const int& a,const int& b) {
        return a>b;
    });
    sm.reserve(n+5);
    sl.reserve(n+5);
    ...
    sl.build(1,1,n);
    sr.build(1,1,n);
    _rep(i,1,n) sm.modify(1,i,i,a[i]+c[i]);
    _rep(i,1,n) sl.modify(1,i,i,a[i]+c[i-1]);
    _rep(i,1,n) sr.modify(1,i,i,a[i]+c[i+1]);
    while (q--) {
        int x,v;
        read(x),read(v);
        if (v>arr[x]) {
            int l=lower_bound(a+1,a+1+n,arr[x])-a;
            int r=upper_bound(a+1,a+1+n,v)-a;
            int mx=-inf,mn=inf;
            if (1<=l-1) {
                auto t=sm.query(1,1,l-1);
                mx=max(mx,t.mx);
                mn=min(mn,t.mn);
            }
            if (l+1<=r-1) {
                auto t=sl.query(1,l+1,r-1);
                mx=max(mx,t.mx);
                mn=min(mn,t.mn);
            }
            auto t=sm.query(1,r,n);
            mx=max(mx,t.mx);
            mn=min(mn,t.mn);
            mx=max(mx,v+c[r-1]);
            mn=min(mn,v+c[r-1]);
            ...
            auto t=sm.query(1,1,n);
            writeln(t.mx-t.mn);
        } else {
            int r=lower_bound(a+1,a+1+n,arr[x])-a;
            int l=upper_bound(a+1,a+1+n,v)-a;
            int mx=-inf,mn=inf;
            if (1<=l-1) {
                auto t=sm.query(1,1,l-1);
                mx=max(mx,t.mx);
            ...
            if (l<=r-1) {
                auto t=sr.query(1,l,r-1);
                mx=max(mx,t.mx);
                mn=min(mn,t.mn);
            }
            if (r+1<=n) {
                auto t=sm.query(1,r+1,n);
                mx=max(mx,t.mx);
                mn=min(mn,t.mn);
            }
            mx=max(mx,v+c[l]);

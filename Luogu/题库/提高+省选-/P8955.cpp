...
inline int query(int u,int cur) {
    if (tr[u].l==tr[u].r) return (cur|tr[u].v)>p?tr[u].l:-1;
    else {
        if ((cur|tr[u<<1].v)>p) return query(u<<1,cur);
        else return query(u<<1|1,cur|tr[u<<1].v);
    }
}
...
int main() {
    ...
    _rep(i,1,q) {
        int l,r,v;
        read(l,r,v);
        vec[l].emplace_back(pii{i,v});
        vec[r+1].emplace_back(pii{i,0});
    }
    build(1,1,q);
    _rep(i,1,n) {
        _for(t,vec[i]) modify(1,t.first,t.second);
        writesp(query(1,a[i]));
    }
    return 0;
}
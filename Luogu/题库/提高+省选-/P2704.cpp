...
int main() {
    ...
    _rep(i,0,2) _rep(j,0,(1<<m)-1) _rep(k,0,(1<<m)-1) f[i][j][k]=-INF;
    f[0][0][0]=0;
    _rep(i,1,n) {
        _rep(j,0,(1<<m)-1) {
            ...
            _rep(k,0,(1<<m)-1) {
                ...
                if (i-1>=1 && (k&arr[i-1])) continue;
                _rep(p,0,(1<<m)-1) {
                    if (k&p) continue;
                    if ((p&(p>>1)) || (p&(p>>2))) continue;
                    if (j&p) continue;
                    if (i-2>=1 && (p&arr[i-2])) continue;
                    f[i%3][j][k]=max(f[i%3][j][k],f[(i+2)%3][k][p]+_popc(j));
                    if (i==n) ans=max(ans,f[i%3][j][k]);
                }
            }
        }
    }

...
vector<int> segment(int n,int m,int k,vector<int> l,vector<int> r) {
    ...
    vector<vector<vector<int>>> f(2,vector<vector<int>>(m+5,vector<int>(m+5,0)));
    ans[1]+=n;
    _rep(i,0,n-1) _rep(j,i+1,n-1) {
        int a=l[i],b=r[i];
        int p=l[j],q=r[j];
        if (a>p) swap(a,p),swap(b,q);
        if (p<=b) f[0][min(b,q)][max(b,q)]++,ans[2]++;
    }
    _rep(t,2,k-1) {
        _rep(i,1,m) _rep(j,1,m) f[t&1^1][i][j]=0;
        _rep(i,1,m) _rep(j,1,m) (f[t&1][i][j]+=f[t&1][i-1][j])%=mod;
        _rep(i,0,n-1) {
            int L=l[i],R=r[i];
            _rep(j,L,m) {
                (f[t&1^1][min(j,R)][max(j,R)]+=f[t&1][L-1][j])%=mod;
                (ans[t+1]+=f[t&1][L-1][j])%=mod;
            }
        }
    }
    return ans;
...
//             r.emplace_back(b);
//         }
//         auto ans=segment(n,m,k,l,r);
//         _rep(i,1,k) writesp(ans[i]);
//         putchar(10);
//     }
//     return 0;
// }
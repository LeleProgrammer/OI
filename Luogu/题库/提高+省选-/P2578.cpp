...
unordered_map<string,string> bk;
queue<string> q;
...
void bfs() {
    ...
    while (!q.empty()) {
        ...
        if (flag.count(cur)) continue;
        flag.emplace(cur);
        if (cur=="012345678") break;
        string g=cur;
        char c=g[5];
        swap(c,g[3]),swap(c,g[4]),swap(c,g[5]);
        if (!dis.count(g) || dis[g]>dis[cur]+1) {
            dis[g]=dis[cur]+1;
            bk[g]=cur;
            q.emplace(g);
        }
        g=cur;
        c=g[0];
        swap(c,g[1]),swap(c,g[2]),swap(c,g[5]),swap(c,g[8]);
        swap(c,g[7]),swap(c,g[6]),swap(c,g[3]),swap(c,g[0]);
        if (!dis.count(g) || dis[g]>dis[cur]+1) {
            dis[g]=dis[cur]+1;
            bk[g]=cur;
            q.emplace(g);
    ...
    if (!dis.count("012345678")) return;
    vector<string> vec;
    string cur="012345678";
    while (true) {

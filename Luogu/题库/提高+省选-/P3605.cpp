...
inline void init() {
    _rrep(i,N*30-1,1) stk.emplace(i);
}

inline void dfs(int u) {
    _for(v,vec[u]) dfs(v),insert(root[u],root[v]);
    ans[u]=query(root[u],1,n,p[u]+1,n);
...
int main() {
    init();
    read(n);
    _rep(i,1,n) read(p[i]),nums.emplace_back(p[i]);
    _rep(i,2,n) {
        int fa;
        read(fa);
        vec[fa].emplace_back(i);
    }
    _sort(nums),_unique(nums);
    _rep(i,1,n) p[i]=LB(nums,p[i])+1;
    dfs(1);

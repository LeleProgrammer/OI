...
void t2p() {
    _rep(i,1,n-1) read(f[i]),d[f[i]]++;
    int i=1,j=1;
    while (i<=n-2) {
        while (d[j]) j++;
        p[i++]=f[j];
        while (i<=n-2 && !--d[p[i-1]] && p[i-1]<j) p[i]=f[p[i-1]],i++;
        j++;
    }
    i64 ans=0;
    _rep(i,1,n-2) ans^=((i64)i*p[i]);
    write(ans);
}

void p2t() {
    _rep(i,1,n-2) read(p[i]),d[p[i]]++;
    p[n-1]=n;
    int i=1,j=1;
    while (i<=n-1) {
        while (d[j]) j++;
        f[j]=p[i];
        while (i<=n-2 && !--d[p[i]] && p[i]<j) f[p[i]]=p[i+1],i++;
        i++,j++;
    }
    i64 ans=0;
    _rep(i,1,n-1) ans^=((i64)i*f[i]);
    write(ans);
}

...
i32 main() {
    ...
    read(n,k);
    _rep(i,1,n) read(b[i]);
    _rep(i,1,k) _rep(j,1,k) scanf("%1d",&s[i][j]);
    _rep(i,1,n) {
        _rep(j,1,k) if (s[b[i]][j] && las[j]) {
            add(i,n+las[j],i-las[j]);
            add(i+n,n+las[j],i-las[j]);
        }
        if (las[b[i]]) add(i+n,las[b[i]]+n,i-las[b[i]]);
        las[b[i]]=i;
    }
    memset(las,0,sizeof(las));
    _rrep(i,n,1) {
        _rep(j,1,k) if (s[b[i]][j] && las[j]) {
            add(i,n+las[j],las[j]-i);
            add(i+n,n+las[j],las[j]-i);
        }
        if (las[b[i]]) add(i+n,las[b[i]]+n,las[b[i]]-i);
        las[b[i]]=i;
    }
    dijkstra(1);

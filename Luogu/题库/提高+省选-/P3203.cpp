...
inline void modify(int pos) {
    int l=L[pos];
    while (pos>=l) {
        nex[pos]=pos+a[pos],cnt[pos]=1;
        while (nex[pos]<=n && B[nex[pos]]==B[pos]) cnt[pos]+=cnt[nex[pos]],nex[pos]=nex[nex[pos]];
        pos--;
    }
}
...
inline int query(int pos) {
    int ans=0;
    while (pos<=n) ans+=cnt[pos],pos=nex[pos];
    return ans;
}

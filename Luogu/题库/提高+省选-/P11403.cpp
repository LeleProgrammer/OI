...
void read_array(int subtask_id,const vector<int>& v) {
    ...
    stack<int> stk;
    _for(i,v) {
        while (!stk.empty() && stk.top()<i) stk.pop(),res.push_back('0');
        stk.emplace(i),res.push_back('1');
    }
    save_to_floppy(res);
}

vector<int> solve_queries(int subtask_id,int n,const string& bits,const vector<int>& a,const vector<int>& b) {
    vector<tup3<int,int,int>> qs;
    _rep(i,0,(int)a.size()-1) qs.emplace_back(tup3<int,int,int>{a[i],b[i],i});
    sort(_all(qs),[&](const tup3<int,int,int>& a,const tup3<int,int,int>& b) {
        return a.b<b.b;
    });
    vector<int> ans;
    _rep(i,0,(int)a.size()-1) ans.emplace_back(-1);
    int cur=-1,ptr=0;
    vector<int> stk;
    _iter(it,qs) {
        int L=it->a,R=it->b,id=it->c;
        while (cur<R) {
            if (bits[ptr]=='0') stk.pop_back();
            else stk.emplace_back(++cur);
            ptr++;
        }
...
//     }
//     read_array(114514,v);
//     auto res=solve_queries(114514,n,bit,L,R);
//     _for(c,res) writeln(c);
//     return 0;
// }

/*
10 7
7 1 4 2 5 3 6 10 8 9
0 9 -1
1 2 -1
2 3 -1
3 4 -1

...
inline void pushup(int u) {
    ...
    tr[u].mn=tr[tr[u].l].mn;
    tr[u].mx=tr[tr[u].r].mx;
    if (tr[tr[u].l].mx+1==tr[tr[u].r].mn) tr[u].cnt--;
}
...
inline void insert(int& u,int l,int r,int p) {
    if (!u) u=newNode();
    if (l==p && r==p) tr[u].mn=l,tr[u].mx=r,tr[u].cnt=1;
    else {
        int mid=l+r>>1;
...
inline void insert(int& u,int v) {
    ...
    if (!u) u=v;
        ...
        insert(tr[u].l,tr[v].l);
        insert(tr[u].r,tr[v].r);
        pushup(u);
        delNode(v);
    }
}
...
int main() {
    _rrep(i,N-1,1) stk.emplace(i);
    read(n,m);
    _rep(i,1,n) read(a[i]),insert(root[a[i]],1,n,i);
    _rep(i,1,N-1) cur+=tr[root[i]].cnt;
    while (m--) {
        int op;
        read(op);
        if (op==1) {

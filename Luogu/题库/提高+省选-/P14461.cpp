...
void solve(int& cur) { // f_{cur}(x) -> f_{cur+1}(x)
    _rep(i,0,m-cur-1) f[i]=f[i+1]*(i+1);
    cur++;
}
...
void process() {
    ...
    _rep(i,0,m) {
        ...
        s*=(int)binom[i];
        while (cur<i*2) solve(cur);
        _rep(j,0,m-cur) F[j]+=f[j]*s;
    }
}
...
i32 main() {
    ...
    _rep(i,0,m) read(g[i]);
    process();
    _rep(i,0,m) swap(f[i],g[i]),swap(F[i],G[i]);
    process();
    _rep(i,0,m) swap(f[i],g[i]),swap(F[i],G[i]);
    if (n&1) {
        _rep(i,0,m) g[i]=G[i],f[i]=F[i],swap(G[i],F[i]);
        _rep(i,0,m-1) G[i]-=f[i+1]*(i+1);
        _rep(i,0,m-1) F[i]+=g[i+1]*(i+1);
    }
    _rep(i,0,m) writesp((int)F[i]); putchar(10);
    _rep(i,0,m) writesp((int)G[i]);
    return 0;
}
...
f[i]=f[i-2]-f[i-2]''
f[i-2]=f[i-4]-f[i-4]''
f[i]=f[i-4]-2f[i-4]''+f[i-4]''''
f[i-4]=f[i-6]-f[i-6]''
f[i]=f[i-6]-3f[i-6]''+3f[i-6]''''-f[i-6]''''''
f[i]=C(3,0)f[i-6]-C(3,1)f[i-6]''+C(3,2)f[i-6]''''-C(3,3)f[i-6]''''''
f[n]=C(k,0)f[n-2k]-C(k,1)f[n-2k]''+... (k=n/2)

...
vector<uint32_t> recover(i32 n) {
    vector<uint32_t> ans(n+1);
    _rep(i,1,n-1) ne[i]=i+1;
    int l=1,r=n;
    while (ne[l]!=r && l!=r) { // l,lmid lmid+1,rmid rmid+1,r
        int len=1,u=l;
        while (u!=r) len++,u=ne[u];
        int lmid,rmid,cur=l;
        _rep(j,1,len/3-1) cur=ne[cur];
        lmid=cur,cur=ne[cur];
        _rep(j,1,len/3-1) cur=ne[cur];
        rmid=cur;
        // cerr<<"test: "<<l<<" "<<lmid<<" "<<rmid<<" "<<r<<" "<<len<<endl;
        int a=query(l,rmid)-query(l,lmid);
        int b=query(lmid+1,r)-query(rmid+1,r);
        if (a<b) r=rmid;
        else if (a>b) l=ne[lmid];
        else {
            int u=lmid;
            while (ne[u]!=ne[rmid]) ne[u]=ne[ne[u]];
        }
    }
    int mn,L,R;
    if ((L=get(l))<(R=get(r))) ans[mn=l]=L;
    else mn=r,ans[mn=r]=R;
    int sum=ans[mn];
    _rep(i,mn+1,n) ans[i]=query(mn,i)+ans[mn]-sum,sum+=ans[i];
    sum=ans[mn];
    _rrep(i,mn-1,1) ans[i]=query(i,mn)+ans[mn]-sum,sum+=ans[i];
    ans.erase(ans.begin());
    return ans;
}

...
int main() {
    read(n),read(m);
    _rep(i,1,n) read(arr[i].l),read(arr[i].r);
    _rep(i,1,m) read(g[i].p),read(g[i].x);
    sort(arr+1,arr+1+n),sort(g+1,g+1+m);
    int cur=0,ans=0;
    _rep(i,1,m) {
        while (cur+1<=n && arr[cur+1].l<=g[i].p) st.emplace(arr[++cur].r);
        while (!st.empty() && (*st.begin())<g[i].p) st.erase(st.begin());
        while (st.size()>g[i].x) st.erase(prev(st.end())),ans--;
    }
    write(n+ans);
    return 0;
}
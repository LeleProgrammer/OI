...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,1,m) read(a[i]);
        _rep(i,1,m) read(b[i]);
        _rep(i,0,M-1) _rep(j,1,m) {
            if (i+a[j]<M) add(i,i+a[j],b[j]);
            if (i-a[j]>=0) add(i,i-a[j],b[j]);
        }
        dijkstra();
        memset(f,0x3f,sizeof(f));
        f[1][0]=f[1][1]=0;
        _rep(i,2,n) _rep(j,1,min(i-1,M-1)) {
            f[i][0]=min(f[i][0],max(f[j][1],f[i-j][0])+dis[j]);
            f[i][1]=min(f[i][1],max(f[j][0],f[i-j][1])+dis[j]);
        }
        if (f[n][0]>=1e18) puts("-1");
        else writeln(f[n][0]);
    }
    return 0;
}
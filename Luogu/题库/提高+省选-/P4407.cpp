...
void Hash(string s) {
    ...
    _rep(i,0,(int)s.size()-1) {
        char c=s[i];
        cur=cur*P+(c-'a'+1),pre[i]=cur;
    }
    cur=0;
    int len=0;
    _rrep(i,(int)s.size()-1,0) {
        char c=s[i];
        cur=cur+pw[len++]*(c-'a'+1),suf[i]=cur;
    }
}
...
int main() {
    ...
    _rep(i,1,n) {
        ...
        readstr(s);
        Hash(s);
        st.emplace(pre[(int)s.size()-1]);
    }
    while (m--) {
        string s;
        readstr(s);
        Hash(s);
        if (st.count(pre[(int)s.size()-1])) puts("-1");
        else {
            unordered_set<u64> ans;
            _rep(i,0,(int)s.size()-1) { // del
                u64 L,R;
                if (i) L=pre[i-1]*pw[(int)s.size()-1-i];
                else L=0;
                R=suf[i+1];
                if (st.count(L+R)) ans.emplace(L+R);
            }
            _rep(i,-1,(int)s.size()-1) { // add
                u64 L,R;
                if (~i) L=pre[i]*pw[(int)s.size()-i];
                else L=0;
                R=suf[i+1];
                _rep(j,1,26) {
                    u64 mid=j*pw[(int)s.size()-i-1];
                    if (st.count(L+R+mid)) ans.emplace(L+R+mid);
                }
            }
            _rep(i,0,(int)s.size()-1) { // replace
                u64 L,R;
                if (i) L=pre[i-1]*pw[(int)s.size()-i];
                else L=0;
                R=suf[i+1];
                _rep(j,1,26) {
                    u64 mid=j*pw[(int)s.size()-i-1];

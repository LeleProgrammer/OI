...
inline void dfs3(int u,int fa) {
    ...
    _graph(i,u) {
        if (e[i]==fa) continue;
        dfs3(e[i],u);
        vec.emplace_back(mxdep[e[i]]-dep[u]);
        chmax(p[u],p[e[i]]);
    }
    sort(_all(vec),[&](const int& a,const int& b) {
        return a>b;
    });
    chmax(p[u],vec[0]+vec[1]);
}

inline void dfs2(int u,int fa) {
    multiset<int> st,pst;
    st.emplace(0),pst.emplace(0);
    _graph(i,u) {
        if (e[i]==fa) continue;
        st.emplace(mxdep[e[i]]-dep[u]);
        pst.emplace(p[e[i]]);
    }
    _graph(i,u) {
        if (e[i]==fa) continue;
        st.erase(LB2(st,mxdep[e[i]]-dep[u]));
        pst.erase(LB2(pst,p[e[i]]));
        g[e[i]]=max(g[u],*prev(st.end()))+1;
        t[e[i]]=max3(t[u],*prev(st.end())+g[u],*prev(pst.end()));
        if (st.size()>=2) chmax(t[e[i]],*prev(st.end())+*prev(prev(st.end())));
        st.emplace(mxdep[e[i]]-dep[u]);
        pst.emplace(p[e[i]]);
    }
    int mx=*prev(st.end());
    int smx=*prev(prev(st.end()));
    chmax(ans,mx+smx+g[u]);
    chmax(ans,mx+smx+t[u]+1);
    // cout<<"Test: "<<u<<": "<<mx<<" "<<smx<<" "<<g[u]<<" "<<t[u]<<endl;
    _graph(i,u) {
        if (e[i]==fa) continue;
        dfs2(e[i],u);
    }

...
namespace SEG_FA {
    ...
    inline int query(int u,int l,int r,int k) {
        if (l==k && r==k) return tr[u].v;
        int mid=l+r>>1;
        if (k<=mid) return query(tr[u].l,l,mid,k);
        else return query(tr[u].r,mid+1,r,k);
    }

    inline void modify(int p,int& u,int l,int r,int pos,int val) {
        u=++idx;
        if (l==pos && r==pos) tr[u].v=val;
        else {
            int mid=l+r>>1;
            if (pos<=mid) {
                tr[u].r=tr[p].r;
                modify(tr[p].l,tr[u].l,l,mid,pos,val);
            } else {
                tr[u].l=tr[p].l;
                modify(tr[p].r,tr[u].r,mid+1,r,pos,val);
            }
        }
    }
}
...
int main() {
    read(n,m);
    SEG_FA::root[0]=SEG_FA::build(1,n);
    SEG_DEP::root[0]=SEG_DEP::build(1,n);
    _rep(tt,1,m) {
        int op;
        read(op);
        if (op==1) {
            int a,b;
            read(a,b);
            while (SEG_FA::query(SEG_FA::root[cur],1,n,a)!=a) a=SEG_FA::query(SEG_FA::root[cur],1,n,a);
            while (SEG_FA::query(SEG_FA::root[cur],1,n,b)!=b) b=SEG_FA::query(SEG_FA::root[cur],1,n,b);
            int da=SEG_DEP::query(SEG_DEP::root[cur],1,n,a);
            int db=SEG_DEP::query(SEG_DEP::root[cur],1,n,b);
            if (da<db) swap(a,b),swap(da,db);
            SEG_FA::modify(SEG_FA::root[cur],SEG_FA::root[tt],1,n,b,a);
            if (da==db) SEG_DEP::modify(SEG_DEP::root[cur],SEG_DEP::root[tt],1,n,a,da+1);
            else SEG_DEP::root[tt]=SEG_DEP::root[cur];
            cur=tt;
        } else if (op==2) {
            int k;
            read(k);
            SEG_FA::root[tt]=SEG_FA::root[k];
            SEG_DEP::root[tt]=SEG_DEP::root[k];
            cur=tt;
        } else {
            int a,b;
            read(a,b);
            ...
            SEG_FA::root[tt]=SEG_FA::root[cur];
            SEG_DEP::root[tt]=SEG_DEP::root[cur];
            cur=tt;
        }
    }
    return 0;

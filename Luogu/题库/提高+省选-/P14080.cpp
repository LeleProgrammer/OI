...
class SEG {
    ...
    void build(int u,int l,int r) {
        tr[u].l=l,tr[u].r=r,tr[u].v.len=(r-l+1),tr[u].v.v=INF,tr[u].v.tag=INF;
        if (l==r) return;
        int mid=l+r>>1;
...
i32 main() {
    ...
    dfs2(1,0);
    seg.build(1,1,n+5);
    _rep(i,1,m) if (!g[i].flag) modify_path<int>(g[i].a,g[i].b,g[i].c,true,[&](int u,int l,int r,int& k){seg.modify(u,l,r,k);});
    _rep(i,1,m) {
        if (!g[i].flag) ans[g[i].i]=sum;
        else {
            int k=query_path<val>(g[i].a,g[i].b,true,[&](int u,int l,int r){return seg.query(u,l,r);},[&](val& u,val& l,val& r){pushup(u,l,r);}).v;
            if (k==INF) ans[g[i].i]=-1;
            else ans[g[i].i]=sum-g[i].c+k;
        }
    }

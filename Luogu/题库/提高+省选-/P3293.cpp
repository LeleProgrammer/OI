...
inline void insert(int p,int& u,int l,int r,int pos) {
    u=++idx;
    if (l==r) {
        tr[u].cnt=tr[p].cnt+1;
        return;
    ...
    int mid=l+r>>1;
    if (pos<=mid) insert(tr[p].l,tr[u].l,l,mid,pos),tr[u].r=tr[p].r;
    else insert(tr[p].r,tr[u].r,mid+1,r,pos),tr[u].l=tr[p].l;
    pushup(u);
}

inline bool query(int p,int u,int l,int r,int L,int R) {
    if (!u) return false;
    if (l>=L && r<=R) return !!(tr[u].cnt-tr[p].cnt);
    int mid=l+r>>1;
    bool ans=false;
    if (L<=mid) ans|=query(tr[p].l,tr[u].l,l,mid,L,R);
    if (R>mid) ans|=query(tr[p].r,tr[u].r,mid+1,r,L,R);
    return ans;
}

int main() {
    read(n,m);
    _rep(i,1,n) read(a[i]),insert(root[i-1],root[i],0,100000,a[i]);
    while (m--) {
        int b,x,l,r;
        read(b,x,l,r);
        int ans=0;
        _rrep(i,25,0) {
            if (kthbit(b,i)) {
                if (!query(root[l-1],root[r],0,100000,ans-x,ans+(1<<i)-1-x)) ans+=(1<<i);
            } else {
                if (query(root[l-1],root[r],0,100000,ans+(1<<i)-x,ans+(1<<i)*2-1-x)) ans+=(1<<i);
            }
        }
        writeln(ans^b);
    }

...
int main() {
    ...
    _rep(i,1,m) g=g*P+(t[i]-'a'),pw*=P;
    u64 cur=0;
    _rep(i,1,n) {
        vec.emplace_back(s[i]);
        cur=cur*P+(s[i]-'a');
        if (vec.size()>m) cur=cur-pw*(vec[(int)vec.size()-m-1]-'a');
        if (cur==g) {
            int p=m;
            while (p--) vec.pop_back();
            cur=0;
            _rep(j,max(0,(int)vec.size()-m),(int)vec.size()-1) cur=cur*P+(vec[j]-'a');
        }
    }
    _iter(it,vec) putchar(*it);
    return 0;
}
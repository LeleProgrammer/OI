...
class SEG {
    ...
    void build(int u,int l,int r) {
        tr[u].l=l,tr[u].r=r,tr[u].v.len=(r-l+1),tr[u].v.v=0;
        if (l==r) return;
        int mid=l+r>>1;
...
SEG<val,pushup,pushdown,int,modify> seg2(N),seg3(N);

void modi(int p,int x) {
    if (p-1) {
        seg2.modify(1,p-1,p-1,x);
        seg3.modify(1,p-1,p-1,x<<1);
        if (p-2) seg3.modify(1,p-2,p-2,x);
    }
    seg2.modify(1,p,n,x<<1);
    seg3.modify(1,p,n,x*3);
}

i32 main() {
    read(n),read(m);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n-1) seg2.modify(1,i,i,a[i]+a[i+1]);
    _rep(i,1,n-2) seg3.modify(1,i,i,a[i]+a[i+1]+a[i+2]);
    while (m--) {
        int op,l,r;
        ...
        if (op==1) {
            ...
            int ans2=seg2.query(1,l,r-1).v;
            int ans3=-inf;
            if (l<=r-2) ans3=seg3.query(1,l,r-2).v;
            int a,b;
            if (3*ans2>2*ans3) a=ans2,b=2;
            else a=ans3,b=3;
            if (!a) puts("0/1");
            else {
                int g=gcd(abs(a),abs(b));
                a/=g,b/=g;
                write(a),putchar('/'),writeln(b);
            }
        }
    }
    return 0;

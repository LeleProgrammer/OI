...
inline void tarjan(int u) {
    ...
    _graph(i,u) {
        if (!dfn[e[i]]) {
            tarjan(e[i]);
            chmin(low[u],low[e[i]]);
        } else if (instk[e[i]]) chmin(low[u],dfn[e[i]]);
    }
    if (dfn[u]==low[u]) {
        scc++;
        int y=-1;
...
inline void topo() {
    queue<int> q;
    _rep(i,1,scc) if (!deg[i]) q.emplace(i);
    while (!q.empty()) {
        int u=q.front(); q.pop();
        _graph(i,u) {
            chmin(mn[e[i]],mn[u]);
...
i32 main() {
    ...
    _rep(i,1,m) if (!dfn[i]) tarjan(i);
    memset(mn,0x3f,sizeof(mn));
    _rep(i,1,n) chmin(mn[id[idx[i]]],i),chmax(mx[id[idx[i]]],i);
    set<pii> st;
    _rep(u,1,m) _graph(i,u) if (id[e[i]]!=id[u]) st.emplace(pii{id[e[i]],id[u]});
    Graph::restore_graph();
    _for(t,st) {
        int fr=t.first,to=t.second;
        deg[to]++;
        add(fr,to);
    }
    topo();
    int ans=0;
    _rep(i,1,n) ans=(ans+i*(mx[id[idx[i]]]-mn[id[idx[i]]]+1))%mod;
    write(ans);
    return 0;
}
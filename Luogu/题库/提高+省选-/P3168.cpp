...
void modify(int p,int u,int l,int r,int k,int t) {
    if (l==k && r==k) tr[u].v=tr[p].v+t,tr[u].sum=tr[p].sum+k*t;
    else {
        int mid=l+r>>1;
        if (k<=mid) {
            tr[u].r=tr[p].r;
            if (!tr[u].l) tr[u].l=++idx;
            modify(tr[p].l,tr[u].l,l,mid,k,t);
        } else {
            tr[u].l=tr[p].l;
            if (!tr[u].r) tr[u].r=++idx;
            modify(tr[p].r,tr[u].r,mid+1,r,k,t);
        }
        pushup(u);
    }
}
...
i32 main() {
    ...
    _rep(i,1,m) {
        int s,e,p;
        read(s,e,p);
        vec[s].emplace_back(pii{p,1});
        vec[e+1].emplace_back(pii{p,-1});
    }
    _rep(i,1,N-1) {
        int las=root[i]=root[i-1];
        _iter(it,vec[i]) {
            int p=it->first,t=it->second;
            root[i]=++idx;
            modify(las,root[i],1,M-1,p,t);
            las=root[i];
        }
    }
    int las=1;
    while (n--) {
        ...
        read(x,a,b,c);
        k=1+(a*las+b)%c;
        int l=1,r=M-1,u=root[x];
        if (tr[u].v<k) {
            writeln(las=tr[u].sum);
            continue;
        }
        while (l<r) {
            int mid=l+r>>1;
            int sum=tr[tr[u].l].v;
            if (k<=sum) u=tr[u].l,r=mid;
            else k-=sum,u=tr[u].r,l=mid+1;
        }
        int dec=(tr[u].v-k)*r;
        writeln(las=query(root[x],1,M-1,1,r)-dec);
    }
    return 0;
}
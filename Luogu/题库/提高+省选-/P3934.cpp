...
inline int ksm(int a,int b,int p,int& flag) {
    if (!b) return 1;
    i128 k=ksm(a,b>>1,p,flag);
    k=k*k;
    if (k>=p) flag=1;
    k%=p;
    if (b&1) {
...
inline pii solve(int l,int r,int p) {
    if (p==1) return {0,1};
    int a=tr.query(l);
    if (a==1) return {1,0};
    if (l==r) {
    ...
    auto g=solve(l+1,r,phi[p]);
    if (g.second) g.first+=phi[p];
    pii ans={0,0};
    ans.first=ksm(a,g.first,p,ans.second);
    return ans;
}

i32 main() {
    Math::init_mul(20000005,false,true);
    read(n,m);
    _rep(i,1,n) read(arr[i]);
    _rrep(i,n,1) arr[i]-=arr[i-1],tr.modify(i,arr[i]);
    while (m--) {
        int op,l,r,p;
        read(op,l,r,p);
        if (op==1) tr.modify(l,p),tr.modify(r+1,-p);
        else {
            auto ans=solve(l,r,p);
            writeln(ans.first);
        }
    }
    return 0;

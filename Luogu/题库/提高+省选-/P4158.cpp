...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,m) scanf("%1d",&arr[i][j]);
    _rep(i,1,n) _rep(j,1,m) cnt[i][j][0]=cnt[i][j-1][0]+(arr[i][j]==0),cnt[i][j][1]=cnt[i][j-1][1]+(arr[i][j]==1);
    _rep(i,1,n) _rep(j,1,m) _rep(k,1,j) _rep(t,1,min(T,j)) f[i][j][t]=max(f[i][j][t],max(f[i][j-1][t],f[i][k-1][t-1]+max(cnt[i][j][0]-cnt[i][k-1][0],cnt[i][j][1]-cnt[i][k-1][1])));
    _rep(i,1,n) _rrep(j,T,0) _rep(k,0,j) g[j]=max(g[j],g[j-k]+f[i][m][k]),ans=max(ans,g[j]);
    write(ans);
    return 0;
}
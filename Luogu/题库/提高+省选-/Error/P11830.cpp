...
inline int check(int mid) {
    ...
    int rmin=0,rmax=0;
    _rep(i,1,n) {
        if (bl[i]<=mid && mid<=br[i]) g+=ar[i];
        else if (br[i]<mid) lmin+=al[i],lmax+=ar[i];
        else rmin+=al[i],rmax+=ar[i];
    }
    int L=max(lmin,rmin);
    int R=min(lmax,rmax);
    if (L<=R) return 1;
...
i32 main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,n) read(al[i],ar[i],bl[i],br[i]);
        int l=1,r=1e9;
        while (l<r) {
            int mid=l+r>>1;
            int t=check(mid);

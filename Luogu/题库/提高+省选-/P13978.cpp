...
int query(int l,int r,int c) {
    if (B[l]==B[r]) {
        pushdown(B[l]);
        int ans=-inf;
        _rep(i,l,r) if (a[i]<c) ans=max(ans,a[i]);
        return ans!=-inf?ans:-1;
    } else {
        pushdown(B[l]),pushdown(B[r]);
        int ans=-inf;
        _rep(i,l,R[l]) if (a[i]<c) ans=max(ans,a[i]);
        _rep(i,L[r],r) if (a[i]<c) ans=max(ans,a[i]);
        _rep(i,B[l]+1,B[r]-1) {
            int pos=lower_bound(b+BL[i],b+BR[i]+1,c-tag[i],[&](int x,int y) {
                return a[x]<y;
            })-b-1;
            if (pos>=BL[i]) ans=max(ans,a[b[pos]]+tag[i]);
        }
        return ans!=-inf?ans:-1;
    }
}

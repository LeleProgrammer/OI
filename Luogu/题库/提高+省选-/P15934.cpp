...
inline void modify(val& u,const vi2& dat) {
    int op=dat.a,k=dat.b;
    if (op==1) u.L=u.R=k,u.llen=1,u.rlen=1,u.sum=1;
    else u.flp^=1,u.L^=1,u.R^=1;
}

SEG<val,pushup,pushdown,vi2,modify> seg(N);

i32 main() {
    read(n,q);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n) seg.modify(1,i,i,vi2{1,a[i]});
    while (q--) {
        int op,l,r;
        read(op,l,r);
        if (op==1) {
            seg.modify(1,l,r,vi2{2,114514});
        } else {

...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,n) _rep(p,0,3) {
        int nx=i+dx[p];
        int ny=j+dy[p];
        if (nx<1 || ny<1 || nx>n || ny>n) continue;
        int inc=0;
        if (nx<i || ny<j) inc=b;
        if (arr[nx][ny]) _rep(g,1,k) add(get(i,j,g),get(nx,ny,k),1,inc+a);
        else {
            _rep(g,1,k) add(get(i,j,g),get(nx,ny,g-1),1,inc);
            _rep(g,1,k) add(get(i,j,g),get(nx,ny,k),1,inc+a+C);
        }
    }
    S=N-1,T=N-2;
    add(S,get(1,1,k),1,0);
    _rep(g,0,k) add(get(n,n,g),T,1,0);
    auto t=solve();
    write(t.second);
    return 0;
}
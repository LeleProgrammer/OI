...
i32 main() {
    ...
    d.emplace(0),d.emplace(n+1);
    _rep(i,1,n) {
        if (a[i]==1) tr.modify(i,1),d.emplace(i);
        else if (a[i]==2) st.emplace(i);
    }
    cur=calc(n);
    _iter(it,st) if (next(it)!=st.end()) cur-=calc(query((*it)+1,(*next(it))-1));
    _iter(it,d) if (next(it)!=d.end()) cur-=calc((*next(it))-(*it)-1);
    while (q--) {
        int x,v;
        read(x),read(v);
        if (a[x]!=v) {
            if (a[x]==1) {
                ...
                int L=*(prev(d.lower_bound(x)));
                int R=*(d.upper_bound(x));
                cur+=calc(query(l+1,r-1));
                cur+=calc(x-L-1);
                cur+=calc(R-x-1);
                cur-=calc(R-L-1);
                d.erase(x);
                a[x]=v,tr.modify(x,-1);
                if (a[x]==2) {
                    cur-=calc(query(l+1,x-1));
                    cur-=calc(query(x+1,r-1));
                    st.emplace(x);
                } else cur-=calc(query(l+1,r-1));
            } else if (a[x]==2) {
                int l=*(prev(st.lower_bound(x)));
                int r=*(st.upper_bound(x));
                int L=*(prev(d.upper_bound(x)));
                int R=*(d.upper_bound(x));
                cur+=calc(query(l+1,x-1));
                cur+=calc(query(x+1,r-1));
                st.erase(x);

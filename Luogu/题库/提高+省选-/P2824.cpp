...
class SEG {
    ...
    void build(int u,int l,int r) {
        tr[u].l=l,tr[u].r=r,tr[u].v.len=(r-l+1),tr[u].v.v=0,tr[u].v.st=-1;
        if (l==r) return;
        int mid=l+r>>1;
...
bool check(int mid) {
    seg.build(1,1,n);
    _rep(i,1,n) seg.modify(1,i,i,p[i]>=mid);
    _rep(i,1,m) {
        int op=arr[i].op,l=arr[i].l,r=arr[i].r;
        int sum=seg.query(1,l,r).v;
        if (!op) {
            if (sum!=r-l+1) seg.modify(1,l,r-sum,0);
            if (sum) seg.modify(1,r-sum+1,r,1);
        } else {
            if (sum) seg.modify(1,l,l+sum-1,1);
            if (sum!=r-l+1) seg.modify(1,l+sum,r,0);
        }
    }
    return !!(seg.query(1,q,q).v);
}
...
int main() {
    ...
    seg.reserve(n+5);
    _rep(i,1,n) read(p[i]);
    _rep(i,1,m) read(arr[i].op),read(arr[i].l),read(arr[i].r);
    read(q);
    int l=1,r=n;
    while (l<r) {
        int mid=(l+r>>1)+1;

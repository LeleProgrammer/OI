...
i32 main() {
    ...
    while (m--) {
        int a,b,c;
        read(a,b,c);
        es.emplace_back(pair<pii,pii>{{a,c},{b,c}});
        vec[a].emplace_back(c);
        vec[b].emplace_back(c);
    }
    _rep(i,1,n) {
        sort(vec[i].begin(),vec[i].end());
        vec[i].erase(unique(vec[i].begin(),vec[i].end()),vec[i].end());
        _for(j,vec[i]) mp[pii{i,j}]=++idx;
        _rep(j,0,(int)vec[i].size()-2) add(mp[pii{i,vec[i][j]}],mp[pii{i,vec[i][j+1]}],vec[i][j+1]-vec[i][j]),add(mp[pii{i,vec[i][j+1]}],mp[pii{i,vec[i][j]}],vec[i][j+1]-vec[i][j]);
    }
    _for(t,es) add(mp[t.first],mp[t.second],0),add(mp[t.second],mp[t.first],0);
    dijkstra(mp[pii{1,0}]);
    int ans=INF;
    _for(i,vec[n]) ans=min(ans,dis[mp[pii{n,i}]]);
    write(ans);
    return 0;
}
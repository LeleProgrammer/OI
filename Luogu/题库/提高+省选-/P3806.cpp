...
int dfs(int u,int fa) {
    if (flag[u]) return sz[u]=0;
    sz[u]=1;
    _graph(i,u) if (e[i]!=fa) sz[u]+=dfs(e[i],u);
    return sz[u];
}
...
void solve(int u) {
    ...
    _graph(i,u) {
        idx=0;
        calc(e[i],u,w[i]);
        _rep(j,1,idx) _rep(k,1,m) {
            int t=q[k]-arr[j];
            if (t<0) continue;
            if (buc[t] || !t) ans[k]=true;
        }
        _rep(j,1,idx) if (arr[j]<=1e7) buc[arr[j]]=true,vec[++vsz]=arr[j];
    }
    _rep(i,1,vsz) buc[vec[i]]=false; vsz=0;
    flag[u]=true;
...
int main() {
    ...
    _rep(i,1,m) read(q[i]);
    solve(1);
    _rep(i,1,m) if (ans[i]) puts("AYE"); else puts("NAY");
    return 0;
}
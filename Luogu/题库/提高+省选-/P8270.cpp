...
int main() {
    ...
    _rep(i,1,m) pos[t[i]-'a'].emplace(i);
    _rep(i,1,n) {
        if (pos[s[i]-'a'].empty()) continue;
        else arr[i]=pos[s[i]-'a'].front(),pos[s[i]-'a'].pop();
    }
    _rep(i,0,17) _rep(j,0,17) if (pos[i].empty() && pos[j].empty()) {
        int las=0;
        bool ok=true;
        _rep(t,1,n) if (s[t]-'a'==i || s[t]-'a'==j) {
            if (arr[t]<las) {
                ok=false;
                break;
            }

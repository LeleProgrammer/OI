...
void pushup(int u) {
    tr[u].a=max(tr[u<<1].a,tr[u<<1|1].a);
    tr[u].b=min(tr[u<<1].b,tr[u<<1|1].b);
    tr[u].ab=max(max(tr[u<<1].ab,tr[u<<1|1].ab),tr[u<<1].a-tr[u<<1|1].b);
    tr[u].ba=max(max(tr[u<<1].ba,tr[u<<1|1].ba),tr[u<<1|1].a-tr[u<<1].b);
    tr[u].aba=max(max(max(tr[u<<1].aba,tr[u<<1|1].aba),tr[u<<1].ab+tr[u<<1|1].a),tr[u<<1].a+tr[u<<1|1].ba);
}
...
void modify(int u,int p,const pii& k) {
    if (tr[u].l==p && tr[u].r==p) {
        if (k.first) tr[u].a=k.first;
        if (k.second) tr[u].b=k.second;
    } else {
        int mid=tr[u].l+tr[u].r>>1;
...
Seg query(int u,int l,int r) {
    ...
    ans.a=max(L.a,R.a);
    ans.b=min(L.b,R.b);
    ans.ab=max(max(L.ab,R.ab),L.a-R.b);
    ans.ba=max(max(L.ba,R.ba),R.a-L.b);
    ans.aba=max(max(max(L.aba,R.aba),L.a+R.ba),L.ab+R.a);
    return ans;
}
...
i32 main() {
    ...
    while (m--) {
        ...
        if (op==1) modify(1,x,pii{y,0});
        else if (op==2) modify(1,x,pii{0,y});
        else writeln(query(1,x,y).aba);
    }
    return 0;
}
...
i32 main() {
    ...
    S=N-1,T=N-2;
    read(n);
    _rep(i,1,n) _rep(j,1,n) read(p[i][j]);
    _rep(i,1,n) _rep(j,1,n) read(q[i][j]);
    _rep(i,1,n) _rep(j,1,n) add_flow(i,n+j,1,-p[i][j]*q[j][i]);
    _rep(i,1,n) add_flow(S,i,1,0),add_flow(n+i,T,1,0);
    auto res=solve();
    write(-res.second);
    return 0;
}
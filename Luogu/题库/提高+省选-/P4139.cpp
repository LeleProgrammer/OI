...
inline int euler_phi(int k) {
    int ans=k;
    _rep(i,2,sqrt(k)) if (k%i==0) {
        ans=ans/i*(i-1);
        while (k%i==0) k/=i;
    }
...
inline int ksm(int a,int b,int p,bool& ans) {
    if (!b) return 1;
    int k=ksm(a,b>>1,p,ans);
    if (b&1) {
        int res=k*k;
        if (res>=p) res%=p,ans=true;
        res*=a;
        if (res>=p) res%=p,ans=true;
        return res;
    }
...
inline int solve(int p) {
    if (p==1) return 1;
    int t=solve(euler_phi(p));
    bool mod=false;
    int res=ksm(2,t,p,mod);
    if (mod) res+=p;
    return res;
}

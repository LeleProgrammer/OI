...
string s[N];
int tr[N][30],idx;
int e[M],ne[M],h[30],tot,indeg[30];
bool st[N];
string ans;

void add(int a,int b) {
    e[tot]=b,ne[tot]=h[a],h[a]=tot++,indeg[b]++;
}
...
bool check(string s) {
    ...
    _rep(i,0,(int)s.size()-1) {
        int c=s[i]-'a';
        _rep(j,0,25) if (tr[u][j] && j!=c) add(c,j);
        u=tr[u][c];
        if (st[u] && i!=(int)s.size()-1) return false;
    }
    queue<int> q;
    int cnt=0;
    _rep(i,0,25) if (!indeg[i]) q.emplace(i);
    ans="";
    while (!q.empty()) {
        int u=q.front(); q.pop();
        ans+=u+'a';
...
int main() {
    read(n);
    _rep(i,1,n) readstr(s[i]),insert(s[i]);
    _rep(i,1,n) if (check(s[i])) writestrln(ans); else writestrln("nemoguce");
    return 0;
}
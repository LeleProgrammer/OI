...
int main() {
    ...
    while (T--) {
        readstr(s);
        bool upp=0,low=0,num=0;
        int O=0,o=0,zer=0;
        _for(c,s) {
            if (islower(c) && c!='o') low=true;
            else if (isupper(c) && c!='O') upp=true;
            else if (isdigit(c) && c!='0') num=true;
            if (c=='O') O++;
            else if (c=='o') o++;
            else if (c=='0') zer++;
        }
        ...
        if (O && !upp) upp=true,O--;
        if (o && !low) low=true,o--;
        if (zer && !num) num=true,zer--;
        int cnt=!upp;
        cnt+=!low,cnt+=!num;
        if (O+o+zer<cnt) puts("-1");
        else writeln(cnt);
    }
    return 0;
}
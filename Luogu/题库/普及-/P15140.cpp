...
int main() {
    ...
    while (T--) {
        ...
        _rep(i,1,n) read(a[i]);
        if (n==1) {
            _rep(i,1,a[1]) if (a[1]%i==0) {
                writeln(a[1]/i);
                break;
            }
            continue;
        } else if (n==2) {
            if (a[1]>a[2]) writeln(a[1]/(a[2]-1));
            else writeln(max(gcd(a[1],a[2]),a[2]/gcd(a[1],a[2])));
            continue;
        }
        _rep(i,1,n) {
            int g=a[i];
            int j=i;
            while (j+1<=n && a[j]<a[j+1]) g=gcd(g,a[++j]);
            if (i==j) continue;
            int ans=max(g,a[j]/g);
            if (i!=1) ans=max(ans,a[i-1]/(g-1));
            if (j!=n) ans=max(ans,a[j+1]);
            writeln(ans);
            break;
        }
    }

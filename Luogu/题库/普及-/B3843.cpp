...
inline bool check() {
    _for(c,t) if (!islower(c) && !isupper(c) && !isdigit(c) && c!='!' && c!='@' && c!='#' && c!='$') return false;
    if (t.size()<6 || t.size()>12) return false;
    int upp=0,low=0,dig=0,spe=0;
    _for(c,t) {
        if (islower(c)) low=1;
        else if (isupper(c)) upp=1;
        else if (isdigit(c)) dig=1;
        else spe=1;
    }
    if (upp+low+dig<2) return false;
    if (!spe) return false;
    return true;
}

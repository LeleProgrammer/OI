...
void solve(int u,int fa) {
    ...
    _graph(i,u) {
        ...
        solve(e[i],u);
        if (flag[e[i]]) {
            cur+=(f[u][0]+f[u][1])*(f[e[i]][0]+f[e[i]][1]);
            f[u][1]+=f[e[i]][0]+f[e[i]][1];
        } else {
            cur+=f[u][0]*f[e[i]][1]+f[u][1]*(f[e[i]][0]+f[e[i]][1]);
            f[u][0]+=f[e[i]][0];
            f[u][1]+=f[e[i]][1];
        }
    }
    f[u][0]+=(deg[u]==1);
    f[u][0]*=max(1LL,inverse(deg[u]-1));
    f[u][1]*=max(1LL,inverse(deg[u]-1));
    cur*=max(1LL,inverse(deg[u]-1));
    ans+=cur;
}
...
i32 main() {
    ...
    while (T--) {
        ...
        dfs(root,0,1);
        _rep(i,1,k) {
            if (dep[o[g[i]].a]>dep[o[g[i]].b]) swap(o[g[i]].a,o[g[i]].b);
            int s=o[g[i]].b;
            flag[s]=true;
        }
        prod=1;
        _rep(i,1,n) prod*=fac[deg[i]-1];
        ans=0;
        solve(root,0);
        writeln((int)(prod*ans));
    }
    return 0;
}
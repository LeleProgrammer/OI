...
double dfs(int pos,int sco,int tar) {
    if (pos>n) return 0.0;
    auto solve=[&](int ptr) {
        if (ptr>tar) return 0.0;
        else return f[tar-ptr];
    };
    double p1=max(solve(sco+s[pos][1]),dfs(pos+1,sco+s[pos][1],tar));
    double p2=max(solve(sco+s[pos][2]),dfs(pos+1,sco+s[pos][2],tar));
    double a=u[pos][0]*solve(sco*c/100)+u[pos][1]*p1+u[pos][2]*p2;
    double b=v[pos][0]*solve(sco*c/100)+v[pos][1]*p1+v[pos][2]*p2;
    return max(a,b);
}
...
int main() {
    ...
    _rep(i,1,n) {
        ...
        _rep(j,1,2) read(s[i][j]);
        _rep(j,0,2) read(u[i][j]),su+=u[i][j];
        _rep(j,0,2) read(v[i][j]),sv+=v[i][j];
        _rep(j,0,2) u[i][j]=u[i][j]/su;
        _rep(j,0,2) v[i][j]=v[i][j]/sv;
    }
    f[0]=1;
    _rep(i,1,m) {
        double l=0,r=1;

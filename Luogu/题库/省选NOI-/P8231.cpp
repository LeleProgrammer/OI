...
void modify(int x,int y,int k) {
    _reps(i,x,n,i+=lowbit(i)) _reps(j,y,m,j+=lowbit(j)) tr[i][j]+=k;
}

int query(int x,int y) {
    int ans=0;
    _rreps(i,x,1,i-=lowbit(i)) _rreps(j,y,1,j-=lowbit(j)) ans+=tr[i][j];
    return ans;
}
...
void solve(int l,int r,vector<int>& que) {
    int ll=l,rr=r;
    if (que.empty()) _rep(i,l,r) _for(g,vec[i]) modify(g.x,g.y,-g.z);
    else if (l==r) {
        _for(i,que) qs[i].ans=l;
        _for(g,vec[l]) modify(g.x,g.y,-g.z);
    } else {
        int mid=(l+r>>1)+1;
        _rep(i,l,mid) _for(g,vec[i]) modify(g.x,g.y,-g.z);
        vector<int> lqu,rqu;
        _for(i,que) {
            int l=qs[i].l,r=qs[i].r;
            int L=qs[i].L,R=qs[i].R;
            int b=qs[i].b;
            if (query(l,r,L,R)<b) lqu.emplace_back(i);
            else rqu.emplace_back(i);
        }
        _rep(i,l,mid) _for(g,vec[i]) modify(g.x,g.y,g.z);
        solve(l,mid-1,lqu);
        solve(mid,r,rqu);
...
i32 main() {
    ...
    // FRW("P8231_1.out");
    read(n),read(m);
    _rep(i,1,n) _rep(j,1,m) read(a[i][j]),modify(i,j,a[i][j]);
    read(q);
    _rep(i,1,q) read(qs[i].l),read(qs[i].r),read(qs[i].L),read(qs[i].R),read(qs[i].b);
    read(t);
    _rep(i,1,t) {
        int p;
        read(p);
        while (p--) {
            ...
            vec[i].emplace_back(Event{x,y,z});
        }
    }
    vector<int> g;
    _rep(i,1,q) g.emplace_back(i);
    solve(1,t,g);
    memset(tr,0,sizeof(tr));
    _rep(i,1,n) _rep(j,1,m) modify(i,j,a[i][j]);
    _for(g,vec[1]) modify(g.x,g.y,-g.z);
    _rep(i,1,q) {
        int l=qs[i].l,r=qs[i].r;
        int L=qs[i].L,R=qs[i].R;
        int b=qs[i].b;
        if (query(l,r,L,R)<b) qs[i].ans=0;
    }
    _rep(i,2,t) _for(g,vec[i]) modify(g.x,g.y,-g.z);
    _rep(i,1,q) {
        int l=qs[i].l,r=qs[i].r;
        int L=qs[i].L,R=qs[i].R;
        int b=qs[i].b;
        if (query(l,r,L,R)>=b) qs[i].ans=-1;
    }
    _rep(i,1,q) writesp(qs[i].ans);
    return 0;
}
...
inline void add(int x) {
    cnt[x]++;
    if (cnt[x]==1) inc[x]=1,mns[N-x]=1;
}

inline void del(int x) {
    cnt[x]--;
    if (!cnt[x]) inc[x]=0,mns[N-x]=0;
}

inline bool qdec(int x) {
    return (inc&(inc>>x)).count();
}

inline bool qadd(int x) {
    return (inc&(mns>>(N-x))).count();
}

inline bool qmul(int x) {
    _rep(i,1,sqrt(x)+1) if (x%i==0 && cnt[i] && cnt[x/i]) return true;
    return false;
}
...
int main() {
    read(n,m),len=sqrt(n);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,m) read(qs[i].op,qs[i].l,qs[i].r,qs[i].x),qs[i].i=i;
    sort(qs+1,qs+1+m);
    int i=1,j=0;
    _rep(t,1,m) {
        int op=qs[t].op,l=qs[t].l,r=qs[t].r,x=qs[t].x,id=qs[t].i;
        while (i<l) del(a[i++]);
        while (i>l) add(a[--i]);
        while (j<r) add(a[++j]);
        while (j>r) del(a[j--]);
        if (op==1) ans[id]=qdec(x);
        else if (op==2) ans[id]=qadd(x);
        else ans[id]=qmul(x);
    }
    _rep(i,1,m) puts(ans[i]?"hana":"bi");
    return 0;
}
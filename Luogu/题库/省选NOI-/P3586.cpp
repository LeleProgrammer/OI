...
i32 main() {
    ...
    _rep(i,1,m) qs[i].b=LB(nums,qs[i].b)+1;
    _rep(i,1,m) {
        char op=qs[i].op;
        if (op=='U') {
            int k,a;
            k=qs[i].a,a=qs[i].b;
            if (arr[k]) cnt.modify(arr[k],-1),sum.modify(arr[k],-nums[arr[k]-1]);
            arr[k]=a;
            if (arr[k]) cnt.modify(arr[k],1),sum.modify(arr[k],nums[arr[k]-1]);
        } else {
            int c,s;
            c=qs[i].a,s=qs[i].b;
            int k=cnt.query(N-1)-cnt.query(s);
            int sm=sum.query(s)+k*nums[s-1];
            if (sm>=c*nums[s-1]) puts("TAK");
            else puts("NIE");
        }
    }
    return 0;

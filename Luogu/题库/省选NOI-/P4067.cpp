...
int T,n,m,k,p;
int f[N][2][2][2],g[N][2][2][2];

inline pii dfs(int pos,int gn,int gm,int gk) {
    if (!~pos) return pii{1,0};
    if (~f[pos][gn][gm][gk]) return pii{f[pos][gn][gm][gk],g[pos][gn][gm][gk]};
    int sf=0,sg=0;
    _rep(i,0,gn?kthbit(n,pos):1) _rep(j,0,gm?kthbit(m,pos):1) if ((i^j)>=kthbit(k,pos) || !gk) {
        auto t=dfs(pos-1,gn && (i==kthbit(n,pos)),gm && (j==kthbit(m,pos)),gk && (i^j)==kthbit(k,pos));
        sf+=t.first,sg+=t.second+(t.first*(i^j)%p)*((1LL<<pos)%p);
        sf%=p,sg%=p;
    }
    return pii{f[pos][gn][gm][gk]=sf,g[pos][gn][gm][gk]=sg};
}
...
i32 main() {
    ...
    while (T--) {
        memset(f,-1,sizeof(f));
        read(n,m,k,p),n--,m--;
        auto res=dfs(62,1,1,1);
        writeln(((res.second-res.first*(k%p))%p+p)%p);
    }
    return 0;
}
...
inline void init_cnt() {
    ...
    while (true) {
        ...
        cur=(cur*10+1)%p;
        if (vis[cur]) {
            i128 len=now-vis[cur]+1;
            i128 rem=n-now;
            i128 g=rem/len;
            i128 md=rem%len;
            _rep(i,0,p-1) if (vis[i]>=vis[cur]) cnt[i]+=g;
            while (md--) {
                las=cur;
                cnt[cur]++;
                cur=(cur*10+1)%p;
...
inline void init_C() {
    _rep(i,0,p-1) _rep(t,0,9) {
        C[i][t]=1;
        if (!cnt[i] && t!=0) {
            C[i][t]=0;
            continue;
        }
        _rrep(j,cnt[i]+t-1,cnt[i]) C[i][t]=(i128)C[i][t]*j%mod;
        _rep(j,1,t) C[i][t]=C[i][t]*Math::inverse(j)%mod;
    }
}
...
i32 main() {
    ...
    init_cnt();
    init_C();
    _rep(i,1,p) _rep(j,0,p-1) _rep(k,0,8) _rep(t,0,k) f[i][j][k]=(f[i][j][k]+f[i-1][((j-t*(i-1))%p+p)%p][k-t]*C[i-1][t])%mod;
    i128 ans=0;
    _rep(i,0,8) ans=(ans+f[p][0][i])%mod;
    write(ans);
    return 0;
}
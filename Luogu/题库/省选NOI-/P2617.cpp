    ...
    void add(int u,int l,int r,int p) {
        ...
        if (l==p && r==p) tr[u].sum++;
            ...
            if (p<=mid) {
                ...
                add(tr[u].r,mid+1,r,p);
            }
            pushup(u);
        }
    }
    
    bool del(int u,int l,int r,int p) {
        // cout<<"del: "<<u<<" "<<l<<" "<<r<<" "<<p<<endl;
        // _assert(u);
        if (l==p && r==p) {
            tr[u].sum--;
            if (!tr[u].sum) return delNode(u),true;
            return false;
        } else {
            int mid=l+r>>1;
            if (p<=mid) {
                if (del(tr[u].l,l,mid,p)) tr[u].l=0;
            } else {
                if (del(tr[u].r,mid+1,r,p)) tr[u].r=0;
            }
            pushup(u);
            if (!tr[u].l && !tr[u].r) {
                if (l!=0 || r!=1e9) delNode(u);
                return true;
            }
            return false;
        }
...
namespace FEN {
    ...
    void add(int x) {
        int y=a[x];
        while (x<=n) DSGT::add(root[x],0,1e9,y),x+=lowbit(x);
    }

    void del(int x) {
        int y=a[x];
        while (x<=n) DSGT::del(root[x],0,1e9,y),x+=lowbit(x);
    }

    void query(int x,vector<int>& ans) {
        while (x) ans.emplace_back(x),x-=lowbit(x);
    }
...
int main() {
    _rrep(i,M-1,1) stk.emplace(i);
    read(n),read(m);
    _rep(i,1,n) read(a[i]),FEN::root[i]=DSGT::newNode();
    FEN::build();
    while (m--) {
        char op[5];
        readstr(op);
        if (!strcmp(op,"Q")) {
            ...
            while (l!=r) {
                int sum=0,mid=l+r>>1;
                _rep(i,0,(int)vec.size()-1) sum+=DSGT::tr[DSGT::tr[vec[i]].l].sum;
                _rep(i,0,(int)dec.size()-1) sum-=DSGT::tr[DSGT::tr[dec[i]].l].sum;
                if (sum>=k) {
                    r=mid;
                    _rep(i,0,(int)vec.size()-1) vec[i]=DSGT::tr[vec[i]].l;
                    _rep(i,0,(int)dec.size()-1) dec[i]=DSGT::tr[dec[i]].l;
                } else {
                    k-=sum;
                    l=mid+1;
                    _rep(i,0,(int)vec.size()-1) vec[i]=DSGT::tr[vec[i]].r;
                    _rep(i,0,(int)dec.size()-1) dec[i]=DSGT::tr[dec[i]].r;
                }
            }
            writeln(l);
        } else {

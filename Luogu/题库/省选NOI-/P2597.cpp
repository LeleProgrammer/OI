...
inline int lca(int a,int b) {
    if (a==b) return a;
    if (dep[a]<dep[b]) swap(a,b);
    _rrep(i,K-1,0) if (dep[fa[a][i]]>=dep[b]) a=fa[a][i];
    if (a==b) return a;
    _rrep(i,K-1,0) if (fa[a][i]!=fa[b][i]) a=fa[a][i],b=fa[b][i];
    return fa[a][0];
}
...
inline void topo() {
    ...
    while (!q.empty()) {
        ...
        _graph(i,u) {
            if (!~g[e[i]]) g[e[i]]=u;
            else g[e[i]]=lca(g[e[i]],u);
            if (!--indeg[e[i]]) {
                q.emplace(e[i]);
                dep[e[i]]=dep[g[e[i]]]+1;
                fa[e[i]][0]=g[e[i]];
                _rep(j,1,K-1) fa[e[i]][j]=fa[fa[e[i]][j-1]][j-1];
                vec.emplace_back(pii{g[e[i]],e[i]});
            }
        }
    }
}
...
inline void solve(int u) {
    ans[u]=1;
    _graph(i,u) solve(e[i]),ans[u]+=ans[e[i]];
}
...
int main() {
    ...
    _rep(i,1,n) {
        int k;
        read(k);
        while (k) add(k,i),indeg[i]++,read(k);
    }
    _rep(i,1,n) if (!indeg[i]) add(0,i),indeg[i]++;
    topo();
    Graph::restore_graph();
    _for(t,vec) add(t.first,t.second);
    solve(0);
    _rep(i,1,n) writeln(ans[i]-1);
    return 0;
}
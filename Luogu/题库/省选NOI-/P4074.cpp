...
struct Query {
    ...
    vector<int> ext;
    _sortf(Query) {
        if (get(a.x)!=get(b.x)) return get(a.x)<get(b.x);
...
inline void add(int x) {
    cnt[t[x]]++;
    cur+=g[cnt[t[x]]]*v[t[x]];
    // cout<<"add: "<<x<<" "<<sta[x]<<" "<<cur<<endl;
}

inline void del(int x) {
    cur-=g[cnt[t[x]]]*v[t[x]];
    cnt[t[x]]--;
    // cout<<"del: "<<x<<" "<<sta[x]<<" "<<cur<<endl;
}
...
i32 main() {
    ...
    _rep(i,1,q) {
        ...
        if (!op) qmod[++ms]={x,y};
            ...
            ++qs;
            int anc=lca(x,y);
            if (anc!=x && anc!=y) qque[qs].ext.emplace_back(anc);
            if (fir[x]>fir[y]) swap(x,y);
            if (anc!=x) qque[qs].ext.emplace_back(x);
            qque[qs]={fir[x],fir[y],ms,qs,qque[qs].ext};
            // cout<<"fr: "<<fir[x]<<" to: "<<fir[y]<<" anc: "<<anc<<" size: "<<qque[qs].ext.size()<<endl;
        }
    }
    _sorta(qque,1,qs);
    int curm=0;
    int l=1,r=0;
    _rep(tt,1,qs) {
        int x=qque[tt].x,y=qque[tt].y,tm=qque[tt].t,i=qque[tt].id;
        while (l<x) {
            if (!sta[id[l]]) add(id[l]);
            else del(id[l]);
            sta[id[l++]]^=1;
        }
        while (l>x) {
        ...
        _for(u,qque[tt].ext) add(u);
        ans[i]=cur;
        _for(u,qque[tt].ext) del(u);
    }
    _rep(i,1,qs) writeln(ans[i]);
    return 0;
}

...
int main() {
    ...
    read(m),read(n);
    _rep(i,1,n) _rep(j,1,m) read(t[i][j]);
    _rep(i,1,n) _rep(j,1,m) _rep(k,1,n) add(i,n+(k-1)*m+j,1,t[i][j]*k);
    _rep(i,1,n) add(S,i,1,0);
    _rep(j,1,m) _rep(k,1,n) add(n+(k-1)*m+j,T,1,0);
    printf("%.2lf",(double)solve().second/n);
    return 0;
}
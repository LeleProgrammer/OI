...
inline void pushdown(int x) {
    _rep(i,BL[x],BR[x]) v[i]+=addv[x];
    i64 cur=addv[x];
    _rrep(i,BR[x],BL[x]) suf[i]+=cur,cur+=addv[x];
    addv[x]=0;
}

inline void rebuild(int x,bool fvec=false) {
    if (fvec) {
        _rep(i,0,N-1) vec[x][i]=-1;
        _rep(i,BL[x],BR[x]) st[i]={a[i],i};
        sort(st+BL[x],st+BR[x]+1);
        _rep(i,BL[x],BR[x]) if (!~vec[x][st[i].a]) vec[x][st[i].a]=i-BL[x];
        _rrep(i,N-2,0) if (!~vec[x][i]) vec[x][i]=vec[x][i+1];
    }
    suf[BR[x]]=v[st[BR[x]].b];
    _rrep(i,BR[x]-1,BL[x]) suf[i]=suf[i+1]+v[st[i].b];
}
...
inline void modify(int p,int w) {
    ...
    _rep(i,L[p],p) v[i]+=w;
    rebuild(B[p]);
    _rep(i,1,B[p]-1) addv[i]+=w;
}

inline i64 query(int p,int d) {
    // cout<<"query: "<<p<<" "<<d<<": ";
    pushdown(B[p]);
    i64 ans=0;
    _rep(i,L[p],p) if (a[i]>=d) ans+=v[i];
    // cout<<"cur: "<<ans<<endl;
    _rep(i,1,B[p]-1) if (~vec[i][d]) ans+=suf[BL[i]+vec[i][d]]+addv[i]*(BR[i]-(BL[i]+vec[i][d])+1);
    return ans;
}
...
int main() {
    ...
    while (m--) {
        ...
        // _rep(i,1,Bn) pushdown(i);
        // cout<<"test v: "; _rep(i,1,n) cout<<v[i]<<" "; cout<<endl;
        // cout<<"test suf: "; _rep(i,1,n) cout<<suf[i]<<" "; cout<<endl;
    }
    return 0;
}
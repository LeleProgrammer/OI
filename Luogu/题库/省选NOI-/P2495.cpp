...
inline void solve(int u,int val) {
    if (flag[u]) dp[u]=val;
        ...
        int sum=0;
        _for(v,G[u]) solve(v.first,v.second),sum+=dp[v.first];
        dp[u]=min(val,sum);
        if (u==1) dp[u]=sum;
    }
}
    ...
    while (m--) {
        ...
        _for(u,vec) {
            ...
            while (true) {
                int top=stk[ptr];
                int sub=stk[ptr-1];
                if (dep[g]>dep[sub]) break;
                G[sub].emplace_back(pii{top,mn[top]});
                dec.emplace(sub);
                ptr--;
            }
            top=stk[ptr];
            ...
            if (g==top) stk[++ptr]=u;
                ...
                G[g].emplace_back(pii{top,mn[top]});
                dec.emplace(g);
                ptr--;
                stk[++ptr]=g;
                stk[++ptr]=u;
        ...
        solve(1,inf);
        _for(u,vec) flag[u]=false;
        _iter(it,dec) G[*it].clear();
        writeln(dp[1]);
    }
    return 0;
}
...
inline void dfs(int u,int d) {
    ...
    _graph(i,u) if (!dep[e[i]]) {
        f[e[i]][0]=u;
        _rep(j,1,K-1) f[e[i]][j]=f[f[e[i]][j-1]][j-1];
        dfs(e[i],d+1),lf=false;
        cnt1+=(u==1);
    }
    if (lf) lvs.emplace_back(u);
}
...
int main() {
    ...
    if (lvs.size()>=n/3) {
        puts("2");
        _rep(i,0,n/3-1) writesp(lvs[i]);
    } else {
        puts("1");
        if (cnt1) lvs.emplace_back(1);
        int l=1,r=(lvs.size())/2+1,cnt=0;
        lvs.emplace(lvs.begin(),-1);
        while (r<lvs.size()) {
            cnt++;
            int a=lvs[l],b=lvs[r];
            int g=lca(a,b);
            queue<int> A;
            stack<int> B;
            while (a!=g) A.emplace(a),a=f[a][0];
            while (b!=g) B.emplace(b),b=f[b][0];
            vector<int> ans;
            while (!A.empty()) ans.emplace_back(A.front()),A.pop();
            ans.emplace_back(g);
            while (!B.empty()) ans.emplace_back(B.top()),B.pop();
            writesp(ans.size());
        ...
        int cur=1;
        while (cnt++<(n+5)/6) writesp(1),writeln(cur++);
    }
    return 0;
}
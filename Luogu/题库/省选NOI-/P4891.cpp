...
namespace Graph {
    ...
    namespace LCA {
        ...
        inline void init() { dep=new int[N]; f=new int*[N]; _rep(i,0,N-1) f[i]=new int[K]; memset(dep,0,sizeof(int)*N),memset(f,0,sizeof(int)*N); }
...
int n,q,len,nn;
int a[N],b[N],c[N];
tup2<int,int> stb[N]; // value, index
int mulb[N];
int L[N],R[N],BL[SQ],BR[SQ],B[N];
tup2<int,int> va[N],vb[N];
int mul[SQ];
int flag[SQ];
...
inline void modify(int l,int r,int x) {
    if (B[l]==B[r]) {
        ...
        _rep(i,l,r) c[i]=x;
        mul[B[l]]=1;
        _rep(i,L[l],R[l]) mul[B[i]]=mul[B[i]]*min(b[i],c[i])%mod;
    } else {
        ...
        _rep(i,L[r],r) c[i]=x;
        mul[B[r]]=1;
        _rep(i,L[r],R[r]) mul[B[i]]=mul[B[i]]*min(b[i],c[i])%mod;
        _rep(i,B[l]+1,B[r]-1) {
            flag[i]=x;
            // !!! 二分可能出现全部 >x 的情况
            if (mulb[BL[i]]>x) mul[i]=Math::ksm(x,BR[i]-BL[i]+1)%mod;
            else {
                int ll=BL[i],rr=BR[i];
                while (ll<rr) { // las (<=x)
                    int mid=(ll+rr>>1)+1;
                    if (stb[mid].a<=x) ll=mid;
                    else rr=mid-1;
                }
                mul[i]=mulb[ll]*Math::ksm(x,BR[i]-ll)%mod;
            }
        }
    }
}
...
inline void change(int pos,int x) {
    ...
    b[pos]=x;
    int ia=0,ib=0;
    vb[++ib]=tup2<int,int>{x,pos};
    _rep(i,L[pos],R[pos]) if (stb[i].b!=pos) va[++ia]=stb[i];
    int i=1,j=1,cur=L[pos];
    while (i<=ia && j<=ib) {
        if (va[i]<vb[j]) stb[cur++]=va[i++];
        else stb[cur++]=vb[j++];
    }
    while (i<=ia) stb[cur++]=va[i++];
    while (j<=ib) stb[cur++]=vb[j++];
    mul[B[pos]]=1;
    _rep(i,L[pos],R[pos]) {
        mul[B[i]]=mul[B[i]]*min(b[i],c[i])%mod;
        if (i!=L[pos]) mulb[i]=mulb[i-1]*stb[i].a%mod;
        else mulb[i]=stb[i].a%mod; // !!! mulb 是以排序后的 stb 为基础生成的
    }
}
...
i32 main() {
    ...
    read(n,q),len=sqrt(n);
    _rep(i,1,n) read(a[i]),c[i]=max(c[i-1],a[i]);
    _rep(i,1,n) read(b[i]),stb[i]=tup2<int,int>{b[i],i};
    _rep(i,1,n) B[i]=get(i),chmax(nn,B[i]);
    _rep(i,1,nn) mul[i]=1;
    _rep(i,1,n) mul[B[i]]=mul[B[i]]*min(b[i],c[i])%mod;
    ...
    _rep(i,1,nn) sort(stb+BL[i],stb+BR[i]+1);
    _rep(i,1,n) if (B[i]!=B[i-1]) mulb[i]=stb[i].a%mod; else mulb[i]=mulb[i-1]*stb[i].a%mod;
    while (q--) {
        int op,x,y;
        read(op,x,y);
        if (!op) {
            a[x]=y;
            if (a[x]>max(flag[B[x]],c[x])) { // !!! 可能标记没下传，不能直接取 c
                int l=x,r=n;
                while (l<r) {
                    int mid=(l+r>>1)+1;
                    if (max(c[mid],flag[B[mid]])<=a[x]) l=mid;
                    else r=mid-1;
                }
                modify(x,r,y);
            }

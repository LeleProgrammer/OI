...
int n,m,sum;
int dep[N],sz[N],son[N],top[N],fa[N],id[N],g[N],cnt;
DSU dsu(N,nullptr);
...
inline void pushup(int u) {
    ...
    int mx=-inf;
    if (tr[u<<1].mx!=tr[u].mx) chmax(mx,tr[u<<1].mx);
    if (tr[u<<1].smx!=tr[u].mx) chmax(mx,tr[u<<1].smx);
    if (tr[u<<1|1].mx!=tr[u].mx) chmax(mx,tr[u<<1|1].mx);
    if (tr[u<<1|1].smx!=tr[u].mx) chmax(mx,tr[u<<1|1].smx);
    tr[u].smx=mx;
}
...
inline pii query(int u,int l,int r) {
    if (tr[u].l>=l && tr[u].r<=r) return pii{tr[u].mx,tr[u].smx};
    else {
        int mid=tr[u].l+tr[u].r>>1;
        if (l<=mid && r>mid) {
            ...
            int mx=-inf;
            if (left.first!=ans.first) chmax(mx,left.first);
            if (left.second!=ans.first) chmax(mx,left.second);
            if (right.first!=ans.first) chmax(mx,right.first);
            if (right.second!=ans.first) chmax(mx,right.second);
            ans.second=mx;
            return ans;
...
inline pii query_path(int u,int v) {
    pii ans={-inf,-inf};
    while (top[u]!=top[v]) {
        if (dep[top[u]]<dep[top[v]]) swap(u,v);
        auto ret=query(1,id[top[u]],id[u]);
        pii nans;
        nans.first=max(ans.first,ret.first);
        int mx=-inf;
        if (ans.first!=nans.first) chmax(mx,ans.first);
        if (ans.second!=nans.first) chmax(mx,ans.second);
        if (ret.first!=nans.first) chmax(mx,ret.first);
        if (ret.second!=nans.first) chmax(mx,ret.second);
        nans.second=mx;
        ans=nans;
        u=fa[top[u]];
    }
    ...
    if (u!=v) {
        auto ret=query(1,id[son[v]],id[u]);
        pii nans;
        nans.first=max(ans.first,ret.first);
        int mx=-inf;
        if (ans.first!=nans.first) chmax(mx,ans.first);
...
i32 main() {
    Graph::init_graph();
    read(n,m);
    _rep(i,1,m) read(arr[i].x,arr[i].y,arr[i].z);
    sort(arr+1,arr+1+m);
    _rep(i,1,m) {
    ...
    _rep(i,1,m) if (!arr[i].flag) {
        int x=arr[i].x,y=arr[i].y,z=arr[i].z;
        auto ret=query_path(x,y);
        chmin(ans,(z==ret.first)?(z-ret.second):(z-ret.first));
    }
    write(sum+ans);
    return 0;
}
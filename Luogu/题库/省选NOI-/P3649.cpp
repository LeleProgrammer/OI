...
inline void extend(int n,int c) {
    int p=las;
    while (n-arr[p].len-1<=0 || s[n-arr[p].len-1]!=s[n]) p=arr[p].fa;
    if (!arr[p].s[c]) {
        int v=++idx,k=arr[p].fa;
        while (n-arr[k].len-1<=0 || s[n-arr[k].len-1]!=s[n]) k=arr[k].fa;
        arr[v].len=arr[p].len+2;
        arr[v].fa=arr[k].s[c];
        arr[p].s[c]=v;
        num[v]=num[arr[v].fa]+1;
    }
    las=arr[p].s[c];
    siz[las]++;
}
...
inline void topo() {
    _rrep(i,idx,1) siz[arr[i].fa]+=siz[i];
    _rep(i,1,idx) chmax(ans,(i64)siz[i]*arr[i].len);
}

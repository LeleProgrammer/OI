...
namespace DSeg {
    ...
    inline void insert(int u,int l,int r,int p) {
        if (l==p && r==p) tr[u].sum++;
            ...
            if (p<=mid) {
                ...
                insert(tr[u].l,l,mid,p);
            } else {
                if (!tr[u].r) tr[u].r=++idx;
                insert(tr[u].r,mid+1,r,p);
            }
            pushup(u);
        }
    }
    ...
    inline int query(int u,int l,int r,int L,int R) {
        ...
        if (l>=L && r<=R) return tr[u].sum;
            ...
            int mid=l+r>>1;
            int ans=0;
            if (L<=mid) ans=ans+query(tr[u].l,l,mid,L,R);
            if (R>mid) ans=ans+query(tr[u].r,mid+1,r,L,R);
            return ans;
        }
    }
}
...
namespace Seg {
    ...
    inline void build(int u,int l,int r) {
        tr[u]={l,r,++idx};
        _rep(i,l,r) DSeg::insert(tr[u].root,1,N-1,arr[i]);
        if (l==r) return;
        int mid=l+r>>1;
        build(u<<1,l,mid);
        build(u<<1|1,mid+1,r);
    ...
    inline int query(int u,int l,int r,int a,int b) {
        if (tr[u].l>=l && tr[u].r<=r) return DSeg::query(tr[u].root,1,N-1,a,b);
        int mid=tr[u].l+tr[u].r>>1;
        int ans=0;
        if (l<=mid) ans=ans+query(u<<1,l,r,a,b);
        if (r>mid) ans=ans+query(u<<1|1,l,r,a,b);
        return ans;
    }
}
...
struct ask {
    int l,r,a,b,i;
    inline friend bool operator < (const ask& a,const ask& b) {
...
inline int query(int l,int r) {
    if (B[l]==B[r]) {
        int ans=0;
        _rep(i,l,r) ans+=!!cnt[i];
        return ans;
    }
    int ans=0;
    _rep(i,B[l]+1,B[r]-1) ans+=sum[i];
    _rep(i,l,R[l]) ans+=!!cnt[i];
    _rep(i,L[r],r) ans+=!!cnt[i];
    return ans;
}

inline void init() {
    len2=sqrt(N);
    _rep(i,1,N-1) B[i]=get(i);
    _rep(i,1,N-1) if (B[i]==B[i-1]) L[i]=L[i-1]; else L[i]=i;
    _rrep(i,N-1,1) if (i==N-1 || B[i]!=B[i+1]) R[i]=i; else R[i]=R[i+1];
}

int main() {
    init();
    read(n,m),len=sqrt(n)+1;
    ...
    int i=1,j=0;
    _rep(t,1,m) {
        int l=qs[t].l,r=qs[t].r,a=qs[t].a,b=qs[t].b,id=qs[t].i;
        while (i<l) del(arr[i++]);
        while (i>l) add(arr[--i]);
        while (j<r) add(arr[++j]);
        while (j>r) del(arr[j--]);
        ans1[id]=Seg::query(1,l,r,a,b);
        ans2[id]=query(a,b);
    }
    _rep(i,1,m) writespln(ans1[i],ans2[i]);
    return 0;
}
...
void modify(int p,int u,int l,int r,int k) {
    if (l==k && r==k) tr[u].v++;
        ...
        if (k<=mid) {
            ...
            modify(tr[p].l,tr[u].l,l,mid,k);
        } else {
            tr[u].l=tr[p].l;
            if (!tr[u].r) tr[u].r=++idx;
            modify(tr[p].r,tr[u].r,mid+1,r,k);
        }
        pushup(u);
    }
}
...
void build(int u,int fa) {
    root[u]=++idx;
    modify(root[fa],root[u],1,2147483647,arr[u]);
...
int main() {
    ...
    while (m--) {
        ...
        int g=lca(u,v);
        int t=f[g][0];
        cerr<<"ask: "<<u<<" "<<v<<" "<<k<<" "<<g<<endl;
        g=root[g],u=root[u],v=root[v],t=root[t];
        int l=1,r=2147483647;
        while (l<r) {
            int mid=(i64)l+r>>1;
            int sum=tr[tr[u].l].v+tr[tr[v].l].v-tr[tr[g].l].v-tr[tr[t].l].v;
            cerr<<"get: "<<l<<" "<<mid<<" "<<r<<" "<<sum<<endl;
            if (k<=sum) g=tr[g].l,u=tr[u].l,v=tr[v].l,t=tr[t].l,r=mid;
            else k-=sum,g=tr[g].r,u=tr[u].r,v=tr[v].r,t=tr[t].r,l=mid+1;
        }
        writeln(las=r);
    }
    return 0;

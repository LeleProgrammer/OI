...
inline void pushdown(int u) {
    ...
    if (~root.set) {
        left.mx=left.set=right.mx=right.set=root.set;
        left.add=left.addp=right.add=right.addp=0;
        root.set=-1;
    }
    if (root.add) {
        left.add+=root.add,right.add+=root.add;
        left.mx+=root.add,right.mx+=root.add;
        root.add=0;
    }
    if (root.addp) {
        left.addp+=root.addp,right.addp+=root.addp;
        left.mx+=root.addp*nums[left.r],right.mx+=root.addp*nums[right.r];
        root.addp=0;
    }
}
...
inline void modify_set(int u,int l,int r,i64 k) {
    if (tr[u].l>=l && tr[u].r<=r) {
        tr[u].set=tr[u].mx=k;
        tr[u].add=tr[u].addp=0;
    } else {
        pushdown(u);
        int mid=tr[u].l+tr[u].r>>1;
...
inline void modify_add(int u,int l,int r,i64 k) {
    if (tr[u].l>=l && tr[u].r<=r) {
        tr[u].mx+=k;
        tr[u].add+=k;
    } else {
...
inline void modify_addp(int u,int l,int r) {
    if (tr[u].l>=l && tr[u].r<=r) {
        tr[u].addp++;
        tr[u].mx+=nums[tr[u].r];
    } else {
        pushdown(u);
        int mid=tr[u].l+tr[u].r>>1;
        if (l<=mid) modify_addp(u<<1,l,r);
        if (r>mid) modify_addp(u<<1|1,l,r);
        pushup(u);
    }
}

inline i64 query(int u,int p) {
...
int main() {
    ...
    _rep(i,1,n) a[i]=LB(nums,a[i]);
    int m=nums.size()-1;
    build(1,1,m);
    _rep(i,1,n) {
        if (a[i]-1) modify_add(1,1,a[i]-1,c);
        modify_add(1,a[i],m,-nums[a[i]]);
        modify_addp(1,a[i],m);
        if (a[i]==1) continue;
        i64 dest=query(1,a[i]);
        if (query(1,1)>dest) modify_set(1,1,a[i]-1,dest);
        else {
            int u=1;
            while (tr[u].l!=tr[u].r) {
                int mid=(tr[u].l+tr[u].r>>1)+1;
                if (mid>=a[i]) u<<=1;
                else if (query(1,mid)>=dest) u<<=1;
                else u<<=1,u|=1;
            }
            modify_set(1,tr[u].l+1,a[i]-1,dest);
        }
    }
    i64 ans=1e18;
    _rep(i,1,m) chmin(ans,query(1,i));
    write(ans);
    return 0;
}
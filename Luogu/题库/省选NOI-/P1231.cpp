...
int main() {
    memset(h,-1,sizeof(h));
    read(n1),read(n2),read(n3),S=N-1,T=N-2;
    int m;
    read(m);
    while (m--) {
        int x,y;
    ...
    while (m--) {
        int x,y;
        read(x),read(y);
        FLOW::add(n1+n2+x,n1+n2*2+y,1);
    }
    _rep(i,1,n1) FLOW::add(S,i,1);
    _rep(i,1,n3) FLOW::add(n1+n2*2+i,T,1);
    _rep(i,1,n2) FLOW::add(n1+i,n1+n2+i,1);
    write(dinic());
    return 0;
}
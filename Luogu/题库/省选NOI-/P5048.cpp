...
inline int query(int l,int r) {
    if (B[l]==B[r] || B[l]+1==B[r]) {
        int mx=0;
        _rep(i,l,r) chmax(mx,++cnt[a[i]]);
        _rep(i,l,r) cnt[a[i]]--;
        return mx;
    } else {
        int ans=g[B[l]+1][B[r]-1];
        _rep(i,l,R[l]) while (pos[i]+ans<=(int)vec[a[i]].size()-1 && vec[a[i]][pos[i]+ans]<=r) ans++;
        _rep(i,L[r],r) while (pos[i]-ans>=0 && vec[a[i]][pos[i]-ans]>=l) ans++;
        return ans;
    }
}
...
int main() {
    ...
    _rep(i,1,Bn) {
        int mx=0;
        _rep(j,i,Bn) {
            _rep(k,BL[j],BR[j]) chmax(mx,++cnt[a[k]]);
            g[i][j]=mx;
        }
        _rep(j,i,Bn) _rep(k,BL[j],BR[j]) cnt[a[k]]--;
    }
    _rep(i,1,n) vec[a[i]].emplace_back(i),pos[i]=(int)vec[a[i]].size()-1;
    while (m--) {
        int l,r;
        read(l,r);
        l^=las,r^=las;
        writeln(las=query(l,r));
    }
    return 0;
}
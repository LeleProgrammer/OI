...
inline void add(int k) {
    cnt[k]++;
    chmax(cur,(i64)nums[k]*cnt[k]);
}

int main() {
    read(n,q),len=sqrt(n);
    _rep(i,1,n) read(x[i]),nums.emplace_back(x[i]);
    _sort(nums),_unique(nums);
    _rep(i,1,n) x[i]=LB(nums,x[i]);
    _rep(i,1,q) read(qs[i].l,qs[i].r),qs[i].i=i;
    sort(qs+1,qs+1+q);
    _rep(a,1,q) {
        int b=a,mx=qs[a].l;
        while (b+1<=q && get(qs[b+1].l)==get(qs[a].l)) chmax(mx,qs[++b].l);
        while (a<=b && get(qs[a].l)==get(qs[a].r)) {
            memset(cnt,0,sizeof(cnt)),cur=0;
            _rep(i,qs[a].l,qs[a].r) add(x[i]);
            ans[qs[a].i]=cur;
            a++;
        }
        int r=mx-1;
        memset(cnt,0,sizeof(cnt)),cur=0;
        _rep(i,a,b) {
            while (r<qs[i].r) add(x[++r]);
            bkcur=cur;
            _rep(i,0,n-1) bk[i]=cnt[i];
            _rrep(j,mx-1,qs[i].l) add(x[j]);
            ans[qs[i].i]=cur;
            cur=bkcur;
            _rep(i,0,n-1) cnt[i]=bk[i];
        }
        a=b;
    }

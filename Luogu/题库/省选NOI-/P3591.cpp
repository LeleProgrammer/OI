...
int n,len;
int a[N],b[N],cc[N];
int dep[N],mxdep[N],son[N],top[N],bot[N];
int fa[N][K];
...
inline void dfs1(int u,int p,int d) {
    ...
    _graph(i,u) {
        ...
        if (mxdep[e[i]]>mxdep[son[u]]) son[u]=e[i];
    }
}

inline void dfs2(int u,int t) {
    top[u]=t;
    down[top[u]].emplace_back(u);
    if (!son[u]) return;
    dfs2(son[u],t);
    _graph(i,u) {
        if (e[i]==son[u] || e[i]==fa[u][0]) continue;
        dfs2(e[i],e[i]);
    }
    if (u==t) {
        int m=mxdep[u]-dep[u];
        int v=u;
        while (v && m--) up[u].emplace_back(v),v=fa[v][0];
    }
}
...
inline int query(int u,int k) {
    ...
    u=fa[u][lg];
    if (!u) return 0;
    if (k<=dep[u]-dep[top[u]]) return down[top[u]][dep[u]-dep[top[u]]-k];
    else return up[top[u]][k-(dep[u]-dep[top[u]])];
}

inline void dfs3(int u) {
    _rep(i,1,len) mp[u][i]=mp[query(u,i)][i]+a[u];
    _graph(i,u) {
...
i32 main() {
    ...
    _rep(i,1,n) read(b[i]);
    _rep(i,1,n-1) read(cc[i]);
    LCA::dfs(1,0,1);
    dfs1(1,0,1);
    dfs2(1,1);
    dfs3(1);
    _rep(i,1,n-1) {
        int u=b[i],v=b[i+1],w=cc[i];
        int g=lca(u,v);
        if (w<=len) {
            int ans=mp[u][w]+mp[v][w];
            u=query(u,(dep[u]-dep[g])/w*w);
            v=query(v,(dep[v]-dep[g])/w*w);
            if (u==v) ans-=a[g];
            u=query(u,w);
            v=query(v,w);
            ans-=mp[u][w]+mp[v][w];

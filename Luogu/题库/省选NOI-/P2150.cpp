...
i32 main() {
    ...
    f[0][0]=1;
    _rep(i,2,n) { // ignore 1 !!
        if (arr[i].gre!=arr[i-1].gre || !arr[i-1].gre) _rep(p,0,255) _rep(q,0,255) if (!(p&q)) f1[p][q]=f2[p][q]=f[p][q];
        _rrep(p,255,0) _rrep(q,255,0) {
            if (p&q) continue;
            if (!(q&arr[i].bit)) f1[p|arr[i].bit][q]=(f1[p|arr[i].bit][q]+f1[p][q])%P;
            if (!(p&arr[i].bit)) f2[p][q|arr[i].bit]=(f2[p][q|arr[i].bit]+f2[p][q])%P;
        }
        if (arr[i].gre!=arr[i+1].gre || !arr[i].gre || i==n) _rep(p,0,255) _rep(q,0,255) if (!(p&q)) f[p][q]=((-f[p][q]+f1[p][q]+f2[p][q])%P+P)%P;
    }
    int ans=0;
    _rep(p,0,255) _rep(q,0,255) if (!(p&q)) ans=(ans+f[p][q])%P;
    write(ans);
    return 0;
}
...
inline int get(int i,int j) {
    if (j>=i && (j^i)>j-i) return 1;
    else return 0;
}

i32 work_sub1(i64 n,i64 x,i64 y) {
    memset(vis,0,sizeof(vis));
    _rep(i,0,(1<<n)-1) _rep(j,0,(1<<n)-1) {
        if (j>=i && (j^i)>j-i) a[i][j]=1;
        else a[i][j]=0;
    }
    queue<pii> q;
    q.emplace(pii{x,y});
    while (!q.empty()) {
        ...
        _rep(p,0,3) {
            ...
            if (!vis[nx][ny]) q.emplace(pii{nx,ny});
        }
    }
    int ans=0;
    _rep(i,0,(1<<n)-1) _rep(j,0,(1<<n)-1) ans+=vis[i][j];
    return ans;
}
...
// i32 work_sub4(i64 n,i64 x,i64 y) {
//     int t=ksm(2,n),s=n-1,cur=1;
//     int ans=t*t%mod;
//     t=t*inv%mod;
//     while (s--) {
//         ans-=calc(1,t-1)*cur;
...
inline void init() {
    ...
    inited=true;
    p1[0]=1;
    _rep(i,1,B-1) p1[i]=p1[i-1]*2%mod;
    i64 b=p1[B-1]*2%mod;
    p2[0]=1;
    _rep(i,1,B-1) p2[i]=p2[i-1]*b%mod;
    q1[0]=1;
    _rep(i,1,B-1) q1[i]=q1[i-1]*3%mod;
    b=q1[B-1]*3%mod;
    q2[0]=1;
    _rep(i,1,B-1) q2[i]=q2[i-1]*b%mod;
}

inline i64 ksm2(i64 n) {
    n%=(mod-1);
    return p2[n/B]*p1[n%B]%mod;
}

inline i64 ksm3(i64 n) {
    n%=(mod-1);
    return q2[n/B]*q1[n%B]%mod;
}
...
i32 work_sub5(i64 n,i64 x,i64 y) {
    n=max(64-_pre0(x),64-_pre0(y))+1;
    rint t=(1LL<<(n-1));
    while (true) {
        if (x<=t-1 && 1<=y-t+1 && y-t+1<=x) return t*(t-1)/2%mod;
        if (x>t) x-=t;
        if (y>t) y-=t;
        t>>=1;
    }
...
extern "C" i32 work(i64 n,i64 x,i64 y) {
    init();
    // if (n<=10) return work_sub1(n,x,y);
    if (get(x,y)==0) return work_sub4(n,x,y);
    if (get(x,y)==1) return work_sub5(n,x,y);
    return -1;
}
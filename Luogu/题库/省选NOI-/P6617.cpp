...
SEG<val,pushup,pushdown,const int,modify> seg(N);

int main() {
    read(n,m,w);
    _rep(i,1,n) read(a[i]),st[min(a[i],w-a[i])].emplace(i);
    _rep(i,0,w) st[i].emplace(0),st[i].emplace(n+1);
    _rep(i,1,n) ne[i]=n+1;
    _rep(i,1,n) {
        int g=min(a[i],w-a[i]);
        auto pos=st[g].upper_bound(i);
        if (a[*pos]+a[i]==w) ne[i]=*pos;
        else ne[i]=n+1;
    }
    _rep(i,1,n) seg.modify(1,i,i,ne[i]);
    while (m--) {
        int op,x,y;
        read(op,x,y);
        if (op==1) {
            int g=min(a[x],w-a[x]);
            st[g].erase(x);
            auto pos=prev(st[g].lower_bound(x));
            if (*pos) {
                ne[*pos]=a[*next(pos)];
                if (a[*next(pos)]+a[*pos]==w) ne[*pos]=*next(pos);
                else ne[*pos]=n+1;
                seg.modify(1,*pos,*pos,ne[*pos]);
            }
            a[x]=y;
            g=min(a[x],w-a[x]);
            st[g].emplace(x);
            pos=st[g].upper_bound(x);
            if (a[*pos]+a[x]==w) ne[x]=*pos;
            else ne[x]=n+1;
            seg.modify(1,x,x,ne[x]);
            pos=prev(st[g].lower_bound(x));
            if (*pos) {
                ne[*pos]=a[*next(pos)];
                if (a[*next(pos)]+a[*pos]==w) ne[*pos]=*next(pos);
                else ne[*pos]=n+1;
            ...
            x^=las,y^=las;
            int k=seg.query(1,x,y).mn;
            if (k<=y) puts("Yes"),las++;
            else puts("No");
        }
        cerr<<"out: ";
        _rep(i,1,n) cerr<<ne[i]<<" ";
        cerr<<endl;
    }
    return 0;
}

...
i32 main() {
    ...
    pre[0]=cnt[0];
    _rep(i,1,n) pre[i]=pre[i-1]+cnt[i];
    f[0][0][0]=1;
    _rep(i,0,n-1) {
        memset(f[i&1^1],0,sizeof(f[i&1^1]));
        _rep(j,0,i) _rep(k,0,i) if (f[i&1][j][k]) {
            if (s[i+1]=='1') {
                f[i&1^1][j][k+1]=(f[i&1^1][j][k+1]+f[i&1][j][k])%mod;
            } else {
                _rep(t,0,min(cnt[j+1],k+1)) f[i&1^1][j+1][k-t+1]=(f[i&1^1][j+1][k-t+1]+f[i&1][j][k]*C(cnt[j+1],t)%mod*C(k+1,t)%mod*Math::f[t])%mod;
            }
            _rep(t,0,min(cnt[j+1],k)) f[i&1^1][j+1][k-t]=(f[i&1^1][j+1][k-t]+f[i&1][j][k]*C(cnt[j+1],t)%mod*C(k,t)%mod*Math::f[t]%mod*(pre[j]-(i-k)))%mod;
        }
    }
    int ans=0;
    _rep(j,0,n-m) ans=(ans+f[n&1][j][n-pre[j]]*Math::f[n-pre[j]])%mod;
    write(ans);
    return 0;
}
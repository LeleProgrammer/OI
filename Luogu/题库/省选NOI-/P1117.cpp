...
i64 ans;
char s[N];
int lsh[N],cnt[N],st[N],sa[N],rk[N],lcp[N],lcs[N];
int rklcp[N],rklcs[N];
int stlcp[N][K],stlcs[N][K];
int sr[N],ed[N];
...
inline void build_sa() {
    ...
    _reps(k,1,n,k<<=1) {
        ...
        swap(st,lsh),lsh[sa[m=1]]=1;
        _rep(i,2,n) lsh[sa[i]]=(st[sa[i]]==st[sa[i-1]] && st[sa[i]+k]==st[sa[i-1]+k])?m:++m;
        if (m==n) break;
    }
...
inline void build_stlcp() {
    _rep(i,1,n) stlcp[i][0]=lcp[i];
    _rep(k,1,K-1) _rep(i,1,n) {
        stlcp[i][k]=stlcp[i][k-1];
        if (i+(1<<(k-1))<=n) chmin(stlcp[i][k],stlcp[i+(1<<(k-1))][k-1]);
    }
}

inline void build_stlcs() {
    _rep(i,1,n) stlcs[i][0]=lcs[i];
    _rep(k,1,K-1) _rep(i,1,n) {
        stlcs[i][k]=stlcs[i][k-1];
        if (i+(1<<(k-1))<=n) chmin(stlcs[i][k],stlcs[i+(1<<(k-1))][k-1]);
    }
}

inline int query_lcp(int l,int r) {
    bool flag=false;
    if (l==5 && r==10) flag=true;
    l=rklcp[l],r=rklcp[r];
    if (l>r) swap(l,r);
    l++;
    int k=log2(r-l+1);
    return min(stlcp[l][k],stlcp[r-(1<<k)+1][k]);
}

inline int query_lcs(int l,int r) {
    l=rklcs[l],r=rklcs[r];
    if (l>r) swap(l,r);
    l++;
    int k=log2(r-l+1);
    return min(stlcs[l][k],stlcs[r-(1<<k)+1][k]);
}
...
int main() {
    ...
    while (T--) {
        ...
        memset(rklcp,0,sizeof(rklcp));
        memset(rklcs,0,sizeof(rklcs));
        m=130;
        readstr(s+1),n=strlen(s+1);
        build_sa();
        build_lcp();
        build_stlcp();
        _rep(i,1,n) rklcp[i]=rk[i];
        m=130;
        reverse(s+1,s+1+n);
        build_sa();
        build_lcs();
        build_stlcs();
        _rep(i,1,n) rklcs[i]=rk[i];
        ans=0;
        _rep(len,1,n>>1) {
            int i=len,j=len<<1;
            while (j<=n) {
                int pre=min(query_lcp(i,j),len);
                int suf=min(query_lcs(n-j+2,n-i+2),len-1);
                if (pre+suf>=len) {
                    sr[i-suf]++,sr[i+pre-len+1]--;
                    ed[j-suf+len-1]++,ed[j+pre]--;
                }
                i+=len,j+=len;
            }
        }
        _rep(i,1,n) sr[i]+=sr[i-1],ed[i]+=ed[i-1];
        _rep(i,1,n-1) ans+=(i64)ed[i]*sr[i+1];
        writeln(ans);
    }
    return 0;
}
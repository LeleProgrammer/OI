...
int main() {
	...
	_rep(tt,1,TT) {
		...
		read(n);
		_rep(i,1,n) read(x[i]),read(y[i]),read(r[i]),read(s[i]);
		int ans=0;
		_rep(i,1,n) {
			if (s[i]>0) add(i,T,s[i]),ans+=s[i];
			else add(S,i,-s[i]);
		}
		_rep(i,1,n) _rep(j,1,n) if (i!=j && (x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j])<=r[j]*r[j]) add(i,j,inf);
		printf("Case #%d: %d\n",tt,ans-dinic());
	}
	return 0;
}
...
char s[N];
int q;
int lsh[N],sa[N],rk[N],st[N],cnt[N],ht[N];
int mx[N];
int spt[N][K];
...
inline void build_spt() {
    _rep(i,1,lens) spt[i][0]=mx[i];
    _rep(k,1,K-1) _rep(i,1,lens) {
        spt[i][k]=spt[i][k-1];
        if (i+(1<<(k-1))<=lens) chmax(spt[i][k],spt[i+(1<<(k-1))][k-1]);
    }
}
...
int main() {
    readstr(s+1),lens=strlen(s+1),s[lens+1]='K';
    readstr(s+2+lens),n=strlen(s+1);
    build_sa();
    build_ht();
    int cur=0;
    _rep(i,1,n-1) {
        if (sa[i]>lens) cur=inf;
        else {
            chmin(cur,ht[i]);
            chmax(mx[sa[i]],cur);
        }

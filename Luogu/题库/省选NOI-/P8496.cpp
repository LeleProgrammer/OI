...
int n,q;
int root[N];
int deq[N],nex[N],pre[N];
int hh[M],tt[M],sz[M];
int idx[M];
stack<int> stk;
stack<int> deqstk;

inline int newDeqNode() {
    int i=deqstk.top(); deqstk.pop();
    deq[i]=nex[i]=pre[i]=0;
    return i;
}

inline void emplace_front(int i,int val) {
    int u=newDeqNode();
    deq[u]=val;
    if (!sz[i]) hh[i]=tt[i]=u;
    else {
...
inline void emplace_back(int i,int val) {
    int u=newDeqNode();
    deq[u]=val;
    if (!sz[i]) hh[i]=tt[i]=u;
    else {
...
inline int get_front(int i) {
    return deq[hh[i]];
}

inline int get_back(int i) {
    return deq[tt[i]];
}
...
inline void modify(int u,int l,int r,int p,int k) {
    // cerr<<"modify: "<<u<<" "<<l<<" "<<r<<" "<<p<<" "<<k<<endl;
    if (l==p && r==p) tr[u].v+=k;
    else {
        int mid=l+r>>1;
        if (p<=mid) {
            if (!tr[u].l) tr[u].l=newNode();
            modify(tr[u].l,l,mid,p,k);
        } else {
            if (!tr[u].r) tr[u].r=newNode();
            modify(tr[u].r,mid+1,r,p,k);
        }
        pushup(u);
        if (tr[u].l && !tr[tr[u].l].v) {
            stk.emplace(tr[u].l);
            tr[u].l=0;
        }
        if (tr[u].r && !tr[tr[u].r].v) {
            stk.emplace(tr[u].r);
            tr[u].r=0;
        }
    }
...
inline void init() {
    _rrep(i,N-1,1) stk.emplace(i),deqstk.emplace(i),tr[i]={0,0,0};
    _rep(i,1,n) root[i]=newNode();
}
    ...
    while (q--) {
        ...
        if (op==1) {
            ...
            while (m--) {
                ...
                v=idx[v];
                x.emplace_back(v);
                cur.emplace_back(root[v]);
                totsz+=sz[v];
            }
            // cerr<<"totsz: "<<totsz<<endl;
            int l=1,r=M-1;
            // while (l<r) {
            //     int mid=l+r>>1;
            //     int sum=0;
            //     _for(i,x) sum+=query(root[i],1,M-1,1,mid);
            //     // cerr<<"query: "<<1<<" "<<mid<<" = "<<sum<<endl;
            //     if (sum>totsz/2) r=mid;
            //     else l=mid+1;
            // }
            i64 dest=totsz/2+1;
            while (l<r) {
            ...
            int cnt=0;
            _for(i,x) cnt+=query(root[i],1,M-1,r,r);
            // cerr<<"possible: "<<r<<" "<<cnt<<endl;
            if (cnt<=totsz/2) puts("-1");
            else writeln(r);
        } else {
            int x1,x2,x3;
            read(x1,x2,x3);
            x1=idx[x1],x2=idx[x2],idx[x3]=x3;
            if (sz[x1]>sz[x2]) { // x1 <- x2
                merge(root[x1],root[x2]);
                while (sz[x2]) {
                    int y=get_front(x2);
                    pop_front(x2);
                    emplace_back(x1,y);
                    // modify(root[x1],1,M-1,y,1);
                    // modify(root[x2],1,M-1,y,-1);
                }
                idx[x3]=x1;
            } else { // x1 -> x2
                merge(root[x2],root[x1]);
                while (sz[x1]) {
                    int y=get_back(x1);
                    pop_back(x1);
                    emplace_front(x2,y);

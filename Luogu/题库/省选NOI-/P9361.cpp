...
int main() {
    read(n),read(m);
    _rep(i,1,m) _rep(j,1,n) read(a[i][j]),b[a[i][j]][i]=j;
    _rep(k,0,K-1) _rep(i,1,m) _rep(j,1,m) _rep(p,1,n) f[k][i][j][p]=n+1;
    _rep(i,1,m) _rep(j,1,m) {
        int mn=n+1;
        _rrep(k,n,1) mn=min(mn,b[a[i][k]][j]),f[0][i][j][k]=mn;
    }
    _rep(k,1,K-1) _rep(i,1,m) _rep(j,1,m) _rep(mid,1,m) _rep(p,1,n) f[k][i][j][p]=min(f[k][i][j][p],f[k-1][mid][j][f[k-1][i][mid][p]]);
    read(q);
    while (q--) {
        int x,y;
        read(x),read(y);
        bool ok=false;
        _rep(i,1,m) if (b[x][i]<b[y][i]) {
            ok=true;
            break;
        }
        if (ok) {
        ...
        _rep(i,1,m) mnp[i]=b[x][i];
        int ans=0;
        _rrep(k,K-1,0) {
            _rep(i,1,m) bac[i]=mnp[i];
            _rep(i,1,m) _rep(j,1,m) mnp[j]=min(mnp[j],f[k][i][j][bac[i]]);
            bool ok=false;
            _rep(i,1,m) if (mnp[i]<=b[y][i]) {
                ok=true;
                break;
            }
            if (ok) _rep(i,1,m) mnp[i]=bac[i];
            else ans+=(1<<k);
        }
        if (ans>n) puts("-1");
        else writeln(ans+2);

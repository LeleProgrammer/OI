...
i32 main() {
    read(p);
    readstr(s+1),n=strlen(s+1),len=sqrt(n);
    read(m);
    _rep(i,1,m) read(qs[i].l,qs[i].r),qs[i].i=i;
    sort(qs+1,qs+1+m);
    int pw=1;
    _rrep(i,n,1) suf[i]=(pw*(s[i]-'0')+suf[i+1])%p,pw=pw*10%p;
    _rep(i,1,n+1) nums.emplace_back(suf[i]);
    sort(nums.begin(),nums.end());
    nums.erase(unique(nums.begin(),nums.end()),nums.end());
    _rep(i,1,n+1) suf[i]=lower_bound(nums.begin(),nums.end(),suf[i])-nums.begin()+1;
    if (p==2) {
        int i=1,j=0;
        _rep(t,1,m) {
        ...
        int i=1,j=0;
        _rep(t,1,m) {
            int l=qs[t].l,r=qs[t].r+1,id=qs[t].i;
            while (i<l) del(suf[i++]);
            while (i>l) add(suf[--i]);
            while (j<r) add(suf[++j]);
            while (j>r) del(suf[j--]);

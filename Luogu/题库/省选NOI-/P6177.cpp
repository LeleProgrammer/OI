...
inline void process() {
    ...
    while (!vec.empty()) {
        ...
        if (!v) continue;
        if (!ok) flag[v]=true,id[v]=++idx,ori[id[v]]=v,up[id[v]][0][arr[v]]=1;
    }
}

inline void dfs(int u,int fa,int las) {
    if (flag[u] && las) {
        lasp[id[u]]=id[las];
        int v=u;
        while (v!=las) up[id[u]][1][arr[v]]=1,v=f[v][0];
        up[id[u]][1][arr[v]]=1;
    }
    _graph(i,u) if (e[i]!=fa) dfs(e[i],u,flag[u]?u:las);
}
...
inline int solve(int u,int v) {
    ...
    if (!same) {
        int cnt=0;
        int bk=cur;
        while (lasp[id[cur]] && dep[ori[lasp[id[cur]]]]>=dep[g]) cur=ori[lasp[id[cur]]],cnt++;
        ansu|=up[id[bk]][cnt];
        while (cur!=g) cur=f[cur][0],ansu[arr[cur]]=1;
    }
    cur=v;
    same=false;
    while (!flag[cur]) {
    ...
    if (!same) {
        ...
        while (lasp[id[cur]] && dep[ori[lasp[id[cur]]]]>=dep[g]) cur=ori[lasp[id[cur]]],cnt++;
        ansv|=up[id[bk]][cnt];
        while (cur!=g) cur=f[cur][0],ansv[arr[cur]]=1;
    }
    auto res=ansu|ansv;
    return res.count();
}
...
int main() {
    ...
    _rep(i,1,n) arr[i]=LB(nums,arr[i]);
    _rep(i,1,n-1) {
        int a,b;
        read(a,b);
        add(a,b),add(b,a);
    ...
    process();
    dfs(1,0,0);
    _rep(t,2,P-1) _rep(i,1,idx) up[i][t]=up[i][1]|up[lasp[i]][t-1];
    while (m--) {
        int u,v;
        read(u,v);
        u^=las;

...
inline void dfs(int u,bool ask) {
    if (ask) vec[u]=ask2(u);
    if (vec[u].size()==1) return;
    vector<int> nex;
    _for(v,vec[u]) if (dep[v]==dep[u]+1) nex.emplace_back(v),ans[v]=u;
    int s=vec[u][randint(0,(int)vec[u].size()-2)];
    if (s==u) s=vec[u].back();
    int son;
    if (dep[s]==dep[u]+1) son=s;
    else {
        _for(v,nex) if (ask1(v,s)==dep[s]-dep[v]) {
            son=v;
            break;
        }
    }
    set<int> st;
    _for(v,vec[u]) st.emplace(v);
    _for(v,nex) if (v!=son) {
        dfs(v,true);
        _for(g,vec[v]) st.erase(g);
    }
    st.erase(u);
    _for(v,st) vec[son].emplace_back(v);
    dfs(son,false);
}
...
int main() {
    srand(time(0));
    read(n);
    _rep(i,2,n) dep[i]=ask1(1,i);
    _rep(i,1,n) vec[1].emplace_back(i);
    dfs(1,false);
    writestr(string{"! "});
    _rep(i,2,n) writesp(ans[i]);
    fflush(stdout);
    return 0;
}
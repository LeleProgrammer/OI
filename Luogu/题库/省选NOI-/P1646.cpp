...
int main() {
	memset(h,-1,sizeof(h)),S=N-1,T=N-2;
	read(n),read(m);
	...
	_rep(i,1,n-1) _rep(j,1,m) {
		...
		read(k);
		add(S,get(i,j)+n*m,k);
		add(get(i,j)+n*m,get(i,j),inf);
		add(get(i,j)+n*m,get(i+1,j),inf);
		res+=k;
	}
	_rep(i,1,n-1) _rep(j,1,m) {
		int k;
		read(k);
		add(get(i,j)+n*m*2,T,k);
		add(get(i,j),get(i,j)+n*m*2,inf);
		add(get(i+1,j),get(i,j)+n*m*2,inf);
		res+=k;
	}
	_rep(i,1,n) _rep(j,1,m-1) {
		int k;
		read(k);
		add(S,get(i,j)+n*m*3,k);
		add(get(i,j)+n*m*3,get(i,j),inf);
		add(get(i,j)+n*m*3,get(i,j+1),inf);
		res+=k;
	}
	_rep(i,1,n) _rep(j,1,m-1) {
		int k;
		read(k);
		add(get(i,j)+n*m*4,T,k);
		add(get(i,j),get(i,j)+n*m*4,inf);
		add(get(i,j+1),get(i,j)+n*m*4,inf);
		res+=k;
	}
	write(res-dinic());
	return 0;

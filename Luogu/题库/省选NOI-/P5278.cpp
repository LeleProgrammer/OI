...
struct val {
    int len;
    int mx,mn,gcd;
    val(): len(0),mx(0),mn(INF),gcd(-1) {}
};

void pushup(val& u,val& l,val& r) {
    u.mx=max(l.mx,r.mx);
    u.mn=min(l.mn,r.mn);
    if (!~l.gcd) u.gcd=r.gcd;
    else if (!~r.gcd) u.gcd=l.gcd;
    else if (!l.gcd || !r.gcd) u.gcd=0;
    else u.gcd=gcd(l.gcd,r.gcd);
}

void pushdown(val& u,val& l,val& r) {}

void modify(val& u,const pii& g) {
    u.mx=u.mn=g.first;
    u.gcd=g.second;
}

SEG<val,pushup,pushdown,const pii,modify> seg(N);
...
void pushup(val2& u,val2& l,val2& r) {
    u.mx=max(l.mx,r.mx);
}

void pushdown(val2& u,val2& l,val2& r) {}

void modify(val2& u,const int& k) {
    u.mx=k;
}

SEG<val2,pushup,pushdown,const int,modify> seg2(N);

int main() {
    read(n),read(m);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n) seg.modify(1,i,i,pii{a[i],abs(a[i]-a[i+1])});
    _rep(i,1,n) {
        if (st[a[i]].size()<2) st[a[i]].emplace(0),st[a[i]].emplace(n+1);
        st[a[i]].emplace(i);
    }
    _rep(i,1,n) {
        auto cur=st[a[i]].lower_bound(i);
        auto l=prev(cur);
        seg2.modify(1,i,i,*l);
    }
    while (m--) {
        ...
        if (op==1) {
            ...
            read(x),read(y);
            x^=las,y^=las;
            auto cur=st[a[x]].lower_bound(x);
            auto l=prev(cur),r=next(cur);
            if (next(r)!=st[a[x]].end()) seg2.modify(1,*r,*r,*l);
            st[a[x]].erase(cur);
            a[x]=y;
            if (st[a[x]].size()<2) st[a[x]].emplace(0),st[a[x]].emplace(n+1);
            st[a[x]].emplace(x);
            cur=st[a[x]].lower_bound(x);
            l=prev(cur),r=next(cur);
            seg2.modify(1,x,x,*l);
            if (next(r)!=st[a[x]].end()) seg2.modify(1,*r,*r,x);
            seg.modify(1,x,x,pii{a[x],abs(a[x]-a[x+1])});
            if (x-1) seg.modify(1,x-1,x-1,pii{a[x-1],abs(a[x-1]-a[x])});
        } else {
            int l,r,k;
            ...
            auto g=seg.query(1,l,r);
            int mn=g.mn,mx=g.mx,gc=seg.query(1,l,r-1).gcd;
            if (!k && mn==mx) {
                puts("Yes");
                las++;
                continue;
            }
            int idxmx=seg2.query(1,l,r).mx;
            if (mx-mn==k*(r-l) && idxmx<l && gc==k) puts("Yes"),las++;
            else puts("No");
        }
    }
    return 0;

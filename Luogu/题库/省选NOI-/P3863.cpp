...
inline void pushdown(int b) {
    if (add[b]) _rep(i,BL[b],BR[b]) v[i]+=add[b],st[i]+=add[b];
    add[b]=0;
}

inline void rebuild(int b) {
    _rep(i,BL[b],BR[b]) st[i]=v[i];
    sort(st+BL[b],st+BR[b]+1);
}
...
inline int query(int l,int r,int y) {
    if (B[l]==B[r]) {
        pushdown(B[l]);
        int ans=0;
        _rep(i,l,r) ans+=(v[i]>=y);
        return ans;
    } else {
        pushdown(B[l]),pushdown(B[r]);
        int ans=0;
        _rep(i,l,R[l]) ans+=(v[i]>=y);
        _rep(i,L[r],r) ans+=(v[i]>=y);
        _rep(i,B[l]+1,B[r]-1) {
            int L=BL[i],R=BR[i]+1;
            while (L<R) {
                int mid=L+R>>1;
                if (st[mid]+add[i]>=y) R=mid;
                else L=mid+1;
            }
            ans+=BR[i]-R+1;
        }
...
i32 main() {
    ...
    _rep(i,1,N-1) L[i]=(B[i]==B[i-1])?L[i-1]:i;
    _rrep(i,N-1,1) R[i]=(B[i]==B[i+1])?R[i+1]:i;
    _rep(i,1,N-1) BL[B[i]]=L[i],BR[B[i]]=R[i];
    _rep(i,1,n) read(a[i]);
    _rep(i,2,q+1) {
        int op;
        read(op);
        if (op==1) {
            int l,r,x;
            read(l,r,x);
            vec[l].emplace_back(pii{i,x});
            vec[r+1].emplace_back(pii{i,-x});
        } else {
            int p,y;
            read(p,y);
            qs[p].emplace_back(piii{++m,{i-1,y}});
        }
    }
    _rep(i,1,n) {
        modify(1,q+1,a[i]-a[i-1]);
        _for(s,vec[i]) modify(s.first,q+1,s.second);
        _for(s,qs[i]) ans[s.first]=query(1,s.second.first,s.second.second);
    }

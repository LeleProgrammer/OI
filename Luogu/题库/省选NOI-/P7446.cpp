...
int n,m,len,las;
int a[N],ext[N];
int B[N],L[N],R[N],BL[N],BR[N],Bn;
int cnt[N],decr[N];

inline void init() {
    _rep(i,1,n) B[i]=i/len+1,Bn=B[i];
...
inline void build(int k) {
    ...
    _rep(i,BL[k],BR[k]) {
        ext[i]=max(a[i],1);
        while (ext[i]!=1 && B[ext[i]]==B[i]) ext[i]=ext[ext[i]];
    }
}
...
inline int query(int u,int v) {
    while (true) {
        int exu=ext[u],exv=ext[v];
        if (cnt[B[u]]==len+1) exu=max(a[u]-decr[B[u]],1);
        if (cnt[B[v]]==len+1) exv=max(a[v]-decr[B[v]],1);
        if (exu==exv) break;
        if (exu<exv) swap(u,v),swap(exu,exv);
        // cout<<"1: "<<u<<" "<<v<<" "<<exu<<" "<<exv<<endl;
        u=exu;
    }
    while (u!=v) {
        // cout<<"2: "<<u<<" "<<v<<endl;
        if (u<v) swap(u,v);
        u=max(a[u]-decr[B[u]],1);
    }
    return u;
}
...
int main() {
    read(n,m),len=sqrt(n);
    _rep(i,2,n) read(a[i]); a[1]=1;
    init();
    _rep(i,1,Bn) build(i);
    while (m--) {
        // cout<<"B: "; _rep(i,1,n) cout<<B[i]<<" "; cout<<endl;
        // cout<<"a: "; _rep(i,1,n) cout<<a[i]-decr[B[i]]<<" "; cout<<endl;
        // cout<<"ext: "; _rep(i,1,n) cout<<ext[i]<<" "; cout<<endl;
        int op;
        read(op);
        if (op==1) {
            int l,r,x;

...
int main() {
    ...
    _rep(i,1,m) read(a[i]);
    bool ok=true;
    _rep(i,1,m) if (a[i]>(n+1)/2) ok=false;
    if (!ok) puts("No");
    else {
        if (!(n&1)) {
            _rep(j,1,m) {
                if (j&1) {
                    _reps(i,1,n,i+=2) if (a[j]) arr[i][j]=1,a[j]--;
                } else {
                    _reps(i,2,n,i+=2) if (a[j]) arr[i][j]=1,a[j]--;
                }
            }
            puts("Yes");
            _rep(i,1,n) {
                _rep(j,1,m) write(arr[i][j]);
                putchar(10);
            }
        } else {
            bool full=false;
            _rep(i,1,m) if (a[i]==(n+1)/2) full=true;
            if (!full) {
                _rep(j,1,m) {
                    if (j&1) {
                        _reps(i,1,n,i+=2) if (a[j]) arr[i][j]=1,a[j]--;
                ...
                vector<int> vec;
                _rep(i,1,m) if (a[i]==(n+1)/2) vec.emplace_back(i);
                int cur=0;
                _rrep(j,vec[0]-1,1) {
                    if (!cur) {
                        _reps(i,2,n,i+=2) if (a[j]) arr[i][j]=1,a[j]--;
                ...
                _rep(t,0,(int)vec.size()-1) {
                    int j=vec[t];
                    _reps(i,1,n,i+=2) if (a[j]) arr[i][j]=1,a[j]--;
                }
                bool flag=true;
                _rep(t,0,(int)vec.size()-2) {
                    int l=vec[t],r=vec[t+1];
                    if (l+1==r) {
                        flag=false;
                        break;
                    }
                    if ((r-l-1)&1) {
                        ...
                        _rep(j,l+1,r-1) {
                            if (!cur) {
                                _reps(i,2,n,i+=2) if (a[j] && !arr[i][j-1] && !arr[i-1][j] && !arr[i+1][j] && !arr[i][j+1]) arr[i][j]=1,a[j]--;
                                _reps(i,1,n,i+=2) if (a[j] && !arr[i][j-1] && !arr[i-1][j] && !arr[i+1][j] && !arr[i][j+1]) arr[i][j]=1,a[j]--;
                            } else {
                                _reps(i,1,n,i+=2) if (a[j] && !arr[i][j-1] && !arr[i-1][j] && !arr[i+1][j] && !arr[i][j+1]) arr[i][j]=1,a[j]--;
                                _reps(i,2,n,i+=2) if (a[j] && !arr[i][j-1] && !arr[i-1][j] && !arr[i+1][j] && !arr[i][j+1]) arr[i][j]=1,a[j]--;
                            }

...
int S(int j) {
    if (~s[j]) return s[j];
    else if (a==1) return s[j]=(binom[j+1]+binom[j]-(j==0)+mod)%mod;
    else if (j==0) return s[j]=(ksm(a,n+1)-a)*inverse(a-1)%mod;
    else return s[j]=(-a*(S(j-1)-(ksm(a,n)*binom[j]%mod)-(ksm(a,n)*binom[j-1]%mod)+(j==0)+(j==1))%mod)*inverse(a-1)%mod;
}
...
i32 main() {
    ...
    st[0][0]=1;
    _rep(i,1,k) _rep(j,1,i) st[i][j]=(st[i-1][j]*j+st[i-1][j-1])%mod;
    binom[0]=1;
    _rep(i,1,k) binom[i]=(binom[i-1]*(n-i+1)%mod)*inverse(i)%mod;
    _rep(i,0,k) ans=(ans+((f[i]*st[k][i]%mod)*S(i)%mod))%mod;
    ans=(ans%mod+mod)%mod;
    write(ans);
    return 0;
}
...
inline void dfs(int u,int fa) {
    vec.emplace_back(col[u]);
    bool flag=true;
    _graph(i,u) if (e[i]!=fa) flag=false,dfs(e[i],u);
    if (flag) insert();
    vec.pop_back();
}
...
inline int extend(int las,int c) {
    int p=las;
    if (arr[arr[p].s[c]].len==arr[p].len+1) return arr[p].s[c];
    int np=++idx;
...
inline void dfs2(int u,int c,int fa) {
    int f=1;
    if (fa) f=extend(fa,c);
    _rep(i,0,10) if (tr[u][i]) dfs2(tr[u][i],i,f);
}
...
i32 main() {
    ...
    _rep(i,1,n) if (deg[i]==1) dfs(i,0);
    dfs2(0,0,0);
    Graph::restore_graph();
    _rep(i,1,idx) _rep(j,0,10) if (arr[i].s[j]) add(arr[i].s[j],i),indeg[i]++;
    topo();
    write(g[1]-1);
    return 0;
}
...
inline void dfs1(int u) {
    if (cnt[reg[u]]>=len) cur[g[reg[u]]]++;
    _rep(i,1,idx) cnt1[reg[u]][i]-=cur[i];
    _graph(i,u) dfs1(e[i]);
    _rep(i,1,idx) cnt1[reg[u]][i]+=cur[i];
}

inline void dfs2(int u) {
    _rep(i,1,idx) cnt2[i][reg[u]]+=cur[i];
    if (cnt[reg[u]]>=len) cur[g[reg[u]]]++;
    _graph(i,u) dfs2(e[i]);
    if (cnt[reg[u]]>=len) cur[g[reg[u]]]--;
}
...
int main() {
    ...
    _rep(i,1,n) {
        ...
        read(reg[i]),cnt[reg[i]]++;
    }
    _rep(i,1,r) if (cnt[i]>=len) g[i]=++idx;
    dfs1(1);
    memset(cur,0,sizeof(cur));
    dfs2(1);
    dfs3(1);
    _rep(i,1,n) {
        st[reg[i]].emplace_back(pii{fir[i],1});
        st[reg[i]].emplace_back(pii{las[i],-1});
        vec[reg[i]].emplace_back(fir[i]);
    }
    _rep(i,1,r) _sort(st[i]),_sort(vec[i]);
    while (q--) {
        int a,b;
        read(a,b);
        if (cnt[b]>=len) writeln(cnt1[a][g[b]]),fflush(stdout);
        else if (cnt[a]<len) {
            int ptr=0,cur=0;
            i64 ans=0;
            _for(i,vec[b]) {
                while (ptr<=(int)st[a].size()-1 && st[a][ptr].first<=i) cur+=st[a][ptr++].second;
                ans+=cur;
            }
            writeln(ans);
            fflush(stdout);
        } else writeln(cnt2[g[a]][b]),fflush(stdout);
    }
    return 0;
}
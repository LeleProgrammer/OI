...
i32 main() {
    ...
    _rep(i,0,n-1) {
        ...
        _rep(j,0,(1<<k)-1) {
            _rep(t,0,k-1) las[t+1]=kthbit(j,t)+las[t];
            int staN=0;
            _rep(t,1,k) {
                if (s[t]=='N') nex[t]=las[t-1]+1;
                else nex[t]=max(nex[t-1],las[t]);
                if (nex[t]-nex[t-1]==1) staN=setbit(staN,t-1);
            }
            _rep(t,0,k-1) las[t+1]=kthbit(j,t)+las[t];
            int staO=0;
            _rep(t,1,k) {
                if (s[t]=='O') nex[t]=las[t-1]+1;
                else nex[t]=max(nex[t-1],las[t]);
                if (nex[t]-nex[t-1]==1) staO=setbit(staO,t-1);
            }
            _rep(t,0,k-1) las[t+1]=kthbit(j,t)+las[t];
            int staI=0;
            _rep(t,1,k) {
                if (s[t]=='I') nex[t]=las[t-1]+1;
                else nex[t]=max(nex[t-1],las[t]);
                if (nex[t]-nex[t-1]==1) staI=setbit(staI,t-1);
            }
            f[i&1^1][staN][1]=(f[i&1^1][staN][1]+f[i&1][j][0]+f[i&1][j][1]+f[i&1][j][2])%mod;
            f[i&1^1][staO][2]=(f[i&1^1][staO][2]+f[i&1][j][1])%mod;
            f[i&1^1][staO][0]=(f[i&1^1][staO][0]+f[i&1][j][0]+f[i&1][j][2])%mod;
            f[i&1^1][staI][0]=(f[i&1^1][staI][0]+f[i&1][j][0]+f[i&1][j][1])%mod;
        }
    }
    _rep(j,0,(1<<k)-1) {
        int t=popc(j);
        ans[t]=(ans[t]+f[n&1][j][0]+f[n&1][j][1]+f[n&1][j][2])%mod;
    }
    _rep(i,0,k) writeln(ans[i]);
    return 0;
}
...
using namespace Tree::LCA;

int q;
int arr[N];
int fir[N],sec[N],idx,root[N];
int tr[N*40][2],cnt[N*40],ptr;

struct ask {
    int op,x,y,z,ans;
} qs[N];

void insert(int p,int u,int k,int t) {
    if (!~t) cnt[u]=cnt[p]+1;
    else {
        int d=((k>>t)&1);
        if (!d) {
            tr[u][1]=tr[p][1];
            if (!tr[u][0]) tr[u][0]=++ptr;
            insert(tr[p][0],tr[u][0],k,t-1);
        } else {
            tr[u][0]=tr[p][0];
            if (!tr[u][1]) tr[u][1]=++ptr;
            insert(tr[p][1],tr[u][1],k,t-1);
        }
        cnt[u]=cnt[tr[u][0]]+cnt[tr[u][1]];
    }
}
...
void build(int u,int fa) {
    fir[u]=++idx;
    root[idx]=++ptr;
    insert(root[idx-1],root[idx],arr[u],35);
    _graph(i,u) if (e[i]!=fa) build(e[i],u);
    sec[u]=++idx;
    root[idx]=root[idx-1];
}

void build2(int u,int fa) {
    root[u]=++ptr;
    insert(root[fa],root[u],arr[u],35);
    _graph(i,u) if (e[i]!=fa) build2(e[i],u);
}
...
i32 main() {
    ...
    _rep(i,1,q) if (qs[i].op==1) {
        ...
        _rrep(t,35,0) {
            if ((z>>t)&1) {
                int sum=cnt[tr[r][0]]-cnt[tr[l][0]];
                if (sum) ans=ans<<1|1,l=tr[l][0],r=tr[r][0];
                else ans<<=1,l=tr[l][1],r=tr[r][1];
            } else {
                int sum=cnt[tr[r][1]]-cnt[tr[l][1]];
                if (sum) ans=ans<<1|1,l=tr[l][1],r=tr[r][1];
                else ans<<=1,l=tr[l][0],r=tr[r][0];
            }
        }
        qs[i].ans=ans;
    }
    ...
    _rep(i,1,q) if (qs[i].op==2) {
        ...
        int z=qs[i].z;
        int ans=0;
        x=root[x],y=root[y],g=root[g],t=root[t];
        _rrep(p,35,0) {
            if ((z>>p)&1) {
                int sum=cnt[tr[x][0]]+cnt[tr[y][0]]-cnt[tr[g][0]]-cnt[tr[t][0]];
                if (sum) ans=ans<<1|1,x=tr[x][0],y=tr[y][0],g=tr[g][0],t=tr[t][0];
                else ans<<=1,x=tr[x][1],y=tr[y][1],g=tr[g][1],t=tr[t][1];
            } else {
                int sum=cnt[tr[x][1]]+cnt[tr[y][1]]-cnt[tr[g][1]]-cnt[tr[t][1]];
                if (sum) ans=ans<<1|1,x=tr[x][1],y=tr[y][1],g=tr[g][1],t=tr[t][1];
                else ans<<=1,x=tr[x][0],y=tr[y][0],g=tr[g][0],t=tr[t][0];
            }
        }
        qs[i].ans=ans;
    }
    _rep(i,1,q) writeln(qs[i].ans);
    return 0;
}

/*
Euler Order: 1 3 7 7 6 6 3 2 5 5 4 4 2 1
*/
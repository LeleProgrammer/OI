...
vector<string> process(string s) {
    ...
    string cur="";
    _for(i,s) {
        if (i==' ') ans.emplace_back(cur),cur="";
        else cur.push_back(i);
    }
    if (!cur.empty()) ans.emplace_back(cur);
    return ans;
}
    ...
    _rep(ttt,1,tt) {
        ...
        _rep(i,3,n) {
            ...
            _for(s,vec) {
                ...
                FLOW::add(i,g,inf);
                FLOW::add(g+5000,n+i,inf);
            }
        }
        _iter(it,st) FLOW::add(it->second,it->second+5000,1);
        writestr("Case #");
        write(ttt);
        writestr(": ");
        writeln(dinic()-K*(n-2));

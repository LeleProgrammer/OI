...
namespace dseg {
    ...
    inline void modify(int& u,int p,int l,int r,i64 k) {
        if (!u) u=++idx;
        if (l==p && r==p) tr[u].sum+=k;
        else {
            int mid=l+r>>1;
            if (p<=mid) modify(tr[u].l,p,l,mid,k);
            else modify(tr[u].r,p,mid+1,r,k);
            pushup(u);
        }
    }
}
...
namespace seg {
    ...
    inline void pushdown(int u) {
        ...
        while (!root.vec.empty()) {
            int k=root.vec.back();
            root.vec.pop_back();
            left.vec.emplace_back(k);
            right.vec.emplace_back(k);
            dseg::modify(left.root,k,0,n<<1,left.r-left.l+1);
            dseg::modify(right.root,k,0,n<<1,right.r-right.l+1);
        }
    }

    inline void modify(int u,int l,int r,int k) {
        dseg::modify(tr[u].root,k,0,n<<1,min(r,tr[u].r)-max(l,tr[u].l)+1);
        if (tr[u].l>=l && tr[u].r<=r) tr[u].vec.emplace_back(k);
        else {
            pushdown(u);
            int mid=tr[u].l+tr[u].r>>1;
            if (l<=mid) modify(u<<1,l,r,k);
    ...
    inline vector<int> query(int u,int l,int r) {
        vector<int> res;
        if (tr[u].l>=l && tr[u].r<=r) {
            res.emplace_back(tr[u].root);
            return res;
        } else {
            pushdown(u);
            int mid=tr[u].l+tr[u].r>>1;
    ...
    while (m--) {
        ...
        if (op==1) {
            ...
            while (l!=r) {
                int mid=l+r>>1;
                i64 sum=0;
                _for(u,g) sum+=dseg::tr[dseg::tr[u].r].sum;
                // cout<<"test: "<<l<<" "<<mid<<" "<<r<<": "<<sum<<" "<<c<<endl;
                if (sum>=c) {
                    _for(u,g) u=dseg::tr[u].r;
                    l=mid+1;
                } else {
                    c-=sum;
                    _for(u,g) u=dseg::tr[u].l;
                    r=mid;
                }
            }
            writeln(r-n);

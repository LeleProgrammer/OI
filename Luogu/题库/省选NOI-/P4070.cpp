...
int s[N];
vector<int> nums;
int lsh[N],st[N],sa[N],rk[N],ht[N],cnt[N];
int spt[N][K];
set<int> stt;
...
inline void build_st() {
    _rep(i,2,n) spt[i][0]=ht[i];
    _rep(k,1,K-1) _rep(i,1,n) {
        spt[i][k]=spt[i][k-1];
        if (i+(1<<(k-1))<=n) chmin(spt[i][k],spt[i+(1<<(k-1))][k-1]);
    }
}
...
i32 main() {
    read(n),m=n;
    _rrep(i,n,1) read(s[i]),nums.emplace_back(s[i]);
    _sort(nums),_unique(nums);
    _rep(i,1,n) s[i]=LB(nums,s[i])+1;
    build_sa();
    build_ht();
    build_st();
    int ans=0;
    _rrep(i,n,1) {
        stt.emplace(rk[i]);
        auto it=stt.find(rk[i]);
        int dec=0;
        if (it!=stt.begin()) {
            int pre=*prev(it);
            chmax(dec,query(pre+1,rk[i]));
        }
        if (next(it)!=stt.end()) {
            int nex=*next(it);
            chmax(dec,query(rk[i]+1,nex));
        }
        ans=ans+(n-i+1)-dec;
        writeln(ans);
    }

...
void solve(int u) {
    st[u]=true;
    _graph(i,u) if (!st[e[i]] && f[i]) solve(e[i]);
}
...
//    FRW("P9879.out");
    ...
    while (tt--) {
        read(n),read(m);
        _rep(i,1,n*m*2+2) h[i]=-1,st[i]=false; tot=0;
        S=n*m*2+1,T=n*m*2+2;
        _rep(i,1,n) _rep(j,1,m) {
            ...
            if (arr[i][j]!=2 && ((i+j)&1)) arr[i][j]^=1;
        }
        int ans=0;
        _rep(i,1,n-1) _rep(j,1,m-1) {
            int g=0,t=1;
            if (arr[i][j]!=g && arr[i+1][j]!=g && arr[i][j+1]!=g && arr[i+1][j+1]!=g) FLOW::add(S,get(i,j),1),ans++;
            if (arr[i][j]!=t && arr[i+1][j]!=t && arr[i][j+1]!=t && arr[i+1][j+1]!=t) FLOW::add(get(i,j)+n*m,T,1),ans++;
            _rep(x,i-1,i+1) _rep(y,j-1,j+1) if (x>=1 && y>=1 && x<n && y<m) FLOW::add(get(i,j),get(x,y)+n*m,INF);
        }
//        cerr<<"test: "<<ans<<endl;
        writeln(ans-dinic());
        solve(S);
        _reps(i,0,tot-1,i+=2) {
            if (e[i]==T && !(st[e[i]]^st[e[i^1]])) {
                auto t=get(e[i^1]-n*m);
                int x=t.first,y=t.second;
//                cerr<<"T: "<<x<<" "<<y<<endl;
                arr[x][y]=arr[x+1][y]=arr[x][y+1]=arr[x+1][y+1]=0;
            } else if (e[i^1]==S && !(st[e[i]]^st[e[i^1]])) {
                auto t=get(e[i]);
                int x=t.first,y=t.second;
//                cerr<<"S: "<<x<<" "<<y<<endl;

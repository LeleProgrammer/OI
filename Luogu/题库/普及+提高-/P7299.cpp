...
int main() {
    read(n,k);
    _rep(i,1,n) arr[i]=i,vec[arr[i]].emplace_back(i);
    while (k--) {
        int a,b;
        read(a,b);
        swap(arr[a],arr[b]);
        vec[arr[a]].emplace_back(a);
        vec[arr[b]].emplace_back(b);
    }
    _rep(i,1,n) if (!dsu.connected(i,arr[i])) dsu.merge(i,arr[i]);
    _rep(i,1,n) _for(j,vec[i]) st[dsu.find(i)].emplace(j);
    _rep(i,1,n) writeln(st[dsu.find(i)].size());
    return 0;
}

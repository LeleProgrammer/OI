...
int main() {
    ...
    _rep(i,0,N-1) _rep(j,0,K-1) dis[i][j]=inf;
    dis[1][0]=0;
    q.emplace(make_pair(dis[1][0],pii{1,0}));
    while (!q.empty()) {
        auto t=q.top(); q.pop();
        int d=t.first;
        ...
        _graph(i,u) {
            if (d>=w[i]) {
                if (dis[e[i]][(g+1)%k]>d+1) {
                    dis[e[i]][(g+1)%k]=d+1;
                    q.emplace(make_pair(dis[e[i]][(g+1)%k],pii{e[i],(g+1)%k}));
                }
            } else {
                int rem=w[i]-d;
                int cnt=rem/k;
                if (rem%k) cnt++;
                if (dis[e[i]][(g+1)%k]>d+1+cnt*k) {
                    dis[e[i]][(g+1)%k]=d+1+cnt*k;
                    q.emplace(make_pair(dis[e[i]][(g+1)%k],pii{e[i],(g+1)%k}));
                }
            }
        }

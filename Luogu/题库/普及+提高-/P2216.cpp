...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,m) read(mx[i][j][0]),mn[i][j][0]=mx[i][j][0];
    _rep(t,1,M-1) _rep(i,1,n) _rep(j,1,m) {
        mn[i][j][t]=mn[i][j][t-1],mx[i][j][t]=mx[i][j][t-1];
        if (i+(1<<(t-1))<=n) mn[i][j][t]=min(mn[i][j][t],mn[i+(1<<(t-1))][j][t-1]),mx[i][j][t]=max(mx[i][j][t],mx[i+(1<<(t-1))][j][t-1]);
        if (j+(1<<(t-1))<=m) mn[i][j][t]=min(mn[i][j][t],mn[i][j+(1<<(t-1))][t-1]),mx[i][j][t]=max(mx[i][j][t],mx[i][j+(1<<(t-1))][t-1]);
        if (i+(1<<(t-1))<=n && j+(1<<(t-1))<=m) mn[i][j][t]=min(mn[i][j][t],mn[i+(1<<(t-1))][j+(1<<(t-1))][t-1]),mx[i][j][t]=max(mx[i][j][t],mx[i+(1<<(t-1))][j+(1<<(t-1))][t-1]);
    }
    int g=log2(k);
    _rep(i,1,n-k+1) _rep(j,1,m-k+1) ans=min(ans,max(max(max(mx[i][j][g],mx[i+k-(1<<g)][j][g]),mx[i][j+k-(1<<g)][g]),mx[i+k-(1<<g)][j+k-(1<<g)][g])-min(min(min(mn[i][j][g],mn[i+k-(1<<g)][j][g]),mn[i][j+k-(1<<g)][g]),mn[i+k-(1<<g)][j+k-(1<<g)][g]));
    write(ans);
    return 0;
}
...
int main() {
    ...
    _rep(j,0,N-1) _rep(k,0,N-1) f[j][k]=-inf;
    f[0][0]=0;
    _rep(i,2,n) _rep(j,1,n/2) _rep(k,j*2,n) if (k>=i) f[j][k]=max(f[j][k],f[j-1][k-i]+a[i]+c[j]-c[n-j+1]);
    _rep(j,0,n/2) _rep(k,j*2,n) ans=max(ans,f[j][k]+(n-k)*a[1]);
    write(ans);
    return 0;
}

/*
f[j][k]=max(f[j][k],f[j-1][k-i]+a[i]+c[j]-c[n-j+1]);
*/
...
i32 main() {
    read(n);
    _rep(i,1,n) read(arr[i].a);
    _rep(i,1,n) read(arr[i].b);
    _rep(i,1,n) arr[i].i=i;
    sort(arr+1,arr+1+n);
    mn=1;
    _rep(i,2,n) {
        ans+=arr[i].a+max(arr[mn].b,arr[i].b);
        res.emplace_back(pii{arr[i].i,arr[mn].i});
        if (arr[i].b<arr[mn].b) mn=i;
    }
    writeln(ans);
    _for(t,res) writespln(t.first,t.second);
    return 0;
}
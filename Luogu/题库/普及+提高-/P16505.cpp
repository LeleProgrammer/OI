...
int main() {
    ...
    _rep(tt,1,T) {
        ...
        while (s--) {
            ...
            read(a);
            scanf(" %c",&g);
            q.emplace(pii{a,g=='L'?0:1});
        }
        int cur=1;
        while (cur<=2000000) {
            pii tail=vec.back(); vec.pop_back(); bd.erase(tail);
            pii head=vec.front();
            ...
            if (y==c+1) y=1;
            if (y==0) y=c;
            vec.emplace_front(pii{x,y});
            if (bd.count(pii{x,y})) break;
            bd.emplace(pii{x,y});
            if (((x+y)&1) && !st.count(pii{x,y})) {
                st.emplace(pii{x,y});
                vec.emplace_back(tail);
            }
            if (!q.empty() && q.front().first==cur) {

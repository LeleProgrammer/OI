...
i32 illumina(i32 n,i32 m,vector<i32> h,vector<i32> a,vector<i32> b,vector<i32> l) {
    vector<i32> flag,g,st,cnt;
    flag.resize(n),g.resize(n),st.resize(n),cnt.resize(n);
    _rep(i,0,m-1) {
        if (flag[a[i]] || flag[b[i]]) continue;
        if (h[a[i]]+h[b[i]]<=l[i]) flag[a[i]]=flag[b[i]]=true;
    }
    _rep(i,0,m-1) {
        if (h[a[i]]>h[b[i]]) swap(a[i],b[i]);
        if (!flag[a[i]] && h[a[i]]<=l[i] && h[b[i]]>l[i]) flag[a[i]]=true;
    }
    _rep(i,0,n-1) p[i]=i;
    _rep(i,0,n-1) if (!flag[i]) cnt[i]=1; else st[i]=true;
    _rep(i,0,m-1) {
        if ((!flag[a[i]] || !flag[b[i]]) && (h[a[i]]<=l[i] && h[b[i]]<=l[i])) {
            if (find(a[i])==find(b[i])) g[find(a[i])]=true;
            else {
                g[find(a[i])]|=g[find(b[i])];
                st[find(a[i])]|=st[find(b[i])];
                cnt[find(a[i])]+=cnt[find(b[i])];
                p[find(b[i])]=find(a[i]);
            }
        }
    }
    ...
    _rep(i,0,n-1) if (find(i)==i) {
        if ((g[find(i)] || st[find(i)])) ans+=cnt[find(i)];
        else if (cnt[find(i)]) ans+=cnt[find(i)]-1;
        else putchar(10);
    }
    return ans;
}
...
//         a.emplace_back(u),b.emplace_back(v),l.emplace_back(w);
//     }
//     write(illumina(n,m,h,a,b,l));
//     return 0;
// }
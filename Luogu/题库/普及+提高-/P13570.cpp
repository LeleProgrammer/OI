...
int main() {
    ...
    while (T--) {
        ...
        bool flaga=true;
        bool flagb=true;
        _rep(i,1,n) if (a[i]=='.') flaga=false;
        _rep(i,1,n) if (b[i]=='.') flagb=false;
        if (flaga^flagb) puts("No");
        else if (flaga && flagb) {
            puts("Yes");
            _rep(i,1,7) {
                _rep(j,1,n) putchar(a[j]);
                putchar(10);
            }
        } else {
            _rep(i,1,n) ans[1][i]=a[i];
            _rep(i,1,n) ans[7][i]=b[i];
            _rep(i,1,n) {
                if (ans[1][i]=='#') ans[2][i]='.';
                else ans[2][i]='#';
            }
            ...
            int posa,posb;
            _rep(i,1,n) if (ans[2][i]=='.' && (ans[2][i-1]=='#' || ans[2][i+1]=='#')) {
                ans[3][i]='#';
                posa=i;
                break;
            }
            _rep(i,1,n) if (i!=posa) ans[3][i]='.';
            _rep(i,1,n) if (ans[6][i]=='.' && (ans[6][i-1]=='#' || ans[6][i+1]=='#')) {
                ans[5][i]='#';
                posb=i;
                break;
            }
            _rep(i,1,n) if (i!=posb) ans[5][i]='.';
            _rep(i,1,n) ans[4][i]='.';
            if (posa==posb) {
                if (posa==1) ans[4][posa+1]='#';
                else ans[4][posa-1]='#';
            } else {
                if (posa>posb) swap(posa,posb);
                if (posa+1==posb) ans[4][posa]='#';
                else _rep(i,posa+1,posb-1) ans[4][i]='#';
            }
            puts("Yes");
            _rep(i,1,7) {
                _rep(j,1,n) putchar(ans[i][j]);
                putchar(10);
            }
        }
    }

...
int num[N];
int col[N];
int sum1[N][2],sum2[N][2],sum3[N][2],cnt[N][2],ans;

i32 main() {
    read(n,m);
    _rep(i,1,n) read(num[i]);
    _rep(i,1,n) read(col[i]);
    _rep(i,1,n) {
        ans=(ans+i*num[i]*cnt[col[i]][i&1]+sum1[col[i]][i&1]+i*sum2[col[i]][i&1]+sum3[col[i]][i&1]*num[i])%mod;
        sum1[col[i]][i&1]=(sum1[col[i]][i&1]+i*num[i])%mod;
        sum2[col[i]][i&1]=(sum2[col[i]][i&1]+num[i])%mod;
        sum3[col[i]][i&1]=(sum3[col[i]][i&1]+i)%mod;
        cnt[col[i]][i&1]++;
    }
    write(ans);
    return 0;
...
x*num[x]*cnt + sum(z*num[z]) + x*sum(num[z]) + sum(z)*num[x]
*/
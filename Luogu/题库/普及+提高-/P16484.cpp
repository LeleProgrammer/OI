...
int main() {
    ...
    _rep(tt,1,T) {
        ...
        _rep(i,1,n) read(a[i]);
        memset(f,0,sizeof(f));
        _rep(i,1,n-2) if (a[i]+k==a[i+1] && a[i+1]+k==a[i+2]) f[i][i+2]=true;
        _rep(len,6,n) _rep(l,1,n-len+1) {
            int r=l+len-1;
            _rep(t,l+1,r-1) if (a[l]+k==a[t] && a[t]+k==a[r] && (f[l+1][t-1] || l+1>t-1) && (f[t+1][r-1] || t+1>r-1)) f[l][r]=true;
            _rep(t,l,r-1) if (f[l][t] && f[t+1][r]) f[l][r]=true;
        }
        _rep(i,1,n) {
            g[i]=g[i-1]+1;
            _rep(j,1,i-2) if (f[j][i]) chmin(g[i],g[j-1]);
        }
        printf("Case #%d: ",tt);
        writeln(g[n]);
    }

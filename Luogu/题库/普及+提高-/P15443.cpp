...
i32 main() {
    ...
    _rep(t,0,k-1) {
        ...
        _rep(i,0,n+m) _rep(j,0,5) if (j<=i && 5-j<=n+m-i && f[t&1][i]) {
            if (j>5-j) f[(t&1)^1][i+5-j]=(f[(t&1)^1][i+5-j]+((C(i,j)*C(n+m-i,5-j)%mod)*inv%mod)*f[t&1][i])%mod;
            else f[(t&1)^1][i-j]=(f[(t&1)^1][i-j]+((C(i,j)*C(n+m-i,5-j)%mod)*inv%mod)*f[t&1][i])%mod;
        }
    }
    write((f[k&1][0]+f[k&1][n+m])%mod);
    return 0;
}
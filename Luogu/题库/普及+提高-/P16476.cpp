...
int T,n;
int prex[M],cntx[M],prey[M],cnty[M];
int x[M],y[M];
...
i32 main() {
    ...
    _rep(tt,1,T) {
        ...
        _rep(i,1,n) {
            int xl,yl,xr,yr;
            read(xl,yl,xr,yr);
            _rep(i,xl,xr) _rep(j,yl,yr) {
                ++idx;
                x[idx]=i,y[idx]=j;
                vecx.emplace_back(x[idx]);
                vecy.emplace_back(y[idx]);
            }
        }
        _sort(vecx),_unique(vecx);
        _sort(vecy),_unique(vecy);
        int xn=vecx.size(),yn=vecy.size();
        _rep(i,1,idx) {
            x[i]=LB(vecx,x[i])+1;
            y[i]=LB(vecy,y[i])+1;
            cntx[x[i]]++,prex[x[i]]+=vecx[x[i]-1];
            cnty[y[i]]++,prey[y[i]]+=vecy[y[i]-1];
        }
        _rep(i,1,xn) cntx[i]+=cntx[i-1],prex[i]+=prex[i-1];
        _rep(i,1,yn) cnty[i]+=cnty[i-1],prey[i]+=prey[i-1];
        int ans=1e18,ansx,ansy;
        _rep(i,1,idx) {
            int a=x[i],b=y[i];
            int disx=vecx[a-1]*cntx[a]-prex[a]+(prex[xn]-prex[a])-vecx[a-1]*(cntx[xn]-cntx[a]);
            int disy=vecy[b-1]*cnty[b]-prey[b]+(prey[yn]-prey[b])-vecy[b-1]*(cnty[yn]-cnty[b]);
            int dis=disx+disy;
            if (dis<ans) ans=dis,ansx=a,ansy=b;
            else if (dis==ans) {
                if (a<ansx) ansx=a,ansy=b;
                else if (a==ansx) chmin(ansy,b);
            }
        }
        printf("Case #%lld: ",tt);
        writespln(vecx[ansx-1],vecy[ansy-1],ans);
    }
    return 0;
}
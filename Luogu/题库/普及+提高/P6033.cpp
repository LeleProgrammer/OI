...
i32 main() {
    ...
    _rep(i,1,N-1) while (cnt[i]--) a.emplace(i);
    int ans=0;
    while (a.size()+b.size()>1) {
        int g[2],idx=0;
        _rep(i,1,2) {
            if (a.empty()) g[idx++]=b.front(),b.pop();
            else if (b.empty()) g[idx++]=a.front(),a.pop();
            else if (a.front()<b.front()) g[idx++]=a.front(),a.pop();
            else g[idx++]=b.front(),b.pop();
        }
        b.emplace(g[0]+g[1]);
        ans+=g[0]+g[1];
    }

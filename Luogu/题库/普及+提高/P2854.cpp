...
i32 main() {
    ...
    _rep(i,1,n) {
        int x,w,f,c;
        read(x),read(w),read(f),read(c);
        vec[x+w].emplace_back(node{w,f,c});
    }
    _rep(i,1,l) _rep(j,0,b) f[i][j]=-inf;
    _rep(i,1,l) _iter(it,vec[i]) _rep(j,it->c,b) f[i][j]=max(f[i][j],f[i-(it->w)][j-(it->c)]+(it->f));
    int ans=-1;
    _rep(i,1,b) ans=max(ans,f[l][i]);
    write(ans);
    return 0;
}
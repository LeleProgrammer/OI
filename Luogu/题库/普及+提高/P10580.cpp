...
i32 main() {
    ...
    while (T--) {
        ...
        if (y!=1) st[y].second=1;
        bool ok=true;
        _iter(it,st) if (it->second.first>it->second.second) ok=false;
        if (!ok) puts("0");
        else {
            int ans=1;
            _iter(it,st) {
                int m=it->second.second-it->second.first+1;
                int a=ksm(m-1,n);
                int b=ksm(m-2,n);
                int all=ksm(m,n);
                // cout<<"test: "<<m<<" "<<a<<" "<<b<<" "<<all<<endl;
                ans=ans*((all-(a*2-b))%mod)%mod;
            }
            writeln((ans+mod)%mod);
        }
    }
...
no min = A
no max = B
no min and no max = C = A n B
no min or no max = D = A u B
ans = |All| - |D|
|D| = |A| + |B| - |C|

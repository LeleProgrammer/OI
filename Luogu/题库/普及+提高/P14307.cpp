        ...
        void dijkstra(int s) {
            ...
            while (!q.empty()) {
                ...
                _graph(i,p) {
                    int nex=dis[u]+1;
                    if (nex<w[i]) {
                        if ((nex&1)==(w[i]&1)) nex=w[i];
                        else nex=w[i]+1;
                    }
                    if (dis[e[i]+(nex&1)*n]>nex) q.emplace(pii{dis[e[i]+(nex&1)*n]=nex,e[i]+(nex&1)*n});
                }
            }
        }
    }
...
int main() {
    ...
    while (T--) {
        ...
        dijkstra(1);
        int ans0=0,ans1=0;
        _rep(i,1,n) ans0=max(ans0,dis[i]),ans1=max(ans1,dis[i+n]);
        // _rep(i,1,n<<1) printf("dis[%d] = %d\n",i,dis[i]);
        if (ans0==INF && ans1==INF) puts("-1");
        else writeln(min(ans0,ans1)*o);
    }
    return 0;
}
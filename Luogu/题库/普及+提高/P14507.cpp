...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,1,n) read(arr[i].a),read(arr[i].b),st.emplace(arr[i].a);
        int cur=-1;
        _rep(i,1,n) {
            if (arr[i].a!=cur+1) break;
            cur++;
            if (!cur) p[arr[i].a]=arr[i].b;
            else p[arr[i].a]=min(p[arr[i].a-1],arr[i].b);
        }
        _rep(i,0,cur) cnt[i]=p[i]-p[i+1];
        // cout<<"cnt: "; _rep(i,0,cur) cout<<cnt[i]<<" ";
        // putchar(10);
        _rep(i,1,q) read(qs[i].m),qs[i].i=i;
        sort(qs+1,qs+1+q);
        if (!~cur) { // no 0 exist
            _rep(i,1,q) {
                int id=qs[i].i,m=qs[i].m;
                if (!m) ans[id]=1;
                else ans[id]=-1;
            }
        ...
        int ptr=1,sum=0,ct=0;
        _rrep(i,cur,0) {
            // cout<<"test: "<<i<<endl;
            while (ptr<=q) {
                // cout<<"test: "<<i<<" "<<ptr<<endl;
                int id=qs[ptr].i,m=qs[ptr].m;
                if (!m) {
                    if (st.count(0)) ans[id]=-1;
                    else ans[id]=1;
                    ptr++;
                    continue;
                }
                ...
                if (d<=0) {
                    if (ct==1) {
                        if (st.count(m)) ans[id]=ct+1;
                        else ans[id]=ct;
                    } else ans[id]=ct;
                    ptr++;
                    continue;
                }
                if (d-cnt[i]*(i+1)>0) break;
                int g=d/(i+1);
                if (d%(i+1)) g++;
                if (ct+g==1) {
                    if (st.count(m)) ans[id]=ct+g+1;
                    else ans[id]=ct+g;
                } else ans[id]=ct+g;
                ptr++;
                continue;

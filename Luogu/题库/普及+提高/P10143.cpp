...
i32 main() {
    init();
    read(c),read(n),read(T);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n) read(t[i]),pre[i]=pre[i-1]+t[i];
    f[0]=1;
    int ans=0;
    _rep(i,1,n) {
        int cur=0;
        _rep(j,0,T-t[i]) cur=(cur+f[j])%mod;
        ans=(ans+(a[i]*pw[n-i]%mod)*cur%mod)%mod;
        _rrep(j,T,t[i]) f[j]=(f[j]+f[j-t[i]])%mod;
    }
    memset(f,0,sizeof(f));
    f[0]=1;
    _rrep(i,n,1) {
        int cur=0;
        _rep(j,0,T-pre[i]) cur=(cur+f[j])%mod;
        ans=(ans+(a[i]*pw[i-1]%mod)*cur%mod)%mod;
        _rrep(j,T,t[i]) f[j]=(f[j]+f[j-t[i]])%mod;
    }
    write(ans);
    return 0;

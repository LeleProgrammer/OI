    ...
    while (T--) {
        ...
        while (m--) {
            ...
            if (*op=='+') {
                int i,j;
                read(i),read(j);
                s[i]={s[j].first,s[j].second};
            } else if (*op=='-') {
                int i,j;
                read(i),read(j);
                s[i]={s[j].first,s[j].second^1};
            } else if (*op=='T') {
                int i;
                read(i);
                s[i]={s[n+1].first,0};
        ...
        _rep(i,1,n) if (s[i].first) add(i,s[i].first,s[i].second),add(s[i].first,i,s[i].second),merge(i,s[i].first);
        _rep(i,1,n+2) if (!flag[i] && !~g[i]) if (!(g[i]=0,dfs(i))) flag[find(i)]=true;
        flag[find(n+2)]=true;
        _rep(i,1,n) if (flag[find(i)]) ans++;
        writeln(ans);
    }
    return 0;
}
...
namespace Graph {
    ...
    namespace TOPO {
        int indeg[N],cnt,ans[N];

        void topo(int n,vector<pii> *vec) {
            ...
            while (!q.empty()) {
                int u=q.front(); q.pop();
                cnt++;
                _iter(it,vec[u]) ans[u]=max(ans[u],ans[it->first]+ans[it->second]);

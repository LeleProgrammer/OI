...
int main() {
    ...
    _rep(i,1,n) {
        _rrep(len,m-1,1) _rep(l,1,m-len+1) {
            int r=l+len-1;
            BigInt<M> a;
            if (l>=2) a=f[l-1][r]+arr[i][l-1]*pw[m-(r-l+1)];
            BigInt<M> b;
            if (r<=m-1) b=f[l][r+1]+arr[i][r+1]*pw[m-(r-l+1)];
            if (a>b) f[l][r]=a;
            else f[l][r]=b;
        }
        BigInt<M> g;
        _rep(j,1,m) if (f[j][j]+pw[m]*arr[i][j]>g) g=f[j][j]+pw[m]*arr[i][j];
        ans=ans+g;
    }
    ans.print();
    return 0;

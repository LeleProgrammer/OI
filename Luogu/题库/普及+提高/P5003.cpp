...
int main() {
    ...
    _rep(i,0,n) _rep(j,0,m) _rep(k,0,1) f[i][j][k]=-inf,g[i][j][k]=inf;
    if (arr[1][1]=='#') {
        puts("-1");
        return 0;
    }
    _rep(i,1,n) _rep(j,1,m) {
        ...
        else if (i==1 && j==2) f[i][j][0]=g[i][j][0]=0;
        else if (i==2 && j==1) f[i][j][1]=g[i][j][1]=0;
        else {
            f[i][j][0]=max(f[i][j-1][0],f[i][j-1][1]+1);
            g[i][j][0]=min(g[i][j-1][0],g[i][j-1][1]+1);
            f[i][j][1]=max(f[i-1][j][0]+1,f[i-1][j][1]);
            g[i][j][1]=min(g[i-1][j][0]+1,g[i-1][j][1]);
        }
    }
    int mx=max(f[n][m][0],f[n][m][1]);
    int mn=min(g[n][m][0],g[n][m][1]);
    if (mn==inf) puts("-1");
    else writesp(mx),write(mn);
    return 0;
}
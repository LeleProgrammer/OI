...
int main() {
    read(n),read(q);
    _rep(i,1,n) read(co[i]),read(fr[i]),read(be[i]);
    _rep(i,0,1) _rep(j,0,N-1) _rep(k,0,N-1) f[i][j][k]=-INF;
    _rep(j,0,N-1) f[0][j][0]=0;
    _rep(i,1,n) _rep(j,0,N-1) _rep(k,0,N-1) {
        f[i&1][j][k]=f[(i&1)^1][j][k];
        if (j-co[i]>=0) f[i&1][j][k]=max(f[i&1][j][k],f[(i&1)^1][j-co[i]][max(k-fr[i],0)]+be[i]);
    }
    while (q--) {
        int co,fr;
        read(co),read(fr);

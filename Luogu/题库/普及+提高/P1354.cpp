...
using namespace Graph::SP;

double x[N],a1[N],b1[N],a2[N],b2[N];
...
int main() {
    memset(h,-1,sizeof(h));
    read(n);
    _rep(i,2,n+1) scanf("%lf %lf %lf %lf %lf",&x[i],&a1[i],&b1[i],&a2[i],&b2[i]);
    a1[1]=a1[n+2]=b1[1]=b1[n+2]=a2[1]=a2[n+2]=b2[1]=b2[n+2]=5.0;
    x[1]=0.0,x[n+2]=10.0;
    n+=2;
    _rep(i,1,n) {
        _rep(j,1,i-1) {
            bool ok=true;
            double k=(a1[i]-a1[j])/(x[i]-x[j]);
            double b=a1[i]-k*x[i];
            _rep(t,j+1,i-1) {
                double y=k*x[t]+b;
                if ((y>=0 && y<=a1[t]) || (y>=b1[t] && y<=a2[t]) || y>=b2[t]) {
                    ok=false;
                    break;
                }
            }
            if (ok) add(j,i,get(x[i],a1[i],x[j],a1[j]));
            ok=true;
            k=(a2[i]-a1[j])/(x[i]-x[j]);
            b=a2[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j,i+n,get(x[i],a2[i],x[j],a1[j]));
            ok=true;
            k=(b1[i]-a1[j])/(x[i]-x[j]);
            b=b1[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j,i+n*2,get(x[i],b1[i],x[j],a1[j]));
            ok=true;
            k=(b2[i]-a1[j])/(x[i]-x[j]);
            b=b2[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j,i+n*3,get(x[i],b2[i],x[j],a1[j]));
            ok=true;
            k=(a1[i]-b1[j])/(x[i]-x[j]);
            b=a1[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n*2,i,get(x[i],a1[i],x[j],b1[j]));
            ok=true;
            k=(a2[i]-b1[j])/(x[i]-x[j]);
            b=a2[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n*2,i+n,get(x[i],a2[i],x[j],b1[j]));
            ok=true;
            k=(b1[i]-b1[j])/(x[i]-x[j]);
            b=b1[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n*2,i+n*2,get(x[i],b1[i],x[j],b1[j]));
            ok=true;
            k=(b2[i]-b1[j])/(x[i]-x[j]);
            b=b2[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n*2,i+n*3,get(x[i],b2[i],x[j],b1[j]));
            ok=true;
            k=(a1[i]-a2[j])/(x[i]-x[j]);
            b=a1[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n,i,get(x[i],a1[i],x[j],a2[j]));
            ok=true;
            k=(a2[i]-a2[j])/(x[i]-x[j]);
            b=a2[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n,i+n,get(x[i],a2[i],x[j],a2[j]));
            ok=true;
            k=(b1[i]-a2[j])/(x[i]-x[j]);
            b=b1[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n,i+n*2,get(x[i],b1[i],x[j],a2[j]));
            ok=true;
            k=(b2[i]-a2[j])/(x[i]-x[j]);
            b=b2[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n,i+n*3,get(x[i],b2[i],x[j],a2[j]));
            ok=true;
            k=(a1[i]-b2[j])/(x[i]-x[j]);
            b=a1[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n*3,i,get(x[i],a1[i],x[j],b2[j]));
            ok=true;
            k=(a2[i]-b2[j])/(x[i]-x[j]);
            b=a2[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n*3,i+n,get(x[i],a2[i],x[j],b2[j]));
            ok=true;
            k=(b1[i]-b2[j])/(x[i]-x[j]);
            b=b1[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n*3,i+n*2,get(x[i],b1[i],x[j],b2[j]));
            ok=true;
            k=(b2[i]-b2[j])/(x[i]-x[j]);
            b=b2[i]-k*x[i];
            _rep(t,j+1,i-1) {
            ...
            if (ok) add(j+n*3,i+n*3,get(x[i],b2[i],x[j],b2[j]));
        }
    }
    dijkstra(1);
    // _rep(i,1,n*2) printf("dis[%d] = %.2lf\n",i,dis[i]);
    printf("%.2lf",dis[n]);
    return 0;
}

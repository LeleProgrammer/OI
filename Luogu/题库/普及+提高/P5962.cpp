...
int main() {
    ...
    _rep(i,1,n) {
        string s;
        readstr(s);
        _rep(j,0,(int)s.size()-1) {
            int l=0;
            while (isdigit(s[j])) l=l*10+s[j++]-'0';
            if (!l) break;
            int r=l;
            if (s[j]=='-') {
                j++;
                r=0;
                while (isdigit(s[j])) r=r*10+s[j++]-'0';
            }
            vec[i].emplace_back(piii{++idx,{l,r}});
        }
    }
    _rep(i,1,idx) p[i]=i;
    _rep(i,2,n) {
        ...
        _rep(j,0,(int)vec[i].size()-1) {
            while (k<vec[i-1].size() && vec[i-1][k].second.second<vec[i][j].second.first) k++;
            while (k<vec[i-1].size() && vec[i-1][k].second.first<=vec[i][j].second.second) merge(vec[i][j].first,vec[i-1][k++].first);
            if (k) k--;
        }
    }
    _rep(i,1,n) _rep(j,0,(int)vec[i].size()-1) cnt[find(vec[i][j].first)]+=vec[i][j].second.second-vec[i][j].second.first+1;
    _rep(i,1,N-1) ans[cnt[i]]++;
    // _rep(i,1,n) _rep(j,0,(int)vec[i].size()-1) cout<<"test: "<<i<<": "<<find(vec[i][j].first)<<" "<<vec[i][j].second.first<<" "<<vec[i][j].second.second<<endl;
    _rrep(i,N-1,1) if (ans[i]) writesp(i),writeln(ans[i]);
    return 0;
}
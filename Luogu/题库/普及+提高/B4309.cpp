...
int main() {
    ...
    while (T--) {
        ...
        _rep(i,1,n) vdcc_nodes[i].clear();
        while (!stk.empty()) stk.pop();
        int cg=0;
        _rep(i,1,n) _rep(j,1,m) scanf(" %c",&arr[i][j]),cg+=arr[i][j]=='G';
        _rep(i,1,n) _rep(j,1,m) if (arr[i][j]=='G') _rep(p,0,3) {
        ...
        int ltk=0;
        _rep(i,1,n) _rep(j,1,m) if (arr[i][j]=='G' && !dfn[get(i,j)]) {
            tarjan(root=get(i,j),-1),ltk++;
            break;
        }
        if (ltk>=2) {
            puts("0");
        ...
        int cnt=0;
        _rep(i,1,n) _rep(j,1,m) cnt+=gd[get(i,j)];
        if (cnt) puts("1");
        else puts("2");
    }
    return 0;

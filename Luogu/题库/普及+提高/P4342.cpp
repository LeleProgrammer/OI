...
inline int solve(int a,char op,int b) {
    if (op=='t') return a+b;
    else return a*b;
}

int main() {
    _rep(i,0,N-1) _rep(j,0,N-1) f[i][j]=-inf,g[i][j]=inf;
    read(n);
    scanf(" %c",&op[n]);
    _rep(i,1,n) {
        read(f[i][i]),g[i][i]=f[i][i];
        if (i!=n) scanf(" %c",&op[i]);
    }
    _rep(i,1,n) f[i+n][i+n]=g[i+n][i+n]=f[i][i],op[i+n]=op[i];
    _rep(len,2,n) _rep(l,1,n*2-len+1) {
        int r=l+len-1;
        _rep(k,l,r-1) {
            chmax(f[l][r],solve(f[l][k],op[k],f[k+1][r]));
            chmax(f[l][r],solve(f[l][k],op[k],g[k+1][r]));
            chmax(f[l][r],solve(g[l][k],op[k],f[k+1][r]));
            chmax(f[l][r],solve(g[l][k],op[k],g[k+1][r]));
            chmin(g[l][r],solve(f[l][k],op[k],f[k+1][r]));
            chmin(g[l][r],solve(f[l][k],op[k],g[k+1][r]));
            chmin(g[l][r],solve(g[l][k],op[k],f[k+1][r]));
            chmin(g[l][r],solve(g[l][k],op[k],g[k+1][r]));
        }
        if (len==n) chmax(ans,f[l][r]);
    }
    writeln(ans);
    _rep(i,1,n) if (f[i][i+n-1]==ans) writesp(i);
    return 0;
}
...
int main() {
    ...
    _rep(u,1,n) {
        int cur=0;
        int v=u;
        while (v && cur<=k) cnt[v][cur++]+=c[u],v=fa[v];
    }
    _rep(u,1,n) _rep(i,1,k) cnt[u][i]+=cnt[u][i-1];
    _rep(u,1,n) {
        int ans=cnt[u][k];
        int las=u,v=fa[u],cur=k-1;
        while (v && ~cur) ans+=cnt[v][cur]-(cur?cnt[las][cur-1]:0),cur--,v=fa[v],las=fa[las];
        writeln(ans);
    }
    return 0;
}
...
int query(int l,int r,int k) {
    ...
    if (B[l]==B[r]) {
        pushdown(B[l]);
        _rep(i,l,r) ans+=(a[i]==k),a[i]=k;
        return ans;
    }
    if (tag[B[l]]!=inf) ans+=(R[l]-l+1)*(tag[B[l]]==k);
    else _rep(i,l,R[l]) ans+=(a[i]==k);
    pushdown(B[l]);
    _rep(i,l,R[l]) a[i]=k;
    if (tag[B[r]]!=inf) ans+=(r-L[r]+1)*(tag[B[r]]==k);
    else _rep(i,L[r],r) ans+=(a[i]==k);
    pushdown(B[r]);
    _rep(i,L[r],r) a[i]=k;
    _rep(i,B[l]+1,B[r]-1) {
        if (tag[i]!=inf) ans+=(BR[i]-BL[i]+1)*(tag[i]==k);
        else _rep(j,BL[i],BR[i]) ans+=(a[j]==k);
        tag[i]=k;
    }
    return ans;
}

...
inline int get(int t,int u) {
    return (t-1)*n+u;
}

inline void spfa() {
    memset(dis,0x3f,sizeof(dis));
    q.emplace(get(1,1)),dis[get(1,1)]=0,flag[get(1,1)]=true;
    while (!q.empty()) {
        int u=q.front(); q.pop();
        flag[u]=false;
        _graph(i,u) if (dis[e[i]]>dis[u]+w[i]) {
            dis[e[i]]=dis[u]+w[i];
            if (!flag[e[i]]) flag[e[i]]=true,q.emplace(e[i]);
        }
    }
}
...
i32 main() {
    ...
    while (m--) {
        int a,b;
        read(a,b);
        _rep(t,1,1001) add(get(t,a),get(t+1,b),-arr[b]);
    }
    spfa();
    _rep(t,1,1001) chmax(ans,-dis[get(t,1)]-cc*(t-1)*(t-1));
    write(ans);
    return 0;
}
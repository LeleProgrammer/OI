...
int main() {
    ...
    _rep(i,1,n) p[i]=i;
    _rep(i,1,n) if (find(i)!=find(arr[i])) p[find(i)]=find(arr[i]);
    _rep(i,1,n) sz[find(i)]++;
    _rep(i,1,n) {
        if (i!=1 && find(i-1)!=find(i)) ans=max(ans,sz[find(i-1)]+sz[find(i)]);
        if (i!=n && find(i+1)!=find(i)) ans=max(ans,sz[find(i+1)]+sz[find(i)]);

...
void init() {
    _rep(i,0,N-1) _rep(j,0,i) {
        if (!j || i==j) binom[i][j]=1;
        else binom[i][j]=(binom[i-1][j]+binom[i-1][j-1])%k;
    }
    _rep(i,0,N-1) _rep(j,0,i) binom[i][j]=!(binom[i][j]%k);
    _rep(i,1,N-1) binom[i][0]+=binom[i-1][0],binom[0][i]+=binom[0][i-1];
    _rep(i,1,N-1) _rep(j,1,N-1) binom[i][j]+=binom[i-1][j]+binom[i][j-1]-binom[i-1][j-1];
}

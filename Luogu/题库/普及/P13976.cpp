...
void pushdown(int x) {
    _rep(i,BL[x],BR[x]) a[i]+=b[x];
    b[x]=0;
}

void modify(int l,int r,int c) {
    if (B[l]==B[r]) _rep(i,l,r) a[i]+=c,s[B[l]]+=c;
    else {
        pushdown(B[l]),pushdown(B[r]);
        _rep(i,l,R[l]) a[i]+=c,s[B[l]]+=c;
        _rep(i,L[r],r) a[i]+=c,s[B[r]]+=c;
        _rep(i,B[l]+1,B[r]-1) b[i]+=c,s[i]+=(BR[i]-BL[i]+1)*c;
    }
}

        ...
        if (s=="right") {
            _rep(i,1,n) {
                ...
                _rrep(j,n,1) {
                    int k=arr[i][j];
                    if (!k) continue;
                    if (k!=las) vec.emplace_back(k),las=k;
                    else vec.pop_back(),vec.emplace_back(k<<1),las=0;
                }
                while (vec.size()!=n) vec.emplace_back(0);
                _rep(j,1,n) arr[i][j]=vec.back(),vec.pop_back();
            }
        } else if (s=="left") {
            _rep(i,1,n) {
                int las=0;
                ...
                while (vec.size()!=n) vec.emplace_back(0);
                _rrep(j,n,1) arr[i][j]=vec.back(),vec.pop_back();
            }
        } else if (s=="down") {
            _rep(j,1,n) {
                int las=0;
                ...
                while (vec.size()!=n) vec.emplace_back(0);
                _rep(i,1,n) arr[i][j]=vec.back(),vec.pop_back();
            }
        } else {
            _rep(j,1,n) {
                int las=0;
                ...
                while (vec.size()!=n) vec.emplace_back(0);
                _rrep(i,n,1) arr[i][j]=vec.back(),vec.pop_back();
            }
        }
        printf("Case #%d:\n",tt);
        _rep(i,1,n) {
            _rep(j,1,n) writesp(arr[i][j]);
            putchar(10);
        }
    }
    return 0;

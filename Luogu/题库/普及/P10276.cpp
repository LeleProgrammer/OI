...
int main() {
    ...
    while (T--) {
        ...
        read(n);
        _rep(i,1,n-1) read(arr[i]),cnt[arr[i]]++;
        if (arr[n-1]!=1 || cnt[1]>=3) {
            puts("-1");
            continue;
        }
        bool ok=true;
        ...
        _rep(i,2,n) {
            ...
            if (!cnt[i]) vec.emplace_back(i);
        }
        if (!ok) {
            puts("-1");
            continue;
        }
        if (vec.size()==1) vec.insert(vec.begin(),1);
        int l=1,r=n,cur=1;
        ans[1]=vec[0],ans[n]=vec[1];
        while (l<r) {
            if (ans[l]<ans[r]) ans[--r]=arr[cur++];
            else ans[++l]=arr[cur++];
        }

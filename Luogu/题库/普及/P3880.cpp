...
bool check(char c) {
    _iter(it,yy) if (c==(*it)) return true;
    return false;
}
...
int main() {
    ...
    _rep(i,1,n) {
        if (isalpha(s[i])) putchar('.'),cntalpha++;
        else putchar(s[i]);
    }
    putchar(10);
    int k=round((double)cntalpha/3.0);
    _rep(i,1,n) {
        if (isalpha(s[i]) && k<=0) putchar('.');
        else putchar(s[i]);
        if (isalpha(s[i])) k--;
    }
    putchar(10);
    k=round((double)cntalpha/3.0);
    int cnt=0;
    _rep(i,1,n) {
        if (!isalpha(s[i])) continue;
        if (k<=0 && check(s[i])) cnt++;
        k--;
    }
    if (!cnt) {
        k=round((double)cntalpha*2.0/3.0);
        _rep(i,1,n) {
            if (isalpha(s[i]) && k<=0) putchar('.');
            else putchar(s[i]);
            if (isalpha(s[i])) k--;
        ...
        k=round((double)cntalpha/3.0);
        _rep(i,1,n) {
            if (isalpha(s[i]) && k<=0 && !check(s[i])) putchar('.');
            else putchar(s[i]);
            if (isalpha(s[i])) k--;
        }
    }

...
int main() {
    ...
    _rep(i,1,n) {
        if (s[i]=='(') stk.emplace(pii{i,0});
        else if (s[i]=='[') stk.emplace(pii{i,1});
        else if (s[i]==')') {
            if (stk.empty() || stk.top().second==1) while (!stk.empty()) stk.pop();
            else {
                f[i]=stk.top().first;
                if (f[stk.top().first-1]) f[i]=f[stk.top().first-1];
                stk.pop();
                if (i-f[i]+1>ans || (i-f[i]+1==ans && f[i]<l)) ans=i-f[i]+1,l=f[i],r=i;
            }
        } else {
            if (stk.empty() || stk.top().second==0) while (!stk.empty()) stk.pop();
            else {
                f[i]=stk.top().first;
                if (f[stk.top().first-1]) f[i]=f[stk.top().first-1];
                stk.pop();

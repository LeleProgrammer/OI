...
inline bool check(int mid) {
    ans=inf;
    unordered_map<u64,int> st;
    u64 cur=0;
    _rep(i,1,mid) cur=cur*P+s[i];
    st[cur]++;
    _rep(i,mid+1,n) {
        cur=cur*P-pw[mid]*s[i-mid]+s[i];
        st[cur]++;
    }
    cur=0;
    _rep(i,1,mid) cur=cur*P+s[i];
    ...
    _rep(i,mid+1,n) {
        cur=cur*P-pw[mid]*s[i-mid]+s[i];
        if (st[cur]==1) chmin(ans,i-mid+1);
    }
    _for(t,st) if (t.second==1) return true;
    return false;
}
...
int main() {
    ...
    check(r);
    _rep(i,ans,ans+r-1) putchar(s[i]);
    return 0;
}
...
int main() {
    read(n);
    _rep(i,1,n) read(arr[i].sum),arr[i].mn=arr[i].sum;
    SEG<val,pushup,pushdown,int,modify> seg(n,arr);
    _rep(i,1,n-2) {
        auto k=seg.query(1,i+1,n);
        double g=(double)(k.sum-k.mn)/(n-i-1);
        if (g>mx) mx=g,ans.clear();
        if (g==mx) ans.emplace_back(i);
    }
    _iter(it,ans) writeln(*it);
    return 0;
}
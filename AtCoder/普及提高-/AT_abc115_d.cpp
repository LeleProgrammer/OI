...
int dfs(int k,int x) {
    ...
    if (x==1) return 0;
    int t=len[k-1];
    if (x<=t+1) return dfs(k-1,x-1);
    else if (x==t+2) return dfs(k-1,t)+1;
    else if (x<=t*2+2) return dfs(k-1,t)+dfs(k-1,x-t-2)+1;
    else return cnt[k];
}
...
signed main() {
    scanf("%lld %lld",&n,&x);
    len[0]=1;
    _rep(i,1,n) len[i]=len[i-1]*2+3;
    cnt[0]=1;
    _rep(i,1,n) cnt[i]=cnt[i-1]*2+1;
    printf("%lld\n",dfs(n,x));
    return 0;
}
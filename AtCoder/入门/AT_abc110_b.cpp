...
int main() {
    scanf("%d %d %d %d",&n,&m,&X,&Y); X++;
    _rep(i,1,n) scanf("%d",&x[i]),maxx=max(maxx,x[i]);
    _rep(i,1,m) scanf("%d",&y[i]),miny=min(miny,y[i]);
    maxx++;
    if (miny<maxx) puts("War");
    else if (maxx<X || miny>Y) puts("War");
    else puts("No War");
    return 0;
}
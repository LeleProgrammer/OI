...
int main() {
    read(l,r,d,u);
    _rep(i,l,r) if (!(abs(i)&1)) {
        int L=max(-abs(i),d),R=min(abs(i),u);
        if (L>R) continue;
        ans+=R-L+1;
        // cout<<"add: "<<i<<" "<<R-L+1<<endl;
    }
    _rep(i,d,u) if (!(abs(i)&1)) {
        int L=max(-abs(i)+1,l),R=min(abs(i)-1,r);
        if (L>R) continue;
        ans+=R-L+1;
        // cout<<"add2: "<<i<<" "<<R-L+1<<endl;
    }
    write(ans);
    return 0;
}
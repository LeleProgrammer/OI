...
i32 main() {
    ...
    _rep(i,0,n-1) {
        int r,c;
        read(r,c);
        int x1=max(1LL,r-h+1),x2=min(H-h+1,r);
        int y1=max(1LL,c-w+1),y2=min(W-w+1,c);
        x2++,y2++;
    ...
    _rep(i,0,2*n-1) {
        ...
        // cout<<"modify: "<<find(seg[i].y1)<<" "<<find(seg[i].y2)-1<<" "<<seg[i].k<<endl;
        modify(1,find(seg[i].y1),find(seg[i].y2)-1,seg[i].k);
    }
    writeln((H-h+1)*(W-w+1)-ans);
    return 0;
}
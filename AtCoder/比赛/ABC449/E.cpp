...
i32 main() {
    ...
    _rep(i,1,m) vec[cnt[i]].emplace_back(i);
    _rep(i,0,m) {
        if (vec[i].empty()) {
            ...
            if (!st.empty()) st.back()++;
            st.emplace_back(0);
            num.emplace_back(vec[i]);
        }
    }
    st.back()=inf;
    // _rep(i,0,(int)st.size()-1) {
    //     cout<<"{ ";
    //     _for(j,num[i]) cout<<j<<" ";
    //     cout<<"} : ";
    //     cout<<st[i]<<endl;
    // }
    read(q);
    _rep(i,1,q) read(qs[i].pos),qs[i].id=i;
    sort(qs+1,qs+1+q);
    int ptr=1;
    while (ptr<=q && qs[ptr].pos<=n) ans[qs[ptr].id]=a[qs[ptr].pos],++ptr;
    int cur=n;
    _set<int> tr;
    _rep(i,0,(int)st.size()-1) {
        _for(j,num[i]) tr.insert(j);
        int sz=tr.size();
        i128 nex=cur+(i128)st[i]*sz;
        while (ptr<=q && qs[ptr].pos<=nex) {
            int inc=qs[ptr].pos-cur;
            inc%=sz;
            if (!inc) inc+=sz;
            ans[qs[ptr].id]=*(tr.find_by_order(inc-1));
            ptr++;
        }
        cur=nex;
    }

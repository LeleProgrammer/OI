...
public:
    ...
    inline void modify(int x,ValueType k) { while (x<=n) tr[x]=(tr[x]+k)%mod,x+=lowbit(x); }
...
i32 main() {
    ...
    _rep(i,1,n) {
        int pos=LB(vec[a[i]],i);
        if (pos+1>=a[i]) {
            int l,r=vec[a[i]][pos-a[i]+1]-1;
            if (pos-a[i]<0) l=1;
            else l=vec[a[i]][pos-a[i]]+1;
            int ret=(tr.query(r)-tr.query(l-1))%mod;
            if (pos+1==a[i]) ret++;
            ans=(ans+ret)%mod;
            tr.modify(i,ret);

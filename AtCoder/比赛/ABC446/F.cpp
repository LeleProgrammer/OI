...
int main() {
    ...
    dfs(1);
    tr.modify(1,1);
    _rep(i,2,n) flag[i]&=flag[i-1];
    _rep(i,1,n) {
        _for(j,vec[i]) if (!(tr.query(j)-tr.query(j-1))) tr.modify(j,1);
        if (tr.query(i)==i && flag[i]) writeln(tr.query(n)-tr.query(i));
        else puts("-1");
    }
    return 0;
}
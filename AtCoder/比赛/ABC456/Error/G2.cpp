...
inline void solve1(int l) {
    _rep(k,0,l) {
        dp[0]=pre[0]=1;
        _rep(i,1,l) {
            dp[i]=pre[i-1];
            if (i-k-1>=1) dp[i]=(dp[i]-pre[i-k-2]+mod)%mod;
...
inline void solve2(int l,int n) {
    pow2[0]=1;
    _rep(i,1,n) pow2[i]=pow2[i-1]*2%mod;
    _rep(k,0,n) {
        if (k>=l) f[k]=pow2[l];
        else {
            int res=0;
            int im=(l+1)/(k+1);
            _rep(j,0,im) {
                int t=C(l+1,j)*pow2[l-j*(k+1)]%mod;
...
i32 main() {
    ...
    _rep(i,0,n+3) gg[i]=1;
    _for(t,cnt) {
        int l=t.first,c=t.second;
        if (l<=P) {
            solve1(l);
            _rep(k,l+1,n) f[k]=f[l];
        } else solve2(l,n);
        _rep(k,0,n) gg[k]=gg[k]*ksm(f[k],c)%mod;
    }
    _rep(k,1,n) {
        int ans=(gg[k]-gg[k-1]+mod)%mod;
        writeln(ans);
    }
    return 0;
}
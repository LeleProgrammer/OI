...
int main() {
    ...
    while (T--) {
        read(n,m);
        Graph::restore_graph(n*10);
        _rep(i,1,n*10) dfn[i]=low[i]=instk[i]=0; ts=0;
        while (!stk.empty()) stk.pop();
        _rep(i,1,m) read(arr[i].a,arr[i].b);
        read(g);
        _rep(i,1,n) readstr(s[i]+1);
        _rep(i,1,m) _rep(j,1,g) {
            int a=arr[i].a,b=arr[i].b;
            if (a==b) continue;
            int p=j+1;
            if (p==g+1) p=1;
            if (s[a][j]=='o' && s[b][p]=='o') add(get(a,j),get(b,p));
            if (s[b][j]=='o' && s[a][p]=='o') add(get(b,j),get(a,p));
        }
        _rep(i,1,n) _rep(j,1,g) {
            int p=j+1;
            if (p==g+1) p=1;
            if (s[i][p]=='o' && s[i][j]=='o') add(get(i,j),get(i,p));
        }
        bool ok=false;
        _rep(i,1,n) if (s[i][1]=='o' && !dfn[get(i,1)]) {
            if (tarjan(get(i,1))) {
                ok=true;
                break;
            }

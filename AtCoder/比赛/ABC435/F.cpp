...
void dfs(int u) {
    if (L[u]) dfs(L[u]);
    if (R[u]) dfs(R[u]);
    f[u]=max(L[u]?f[L[u]]+u-L[u]:0,R[u]?f[R[u]]+R[u]-u:0);
}

int main() {
    read(n);
    _rep(i,1,n) read(p[i]),root=p[i]==n?i:root;
    _rep(i,1,n) {
        int las=0;
        while (!stk.empty() && p[i]>p[stk.top()]) las=stk.top(),stk.pop();
        L[i]=las;
        if (!stk.empty()) R[stk.top()]=i;
        stk.emplace(i);
    }
    // _rep(i,1,n) cout<<L[i]<<" "<<R[i]<<endl;
    dfs(root);
    write(f[root]);
    return 0;
}
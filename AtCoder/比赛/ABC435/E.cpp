...
int main() {
    ...
    while (q--) {
        ...
        while (it!=st.end()) {
            ...
            if (L!=it->first) add.emplace_back(pii{it->first,L-1});
            if (R!=it->second) add.emplace_back(pii{R+1,it->second});
            ++it;
        }
        while (!del.empty()) st.erase(del.back()),del.pop_back();
        while (!add.empty()) st[add.back().first]=add.back().second,add.pop_back();
        writeln(cur);
        // cout<<"Test: "<<endl;
        // _iter(it,st) cout<<it->first<<" "<<it->second<<endl;
    }
    return 0;
}
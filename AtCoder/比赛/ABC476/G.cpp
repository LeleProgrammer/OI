...
i32 main() {
    _rep(i,0,N-1) {
        f[i][0]=1;
        _rep(j,1,i) f[i][j]=f[i-1][j-1]+f[i-1][j];
    }
    read(T);
    while (T--) {
        read(l,r);
        ...
        while (cur<=r) {
            int cnt=cnt0(cur);
            while (cur+(1LL<<cnt)-1>r) cnt--;
            vec.emplace_back(pii{cur,cnt});
            cur+=(1LL<<cnt);
        }
        memset(g,0,sizeof(g));
        _for(t,vec) {
            ...
            _rep(i,0,k) if (pc+i<=N-1) cnt[pc+i]+=f[k][i];
            suf[N-1]=g[N-1];
            _rrep(i,N-2,0) suf[i]=max(suf[i+1],g[i]);
            _rep(i,0,N-1) {
                if (cnt[i]) chmax(g[i],suf[i]+cnt[i]);
                chmax(ans,g[i]);
            }
        }
        writeln(ans);

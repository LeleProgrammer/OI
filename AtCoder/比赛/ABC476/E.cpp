...
struct val {
    ...
    int mn,mnp;
    int mx,mxp;
    val(): l(0),r(0),mn(inf),mnp(0),mx(-inf),mxp(0) {}
};

inline void pushup(val& u,val& l,val& r) {
    u.mn=min(l.mn,r.mn);
    u.mx=max(l.mx,r.mx);
    if (u.mn==l.mn) u.mnp=l.mnp;
    else u.mnp=r.mnp;
    if (u.mx==l.mx) u.mxp=l.mxp;
    else u.mxp=r.mxp;
}
...
int main() {
    read(n,m);
    _rep(i,1,n) read(p[i]),seg.modify(1,i,i,p[i]);
    while (m--) {
        int l,r;
        read(l,r);
        auto ret=seg.query(1,l,r);
        seg.modify(1,ret.mnp,ret.mnp,ret.mx);
        seg.modify(1,ret.mxp,ret.mxp,ret.mn);
    }
    _rep(i,1,n) writesp(seg.query(1,i,i).mx);
    return 0;
}
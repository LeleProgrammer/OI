...
int arr[N][N];
int pre[N][N],suf[N][N];
int pa[N][N],pb[N][N],sa[N][N],sb[N][N];
...
i32 main() {
    ...
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n) read(b[i]);
    _rep(i,1,n) _rep(j,1,n) arr[i][j]=a[i]*b[j]%m;
    _rep(i,1,n) _rep(j,1,n) pre[i][j]=pre[i][j-1]+arr[i][j];
    _rep(i,1,n) _rrep(j,n,1) suf[i][j]=suf[i][j+1]+arr[i][j];
    _rep(i,1,n) _rep(j,1,n) pa[i][j]=pa[i-1][j-1]+pre[i][j];
    _rrep(i,n,1) _rep(j,1,n) pb[i][j]=pb[i+1][j-1]+pre[i][j];
    _rep(i,1,n) _rrep(j,n,1) sa[i][j]=sa[i-1][j+1]+suf[i][j];
    _rrep(i,n,1) _rrep(j,n,1) sb[i][j]=sb[i+1][j+1]+suf[i][j];
    _rep(i,1,n) _rep(j,1,n) lu[i][j]=lu[i-1][j]+pre[i][j];
    _rep(i,1,n) _rrep(j,n,1) ru[i][j]=ru[i-1][j]+suf[i][j];
    _rrep(i,n,1) _rep(j,1,n) ld[i][j]=ld[i+1][j]+pre[i][j];
    _rrep(i,n,1) _rrep(j,n,1) rd[i][j]=rd[i+1][j]+suf[i][j];
    _rep(i,1,n) _rep(j,1,n) f[1][1]+=arr[i][j]*max(i-1,j-1);
    _rep(i,1,n) {
        if (i!=1) f[i][1]=f[i-1][1]+(ru[i-1][1]-sa[i-1][2])-(rd[i][1]-sb[i][2]);
        _rep(j,2,n) f[i][j]=f[i][j-1]+pa[i][j-1]+pb[i+1][j-2]-sa[i][j]-sb[i+1][j+1];
    }
    _rep(i,1,n) _rep(j,1,n) ans^=(f[i][j]+(i-1)*n+(j-1));
    write(ans);
    return 0;
}
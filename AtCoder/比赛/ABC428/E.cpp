...
void solve_u(int u,int fa,int d) {
    udis[u]=d;
    _graph(i,u) {
        if (e[i]==fa) continue;
        solve_u(e[i],u,d+1);
...
void solve_v(int u,int fa,int d) {
    vdis[u]=d;
    _graph(i,u) {
        if (e[i]==fa) continue;
        solve_v(e[i],u,d+1);
...
int main() {
    ...
    dfs(1,0,1);
    int u=1;
    _rep(i,2,n) if (dep[i]>=dep[u]) u=i;
    dfs(u,0,1);
    int v=1;
    _rep(i,2,n) if (dep[i]>=dep[v]) v=i;
    solve_u(u,0,1);
    solve_v(v,0,1);
    _rep(i,1,n) {
        if (udis[i]>vdis[i]) writeln(u);
        else if (udis[i]<vdis[i]) writeln(v);
        else writeln(max(u,v));
    }
    return 0;
}
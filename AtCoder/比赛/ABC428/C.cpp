...
int main() {
    ...
    while (T--) {
        ...
        if (op==1) {
            ...
            if (c=='(') {
                vec.emplace_back('(');
                if (cnt.empty()) cnt.emplace_back(1);
                else cnt.emplace_back(cnt.back()+1);
            } else {
                vec.emplace_back(')');
                if (cnt.empty()) cnt.emplace_back(-1);
                else cnt.emplace_back(cnt.back()-1);
                if (cnt.back()<0 && las==inf) las=(int)cnt.size()-1;
            }
            if (las==inf && cnt.back()==0) puts("Yes");
            else puts("No");
        } else {
            if ((int)vec.size()-1==las) las=inf;
            vec.pop_back();
            cnt.pop_back();
            if (las==inf && (cnt.empty() || cnt.back()==0)) puts("Yes");
            else puts("No");
        }
    }
    return 0;

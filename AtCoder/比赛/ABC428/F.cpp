...
pii getPos(int i) {
    auto k=seg.query(1,i,i);
    int l,r;
    if (k.type==0) l=k.pos,r=k.pos+len[i];
    else l=k.pos-len[i],r=k.pos;
    return pii{l,r};
}

i32 main() {
    read(n);
    _rep(i,1,n) read(len[i]),seg.modify(1,i,i,dat{0,0});
    read(q);
    while (q--) {
        int op,x;
        read(op),read(x);
        if (op==1) {
            int l=getPos(x).first;
            seg.modify(1,1,x,dat{0,l});
        } else if (op==2) {
            int r=getPos(x).second;
            seg.modify(1,1,x,dat{1,r});
        } else {
            int l=1,r=n;
            while (l<r) {
                int mid=l+r>>1;
                auto pos=getPos(mid);
                if (x>=pos.first && x<pos.second) r=mid;
                else l=mid+1;
            }
            auto pos=getPos(r);
            if (x<pos.first || x>=pos.second) puts("0");
            else writeln(n-r+1);
        }
    }
    return 0;

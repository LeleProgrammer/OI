...
int main() {
    memset(h,-1,sizeof(h));
    read(n),S=N-1,T=N-2;
    _rep(i,1,n) read(x[i]),read(r[i]),vec.emplace_back(x[i]-r[i]),vec.emplace_back(x[i]+r[i]);
    sort(vec.begin(),vec.end());
    vec.erase(unique(vec.begin(),vec.end()),vec.end());
    _rep(i,1,n) {
        int L=n+(lower_bound(vec.begin(),vec.end(),x[i]-r[i])-vec.begin())+1;
        int R=n+(lower_bound(vec.begin(),vec.end(),x[i]+r[i])-vec.begin())+1;
        FLOW::add(S,i,1);
        FLOW::add(i,L,1);
        FLOW::add(i,R,1);
    }
    _rep(i,0,(int)vec.size()-1) FLOW::add(n+i+1,T,1);
    write(dinic());
    return 0;
}
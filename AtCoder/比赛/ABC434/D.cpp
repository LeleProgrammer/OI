...
int main() {
    ...
    int cur=0;
    _rep(i,1,2000) _rep(j,1,2000) s[i][j]=((i)?s[i-1][j]:0)+((j)?s[i][j-1]:0)-((i && j)?s[i-1][j-1]:0)+p[i-1][j-1],cur+=!s[i][j];
    _rep(i,0,2000) _rep(j,0,2000) g[i][j]=((i)?g[i-1][j]:0)+((j)?g[i][j-1]:0)-((i && j)?g[i-1][j-1]:0)+(s[i][j]==1);
    // cout<<"test: "<<endl;
    // _rep(i,1,5) {
    //     _rep(j,1,6) writesp(g[i][j]);
    //     putchar(10);
    // }
    _rep(i,1,n) {
        int u=arr[i].u;
        ...
        int l=arr[i].l;
        int r=arr[i].r;
        writeln(g[d][r]-g[d][l-1]-g[u-1][r]+g[u-1][l-1]+cur);
    }
    return 0;
}
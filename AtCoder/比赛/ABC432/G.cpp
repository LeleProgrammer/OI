...
void solve() {
    vector<int> p,q;
    p.resize(G+1),q.resize(G+1);
    _rep(i,0,G) p[i]=(cnta[i]*f[i])%mod,q[i]=g[G-i];
    s=NTT::mul(p,q);
}
...
i32 main() {
    init_fact();
    read(n),read(m);
    _rep(i,1,n) read(a[i]),cnta[a[i]]++,G=max(G,a[i]);
    _rep(i,1,m) read(b[i]),cntb[b[i]]++,G=max(G,b[i]);
    solve();
    int ans=0;
    _rep(i,0,G) ans=(ans+cntb[i]*(s[G+i]*g[i]%mod)%mod)%mod;
    write(ans);
    return 0;
}
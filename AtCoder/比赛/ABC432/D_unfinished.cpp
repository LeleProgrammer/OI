...
i32 main() {
    ...
    _rep(i,1,n) {
        ...
        if (c=='X') {
            _iter(it,st) {
                if (it->first<arr[i].a) it->second-=b;
                else it->second+=b;
            }
        } else {
            _iter(it,st) {
                auto t=prev(g[it->first].upper_bound(a));
                if (t->first!=a-(it->second)) g[it->first][a-(it->second)]=t->second;
                _iter(itt,g[it->first]) {
                    if (itt->first<a-(it->second)) itt->second-=b;
                    else itt->second+=b;
                }
            }
        }
    ...
    _iter(it,st) {
        ...
        _iter(itt,g[it->first]) {
            cout<<"    test: "<<itt->first<<" "<<itt->second<<endl;
        }
    }
    return 0;
}
...
inline void bfs() {
    ...
    while (!q.empty()) {
        ...
        _rep(p,0,7) {
            ...
            if (dis[nx][ny]>dis[i][j]+1) dis[nx][ny]=dis[i][j]+1,q.emplace(pii{nx,ny});
        }
    }
}
...
i32 main() {
    ...
    _rep(i,1,n) _rep(j,0,m) dis[i].emplace_back(inf),flag[i].emplace_back(false);
    _rep(g,1,2) {
        _rep(i,1,n) t[i]=s[i];
    ...
    bfs();
    _rep(i,1,n) _rep(j,1,m) if (s[i][j]=='.' && dis[i][j]%2==0 && dis[i][j]!=inf) s[i][j]='#';
    _rep(i,1,n) writestrln(s[i]);
    return 0;
}
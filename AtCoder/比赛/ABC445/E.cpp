...
inline void sieve(int k,int id) {
    num[id].clear();
    _rep(i,2,sqrt(k)+1) if (k%i==0) {
        int cnt=0;
        while (k%i==0) k/=i,cnt++;
        num[id][i]=cnt;
        st[i].emplace_back(cnt);
    }
    if (k!=1) {
        st[k].emplace_back(1);
        num[id][k]=1;
...
i32 main() {
    ...
    while (T--) {
        read(n),st.clear();
        _rep(i,1,n) read(a[i]);
        _rep(i,1,n) sieve(a[i],i);
        int g=1;
        _iter(it,st) {
            _sort(it->second);
            int p=(it->second).back();
            g=g*ksm(it->first,p)%mod;
        }
        _rep(i,1,n) {
            int cur=g;
            _iter(it,num[i]) {
                if (st[it->first].back()==it->second) {
                    int nex;
                    if (st[it->first].size()>=2) nex=st[it->first][(int)st[it->first].size()-2];
                    else nex=0;
                    cur=cur*inverse(ksm(it->first,it->second))%mod;
                    cur=cur*ksm(it->first,nex)%mod;
                }
            }
            writesp(cur);
        }

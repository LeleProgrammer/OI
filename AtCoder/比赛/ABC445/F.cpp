...
i32 main() {
    ...
    read(n),read(k);
    _rep(i,1,n) _rep(j,1,n) read(f[0][i][j]);
    _rep(p,1,P-1) _rep(t,1,n) _rep(i,1,n) _rep(j,1,n) chmin(f[p][i][j],f[p-1][i][t]+f[p-1][t][j]);
    memset(ans,0x3f,sizeof(ans));
    _rep(i,1,n) ans[i][i]=0;
    _rep(p,0,P-1) if (k&(1LL<<p)) {
        memset(nex,0x3f,sizeof(nex));
        _rep(t,1,n) _rep(i,1,n) _rep(j,1,n) chmin(nex[i][j],ans[i][t]+f[p][t][j]);
        _rep(i,1,n) _rep(j,1,n) ans[i][j]=nex[i][j];
    }
    _rep(i,1,n) writeln(ans[i][i]);
    return 0;
}
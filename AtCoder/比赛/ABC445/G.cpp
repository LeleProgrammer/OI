...
inline void bfs2() {
    ...
    while (!q.empty()) {
        ...
        if (vis[u]) continue;
        vis[u]=true;
        _graph(i,u) if (f[i]>0 && !vis[e[i]]) q.emplace(e[i]);
    }
}
...
int main() {
    ...
    DINIC::init();
    read(n,A,B),S=N-1,T=N-2;
    int g=Math::gcd(A,B);
    _rep(i,-1,1) _rep(j,-1,1) if (i && j && (A || i==1) && (B || j==1)) dx.emplace_back(i*A),dy.emplace_back(j*B);
    if (A!=B) {
        _rep(i,-1,1) _rep(j,-1,1) if (i && j && (B || i==1) && (A || j==1)) dx.emplace_back(i*B),dy.emplace_back(j*A);
    }
    _rep(i,1,n) readstr(s[i]+1);
    int pp=dx.size();
    if (((A/g)&1)^((B/g)&1)) {
        _rep(i,1,n) _rep(j,1,n) if (check(i,j) && ((i/g+j/g)&1)) {
            add_flow(S,get(i,j),1);
            _rep(p,0,pp-1) {
                int nx=i+dx[p];
                int ny=j+dy[p];
                if (check(nx,ny)) add_flow(get(i,j),get(nx,ny),inf);
            }
        } else if (check(i,j) && !((i/g+j/g)&1)) {
            add_flow(get(i,j),T,1);
        }
        DINIC::dinic();
        bfs2();
        _rep(i,1,n) _rep(j,1,n) if (check(i,j)) {
            if ((i/g+j/g)&1) {
                if (!vis[get(i,j)]) flag[get(i,j)]=true;
            } else {
                if (vis[get(i,j)]) flag[get(i,j)]=true;
            }
        }
        _rep(i,1,n) {
            _rep(j,1,n) {
                if (s[i][j]=='#') putchar('#');
                else if (!flag[get(i,j)]) putchar('o');
                else putchar('.');
            }
            putchar(10);
        }
    } else {
        _rep(i,1,n) _rep(j,1,n) if (check(i,j) && ((i/g)&1)) {
            add_flow(S,get(i,j),1);
            _rep(p,0,pp-1) {
                int nx=i+dx[p];
                int ny=j+dy[p];

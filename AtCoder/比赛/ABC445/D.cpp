...
int main() {
    read(h,w,n);
    _rep(i,1,n) read(arr[i].h,arr[i].w),arr[i].id=i,brr[i]=arr[i];
    sort(arr+1,arr+1+n,[&](const node& a,const node& b) {
        return a.w<b.w;
    });
    sort(brr+1,brr+1+n,[&](const node& a,const node& b) {
        return a.h<b.h;
    });
    int pa=n,pb=n;
    while (w && h) {
        while (pa && vis[arr[pa].id]) pa--;
        while (pb && vis[brr[pb].id]) pb--;
        if (pa && arr[pa].w==w) {
            ans[arr[pa].id]={x,y};
            x+=arr[pa].h;
            h-=arr[pa].h;
            vis[arr[pa].id]=true;
        } else if (pb && brr[pb].h==h) {
            ans[brr[pb].id]={x,y};
            y+=brr[pb].w;
            w-=brr[pb].w;
            vis[brr[pb].id]=true;
    ...
    _rep(i,1,n) writesp(ans[i].first),writeln(ans[i].second);
    return 0;
}
...
int main() {
    read(n,q);
    _rep(i,1,n) read(arr[i].x,arr[i].y),arr[n+i]=arr[i];
    _rep(i,1,n<<1) {
        sumc[i]=cross(arr[i],arr[i+1]);
        sumx[i]=sumc[i]*(arr[i].x+arr[i+1].x);
        sumy[i]=sumc[i]*(arr[i].y+arr[i+1].y);
    }
    _rep(i,1,n<<1) sumc[i]+=sumc[i-1],sumx[i]+=sumx[i-1],sumy[i]+=sumy[i-1];
    while (q--) {
        int u,v;
        read(u,v);
        if (u>v) v+=n;
        // cout<<"test: "<<u<<" "<<v<<" "<<n<<endl;
        double s=(sumc[v-1]-sumc[u-1]+cross(arr[v],arr[u]))/2.0;
        double x=(sumx[v-1]-sumx[u-1]+cross(arr[v],arr[u])*(arr[v].x+arr[u].x));
        double y=(sumy[v-1]-sumy[u-1]+cross(arr[v],arr[u])*(arr[v].y+arr[u].y));
        x/=s,y/=s;
        x/=6.0,y/=6.0;
        printf("%.15Lf %.15Lf\n",x,y);
    }

...
int main() {
    ...
    _rep(i,1,n) {
        bool ok=true;
        _rep(j,1,m) if (s[i][j]=='#') ok=false;
        fx[i]=ok;
    }
    _rep(i,1,m) {
        bool ok=true;
        _rep(j,1,n) if (s[j][i]=='#') ok=false;
        fy[i]=ok;
    }
    ...
    _rep(i,1,n) _rep(j,1,m) if (s[i][j]=='.' && fx[i] && fy[j]) add(0,get(i,j),0);
    dijkstra(0);
    int ans=0;
    _rep(i,1,n) _rep(j,1,m) if (dis[get(i,j)]<=k) ans++;
    write(ans);
    return 0;
}
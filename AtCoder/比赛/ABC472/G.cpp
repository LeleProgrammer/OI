...
int main() {
    ...
    DINIC::init(); S=N-1,T=N-2;
    read(n,m);
    _rep(i,1,n) readstr(s[i]+1);
    _rep(i,1,n) _rep(j,1,m) if (s[i][j]!='#') {
        _rrep(t,i-1,1) {
            if (s[t][j]=='#') break;
            add_flow(get(i,j),get(t,j),inf);
        }
        _rrep(t,j-1,1) {
            if (s[i][t]=='#') break;
            add_flow(get(i,j),get(i,t),inf);
        }
        _rep(t,j+1,m) {
            if (s[i][t]=='#') break;
            add_flow(get(i,j),get(i,t),inf);
        }
        if (s[i][j]=='+') add_flow(S,get(i,j),1),ans++;
        else add_flow(get(i,j),T,1);
    }
    write(ans-DINIC::dinic());
    return 0;
}
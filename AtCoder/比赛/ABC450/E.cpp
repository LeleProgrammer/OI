...
i32 main() {
    ...
    readstr(b+1),m=strlen(b+1);
    _rep(i,1,n) cnt[1][a[i]-'a']++; len[1]=n;
    _rep(i,1,m) cnt[2][b[i]-'a']++; len[2]=m;
    _rep(i,1,n) _rep(j,0,25) cnta[i][j]=cnta[i-1][j]+(a[i]-'a'==j);
    _rep(i,1,m) _rep(j,0,25) cntb[i][j]=cntb[i-1][j]+(b[i]-'a'==j);
    _rep(i,3,P-1) {
        _rep(j,0,25) cnt[i][j]=cnt[i-1][j]+cnt[i-2][j];

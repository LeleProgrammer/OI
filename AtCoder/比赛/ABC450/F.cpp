...
inline void modify(val& u,const pii& k) {
    if (k.first==0) u.v=(u.v+k.second)%mod;
    else u.mul=u.mul*k.second%mod,u.v=u.v*k.second%mod;
}

SEG<val,pushup,pushdown,pii,modify> seg(N);

i32 main() {
    _rep(i,0,N*4-1) seg.tr[i].mul=1;
    read(n,m);
    _rep(i,1,m) read(arr[i].l,arr[i].r);
    sort(arr+1,arr+1+m);
    seg.modify(1,1,1,pii{0,1});
    _rep(i,1,m) {
        int sum=seg.query(1,arr[i].l,arr[i].r-1).v;
        seg.modify(1,arr[i].r,n,pii{1,2});
        seg.modify(1,arr[i].r,arr[i].r,pii{0,sum});
    }
    write(seg.query(1,n,n).v);
    return 0;
}
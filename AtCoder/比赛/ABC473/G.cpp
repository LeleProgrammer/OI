...
inline vector<int> poly_shift(vector<int> poly,int k) {
    // cout<<"shift: "; _for(t,poly) cout<<t<<" "; cout<<endl;
    int sz=(int)poly.size()-1;
    _rep(i,0,sz) poly[i]=poly[i]*f[i]%mod;
    _rev(poly);
    vector<int> vec;
    int cur=1;
    _rep(i,0,sz) {
        vec.emplace_back(cur*g[i]%mod);
        cur=cur*k%mod;
    }
    // cout<<"poly: "; _for(t,poly) cout<<t<<" "; cout<<endl;
    // cout<<"vec: "; _for(t,vec) cout<<t<<" "; cout<<endl;
    poly=polymul(poly,vec);
    poly.resize(sz+1);
    _rev(poly);
    _rep(i,0,sz) poly[i]=poly[i]*g[i]%mod;
    // cout<<"res: "; _for(t,poly) cout<<t<<" "; cout<<endl;
    return poly;
}

inline vector<int> build(int n) {
    if (!n) {
        vector<int> res;
        res.emplace_back(1);
        return res;
    ...
    vector<int> nex=build(n>>1);
    nex=polymul(nex,poly_shift(nex,n>>1));
    if (n&1) {
        vector<int> res(nex.size()+1,0);
        _rep(i,0,(int)res.size()-2) res[i]=(res[i]+nex[i]*(n-1))%mod;
        _rep(i,1,(int)res.size()-1) res[i]=(res[i]+nex[i-1])%mod;
        return res;
    }
    return nex;
}

...
inline void init() {
    pw[0]=1;
    _rep(i,1,N-1) pw[i]=pw[i-1]*10%3;
    _rep(i,0,9) f[0][i][i%3][setbit(0,i)]=1;
    _rep(i,1,N-1) _rep(j,0,9) _rep(k,0,9) _rep(sta,0,(1<<10)-1) if (kthbit(sta,k)) _rep(rem,0,2) (f[i][j][(rem+pw[i]*j)%3][setbit(sta,j)]+=f[i-1][k][rem][sta])%=mod;
}

inline int solve() {
    _reva(s,0,m);
    _rep(i,0,m) s[i]=s[i]-'0';
    int jw=1,i=0;
    while (jw) {
        s[i]+=jw;
        jw=s[i]/10;
    ...
    int ans=0;
    _rep(i,0,m-1) _rep(j,1,9) {
        _rep(sta,0,(1<<10)-1) if (popc(sta)!=3 && !kthbit(sta,3)) (ans+=f[i][j][0][sta])%=mod;
        _rep(sta,0,(1<<10)-1) if (popc(sta)!=3 && kthbit(sta,3)) (ans+=f[i][j][1][sta]+f[i][j][2][sta])%=mod;
        _rep(sta,0,(1<<10)-1) if (popc(sta)==3 && !kthbit(sta,3)) (ans+=f[i][j][1][sta]+f[i][j][2][sta])%=mod;
    }
    _rep(i,1,s[m]-1) {
        _rep(sta,0,(1<<10)-1) if (popc(sta)!=3 && !kthbit(sta,3)) (ans+=f[m][i][0][sta])%=mod;
        _rep(sta,0,(1<<10)-1) if (popc(sta)!=3 && kthbit(sta,3)) (ans+=f[m][i][1][sta]+f[m][i][2][sta])%=mod;
        _rep(sta,0,(1<<10)-1) if (popc(sta)==3 && !kthbit(sta,3)) (ans+=f[m][i][1][sta]+f[m][i][2][sta])%=mod;
        // cout<<"highest: "<<i<<" "<<ans<<endl;
    }
    int cur=setbit(0,s[m]),rem=s[m]%3;
    _rrep(i,m-1,0) {
        rem=rem*10%3;
        // cout<<"now: "<<cur<<" "<<rem<<endl;
        _rep(j,0,s[i]-1) {
            _rep(sta,0,(1<<10)-1) if (popc(sta|cur)!=3 && !kthbit(sta|cur,3)) (ans+=f[i][j][(3-rem)%3][sta])%=mod;
            _rep(sta,0,(1<<10)-1) if (popc(sta|cur)!=3 && kthbit(sta|cur,3)) (ans+=f[i][j][(4-rem)%3][sta]+f[i][j][(5-rem)%3][sta])%=mod;
            _rep(sta,0,(1<<10)-1) if (popc(sta|cur)==3 && !kthbit(sta|cur,3)) (ans+=f[i][j][(4-rem)%3][sta]+f[i][j][(5-rem)%3][sta])%=mod;
            // cout<<"sub: "<<i<<" "<<j<<" "<<ans<<endl;
        }
        cur=setbit(cur,s[i]);
        rem=(rem+s[i])%3;
    }

...
i32 main() {
    ...
    _rep(a,1,9) _rep(b,0,9) _rep(c,0,9) _rep(d,0,9) _rep(e,0,9) _rep(f,0,9) pre[a][b][c][d][e][f]+=pre[a-1][b][c][d][e][f];
    _rep(a,0,9) _rep(b,1,9) _rep(c,0,9) _rep(d,0,9) _rep(e,0,9) _rep(f,0,9) pre[a][b][c][d][e][f]+=pre[a][b-1][c][d][e][f];
    _rep(a,0,9) _rep(b,0,9) _rep(c,1,9) _rep(d,0,9) _rep(e,0,9) _rep(f,0,9) pre[a][b][c][d][e][f]+=pre[a][b][c-1][d][e][f];
    _rep(a,0,9) _rep(b,0,9) _rep(c,0,9) _rep(d,1,9) _rep(e,0,9) _rep(f,0,9) pre[a][b][c][d][e][f]+=pre[a][b][c][d-1][e][f];
    _rep(a,0,9) _rep(b,0,9) _rep(c,0,9) _rep(d,0,9) _rep(e,1,9) _rep(f,0,9) pre[a][b][c][d][e][f]+=pre[a][b][c][d][e-1][f];
    _rep(a,0,9) _rep(b,0,9) _rep(c,0,9) _rep(d,0,9) _rep(e,0,9) _rep(f,1,9) pre[a][b][c][d][e][f]+=pre[a][b][c][d][e][f-1];
    read(q);
    while (q--) {
        string x,y;
        readstr(x),readstr(y);
        _rep(i,0,5) x[i]-='0',y[i]-='0';
        bool ok=true;
        _rep(i,0,5) if (x[i]>y[i]) ok=false;
        if (!ok) {
            puts("0");
            continue;
        }
        ...
        _rep(sta,0,(1<<6)-1) {
            _rep(i,0,5) {
                if (kthbit(sta,i)) g[i]=x[i]-1;
                else g[i]=y[i];
            }
            bool ok=true;
            _rep(i,0,5) if (g[i]<0) ok=false;
            if (!ok) continue;
            ans+=pre[g[0]][g[1]][g[2]][g[3]][g[4]][g[5]]*((popc(sta)&1)?-1:1);
        }
        writeln(ans);
    }
    return 0;

...
void process_x(int x) {
    if (okx[x]) return;
    okx[x]=true;
    _iter(it,tx[x]) {
        ay[*it].emplace(x);
        if (ax[x].count(*it)) axy.emplace(pii{x,*it});
    }
}

void process_y(int y) {
    if (oky[y]) return;
    oky[y]=true;
    _iter(it,ty[y]) {
        ax[*it].emplace(y);
        if (ay[y].count(*it)) axy.emplace(pii{*it,y});
    }
}
...
int main() {
    ...
    while (T--) {
        read(n),read(m);
        axy.clear();
        _rep(i,1,n) tx[i].clear(),ax[i].clear(),okx[i]=false;
        _rep(i,1,m) ty[i].clear(),ay[i].clear(),oky[i]=false;
        _rep(i,1,n) {
            ans[i].clear();
            ans[i].emplace_back(-1);
            _rep(j,1,m) ans[i].emplace_back(0),tx[i].emplace(j),ty[j].emplace(i);
        }
        _rep(i,1,n*m) fx[i]=fy[i]=0;
        _rep(i,1,n) read(x[i]),fx[x[i]]=i;
        _rep(i,1,m) read(y[i]),fy[y[i]]=i;
        bool ok=true;
        _rrep(k,n*m,1) {
            if (fx[k] && fy[k]) {
                ans[fx[k]][fy[k]]=k;

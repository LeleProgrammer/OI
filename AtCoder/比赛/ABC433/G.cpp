...
void solve() {
    _rep(i,1,cnt) indeg[i]=f[i]=0,h[i]=-1; tot=0;
    _rep(i,1,cnt) _rep(j,0,25) if (arr[i].s[j]) add(arr[i].s[j],i);
    queue<int> q;
    _rep(i,1,cnt) if (!indeg[i]) q.emplace(i);
    while (!q.empty()) {
        int u=q.front(); q.pop();
        _graph(i,u) {
            f[e[i]]|=!f[u];

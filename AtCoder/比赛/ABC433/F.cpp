...
void solve(int k) {
    cnt=0;
    _rep(i,1,n) if (s[i]==k || s[i]==k+1) arr[++cnt]=s[i];
    suf[cnt+1]=0;
    _rrep(i,cnt,1) suf[i]=suf[i+1]+(arr[i]==k+1);
    int pre=0;
    _rep(i,1,cnt) {
        if (!suf[i]) break;
        if (arr[i]==k) ans+=C(pre+suf[i],suf[i]-1),pre++;
    }
}

i32 main() {
    init_fact();
    readstr(in+1),n=strlen(in+1);
    _rep(i,1,n) s[i]=in[i]-'0';
    _rep(k,0,8) solve(k);
    write((int)ans);
    return 0;
}
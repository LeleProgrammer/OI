...
inline int& s(int j,int i) {
    if (i>=0) return f[j&1][i];
    else return g[j&1][-i];
}
...
i32 main() {
    ...
    _rep(i,1,n) {
        int w,h,b;
        read(w),read(h),read(b);
        arr[i]={w,h,b};
    }
    _rep(i,-(M-1),M-1) s(0,i)=s(1,i)=-inf;
    s(0,0)=0;
    _rep(i,1,n) _rep(j,-(M-1)+arr[i].w,M-1-arr[i].w) s(i,j)=max(s(i-1,j-arr[i].w)+arr[i].b,s(i-1,j+arr[i].w)+arr[i].h);
    int ans=0;
    _rep(j,0,M-1) ans=max(ans,s(n,j));
    write(ans);
    return 0;
}
...
namespace Graph {
    ...
    namespace SP {
        ...
        void dijkstra(int s) {
            _rep(i,0,n*m*4) dis[i]=inf,flag[i]=false;
            q.push({dis[s]=0,s});
            while (!q.empty()) {
                auto tp=q.top(); q.pop();
                int u=tp.second;
...
int main() {
    ...
    while (T--) {
        ...
        _rep(i,1,n) _rep(j,1,m) _rep(k,0,3) h[get(i,j,k)]=-1; tot=0;
        _rep(i,1,n) _rep(j,1,m) {
            ...
            int d=get(i,j,2);
            if (s[i][j]=='A') {
                add(l,r,0),add(r,l,0),add(u,d,0),add(d,u,0);
                add(l,d,1),add(d,l,1),add(u,r,1),add(r,u,1);
                add(l,u,1),add(u,l,1),add(d,r,1),add(r,d,1);
            } else if (s[i][j]=='B') {
                add(l,r,1),add(r,l,1),add(u,d,1),add(d,u,1);
                add(l,d,0),add(d,l,0),add(u,r,0),add(r,u,0);
                add(l,u,1),add(u,l,1),add(d,r,1),add(r,d,1);
            } else {
                add(l,r,1),add(r,l,1),add(u,d,1),add(d,u,1);
                add(l,d,1),add(d,l,1),add(u,r,1),add(r,u,1);
                add(l,u,0),add(u,l,0),add(d,r,0),add(r,d,0);
            }
            if (i>1) add(u,get(i-1,j,2),0);
            if (j>1) add(l,get(i,j-1,1),0);
            if (i<n) add(d,get(i+1,j,0),0);
            if (j<m) add(r,get(i,j+1,3),0);
        }
        dijkstra(get(1,1,3));
        // _rep(i,1,n) _rep(j,1,m) _rep(k,0,3) printf("%d %d %d %d\n",i,j,k,dis[get(i,j,k)]);
        writeln(dis[get(n,m,1)]);
    }
    return 0;
}
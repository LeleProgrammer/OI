...
i32 main() {
    read(n);
    _rep(i,1,n) read(a[i]);
    vec.emplace_back(vi2{-1,-1});
    _rep(i,1,n) {
        int j=i;
        while (j+1<=n && a[j+1]==a[j]) j++;
        vec.emplace_back(vi2{a[i],j-i+1});
        i=j;
    }
    _rep(i,1,(int)vec.size()-1) extend(vec[i].a,i);
    _rep(i,1,cnt) ord.emplace_back(i);
    sort(_all(ord),[&](const int& a,const int& b) {
        return arr[a].len>arr[b].len;
    });
    // cout<<"ord: "; _for(g,ord) cout<<g<<" "; putchar(10);
    // cout<<"vec: "; _for(g,vec) printf("(%lld, %lld) ",g.a,g.b); putchar(10);
    _rep(i,1,(int)vec.size()-1) if (vec[i].b>=vec[i].a) {
        int j=i;
        while (j+1<=(int)vec.size()-1) {
            if (vec[j+1].b==vec[j+1].a) j++;
            else if (vec[j+1].b>vec[j+1].a) {
                j++;
                break;
            } else break;
        }
        seq[j].emplace_back(i);
        // cout<<"seq: "<<i<<" "<<j<<endl;
        if (vec[j].a==vec[j].b) i=j;
        else if (i==j || j==(int)vec.size()-1) i=j;
        else i=j-1;
    }
    int cur=inf;
    _rrep(i,(int)vec.size()-1,1) {
        _for(g,seq[i]) chmin(cur,g);
        L[i]=cur;
        if (L[i]<=i) mxl[i]=i-L[i]+1;
    }
    _rep(i,1,(int)vec.size()-1) chmax(mxsta[sta[i]],mxl[i]);
    _for(u,ord) {
        if (u==1) continue;
        chmax(mxsta[arr[u].fa],mxsta[u]);
    }
    int ans=0;
    _rep(u,2,cnt) {
        int A=min(arr[u].len,mxsta[u]);
        int B=arr[arr[u].fa].len+1;
        if (B<=A) ans+=A-B+1;
    }
    write(ans);
    return 0;

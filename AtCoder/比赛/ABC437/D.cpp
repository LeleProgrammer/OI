...
i32 main() {
    ...
    _rep(i,2,m) pre[i]=(pre[i-1]+(b[i]-b[i-1])*(i-1))%mod;
    _rrep(i,m-1,1) suf[i]=(suf[i+1]+(b[i+1]-b[i])*(m-i))%mod;
    int ans=0;
    _rep(i,1,n) {
        int pos=lower_bound(b+1,b+1+m,a[i])-b;
        ans=(ans+pre[pos-1]+(a[i]-b[pos-1])*(pos-1)+suf[pos]+(b[pos]-a[i])*(m-pos+1))%mod;
    }
    write(ans);
    return 0;
}
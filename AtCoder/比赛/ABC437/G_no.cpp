...
bool check(int a,int b,int c,int d,int A,int B,int C) {
    if (A>b+c+d) return false;
    else if (B>a+c+d) return false;
    else if (C>a+b+d) return false;
...
bool dfs(int u,int fa) {
    int ok=0,c0=0,c1=0,c2=0;
    int k0=0,k1=0,k2=0,cur=c[u];
    _rep(i,1,deg[u]) {
        if (cur==0) k0++;
        else if (cur==1) k1++;
        else k2++;
    ...
    _graph(i,u) {
        if (e[i]==fa) continue;
        if (!dfs(e[i],u)) return false;
        if (f[e[i]][0]+f[e[i]][1]+f[e[i]][2]>1) ok++;
        else if (f[e[i]][0]+f[e[i]][1]+f[e[i]][2]==1) {
            if (f[e[i]][0]) c0++;
            else if (f[e[i]][1]) c1++;
            else if (f[e[i]][2]) c2++;
        } else return false;
    ...
    if (fa) {
        if (k0) {
            k0--;
            if (check(c0,c1,c2,ok,k0,k1,k2)) f[u][0]=1;
            k0++;
        }
        if (k1) {
            k1--;
            if (check(c0,c1,c2,ok,k0,k1,k2)) f[u][1]=1;
            k1++;
        }
        if (k2) {
            k2--;
            if (check(c0,c1,c2,ok,k0,k1,k2)) f[u][2]=1;
            k2++;
        }
    } else if (check(c0,c1,c2,ok,k0,k1,k2)) f[u][0]=1;
    return f[u][0]+f[u][1]+f[u][2];
}

void solve(int u,int fa,int k) {
    cerr<<"solve: "<<u<<endl;
    if (~fa) vec[fa].emplace_back(k);
    int ok=0,c0=0,c1=0,c2=0;
    int k0=0,k1=0,k2=0,cur=c[u];
    _rep(i,1,deg[u]) {
        if (cur==0) k0++;
    ...
    _graph(i,u) {
        if ((i^1)==fa) continue;
        if (f[e[i]][0]+f[e[i]][1]+f[e[i]][2]>1) ok++,d.emplace_back(i);
        else if (f[e[i]][0]+f[e[i]][1]+f[e[i]][2]==1) {
            if (f[e[i]][0]) c0++,a.emplace_back(i);
            else if (f[e[i]][1]) c1++,b.emplace_back(i);
            else if (f[e[i]][2]) c2++,c.emplace_back(i);
        }
    }
    cerr<<"solve: "<<u<<" "<<k0<<" "<<k1<<" "<<k2<<endl;
    while (k0--) {
        if (!b.empty()) {
            int g=b.back(); b.pop_back();
            vec[g].emplace_back(0);
            solve(e[g],g,1);
        } else if (!c.empty()) {
            int g=c.back(); c.pop_back();
            vec[g].emplace_back(0);
            solve(e[g],g,2);
        } else if (!d.empty()) {
            int g=d.back(); d.pop_back();
            vec[g].emplace_back(0);
            solve(e[g],g,0);
        } else __assert(false);
    }
    ...
    while (k1--) {
        if (!a.empty()) {
            int g=a.back(); a.pop_back();
            vec[g].emplace_back(1);
            solve(e[g],g,0);
        } else if (!c.empty()) {
            int g=c.back(); c.pop_back();
            vec[g].emplace_back(1);
...
int main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,n) h[i]=-1,deg[i]=0; tot=0;
        _rep(i,1,n) {
            char g;
            scanf(" %c",&g);
            if (g=='R') c[i]=0;
        ...
        bool ok=false;
        _rep(root,1,n) {
            _rep(i,1,n) f[i][0]=f[i][1]=f[i][2]=0;
            _rep(i,0,n*2) vec[i].clear();
            bool res=dfs(root,0);
            cerr<<"test: "<<root<<": ";
            _rep(i,1,n) cerr<<(f[i][0]+f[i][1]+f[i][2])<<" ";
            cerr<<endl;
            if (res) {
                ok=true;
                solve(root,-1,0);
        ...
        if (!ok) puts("No");
            ...
            while (ok<n-1) {
                _rep(i,0,n*2) {
                    ...
                    cerr<<"out: "<<i<<" "<<a<<" "<<b<<endl;
                    if (a==c[e[i^1]] && b==c[e[i]]) {
                        writesp(i+2>>1);
                        c[e[i^1]]=(c[e[i^1]]+1)%mod;
                        c[e[i]]=(c[e[i]]+1)%mod;
                        done[i]=true;
                        ok++;
                        break;

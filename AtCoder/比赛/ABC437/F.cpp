...
SEG<val,pushup,pushdown,const int,modify> a(N),b(N),c(N),d(N);

i32 main() {
    read(n,q);
    _rep(i,1,n) read(x[i],y[i]);
    _rep(i,1,n) {
        a.modify(1,i,i,x[i]+y[i]);
        b.modify(1,i,i,x[i]-y[i]);
        c.modify(1,i,i,-x[i]+y[i]);
        d.modify(1,i,i,-x[i]-y[i]);
    }
    while (q--) {
        int op;
        read(op);
        if (op==1) {
            ...
            int A=a.query(1,l,r).v-x-y;
            int B=b.query(1,l,r).v-x+y;
            int C=c.query(1,l,r).v+x-y;
            int D=d.query(1,l,r).v+x+y;
            writeln(max(max(max(A,B),C),D));
        }
    }
    return 0;
}
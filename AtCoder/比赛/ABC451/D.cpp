...
inline void init() {
    _reps(i,1,1e9,i<<=1) vec.emplace_back(to_string(i));
}

inline i64 to_int(const string& s) {
    i64 ans=0;
    _for(c,s) ans=ans*10+c-'0';
    return ans;
}

inline void dfs(string cur) {
    i64 val=to_int(cur);
    if (st.count(val)) return;
    ans.emplace_back(val);
    st.emplace(val);
    _for(nex,vec) {
        if (cur.size()+nex.size()>=10) return;
        dfs(cur+nex);
    }
}

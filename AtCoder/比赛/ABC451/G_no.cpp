...
inline int solve(int x) {
    _rrep(i,P-1,0) if (kthbit(x,i) && bs[i]) x^=bs[i];
    return x;
}
...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,1,idx) cnt[i]=0,tr[i][0]=tr[i][1]=0;
        idx=0;
        _rep(i,1,n) dis[i]=-1;
        _rep(i,1,m) {
            int a,b,c;
        ...
        _rep(i,1,m) {
            ...
            if (p[b]==a) swap(a,b);
            if (p[a]==b && dis[a]==(dis[b]^c)) continue;
            insert(dis[a]^dis[b]^c);
        }
        _rep(i,1,n) res[i]=solve(dis[i]);
        int ans=0;
        _rep(i,1,n) ans+=query(res[i]),ins(res[i]);
        writeln(ans);
    }
    return 0;
}
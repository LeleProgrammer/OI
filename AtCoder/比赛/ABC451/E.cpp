...
int main() {
    ...
    LCA::init();
    read(n);
    _rep(i,1,n-1) _rep(j,i+1,n) read(a[i][j]),vec.emplace_back(vi3{a[i][j],i,j});
    _sort(vec);
    int dest=n-2;
    _rep(i,0,dest) {
        if (dsu.connected(vec[i].b,vec[i].c)) {
            dest=min(dest+1,(int)vec.size());
            continue;
        }
        add(vec[i].b,vec[i].c,vec[i].a),add(vec[i].c,vec[i].b,vec[i].a);
        dsu.merge(vec[i].b,vec[i].c);
        // cout<<"add: "<<vec[i].b<<" "<<vec[i].c<<endl;
    }
    dfs(1);
    int cnt=0;
    _rep(i,1,n) cnt+=vis[i];
    if (cnt!=n) {
        puts("No");
        return 0;
    }
    ...
    _rep(i,1,n-1) _rep(j,i+1,n) {
        int g=lca(i,j);
        int dis=pre[i]+pre[j]-2*pre[g];
        // cout<<"test: "<<i<<" "<<j<<": "<<dis<<" "<<a[i][j]<<endl;
        if (dis!=a[i][j]) {
            ok=false;
            break;
        }

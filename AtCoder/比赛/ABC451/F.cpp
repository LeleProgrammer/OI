...
inline void flp(int u) {
    c[u]^=1;
    _graph(i,u) if (c[e[i]]==c[u]) flp(e[i]);
}
...
int main() {
    ...
    while (q--) {
        ...
        if (dsu.find(a)==dsu.find(b)) {
            ...
            ans-=min(cnt[b][0],cnt[b][1]);
            if (g) {
                if (sz[a]>sz[b]) swap(a,b);
                flp(a);
                swap(cnt[a][0],cnt[a][1]);
            }
            dsu.merge(a,b);
            ans+=min(cnt[a][0],cnt[a][1]);
            add(aa,bb),add(bb,aa);
        }
        // cout<<"col: "; _rep(i,1,n) cout<<c[i]<<" "; putchar(10);
        if (!ok) puts("-1");
        else writeln(ans);
    }
    return 0;

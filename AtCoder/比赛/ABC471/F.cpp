...
inline i64 get(const string& s) {
    i64 k=0;
    _for(c,s) k=k*10+c-'0';
    return k;
}

inline int get0(const string& s) {
    int ans=0;
    _for(c,s) {
        if (c=='0') ans++;
        else break;
...
int main() {
    ...
    _rep(i,1,n) {
        readstr(s[i]);
        mp[s[i].size()].emplace_back(s[i]);
    }
    _for(t,mp) sort(_all(t.second),[&](const string& a,const string& b) {
        return a>b;
    });
    vector<string> vec;
    _riter(it,mp) {
    ...
    bool ok=false;
    _for(t,vec) if (t[0]!='0') ok=true;
    if (ok) {
        sort(_all(vec),[&](const string& a,const string& b) {
            if (a+b!=b+a) return a+b>b+a;
            else return a.size()>b.size();
        });
        _for(t,vec) writestr(t);
    } else {
        string ans1;
        sort(_all(vec),[&](const string& a,const string& b) {
            int a0=get0(a),b0=get0(b);
            if (a0!=b0) return a0<b0;
            else return a+b>b+a;
        });
        sort(next(vec.begin()),vec.end(),[&](const string& a,const string& b) {
            if (a+b!=b+a) return a+b>b+a;
            else return a.size()>b.size();
        });
        bool st=false;
        ...
        string inc;
        _iter(it,mp) {
            _for(t,it->second) if (!tak.count(t)) {
                if (get(t)>get(inc)) inc=t;
            }
        }
        while (!inc.empty() && inc[0]=='0') inc.erase(inc.begin());
        ans2=inc;
        _for(t,vec) ans2+=t;
        process(ans1),process(ans2);

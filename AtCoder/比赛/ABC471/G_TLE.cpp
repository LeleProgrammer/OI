...
template<const int N> struct Bitset {
	...
	inline u64 get(size_t idx) { return (bit[idx>>6]>>(idx&63))&1; }
	inline void set(size_t idx) { bit[idx>>6]|=(1ull<<(idx&63)); }
	inline void clr(size_t idx) { bit[idx>>6]^=(1ull<<(idx&63)); }
	inline void flp(size_t idx) { bit[idx>>6]^=(1ull<<(idx&63)); }
	u64& operator [] (size_t idx) { return bit[idx]; }
	const u64& operator [] (size_t idx) const { return bit[idx]; }
	inline friend Bitset<N> operator | (const Bitset<N>& a,const Bitset<N>& b) {
		Bitset<N> res;
		res.reset();
		_rep(i,0,N-1) res[i]=(a[i]|b[i]);
		return res;
	}
	inline friend Bitset<N> operator ^ (const Bitset<N>& a,const Bitset<N>& b) {
		Bitset<N> res;
...
Bitset<P> g[M];
int ans[M];
unordered_map<u64,int> cnt[P];

i32 main() {
    read(n,k,sta,m);
    _rep(i,1,m) read(b[i]);
    _rep(i,0,k-1) read(v[i]);
    _rep(i,1,n) {
        if (i<=m) a[i]=b[i];
        else {
            u64 x=(((sta>>18)^sta)>>27)%(1LL<<32);
            u64 r=(sta>>59);
            u64 y=((x>>r)+(x<<(32-r)))%(1LL<<32);
            a[i]=y%k;
            sta=(sta*6364136223846793005ULL+2026081520260815ULL);
        }
    }
    _rep(i,0,k-1) _rep(j,0,k-1) if (v[(i+j)%k]) g[i].set(j);
    // _rep(i,0,k-1) {
    //     cout<<i<<": ";
    //     _rep(j,0,k-1) cout<<g[i][j];
    //     cout<<endl;
    // }
    _rep(i,0,n) {
        Bitset<P> t;
        if (i>=1 && i<=n-1) t=(g[a[i]]^g[a[i+1]]);
        else if (!i) t=g[a[i+1]];
        else t=g[a[i]];
        // _rep(j,0,k-1) if (t.get(j)) ans[j]++;
        _rep(j,0,P-1) {
            // if (t[j]) cout<<"test: "<<j<<" "<<t[j]<<endl;
            cnt[j][t[j]]++;
        }
    }
    _rep(i,0,P-1) {
        int st=i*64;
        _for(t,cnt[i]) {
            _rep(k,0,63) if (kthbit(t.first,k)) ans[st+k]+=t.second;
        }
    }
    _rep(i,0,k-1) writeln(ans[i]>>1);
    return 0;
}

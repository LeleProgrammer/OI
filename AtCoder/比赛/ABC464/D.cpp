...
i32 main() {
    ...
    while (T--) {
        ...
        readstr(s+1);
        _rep(i,1,n) read(x[i]);
        _rep(i,1,n-1) read(y[i]);
        _rep(i,1,n) if (s[i]=='R') t[i]=0; else t[i]=1;
        f[1][t[1]]=0,f[1][t[1]^1]=-x[1];
        _rep(i,2,n) {
            if (t[i]==0) {
                f[i][t[i]]=max(f[i-1][0],f[i-1][1]);
                f[i][t[i]^1]=max(f[i-1][0]+y[i-1],f[i-1][1])-x[i];
            } else {
                f[i][t[i]]=max(f[i-1][0]+y[i-1],f[i-1][1]);
                f[i][t[i]^1]=max(f[i-1][0],f[i-1][1])-x[i];
            }
        }

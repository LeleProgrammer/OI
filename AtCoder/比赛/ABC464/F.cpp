...
i32 main() {
    ...
    read(n,x);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n) tot=(tot+a[i])%mod;
    nl=n/2,nr=n-nl;
    _rep(t,0,(1<<nl)-1) {
        int sum=0,cnt=0;
        _rep(i,1,nl) if (kthbit(t,i-1)) {
            sum+=a[i];
            cnt++;
        }
        vecl.emplace_back(pii{sum,cnt});
    }
    _rep(t,0,(1<<nr)-1) {
        int sum=0,cnt=0;
        _rep(i,1,nr) if (kthbit(t,i-1)) {
            sum+=a[nl+i];
            cnt++;
        }
        vecr.emplace_back(pii{sum,cnt});
    }
    _rsort(vecl);
    _sort(vecr);
    int j=-1;
    _for(t,vecl) {
        int suml=t.first,cntl=t.second;
        int lmt=x-suml;
        suml%=mod;
        while (j+1<=(int)vecr.size()-1 && vecr[j+1].first<lmt) {
            ++j;
            int cntr=vecr[j].second;
            int sumr=vecr[j].first;
            calc[cntr]=(calc[cntr]+1)%mod;
            vecs[cntr]=(vecs[cntr]+sumr)%mod;
        }
        _rep(i,0,nr) {
            if (!calc[i]) continue;
            int k=cntl+i;
            f[k]=(f[k]+calc[i])%mod;
            g[k]=(g[k]+suml*calc[i]+vecs[i])%mod;
        }
    }
    int ans=0;
    _rep(i,0,n-1) ans=(ans+(tot*f[i]%mod-g[i])*Math::f[i]%mod*Math::f[n-1-i])%mod;
    ans=ans*Math::g[n]%mod;
    ans=(ans%mod+mod)%mod;
    write(ans);
    return 0;
}
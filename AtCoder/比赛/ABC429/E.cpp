...
char s[N];
pii dis[N],sdis[N]; // dis, S
queue<int> q;

void bfs() {
    _rep(i,0,N-1) dis[i]={INF,0},sdis[i]={INF,0};
    _rep(i,1,n) if (s[i]=='S') q.emplace(i),dis[i]={0,i};
    while (!q.empty()) {
        int u=q.front(); q.pop();
        _graph(i,u) {
            vector<pii> vec;
            vec.emplace_back(dis[e[i]]);
            vec.emplace_back(sdis[e[i]]);
            vec.emplace_back(pii{dis[u].first+1,dis[u].second});
            sort(vec.begin(),vec.end(),[&](const pii& a,const pii& b) {
                return a.first<b.first;
            });
            if (vec[0].second==vec[1].second) vec.erase(vec.begin()+1);
            else vec.pop_back();
            if (dis[e[i]].first!=vec[0].first || sdis[e[i]].first!=vec[1].first) q.emplace(e[i]);
            dis[e[i]]=vec[0],sdis[e[i]]=vec[1];
        }
        _graph(i,u) {
            vector<pii> vec;
            vec.emplace_back(dis[e[i]]);
            vec.emplace_back(sdis[e[i]]);
            vec.emplace_back(pii{sdis[u].first+1,sdis[u].second});
            sort(vec.begin(),vec.end(),[&](const pii& a,const pii& b) {
                return a.first<b.first;
            });
            if (vec[0].second==vec[1].second) vec.erase(vec.begin()+1);
...
int main() {
    ...
    readstr(s+1);
    bfs();
    _rep(i,1,n) if (s[i]=='D') writeln(dis[i].first+sdis[i].first);
    // _rep(i,1,n) printf("test: %d %d\n",dis[i].second,sdis[i].second);
    return 0;
}

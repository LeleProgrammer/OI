...
i32 main() {
    ...
    nums.emplace_back(0),nums.emplace_back(m-1);
    sort(nums.begin(),nums.end());
    nums.erase(unique(nums.begin(),nums.end()),nums.end());
    _rep(i,1,n) a[i]=lower_bound(nums.begin(),nums.end(),a[i])-nums.begin();
    int k=nums.size()-1;
    _rep(i,1,n) sum[a[i]]++,sum[a[i]+n]++;
    p[0]=sum[0];
    _rep(i,1,n<<1) p[i]=p[i-1]+sum[i];
    int las=0;
    int ans=0;
    _rep(i,1,k) {
        int pos=nums[i];
        ...
        while (l<r) {
            int mid=l+r>>1;
            if (p[mid]-p[i-1]>=c) r=mid;
            else l=mid+1;
        }
        int res=p[r]-p[i-1];
        // cout<<"Test: "<<nums[i]<<" "<<nums[las]<<" "<<res<<endl;
        ans+=res*(nums[i]-nums[las]);
        las=i;
    }
    int l=0,r=n<<1;
    while (l<r) {

...
inline void pushdown(val& u,val& l,val& r) {
    if (u.add) {
        l.add=(l.add+u.add)%mod;
        r.add=(r.add+u.add)%mod;
        l.sum2=(l.sum2+(l.r-l.l+1)*(u.add*u.add%mod)+u.add*l.sum*2%mod)%mod;
        r.sum2=(r.sum2+(r.r-r.l+1)*(u.add*u.add%mod)+u.add*r.sum*2%mod)%mod;
        l.sum=(l.sum+(l.r-l.l+1)*u.add)%mod;
        r.sum=(r.sum+(r.r-r.l+1)*u.add)%mod;
        u.add=0;
    }
}
...
inline void modify(val& u,const int& k) {
    u.add=(u.add+k)%mod;
    u.sum2=(u.sum2+(u.r-u.l+1)*(k*k%mod)+k*u.sum*2%mod)%mod;
    u.sum=(u.sum+(u.r-u.l+1)*k)%mod;
}

SEG<val,pushup,pushdown,int,modify> seg(N);

i32 main() {
    int inv=Math::inverse(2);
    read(n,q);
    while (q--) {
        int l,r,a;
        read(l,r,a);
        seg.modify(1,l,r,a);
        auto ret=seg.query(1,l,r);
        int sum=ret.sum,sum2=ret.sum2;
        writeln(((sum*sum-sum2)%mod*inv%mod+mod)%mod);
    }
    return 0;
}
...
i32 main() {
    read(n,m);
    _rep(i,1,n) read(arr[i].p,arr[i].v);
    _rep(i,0,n) _rep(j,0,m) f[i][j]=-INF;
    f[0][0]=0;
    _rep(i,1,n) {
        _rep(j,0,arr[i].p-1) f[i][j]=f[i-1][j];
        _rep(j,arr[i].p,m) f[i][j]=max(f[i][j],max(f[i-1][j],f[i-1][j-arr[i].p]+arr[i].v));
    }
    int mx=-INF;
    _rep(i,0,m) mx=max(mx,f[n][i]);
    // cout<<"mx: "<<mx<<" "<<f[n][m]<<endl;
    _rep(i,0,m) if (mx==f[n][i]) flag[n][i]=true;
    _rrep(i,n,1) {
        bool a0=true,a1=true,a2=false;
        _rep(j,0,m) if (flag[i][j]) {
            // cout<<"test: "<<i<<" "<<j<<endl;
            bool A=f[i][j]==f[i-1][j];
            bool B=false;
            if (j>=arr[i].p && f[i][j]==f[i-1][j-arr[i].p]+arr[i].v) B=true;
            if (B) a0=false;
            if (A) a1=false;
            if (A) flag[i-1][j]=true;
            if (B) flag[i-1][j-arr[i].p]=true;
        }
        // cout<<i<<": "<<a0<<" "<<a1<<endl;
        if (!a0 && !a1) a2=true;
        if (a2) res.emplace_back('B');
        else if (a0) res.emplace_back('C');

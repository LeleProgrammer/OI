...
inline int cross(const node& a,const node& b) {
    return a.x*b.y-a.y*b.x;
}

inline int dot(const node& a,const node& b) {
    return a.x*b.x+a.y*b.y;
}
...
i32 main() {
    read(T);
    while (T--) {
        read(px,py,qx,qy,rx,ry,sx,sy);
        node a={qx-px,qy-py},b={sx-rx,sy-ry};
        if (cross(a,b)!=0) puts("Yes");
        else {
            node c={px+qx,py+qy},d={rx+sx,ry+sy};
            if (dot(c-d,a)!=0) puts("No");
            else puts("Yes");
        }
    }
    return 0;

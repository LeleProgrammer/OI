...
i32 main() {
    ...
    _rep(i,0,n-2) read(b[i]);
    _rep(i,1,n) c[i]=((b[i-1]-c[i-1])%m+m)%m;
    _rep(i,0,n-1) {
        d[i]=((c[i]-a[i])%m+m)%m;
        ret+=d[i];
        if ((i+1)&1) rem++;
    ...
    _rep(i,0,n-1) {
        if ((i+1)&1) {
            if (d[i]) vec.emplace_back(pii{m-d[i],-m});
        } else {
            if (d[i]!=m-1) vec.emplace_back(pii{d[i]+1,m});
        }
    }
    ans=ret;
    _sort(vec);
    ...
    while (i<=(int)vec.size()-1) {
        int t=vec[i].first;
        int sum=0;
        while (i<=(int)vec.size()-1 && vec[i].first==t) sum+=vec[i].second,++i;
        if (t-1>=cur) chmin(ans,ret+rem*(t-1)+g);
        g+=sum;
        chmin(ans,ret+rem*t+g);
        cur=t;
    }
    if (m-1>=cur) chmin(ans,ret+rem*(m-1)+g);
    write(ans);
    return 0;
}
...
int dx[]={0,1,0,-1};
int dy[]={1,0,-1,0};
int dis[K*3][K*3][K][K][K][K];
bool flag[K*3][K*3][K][K][K][K];
int ans=inf;
bool emp[K][K][K][K];
...
void bfs() {
    _rep(i,1,n*3) _rep(j,1,m*3) _rep(a,1,n) _rep(b,1,n) _rep(c,1,m) _rep(d,1,m) dis[i][j][a][b][c][d]=inf;
    queue<node> q;
    q.emplace(node{tx,ty,1,n,1,m});
    dis[tx][ty][1][n][1][m]=0;
    while (!q.empty()) {
        auto u=q.front(); q.pop();
        int i=u.i,j=u.j,a=u.a,b=u.b,c=u.c,d=u.d;
        if (flag[i][j][a][b][c][d]) continue;
        flag[i][j][a][b][c][d]=true;
        if (emp[a][b][c][d]) ans=min(ans,dis[i][j][a][b][c][d]);
        _rep(p,0,3) {
            int ii=i+dx[p];
            int jj=j+dy[p];
            int aa=ii-tx+1-n;
            ...
            dd=max(dd,d);
            if (aa>bb || cc>dd) {
                ans=min(ans,dis[i][j][a][b][c][d]+1);
                continue;
            }
            if (dis[ii][jj][aa][bb][cc][dd]<=dis[i][j][a][b][c][d]+1) continue;
            dis[ii][jj][aa][bb][cc][dd]=dis[i][j][a][b][c][d]+1;
            q.emplace(node{ii,jj,aa,bb,cc,dd});
        }
    }
}

int main() {
    read(n),read(m),g=max(n,m);
    _rep(i,n+1,n*2) _rep(j,m+1,m*2) {
        char c;
        scanf(" %c",&c);
        if (c=='#') s[i][j]=1;
        else if (c=='T') tx=i,ty=j;
    }
    _rep(i,1,n) _rep(j,1,m) _rep(p,i,n) _rep(q,j,m) {
        int cnt=0;
        _rep(a,i,p) _rep(b,j,q) cnt+=s[a+n][b+m];
        if (cnt) continue;
        emp[i][j][p][q]=true;
    }
    bfs();

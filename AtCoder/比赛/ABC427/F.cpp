...
int a[N];
bool flag[N];
unordered_map<int,int> cnt1[2],cnt2[2];

void dfs1(int u,int cur) {
    if (u==n/2+1) cnt1[flag[n/2]][cur]++;
    else {
        if (!flag[u-1]) {
            flag[u]=1;
            dfs1(u+1,(cur+a[u])%m);
...
int main() {
    ...
    i64 ans=0;
    _iter(it,cnt1[0]) ans+=(i64)(it->second)*(cnt2[0][(m-it->first)%m]+cnt2[1][(m-it->first)%m]);
    _iter(it,cnt1[1]) ans+=(i64)(it->second)*cnt2[0][(m-it->first)%m];
    write(ans);
    return 0;
}

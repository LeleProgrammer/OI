...
int main() {
    ...
    bool ok=true;
    _rep(i,1,n) {
        if (g[s[i]-'a'+1] && g[s[i]-'a'+1]!=t[i]-'a'+1) {
            ok=false;
            break;
        }
        g[s[i]-'a'+1]=t[i]-'a'+1;
    ...
    int cnt=0;
    _rep(i,1,26) cnt+=app[i];
    if (cnt==26 && strcmp(s+1,t+1)) {
        puts("-1");
        return 0;
    }
    _rep(i,1,26) if (!g[i]) g[i]=i;
    int ans=0;
    _rep(i,1,26) ans+=(g[i]!=i),rev[g[i]]++;
    _rep(i,1,26) {
        if (rev[i]<=1 || vis[i]) continue;
        dfs(i,0);
    }
    _rep(i,1,26) if (!vis[i]) dfs(i,i);
    // cout<<ans<<" "<<ext<<endl;
    write(ans+ext);
    return 0;
}
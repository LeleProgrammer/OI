...
i32 main() {
    ...
    _rep(i,1,n) _rep(j,1,m) {
        ...
        _rep(p,0,3) {
            if (s[i][j]=='S' || s[i][j]=='G' || s[i][j]=='.') {
                if (i-1>=1 && s[i-1][j]!='#') add(get(i,j,p),get(i-1,j,0),1);
                if (i+1<=n && s[i+1][j]!='#') add(get(i,j,p),get(i+1,j,2),1);
                if (j-1>=1 && s[i][j-1]!='#') add(get(i,j,p),get(i,j-1,3),1);
                if (j+1<=m && s[i][j+1]!='#') add(get(i,j,p),get(i,j+1,1),1);
            } else if (s[i][j]=='o') {
                if (p==0 && i-1>=1 && s[i-1][j]!='#') add(get(i,j,p),get(i-1,j,0),1);
                if (p==2 && i+1<=n && s[i+1][j]!='#') add(get(i,j,p),get(i+1,j,2),1);
                if (p==3 && j-1>=1 && s[i][j-1]!='#') add(get(i,j,p),get(i,j-1,3),1);
                if (p==1 && j+1<=m && s[i][j+1]!='#') add(get(i,j,p),get(i,j+1,1),1);
            } else if (s[i][j]=='x') {
                if (p!=0 && i-1>=1 && s[i-1][j]!='#') add(get(i,j,p),get(i-1,j,0),1);
                if (p!=2 && i+1<=n && s[i+1][j]!='#') add(get(i,j,p),get(i+1,j,2),1);
                if (p!=3 && j-1>=1 && s[i][j-1]!='#') add(get(i,j,p),get(i,j-1,3),1);
    ...
    dijkstra(get(sx,sy,0));
    int mn=min4(dis[get(tx,ty,0)],dis[get(tx,ty,1)],dis[get(tx,ty,2)],dis[get(tx,ty,3)]);
    if (mn>5000000) puts("No");
    else {
        puts("Yes");
        Graph::restore_graph();
        _rep(i,1,n) _rep(j,1,m) {
            ...
            _rep(p,0,3) {
                if (s[i][j]=='S' || s[i][j]=='G' || s[i][j]=='.') {
                    if (i-1>=1 && s[i-1][j]!='#') add2(get(i,j,p),get(i-1,j,0),1);
                    if (i+1<=n && s[i+1][j]!='#') add2(get(i,j,p),get(i+1,j,2),1);
                    if (j-1>=1 && s[i][j-1]!='#') add2(get(i,j,p),get(i,j-1,3),1);
                    if (j+1<=m && s[i][j+1]!='#') add2(get(i,j,p),get(i,j+1,1),1);
                } else if (s[i][j]=='o') {
                    if (p==0 && i-1>=1 && s[i-1][j]!='#') add2(get(i,j,p),get(i-1,j,0),1);
                    if (p==2 && i+1<=n && s[i+1][j]!='#') add2(get(i,j,p),get(i+1,j,2),1);
                    if (p==3 && j-1>=1 && s[i][j-1]!='#') add2(get(i,j,p),get(i,j-1,3),1);
                    if (p==1 && j+1<=m && s[i][j+1]!='#') add2(get(i,j,p),get(i,j+1,1),1);
                } else if (s[i][j]=='x') {
                    if (p!=0 && i-1>=1 && s[i-1][j]!='#') add2(get(i,j,p),get(i-1,j,0),1);
                    if (p!=2 && i+1<=n && s[i+1][j]!='#') add2(get(i,j,p),get(i+1,j,2),1);
                    if (p!=3 && j-1>=1 && s[i][j-1]!='#') add2(get(i,j,p),get(i,j-1,3),1);
        ...
        int cur;
        if (mn==dis[get(tx,ty,0)]) cur=get(tx,ty,0);
        if (mn==dis[get(tx,ty,1)]) cur=get(tx,ty,1);
        if (mn==dis[get(tx,ty,2)]) cur=get(tx,ty,2);
        if (mn==dis[get(tx,ty,3)]) cur=get(tx,ty,3);
        vector<char> res;
        while (true) {
            if (cur==get(sx,sy,0)) break;
            int t=(cur-1)/(n*m);
            if (t==0) {
                res.emplace_back('U');
            } else if (t==1) {
            ...
            _graph(i,cur) if (dis[e[i]]==dis[cur]-1) {
                cur=e[i];
                break;
            }
        }

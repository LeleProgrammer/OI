...
i32 main() {
    ...
    _rep(i,1,n) {
        ...
        L[arr[i].l].emplace_back(i);
        R[arr[i].r].emplace_back(i);
    }
    int ans=0;
    _rep(cntl,1,n-1) {
        int cntr=n-cntl;
        _for(i,L[cntl]) {
            if (cnt[i]==1) cntb--,cnt2++;
            else cnta++;
            cnt[i]++;
        }
        _for(i,R[cntl-1]) {
            if (cnt[i]==2) cnt2--,cntb++;
            else cnta--;
            cnt[i]--;
        }
        _for(i,L[cntr+1]) {
            if (cnt[i]==2) cnt2--,cnta++;
            else cntb--;
            cnt[i]--;
        }
        _for(i,R[cntr]) {
            if (cnt[i]==1) cnta--,cnt2++;
            else cntb++;
            cnt[i]++;
        }
        if (cnta+cnt2+cntb!=n) continue;
        if (cntl-cnta<0) continue;
        if (cntl-cnta>cnt2) continue;
        ans=(ans+C(cnt2,cntl-cnta))%mod;
    }
    write(ans);
    return 0;
}
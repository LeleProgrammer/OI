...
const int inf=1e18;
const __float128 pi=acosl((__float128)-1.0);
const __float128 eps=(__float128)1e-10*(__float128)1e-10;
...
#define _unique(x) (x).erase(unique(all(x)),x.end())
...
template<typename A,typename B> inline auto LB(const A& a,const B& b) { return (a).lower_bound(b); }
template<typename A,typename B> inline auto UB(const A& a,const B& b) { return (a).upper_bound(b); }
...
template<typename T1,typename T2> struct tup2 {
    T1 a; T2 b;
    friend bool operator < (const tup2& x,const tup2& y) {
        return x.b*y.a<x.a*y.b;
    }
    friend bool operator == (const tup2& x,const tup2& y) {
        return x.a==y.aa && x.b==y.b;
    }
...
namespace IO2 {
    ...
    inline void init() { inbuf=new char[buf_sz]; outbuf=new char[buf_sz]; memset(inbuf,0,sizeof(inbuf)); memset(outbuf,0,sizeof(outbuf)); inbuf[0]=outbuf[0]='\0'; inptr=inend=inbuf; outptr=outbuf; }
...
namespace Math {
    ...
    inline void init_mul(bool fmu=false,bool fphi=false) {
        ...
        pri=new int[N];
        flag=new bool[N];
        if (fmu) mu=new int[N],memset(mu,0,sizeof(flag));
        if (fphi) phi=new int[N],memset(phi,0,sizeof(phi));
...
public:
    ...
    Mint operator ++ (int) { Mint tmp=*this; ++a; return tmp; }
...
public:
    ...
    BigInt() { arr=new int[n]; memset(arr,0,sizeof(int)*n); }
    BigInt& operator = (const BigInt& b) { memcpy(arr,b.arr,sizeof(int)*n); return *this; }
    ~BigInt() { delete[] arr; }
...
namespace Graph {
    ...
    inline void restore_graph() { memset(h,-1,sizeof(h)),tot=0; }
    ...
    namespace LCA {
        ...
        inline void init() { dep=new int[N]; f=new int*[N]; _rep(i,0,N-1) f[i]=new int[K]; memset(dep,0,sizeof(dep)),memset(f,0,sizeof(f)); }
        inline void restore() { memset(dep,0,sizeof(dep)),memset(f,0,sizeof(f)); }
    ...
    namespace DINIC {
        ...
        inline void init() { S=T=0; cur=new int[N]; dep=new int[N]; memset(cur,0,sizeof(cur)); memset(dep,0,sizeof(dep)); }
    ...
    namespace MCMF {
        ...
        inline void init() { S=T=0; dis=new int[N]; flow=new int[N]; bk=new int[N]; flag=new bool[N]; _rep(i,0,N-1) dis[i]=flow[i]=inf; memset(bk,0,sizeof(bk)); memset(flag,0,sizeof(flag)); }
...
public:
    ...
    Splay() {}
    Splay(size_t n) { this->n=n; tr=new ValueType[N]; memset(tr,0,sizeof(tr)); }
    ~Splay() { delete[] tr; }
    Splay(const Splay&)=delete;
    Splay operator=(const Splay&)=delete;
    
    inline void reserve(size_t n) { if (tr!=nullptr) delete[] tr; tr=new ValueType[N]; memset(tr,0,sizeof(tr)); this->n=n; root=idx=0; }
...
public:
    ...
    Queue(const Queue& oth): head(nullptr),tail(nullptr),length(0) {
        node* curr=oth.head;
        while (curr) {
            emplace(curr->value);
            curr=curr->next;
    ...
    Queue(Queue&& oth) noexcept: head(oth.head),tail(oth.tail),length(oth.length) {
        oth.head=nullptr;
        oth.tail=nullptr;
        oth.length=0;
    }
    ...
    inline void swap(Queue& oth) noexcept {
        swap(head,oth.head);
        swap(tail,oth.tail);
    ...
    inline void emplace(const T& value) {
        node* u=new node{value,nullptr};
        if (empty()) head=tail=u;
        else tail->next=u,tail=u;
        ++length;
    }

    template<typename... S> inline void emplace(const T& v1,const S&... v2) { emplace(v1),emplace(v2...); }
...
public:
    ...
    Stack(const Stack& oth): head(nullptr),tail(nullptr),length(0) {
        node* curr=oth.head;
        while (curr) {
            emplace(curr->value);
            curr=curr->next;
    ...
    Stack(Stack&& oth) noexcept: head(oth.head),tail(oth.tail),length(oth.length) {
        oth.head=nullptr;
        oth.tail=nullptr;
        oth.length=0;
    }
    ...
    inline void swap(Stack& oth) noexcept {
        swap(head,oth.head);
        swap(tail,oth.tail);
        swap(length,oth.length);
    }
    ...
    inline void clear() { while (!empty()) { node* tmp=head; head=head->next; delete tmp; } tail=nullptr; length=0; }
    inline size_t size() const { return length; }
    inline T& top() { if (empty()) __assert(false); return head->value; }
    inline T& bottom() { if (empty()) __assert(false); return tail->value; }

    inline void emplace(const T& value) {
        node* u=new node{value,head};
        if (empty()) head=tail=u;
        else head=u;
        ++length;
    }
...
i32 main() {
    read(n,q);
    _rep(i,1,n) {
        read(arr[i].a,arr[i].b);
        g[i]=fabsl(atan2l((f128)arr[i].b,(f128)arr[i].a));
        if (arr[i].a>0 && arr[i].b<0) g[i]=-g[i];
        else if (arr[i].a<0 && arr[i].b>0) g[i]=pi-g[i];
        else if (arr[i].a<0 && arr[i].b<0) g[i]=-pi+g[i];
        vec.emplace_back(g[i]);
    }
    sort(_all(vec));
    // _iter(it,vec) cout<<"test: "<<(long double)*it<<endl;
    while (q--) {
        int a,b;
        read(a,b);
        int A=upper_bound(_all(vec),g[a]+eps)-vec.begin()-1;
        int B=lower_bound(_all(vec),g[b]-eps)-vec.begin();
        // cout<<"get: "<<A<<" "<<B<<endl;
        if (A>=B) writeln(A-B+1);
        else writeln((int)vec.size()-(B-A+1)+2);
    }
    return 0;
}
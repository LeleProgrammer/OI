...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,n) if (s[i][j]=='.') arr[i][j]=0; else arr[i][j]=1;
    _rep(i,1,n) {
        _rep(j,1,n) pre1[i][j]=pre1[i][j-1]+(arr[i][j]==1);
        _rrep(j,n,1) suf0[i][j]=suf0[i][j+1]+(arr[i][j]==0);
    }
    _rep(i,1,n) {
        _rrep(j,n,0) f[i][j+1]=f[i-1][j+1]+pre1[i][n-j]+suf0[i][n-j+1];
        _rep(j,1,n) f[i][j+1]=min(f[i][j+1],f[i][j]);
        // cout<<"test: "<<i<<": ";
        // _rep(j,0,n) writesp(seg.query(1,j+1,j+1).v);
        // putchar(10);
    }
    write(f[n][n+1]);
    return 0;

...
struct Matrix {
    ...
    inline friend Matrix operator * (Matrix a,Matrix b) {
        ...
        Matrix mat;
        mat.n=n,mat.m=p;
        _rep(i,1,n) _rep(j,1,p) _rep(k,1,m) (mat[i][j]+=a[i][k]*b[k][j])%=mod;
        return mat;
    }
} G,B;
...
inline void build() {
    queue<int> q;
    _rep(c,0,25) if (tr[0][c]) q.emplace(tr[0][c]);
    while (!q.empty()) {
        int u=q.front(); q.pop();
        if (flag[ne[u]]) flag[u]=true;
        _rep(c,0,25) {
            if (!tr[u][c]) tr[u][c]=tr[ne[u]][c];
            else ne[tr[u][c]]=tr[ne[u]][c],q.emplace(tr[u][c]);
        }
    }
}
...
i32 main() {
    ...
    B=B*G;
    int ans=0;
    _rep(i,0,idx) if (!flag[i]) (ans+=B[1][i+1])%=mod;
    write(ans);
    return 0;
}
...
inline bool check(int mid) {
    ...
    int rem=mid,cur=0,delta=0;
    _rep(i,1,n) {
        cur+=a[i];
        if (rem*b[i]>cur) {
            while (!q.empty() && q.top().first-delta-b[i]>=0) {
                auto g=q.top(); q.pop();
                int inc=q.top().first-delta-b[i];
                int dis=rem*b[i]-cur;
                int cnt=min(g.second,dis/inc+!!(dis%inc));
                rem+=cnt,cur+=(q.top().first-delta)*cnt;
                g.second-=cnt;
                if (g.second) q.emplace(g);
            }
        }
        if (rem*b[i]>cur) return false;
        cur-=rem*b[i];
        delta+=b[i];
        // cout<<"dec: "<<cur<<endl;
        int t=cur/c[i];
        ...
        rem-=t;
        cur-=c[i]*t;
        q.emplace(pii{c[i]+delta,t});
        if (!rem) return true;
        // cout<<mid<<" after: "<<i<<": "<<rem<<" "<<cur<<endl;
    }
    return false;
}

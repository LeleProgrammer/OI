...
i32 main() {
    ...
    _rep(i,1,m) st.emplace(pii{a[i],b[i]});
    m=st.size();
    _for(t,st) d[t.first]++,d[t.second]++;
    _rep(i,1,n) cnt[d[i]].emplace_back(i);
    _rep(i,0,m) {
        int j=m-i;
        if (i>j+2) break;
        // cout<<"now: "<<i<<" "<<j<<endl;
        _for(u,cnt[j]) vec.emplace_back(u);
        _for(u,cnt[i]) {
            _for(v,vec) {
                if (u==v) continue;
                if (d[u]>d[v]) continue;
                int tot=d[u]+d[v]-st.count(pii{min(u,v),max(u,v)});
                // cout<<"test: "<<u<<" "<<v<<" "<<tot<<endl;
                if (tot==m) ans.emplace(pii{min(u,v),max(u,v)});
            }
        }
    }
    write(ans.size());

...
inline bool check(double mid) {
    _rep(i,1,n) if (s[i]=='o') g[i]=1-mid; else g[i]=-mid;
    _rep(i,1,n) g[i]=g[i-1]+g[i];
    mn[0]=g[0];
    _rep(i,1,n) mn[i]=min(g[i],mn[i-1]);
    int l=0;
    _rep(r,1,n) {
        while (l<=r-1 && p[r]-p[l]>=k) ++l;
        if (!l) continue;
        if (g[r]-mn[l-1]>=0) return true;
    }
    return false;
}

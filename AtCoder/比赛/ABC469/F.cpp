...
inline void sieve(int k) {
    vec.clear();
    _rep(i,2,sqrt(k)) {
        if (k%i!=0) continue;
        int cnt=0;
        while (k%i==0) cnt++,k/=i;
        vec.emplace_back(pii{i,cnt});
    }
    if (k!=1) vec.emplace_back(pii{k,1});
}

inline void solve(int pos,int cur,int& ans) {
    // cout<<"solve: "<<pos<<" "<<cur<<endl;
    if (pos==(int)vec.size()) {
        if (cnt[cur]>=2 && mx[cur]>lmt) chmax(ans,cur);
        return;
    }
    _rep(i,0,vec[pos].second) {
        solve(pos+1,cur,ans);
        cur*=vec[pos].first;
    }
}
...
i32 main() {
    ...
    _rep(i,1,n) {
        _rep(j,1,sqrt(a[i])) {
            if (a[i]%j!=0) continue;
            cnt[j]++,mx[j]=a[i];
            if (j*j!=a[i]) cnt[a[i]/j]++,mx[a[i]/j]=a[i];
        }
    }
    // cout<<"debug: "<<mx[5]<<endl;
    int res=0;
    _rep(i,1,n) {
        sieve(a[i]);
        int ans=0;

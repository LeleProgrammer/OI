...
int n;
int a[N],b[N];
int mxmx,mxsmx,mxx,smxsmx,smxx,xx;
map<pii,int> mp;

inline int calc(int c,int k) {
    if (mp.count(pii{c,k})) return mp[pii{c,k}];
    int ans=0;
    _rep(i,0,k) ans=(ans+C(k,i)*inverse(c+i))%mod;
    ans=ans*inverse(ksm(2,k))%mod;
    return mp[pii{c,k}]=ans;
}
...
i32 main() {
    ...
    int mx=0;
    _rep(i,1,n) chmax(mx,max(a[i],b[i]));
    _rep(i,1,n) {
        int c=a[i],d=b[i];
        if (c<d) swap(c,d);
        if (c==mx) {
            if (d==mx) mxmx++;
            else if (d==mx-1) mxsmx++;
            else mxx++;
        } else if (c==mx-1) {
            if (d==mx-1) smxsmx++;
            else smxx++;
    ...
    _rep(i,1,n) {
        ...
        if (a[i]<mx-1) writesp(0);
        else {
            if (a[i]==mx) {
                int ans1=0;
                if (mxmx==0 && b[i]!=mx) {
                    int p=inverse(ksm(2,mxsmx+mxx));
                    int cnt=1+mxsmx*2+mxx+smxsmx;
                    if (b[i]==mx-1) cnt++,p=p*inverse(2)%mod;
                    else p=p*inverse(2)%mod;
                    ans1=calc(cnt,smxx)*p%mod;
                }
                int p=inverse(2);
                int cnt=1+mxmx;
                int ans2=calc(cnt,mxsmx+mxx)*p%mod;
                int res=(ans1+ans2)%mod;
                writesp(res);
            } else {
                if (mxmx) writesp(0);
                else {
                    int p=inverse(ksm(2,mxsmx+mxx));
                    int cnt=1+mxsmx*2+mxx+smxsmx;
                    if (b[i]==mx) cnt++,p=p*inverse(2)%mod;
                    else if (b[i]==mx-1) p=p*inverse(2)%mod;
                    else p=p*inverse(2)%mod;
                    int res=calc(cnt,smxx)*p%mod;
                    writesp(res);
                }
            }
        }
        ...
        if (b[i]<mx-1) writesp(0);
            ...
            if (b[i]==mx) {
                ...
                if (mxmx==0 && a[i]!=mx) {
                    int p=inverse(ksm(2,mxsmx+mxx));
                    int cnt=1+mxsmx*2+mxx+smxsmx;
                    if (a[i]==mx-1) cnt++,p=p*inverse(2)%mod;
                    else p=p*inverse(2)%mod;
                    ans1=calc(cnt,smxx)*p%mod;
                }
                int p=inverse(2);
                ...
                int res=(ans1+ans2)%mod;
                writesp(res);
                // cout<<"no 96: "<<ans1*96%mod<<" "<<ans2*96%mod<<" "<<res*96%mod<<endl;
            } else {
                if (mxmx) writesp(0);
                else {
                    int p=inverse(ksm(2,mxsmx+mxx));
                    int cnt=1+mxsmx*2+mxx+smxsmx;
                    if (a[i]==mx) cnt++,p=p*inverse(2)%mod;
                    else if (a[i]==mx-1) p=p*inverse(2)%mod;
                    else p=p*inverse(2)%mod;
                    int res=calc(cnt,smxx)*p%mod;
                    writesp(res);
                }

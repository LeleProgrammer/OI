...
inline void init() {
    pw2[0]=1;
    _rep(i,1,N-1) pw2[i]=pw2[i-1]*2%mod;
    _rep(i,0,N-1) invpw2[i]=inverse(pw2[i]);
}

inline int solve(int n,int x) {
    int tot=0;
    _rep(k,0,n) tot=(tot+C(n,k)*(abs(k*2-(n+x))%mod))%mod;
    return tot*invpw2[n]%mod;
}
...
inline void process(int n) {
    ...
    _rep(k,1,n) {
        int c=C(n,k);
        vecs[k]=(vecs[k-1]+c)%mod;
        vecl[k]=(vecl[k-1]+k*c)%mod;
    }
}

inline int solve2(int n,int x) {
    int tot=0;
    int spl;
    if (n+x>=0) spl=(n+x+1)/2;  
    else spl=n+x/2;
    if (spl<0) spl=0;
    if (spl>n+1) spl=n+1;
    if (spl>0) {
    ...
    if (spl<=n) {
        int c=vecs[n]-vecs[spl-1];
        int c2=vecl[n]-vecl[spl-1];
        int t=c2*2%mod;
        t=(t-(n+x)*c)%mod;
        t=(t%mod+mod)%mod;
        tot=(tot+t)%mod;
...
i32 main() {
    ...
    init();
    read(T);
    _rep(i,1,T) read(arr[i].n,arr[i].x),arr[i].i=i;
    sort(arr+1,arr+1+T);
    int i=1;
    while (i<=T) {
        int j=i;
        int cur=arr[i].n;
        while (j<=T && arr[j].n==cur) j++;
        if (cur<=B) _rep(k,i,j-1) ans[arr[k].i]=solve(cur,arr[k].x);
        else {
            process(cur);
            _rep(k,i,j-1) ans[arr[k].i]=solve2(cur,arr[k].x);
        }
        i=j;
    }
    _rep(i,1,T) writeln(ans[i]);
    return 0;
}
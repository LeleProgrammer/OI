...
inline void bfs() {
    queue<pii> q;
    _rep(i,0,N-1) _rep(j,0,9) dis[i][j]=inf;
    _rep(i,1,9) {
        if (i%n==0) {
            write(i);
            exit(0);
    ...
    while (!q.empty()) {
        ...
        int m=u.first,las=u.second;
        _rep(i,las,9) {
            if ((m*10+i)%n==0) {
                string res;
                res.push_back(i+'0');
                while (~bk[m][las].a) {
                    res.push_back(bk[m][las].c+'0');
                    int mm=bk[m][las].a,lass=bk[m][las].b;
                    m=mm,las=lass;
                }
                res.push_back(bk[m][las].c+'0');
                _riter(it,res) putchar(*it);
                exit(0);
            }
            if (!flag[(m*10+i)%n][i]) {
                flag[(m*10+i)%n][i]=true;
                bk[(m*10+i)%n][i]={m,las,i};
                q.emplace(pii{(m*10+i)%n,i});
            }
        }
    }
}

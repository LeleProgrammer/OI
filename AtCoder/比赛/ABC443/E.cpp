...
inline void bfs() {
    ...
    while (!q.empty()) {
        ...
        if (j-1>=1 && (arr[i-1][j-1]=='.' || flag[i-1][j-1])) q.emplace(pii{i-1,j-1});
        if (arr[i-1][j]=='.' || flag[i-1][j]) q.emplace(pii{i-1,j});
        if (j+1<=n && (arr[i-1][j+1]=='.' || flag[i-1][j+1])) q.emplace(pii{i-1,j+1});
    }
}
...
int main() {
    ...
    while (T--) {
        read(n,g);
        _rep(i,1,n) _rep(j,1,n) scanf(" %c",&arr[i][j]);
        _rep(i,1,n) _rep(j,1,n) vis[i][j]=flag[i][j]=false;
        _rep(j,1,n) _rrep(i,n,1) if (arr[i][j]=='#') {
            flag[i][j]=true;
            break;
        }
        bfs();
        _rep(i,1,n) if (vis[1][i]) putchar('1'); else putchar('0');
        putchar(10);
    }
    return 0;
}
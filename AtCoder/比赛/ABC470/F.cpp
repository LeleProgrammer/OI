...
i32 main() {
    ...
    read(n,m);
    readstr(s+1);
    _rep(i,1,m) read(a[i],b[i]),add(a[i],b[i]),add(b[i],a[i]);
    bool flag=false;
    _rep(i,1,n) if (!~col[i]) {
        bool ok=true;
        dfs(i,0,ok);
    ...
    _rep(i,1,n) p[i]=i;
    _rep(i,1,m) merge(a[i],b[i]);
    _rep(i,1,n) vec[find(i)].emplace_back(i);
    int ans=1;
    bool have=false;
    _rep(i,1,n) {
        auto &v=vec[find(i)];

...
inline double dfs(int c,int k,int l) {
    if (!l) return 0;
    if (!c) return !!k;
    if (f[c][k][l]>=0) return f[c][k][l];
    f[c][k][l]=0;
    double u=c*2+k;
    double tot=c+k;
    double per=1.0/tot;
    if (k) f[c][k][l]+=(k/u)*(per+(1.0-per)*dfs(c,k-1,l));
    if (c) {
        f[c][k][l]+=(c*2.0/u)/(u-1)*(per+(1.0-per)*dfs(c-1,k,l));
        if (k && l-1) f[c][k][l]+=(c*2.0/u)*k/(u-1)*(per+(1.0-per)*dfs(c-1,k,l-1));
        if (c*2-2>0 && l-1) f[c][k][l]+=(c*2.0/u)*(c*2.0-2)/(u-1)*dfs(c-2,k+2,l-1);
    }
    return f[c][k][l];
}
...
i32 main() {
    read(n,l);
    _rep(i,1,n) read(a[i]);
    _rep(i,0,N-1) _rep(j,0,N-1) _rep(k,0,N-1) f[i][j][k]=-1;
    double sum=0;
    _rep(i,1,n) sum+=a[i];
    printf("%.12lf",dfs(n,0,l)*sum);
    return 0;
}
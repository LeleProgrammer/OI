...
constexpr inline int calc(int a,int b) {
    return (a+b)*(b-a+1)/2;
}
    ...
    while (T--) {
        ...
        _rrep(i,n-1,1) {
            ...
            while (a[i]>=a[i+1]) {
                ...
                // cout<<"take: "<<pos<<" "<<cnt<<endl;
                int len=pos-i;
                int k=(a[i]-a[i+1])/(len+1);
                chmin(k,cnt);
                // cout<<"k: "<<k<<endl;
                cnt-=k;
                a[i+1]+=k;
                ...
                if (a[i]>=a[i+1]) {
                    ...
                    if (d!=len) q.emplace(pii{i+len-d,1});
                    cnt--;
                }
                if (cnt) q.emplace(pii{pos,cnt});
            }
            if (a[i]+1<a[i+1]) q.emplace(pii{i,a[i+1]-a[i]-1});
            // cout<<"test: "<<i<<": "<<a[i]<<" ans: "<<ans<<endl;
        }
        writeln(ans);
    }
    return 0;

...
i32 main() {
    ...
    _rep(i,0,N-1) _rep(j,0,M-1) f[i][j]=-1e18;
    f[0][0]=0;
    _rep(i,1,n) _rep(j,0,m) {
        if (arr[i].b>=t) f[i][j]=f[i-1][j]+arr[i].a;
        else if (j>=arr[i].c) f[i][j]=max(f[i-1][j],(i64)(f[i-1][j-arr[i].c]+arr[i].a));
        else f[i][j]=f[i-1][j];
    }
    i64 ans=0;

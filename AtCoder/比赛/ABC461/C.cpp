...
int main() {
    ...
    _rep(i,1,n) _sort(vec[i]);
    _rep(i,1,n) if (!vec[i].empty()) p.emplace_back(i);
    sort(_all(p),[&](const int& a,const int& b) {
        return vec[a].back()>vec[b].back();
    });
    i64 ans=0;
    _rep(i,1,m) {
        int t=p[i-1];
    ...
    vector<int> rem;
    _rep(i,1,n) _for(g,vec[i]) rem.emplace_back(g);
    _sort(rem);
    _rep(i,1,k-m) ans+=rem.back(),rem.pop_back();
    write(ans);
    return 0;
}
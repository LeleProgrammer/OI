...
int main() {
    ...
    _rep(i,1,n) _rep(j,1,m) p[i][j]=p[i][j-1]+s[i][j]-'0';
    i64 ans=0;
    _rep(len,1,m) _rep(st,1,m-len+1) {
        int l=1,cur=0,L=1,cur2=0;
        _rep(r,1,n) {
            cur+=p[r][st+len-1]-p[r][st-1];
            cur2+=p[r][st+len-1]-p[r][st-1];
            while (cur>k) cur-=p[l][st+len-1]-p[l][st-1],l++;
            while (cur2>=k && L<=r) cur2-=p[L][st+len-1]-p[L][st-1],L++;
            if (cur==k) ans+=L-l;
        }
    }
    write(ans);

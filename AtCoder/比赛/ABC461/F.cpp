...
int n,tot=1;
vector<int> vecp,vecc;
vector<int> rev[N];
int cnt[N][P],sum[N][P];
int ccnt[N][P],csum[N][P];
...
i32 main() {
    Math::init_fact();
    read(n);
    _rep(i,2,sqrt(n)) if (n%i==0) {
        int cnt=0;
        while (n%i==0) n/=i,cnt++;
        vecp.emplace_back(i);
        vecc.emplace_back(cnt);
    }
    if (n!=1) {
        vecp.emplace_back(n);
        vecc.emplace_back(1);
    ...
    _rep(i,0,tot-1) {
        int j=i;
        _rep(t,0,(int)vecc.size()-1) {
            rev[i].emplace_back(j%(vecc[t]+1));
            j/=(vecc[t]+1);
        }
    }
    int tar=query(vecc);
    ...
    _rep(id,0,tot-1) {
        int d=1;
        _rep(i,0,(int)vecp.size()-1) _rep(j,0,rev[id][i]-1) d*=vecp[i];
        div.emplace_back(pii{d,id});
    }
    int kk=0;
    _for(x,vecc) kk+=x;
    ++kk;
    ...
    _for(val,div) {
        ...
        _rep(st,0,tot-1) {
            ...
            vector<int> nxt;
            bool ok=true;
            _rep(i,0,(int)vecp.size()-1) {
                // cout<<"now: "<<i<<" "<<vecp.size()-1<<endl;
                nxt.emplace_back(rev[st][i]+rev[z][i]);
                if (nxt[i]>vecc[i]) {
                    ok=false;
                    break;
                }
            ...
            int ns=query(nxt);
            _rep(k,0,kk-1) {
                if (!ccnt[st][k]) continue;
                cnt[ns][k+1]=(cnt[ns][k+1]+ccnt[st][k])%mod;
                int add=(csum[st][k]+d*ccnt[st][k])%mod;
                sum[ns][k+1]=(sum[ns][k+1]+add)%mod;
            }
        }
    }
    int ans=0;
    _rep(i,1,kk) ans=(ans+Math::f[i]*sum[tar][i])%mod;
    write(ans);
    return 0;
}
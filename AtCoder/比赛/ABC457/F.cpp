...
i32 main() {
    ...
    inv[1]=1;
    _rep(i,2,N-1) inv[i]=Math::inverse(i);
    f[n]=f[n-1]=1;
    cur.emplace_back(n);
    ...
    _rrep(i,n-2,1) {
        if (d[i]==cd) {
            if (f[i+cd]) {
                int v=f[i+cd];
                if (!f[i]) cur.emplace_back(i);
                (f[i]+=v*inv[n-i-1])%=mod;
                f[i+cd]=v*(n-i)%mod*inv[n-i-1]%mod;
            }
            (mul*=(n-i-1))%=mod;
        } else {
            memset(g,0,sizeof(g));
            ...
            _for(p,cur) {
                if (p==i+d[i]) {
                    if (!g[p]) nex.emplace_back(i);
                    (g[i]+=f[p]*mul)%=mod;
                    if (!g[p]) nex.emplace_back(p);
                    (g[p]+=f[p]*mul)%=mod;
                }
            }
            if (nex.empty()) {

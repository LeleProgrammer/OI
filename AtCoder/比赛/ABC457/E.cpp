...
SEG<val,pushup,nullptr,int,modify> segl(N),segr(N);

int main() {
    read(n,m);
    _rep(i,1,m) read(arr[i].l,arr[i].r);
    _rep(i,1,m) L[arr[i].l].emplace_back(arr[i].r);
    _rep(i,1,m) R[arr[i].r].emplace_back(arr[i].l);
    _rep(i,1,n) sort(_all(L[i]));
    _rep(i,1,n) sort(_all(R[i]));
    _rep(i,1,n) if (!L[i].empty()) segl.modify(1,i,i,L[i][0]);
    _rep(i,1,n) if (!R[i].empty()) segr.modify(1,i,i,R[i][0]);
    read(q);
    while (q--) {
        int s,t;
        read(s,t);
        ...
        if (L[s][lp]<=R[t][rp]-2) puts("No");
        else {
            if (L[s][lp]!=t || R[t][rp]!=s) puts("Yes");
            else {
                bool ok=false;
                lp--;
                if (~lp) ok=true;
                rp++;
                if (rp!=R[t].size()) ok=true;
                if (ok) puts("Yes");
                else {
                    if (s==t || s+1==t) puts("No");
                    else {
                        int mnl=segl.query(1,s+1,t-1).mn;
                        int mxr=segr.query(1,s+1,t-1).mx;
                        if (mnl<=t || mxr>=s) puts("Yes");
                        else puts("No");
                    }
                }
            }
